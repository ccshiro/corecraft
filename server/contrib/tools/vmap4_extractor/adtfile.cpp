    ...
    std::set<std::string>& failedPaths)
{
    ...
    xMap = TempMapNumber.substr(TempMapNumber.find("_") + 1,
        (TempMapNumber.find_last_of("_") - 1) - (TempMapNumber.find("_")));
    yMap = TempMapNumber.substr(TempMapNumber.find_last_of("_") + 1,
        (TempMapNumber.length()) - (TempMapNumber.find_last_of("_")));
    Adtfilename.erase(
        (Adtfilename.length() - xMap.length() - yMap.length() - 2),
        (xMap.length() + yMap.length() + 2));
    string AdtMapNumber =
        xMap + ' ' + yMap + ' ' + GetPlainName((char*)Adtfilename.c_str());
        ...
        else if (!strcmp(fourcc, "MDDF"))
        {
            if (size)
            {
                ...
                for (int i = 0; i < nMDX; ++i)
                {
                    uint32 id;
                    ADT.read(&id, 4);
                    ModelInstance inst(ADT, ModelInstansName[id].c_str(),
                        map_num, tileX, tileY, dirfile);
                }
        ...
        else if (!strcmp(fourcc, "MODF"))
        {
            if (size)
            {
                ...
                for (int i = 0; i < nWMO; ++i)
                {
                    uint32 id;
                    ADT.read(&id, 4);
                    WMOInstance inst(ADT, WmoInstansName[id].c_str(), map_num,
                        tileX, tileY, dirfile);
                }
