...
const char* szRawVMAPMagic = "VMAPb41";
...
bool ExtractWmo()
{
    ...
    // const char* ParsArchiveNames[] = {"patch-2.MPQ", "patch.MPQ",
         ...
         ar_itr != gOpenArchives.end() && success; ++ar_itr)
    {
        ...
        for (auto fname = filelist.begin(); fname != filelist.end() && success;
             ++fname)
        {
...
void ParsMapFiles()
{
    ...
    if (!failedPaths.empty())
    {
        ...
        printf(
            "A few not found models can be expected and are not alarming.\n");
    }
}
...
void getGamePath()
{
    ...
    l = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
        ...
        KEY_QUERY_VALUE, &key);
    // l = RegOpenKeyEx(HKEY_LOCAL_MACHINE,"SOFTWARE\\Blizzard
    // Entertainment\\Burning Crusade Closed Beta",0,KEY_QUERY_VALUE,&key);
...
bool fillArchiveNameVector(std::vector<std::string>& pArchiveNames)
{
    ...
    {
        pArchiveNames.push_back(
            in_path + locale + "/locale-" + locale + ".MPQ");
        pArchiveNames.push_back(
            in_path + locale + "/expansion-locale-" + locale + ".MPQ");
    }
    ...
    for (auto& locale : locales)
    {
        printf("Locale: %s\n", locale.c_str());
        sprintf(
            path, "%s%s/patch-%s", input_path, locale.c_str(), locale.c_str());
        if (scan_patches(path, pArchiveNames))
            foundOne = true;
    }
...
bool processArgv(int argc, char** argv, const char* versionString)
{
    ...
    if (!result)
    {
        ...
        printf("%s [-?][-s][-l][-d <path>]\n", argv[0]);
        printf(
            "   -s : (default) small size (data size optimization), ~500MB "
            "less vmap data.\n");
        printf(
            "   -l : large size, ~500MB more vmap data. (might contain more "
            "details)\n");
...
int main(int argc, char** argv)
{
    ...
    else
    {
        ...
        {
            printf(
                "Your output directory seems to be polluted, please use an "
                "empty directory!\n");
    ...
    {
        auto archive = new MPQArchive(archiveName.c_str());
    ...
    {
        printf(
            "FATAL ERROR: None MPQ archive found by path '%s'. Use -d option "
            "with proper path.\n",
            input_path);
        return 1;
    }
