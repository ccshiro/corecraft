...
bool WMOGroup::open()
{
    ...
    while (!f.isEof())
    {
        ...
        else if (!strcmp(fourcc, "MLIQ"))
        {
            ...
            ios_base::app);
            llog << filename;
            llog << "\nbbox: " << bbcorn1[0] << ", " << bbcorn1[1] << ", " <<
            bbcorn1[2] << " | " << bbcorn2[0] << ", " << bbcorn2[1] << ", " <<
            bbcorn2[2];
            llog << "\nlpos: " << hlq->pos_x << ", " << hlq->pos_y << ", " <<
            hlq->pos_z;
            llog << "\nx-/yvert: " << hlq->xverts << "/" << hlq->yverts << "
            size: " << size << " expected size: " << 30 +
            hlq->xverts*hlq->yverts*8 + hlq->xtiles*hlq->ytiles << std::endl;
            llog.close(); */
        }
        f.seek((int)nextpos);
    }
...
std::set<std::string> wmos_with_liquid;

int WMOGroup::ConvertToVMAPGroupWmo(
    FILE* output, WMORoot* rootWMO, bool pPreciseVectorData)
{
    ...
    else
    {
        ...
        auto IndexRenum = new int[nVertices];
        ...
        // write vertices
        int VERT[] = {0x54524556, int(nColVertices * 3 * sizeof(float) + 4),
            nColVertices}; // "VERT"
    ...
    if (LiquEx_size != 0)
    {
        auto root_name = rootWMO->get_filename();
        auto pos = root_name.rfind('\\');
        if (pos == std::string::npos)
        {
            printf("Error: '%s' does not contain an expected '\\'!\n",
                root_name.c_str());
            exit(-1);
        }
        root_name = root_name.substr(pos + 1);
        std::transform(
            root_name.begin(), root_name.end(), root_name.begin(), ::tolower);
        wmos_with_liquid.insert(root_name);

        int LIQU_h[] = {0x5551494C, int(sizeof(WMOLiquidHeader) + LiquEx_size +
                                        hlq->xtiles * hlq->ytiles)}; // "LIQU"
        ...
        ios_base::app);
        llog << filename;
        llog << ":\ntype: " << hlq->type << " (root:" << rootWMO->liquidType <<
        " group:" << liquidType << ")\n";
        llog.close(); */
...
std::map<uint32 /*map*/, std::vector<std::pair<uint32, uint32>>> used_tiles;
// static void wmoinst_with_liquid(uint32 map,

WMOInstance::WMOInstance(MPQFile& f, const char* WmoInstName, uint32 mapID,
    uint32 tileX, uint32 tileY, FILE* pDirfile)
{
    ...
    std::string wmo_name_lowercase(WmoInstName);
    std::transform(wmo_name_lowercase.begin(), wmo_name_lowercase.end(),
        wmo_name_lowercase.begin(), ::tolower);
    if (wmos_with_liquid.find(wmo_name_lowercase) != wmos_with_liquid.end())
        flags |= MOD_HAS_LIQUID;

    // write mapID, tileX, tileY, Flags, ID, Pos, Rot, Scale, Bound_lo,
    // Bound_hi, name
