...
const char* CONF_mpq_list[] = {
    "common.MPQ", "common-2.MPQ", "lichking.MPQ", "expansion.MPQ", "patch.MPQ",
    "patch-2.MPQ", "patch-3.MPQ", "patch-4.MPQ", "patch-5.MPQ",
};

static char* const langs[] = {"enGB", "enUS", "deDE", "esES", "frFR", "koKR",
...
void Usage(char* prg)
{
    printf(
        ...
        "-f height stored as int (less map size but lost some accuracy) 1 by "
        "default\n"
...
bool ConvertADT(char* filename, char* filename2, int cell_y, int cell_x)
{
    ...
    for (auto& area_flag : area_flags)
    {
        for (auto& elem : area_flag)
        {
            if (elem != areaflag)
            {
                fullAreaData = true;
    ...
    {
        for (auto& elem : V8)
            for (auto& elem_x : elem)
                if (elem_x < CONF_use_minHeight)
                    elem_x = CONF_use_minHeight;
        for (int y = 0; y <= ADT_GRID_SIZE; y++)
            for (int x = 0; x <= ADT_GRID_SIZE; x++)
    ...
    if (!(heightHeader.flags & MAP_HEIGHT_NO_HEIGHT))
    {
        ...
        if (CONF_allow_float_to_int)
        {
            ...
            if (diff < CONF_float_to_int8_limit) // As uint8 (max accuracy =
    ...
    if (map.liquidMapOffset)
    {
        ...
        if (!(liquidHeader.flags & MAP_LIQUID_NO_HEIGHT))
        {
            for (int y = 0; y < liquidHeader.height; y++)
                fwrite(
                    &liquid_height[y +
                                   liquidHeader.offsetY][liquidHeader.offsetX],
                    sizeof(float), liquidHeader.width, output);
        }
    }
...
void ExtractMapsFromMpq()
{
    ...
    for (uint32 z = 0; z < map_count; ++z)
    {
        ...
        sprintf(mpq_map_name, "World\\Maps\\%s\\%s.wdt", map_ids[z].name,
            map_ids[z].name);
        WDT_file wdt;
        ...
        for (uint32 y = 0; y < WDT_MAP_SIZE; ++y)
        {
            for (uint32 x = 0; x < WDT_MAP_SIZE; ++x)
            {
                if (!wdt.main->adt_list[y][x].exist)
                    continue;
                sprintf(mpq_filename, "World\\Maps\\%s\\%s_%u_%u.adt",
                    map_ids[z].name, map_ids[z].name, x, y);
                sprintf(output_filename, "%s/maps/%03u%02u%02u.map",
                    output_path, map_ids[z].id, y, x);
...
void LoadLocaleMPQFiles(int const locale)
{
    ...
    sprintf(filename, "%s/Data/%s/locale-%s.MPQ", input_path, langs[locale],
        langs[locale]);
    ...
    for (int i = 1; i < 5; ++i)
    {
        ...
        sprintf(filename, "%s/Data/%s/patch-%s%s.MPQ", input_path,
            langs[locale], langs[locale], ext);
