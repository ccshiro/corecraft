...
/**************************************************************************/
void TerrainBuilder::getLoopVars(
    Spot portion, int& loopStart, int& loopEnd, int& loopInc)
{
    switch (portion)
    {
    case ENTIRE:
    ...
    uint32 mapID, uint32 tileX, uint32 tileY, MeshData& meshData, Spot portion)
{
    ...
    if (fread(&fheader, sizeof(GridMapFileHeader), 1, mapFile) != 1 ||
    ...
    bool haveTerrain = false, haveLiquid = false;
    ...
    if (haveTerrain)
    {
        ...
        if (hheader.flags & MAP_HEIGHT_AS_INT8)
        {
            ...
            if (count != expected)
                printf(
                    "TerrainBuilder::loadMap: Failed to read V9 and V8 data "
                    "expected count %d, read %d (height format was int8)\n",
                    expected, count);
        ...
        else if (hheader.flags & MAP_HEIGHT_AS_INT16)
        {
            ...
            if (count != expected)
                printf(
                    "TerrainBuilder::loadMap: Failed to read V9 and V8 data "
                    "expected count %d, read %d (height format was int16)\n",
                    expected, count);
        ...
        else
        {
            ...
            if (count != expected)
                printf(
                    "TerrainBuilder::loadMap: Failed to read V9 and V8 data "
                    "expected count %d, read %d (height format was float)\n",
                    expected, count);
        }
        ...
        if (fheader.holesSize != 0)
        {
            ...
            if (fread(holes, fheader.holesSize, 1, mapFile) != 1)
                printf(
                    "TerrainBuilder::loadMap: Failed to read hole data "
                    "expected count 1, read 0\n");
        }
    ...
    if (haveLiquid)
    {
        ...
        if (fread(&lheader, sizeof(GridMapLiquidHeader), 1, mapFile) != 1)
            printf(
                "TerrainBuilder::loadMap: Failed to read liquid header data "
                "expected 1, read 0\n");
        ...
        if (!(lheader.flags & MAP_LIQUID_NO_TYPE))
            if (fread(liquid_type, sizeof(liquid_type), 1, mapFile) != 1)
                printf(
                    "TerrainBuilder::loadMap: Failed to read liquid type data "
                    "expected 1, read 0\n");
        ...
        if (!(lheader.flags & MAP_LIQUID_NO_HEIGHT))
        {
            ...
            int count = fread(liquid_map, sizeof(float), toRead, mapFile);
            if (count != (int)toRead)
                printf(
                    "TerrainBuilder::loadMap: Failed to read liquid map data "
                    "expected %u, read %u\n",
                    count, toRead);
        }
    ...
    for (int i = loopStart; i < loopEnd; i += loopInc)
    {
        for (int j = 0; j < 2; ++j)
        {
            ...
            if (/*!liquid_type ||*/ !meshData.liquidVerts.size() ||
                !ltriangles.size()) // FIXME: liquid_type never null
                useLiquid = false;
            else
            {
            ...
            useLiquid = false; // FIXME: See MapBuilder.cpp FIXME
...
/**************************************************************************/
void TerrainBuilder::getHeightTriangle(
    int square, Spot triangle, int* indices, bool liquid /* = false*/)
{
...
/**************************************************************************/
bool TerrainBuilder::loadVMap(
    uint32 mapID, uint32 tileX, uint32 tileY, MeshData& meshData)
{
    auto vmapManager = new VMapManager2(true);
    ...
    do
    {
        ...
        for (uint32 i = 0; i < count; ++i)
        {
            ...
            WorldModel* worldModel =
                const_cast<WorldModel*>(instance.getWorldModel());
            if (!worldModel)
                continue;
            ...
            // build mesh data for single model instance
            buildSingleModel(
                meshData, worldModel, isM2, scale, position, rotation, false);
        }
    } while (false);
...
void TerrainBuilder::buildSingleModel(MeshData& meshData, WorldModel* model,
    bool isM2, float scale, G3D::Vector3 position, G3D::Matrix3 rotation,
    bool transport)
{
    assert(model);

    vector<GroupModel> groupModels;
    model->getGroupModels(groupModels);
    ...
    for (auto& groupModel : groupModels)
    {
        ...
        liquid = nullptr; // FIXME: This is a temporary fix; water paths are not
                          // handled
        // correctly, and where the surface breaks the pathgen will just
        // stop. The correct way would of course be to fix this instead of
        // ignoring the liquid, but the core currently does special handling
        // of liquid and this isn't even needed (except for one case that's
        // currently a FIXME in the core). When we do need it, this issue
        // needs to be corrected.

        // first handle collision mesh
        if (transport) // don't transform for transports
            transformedVertices = tempVertices;
        else
            transform(
                tempVertices, transformedVertices, scale, rotation, position);
        ...
        if (liquid)
        {
            ...
            for (uint32 x = 0; x < vertsX; ++x)
                ...
                {
                    vert = Vector3(corner.x + x * GRID_PART_SIZE,
                        corner.y + y * GRID_PART_SIZE, data[y * vertsX + x]);
            ...
            {
                meshData.liquidTris.append(liqTris[i * 3 + 1] + liqOffset,
                    liqTris[i * 3 + 2] + liqOffset, liqTris[i * 3] + liqOffset);
...
/**************************************************************************/
void TerrainBuilder::transform(vector<Vector3>& source,
    vector<Vector3>& transformedVertices, float scale, G3D::Matrix3& rotation,
    Vector3& position)
{
    for (auto& elem : source)
    {
        // apply tranform, then mirror along the horizontal axes
        Vector3 v((elem)*rotation * scale + position);
        v.x *= -1.f;
        v.y *= -1.f;
...
/**************************************************************************/
void TerrainBuilder::copyVertices(
    vector<Vector3>& source, G3D::Array<float>& dest)
{
    for (auto& elem : source)
    {
        dest.push_back((elem).y);
        dest.push_back((elem).z);
        dest.push_back((elem).x);
    }
}

/**************************************************************************/
void TerrainBuilder::copyIndices(
    vector<MeshTriangle>& source, G3D::Array<int>& dest, int offset, bool flip)
{
    if (flip)
    {
        for (auto& elem : source)
        {
            dest.push_back((elem).idx2 + offset);
            dest.push_back((elem).idx1 + offset);
            dest.push_back((elem).idx0 + offset);
        }
    }
    else
    {
...
/**************************************************************************/
void TerrainBuilder::copyIndices(
    G3D::Array<int>& source, G3D::Array<int>& dest, int offset)
{
...
/**************************************************************************/
void TerrainBuilder::cleanVertices(
    G3D::Array<float>& verts, G3D::Array<int>& tris)
{
    map<int, int> vertMap;
...
/**************************************************************************/
void TerrainBuilder::loadOffMeshConnections(uint32 mapID, uint32 tileX,
    uint32 tileY, MeshData& meshData, const char* offMeshFilePath)
{
