...
{
    LOG_DEBUG(logging, "DEBUG: Gossip selection, sender: %d, action: %d",
        sender, action);
...
MANGOS_DLL_EXPORT
bool GOGossipSelect(
    Player* pPlayer, GameObject* pGo, uint32 sender, uint32 action)
{
    LOG_DEBUG(logging, "DEBUG: GO Gossip selection, sender: %u, action: %u",
        sender, action);
...
MANGOS_DLL_EXPORT
bool GossipSelectWithCode(Player* player, Creature* _Creature, uint32 sender,
    uint32 action, const char* sCode)
{
    LOG_DEBUG(logging, "DEBUG: Gossip selection, sender: %d, action: %d",
        sender, action);
    ...
    return tmpscript->pGossipSelectWithCode(
        player, _Creature, sender, action, sCode);
}

MANGOS_DLL_EXPORT
bool GOGossipSelectWithCode(Player* pPlayer, GameObject* pGo, uint32 sender,
    uint32 action, const char* sCode)
{
    LOG_DEBUG(logging, "DEBUG: GO Gossip selection, sender: %u, action: %u",
        sender, action);
    ...
    return tmpscript->pGOGossipSelectWithCode(
        pPlayer, pGo, sender, action, sCode);
}
...
MANGOS_DLL_EXPORT
bool ChooseReward(
    Player* player, Creature* _Creature, Quest* _Quest, uint32 opt)
{
    Script* tmpscript = m_scripts[_Creature->GetScriptId()];
...
MANGOS_DLL_EXPORT
bool EffectDummyGameObj(Unit* caster, uint32 spellId, SpellEffectIndex effIndex,
    GameObject* gameObjTarget)
{
    ...
    return tmpscript->pEffectDummyGameObj(
        caster, spellId, effIndex, gameObjTarget);
}

MANGOS_DLL_EXPORT
bool EffectDummyCreature(
    Unit* caster, uint32 spellId, SpellEffectIndex effIndex, Creature* crTarget)
{
...
bool EffectDummyItem(
    Unit* caster, uint32 spellId, SpellEffectIndex effIndex, Item* itemTarget)
{
...
void ScriptedAI::UpdateAI(const uint32)
{
    ...
    if (m_creature->isAlive() && m_creature->SelectHostileTarget() &&
        m_creature->getVictim())
    {
        // If we are within range melee the target
        if (m_creature->IsWithinDistInMap(
                m_creature->getVictim(), ATTACK_DISTANCE))
        {
