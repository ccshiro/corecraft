...
bool SqlStatement::Execute()
{
    ...
    {
        logging.error(
            "SQL ERROR: wrong amount of parameters (%i instead of %i)",
            args->boundParams(), arguments());
        logging.error(
            "SQL ERROR: statement: %s", m_pDB->GetStmtString(ID()).c_str());
        assert(false);
        return false;
    }
...
SqlPlainPreparedStatement::SqlPlainPreparedStatement(
    const std::string& fmt, SqlConnection& conn)
...
void SqlPlainPreparedStatement::bind(const SqlStmtParameters& holder)
{
    ...
    if (m_nParams != static_cast<uint32>(holder.boundParams()))
    {
        assert(false);
        return;
    }
...
void SqlPlainPreparedStatement::DataToString(
    const SqlStmtFieldData& data, std::ostringstream& fmt)
{
    switch (data.type())
    {
    case FIELD_BOOL:
...
void SqlStmtFieldData::set(bool val)
{
    m_type = FIELD_BOOL;
    m_binaryData.ui8 = val;
}

void SqlStmtFieldData::set(uint8 val)
{
    m_type = FIELD_UI8;
    m_binaryData.ui8 = val;
}

void SqlStmtFieldData::set(int8 val)
{
    m_type = FIELD_I8;
    m_binaryData.i8 = val;
}

void SqlStmtFieldData::set(uint16 val)
{
    m_type = FIELD_UI16;
    m_binaryData.ui16 = val;
}

void SqlStmtFieldData::set(int16 val)
{
    m_type = FIELD_I16;
    m_binaryData.i16 = val;
}

void SqlStmtFieldData::set(uint32 val)
{
    m_type = FIELD_UI32;
    m_binaryData.ui32 = val;
}

void SqlStmtFieldData::set(int32 val)
{
    m_type = FIELD_I32;
    m_binaryData.i32 = val;
}

void SqlStmtFieldData::set(uint64 val)
{
    m_type = FIELD_UI64;
    m_binaryData.ui64 = val;
}

void SqlStmtFieldData::set(int64 val)
{
    m_type = FIELD_I64;
    m_binaryData.i64 = val;
}

void SqlStmtFieldData::set(float val)
{
    m_type = FIELD_FLOAT;
    m_binaryData.f = val;
}

void SqlStmtFieldData::set(double val)
{
    m_type = FIELD_DOUBLE;
    m_binaryData.d = val;
}

void SqlStmtFieldData::set(const char* val)
{
    m_type = FIELD_STRING;
    m_szStringData = val;
}

void SqlStmtFieldData::set(std::string val)
{
    m_type = FIELD_STRING;
    m_szStringData = std::move(val);
}
