...
DatabasePostgre::DatabasePostgre()
{
    ...
    if (db_count++ == 0)
    {
        if (!PQisthreadsafe())
        {
            logging.error("FATAL ERROR: PostgreSQL libpq isn't thread-safe.");
            exit(1);
        }
    }
}
...
bool PostgreSQLConnection::Initialize(const char* infoString)
{
    ...
    if (host == ".")
        mPGconn = PQsetdbLogin(NULL,
            port_or_socket_dir == "." ? NULL : port_or_socket_dir.c_str(), NULL,
            NULL, database.c_str(), user.c_str(), password.c_str());
    else
        mPGconn = PQsetdbLogin(host.c_str(), port_or_socket_dir.c_str(), NULL,
            NULL, database.c_str(), user.c_str(), password.c_str());

    /* check to see that the backend connection was successfully made */
    if (PQstatus(mPGconn) != CONNECTION_OK)
    {
        logging.error("Could not connect to Postgre database at %s: %s",
    ...
    LOG_DEBUG(logging, "Connected to Postgre database %s@%s:%s/%s",
        user.c_str(), host.c_str(), port_or_socket_dir.c_str(),
        database.c_str());
    logging.info("PostgreSQL server ver: %d", PQserverVersion(mPGconn));
    return true;
}

bool PostgreSQLConnection::_Query(
    const char* sql, PGresult** pResult, uint64* pRowCount, uint32* pFieldCount)
{
    if (!mPGconn)
        return false;
    ...
    *pResult = PQexec(mPGconn, sql);
    ...
    if (PQresultStatus(*pResult) != PGRES_TUPLES_OK)
    {
        ...
        logging.error("SQL %s", PQerrorMessage(mPGconn));
        PQclear(*pResult);
        return false;
    }
    else
...
QueryResult* PostgreSQLConnection::Query(const char* sql)
{
    ...
    QueryResultPostgre* queryResult =
        new QueryResultPostgre(result, rowCount, fieldCount);
...
bool PostgreSQLConnection::_TransactionCmd(const char* sql)
{
    ...
    {
        logging.error("SQL: %s", sql);
        logging.error("SQL ERROR: %s", PQerrorMessage(mPGconn));
        return false;
    }
    else
    {
...
unsigned long PostgreSQLConnection::escape_string(
