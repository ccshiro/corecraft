...
public:
    ...
    virtual unsigned long escape_string(
...
public:
    ...
    bool AsyncQuery(
        Class* object, void (Class::*method)(QueryResult*), const char* sql);
    template <class Class, typename ParamType1>
    bool AsyncQuery(Class* object,
        void (Class::*method)(QueryResult*, ParamType1), ParamType1 param1,
        const char* sql);
    ...
    bool AsyncQuery(void (*method)(QueryResult*, ParamType1), ParamType1 param1,
        const char* sql);
    ...
    bool AsyncQuery(
        void (*method)(QueryResult*, ParamType1, ParamType2, ParamType3),
        ParamType1 param1, ParamType2 param2, ParamType3 param3,
        const char* sql);
    // PQuery / member
    template <class Class>
    bool AsyncPQuery(Class* object, void (Class::*method)(QueryResult*),
    ...
    bool AsyncPQuery(Class* object,
        void (Class::*method)(QueryResult*, ParamType1, ParamType2, ParamType3),
        ParamType1 param1, ParamType2 param2, ParamType3 param3,
        const char* format, ...) ATTR_PRINTF(7, 8);
    // PQuery / static
    template <typename ParamType1>
    bool AsyncPQuery(void (*method)(QueryResult*, ParamType1),
        ParamType1 param1, const char* format, ...) ATTR_PRINTF(4, 5);
    template <typename ParamType1, typename ParamType2>
    ...
    bool AsyncPQuery(
        void (*method)(QueryResult*, ParamType1, ParamType2, ParamType3),
        ParamType1 param1, ParamType2 param2, ParamType3 param3,
        const char* format, ...) ATTR_PRINTF(6, 7);
    template <class Class>
    // QueryHolder
    ...
    operator bool() const
    {
        return m_Connections.size() && m_pAsyncConn != nullptr;
    }

    // escape string generation
    void escape_string(std::string& str);

    // must be called before first query in thread (one time for thread using
    // one from existing Database objects)
    virtual void ThreadStart();
    // must be called before finish thread run (one time for thread using one
    // from existing Database objects)
    virtual void ThreadEnd();

    // set database-wide result queue. also we should use object-bases and not
    // thread-based result queues
...
protected:
    Database()
      : m_ConnectionPoolSize(1), m_ConnectionIndex(-1), m_pAsyncConn(nullptr),
        m_pResultQueue(nullptr), m_threadBody(nullptr), m_delayThread(nullptr),
        m_bAllowAsyncTransactions(false), m_iStmtIndex(-1), m_logSQL(false),
        m_pingIntervallms(0)
    {
        // m_ConnectionIndex starts at -1 to ensure that our first connection is
        // the 0th in the pool (see Database::getQueryConnection)
    }
    ...
    public:
        ...
        SqlTransaction* init();
        // gets pointer on current transaction object. Returns NULL if
        // transaction was not initiated
    ...
    // per-thread based storage for SqlTransaction object initialization - no
    // locking is required
    typedef boost::thread_specific_ptr<Database::TransHelper> DBTransHelperTSS;
    Database::DBTransHelperTSS m_TransStorage;
    void validate_trans_ptr()
    {
        if (!m_TransStorage.get())
            m_TransStorage.reset(new Database::TransHelper);
    }
    ...
    uint32 m_ConnectionIndex;
    std::recursive_mutex m_ConnectionIndexMutex;

    // lets use pool of connections for sync queries
    typedef std::vector<SqlConnection*> SqlConnectionContainer;
    SqlConnectionContainer m_Connections;

    // only one single DB connection for transactions
    SqlConnection* m_pAsyncConn;

    MaNGOS::locked_queue<MaNGOS::IQueryCallback*>*
        m_pResultQueue;           ///< Transaction queues from diff. threads
    SqlDelayThread* m_threadBody; ///< Pointer to delay sql executer (owned by
    /// m_delayThread)
    boost::thread* m_delayThread; ///< Pointer to executer thread

    bool m_bAllowAsyncTransactions; ///< flag which specifies if async
    /// transactions are enabled

    // PREPARED STATEMENT REGISTRY
    mutable std::recursive_mutex m_stmtMutex;
