...
DatabaseMysql::DatabaseMysql()
{
    ...
    if (db_count++ == 0)
    {
        ...
        {
            logging.error("FATAL ERROR: Used MySQL library isn't thread-safe.");
...
bool MySQLConnection::Initialize(const char* infoString)
{
    ...
    if (!mysqlInit)
    {
        logging.error("Could not initialize Mysql connection");
        return false;
    }
    ...
    if (!mMysql)
    {
        logging.error("Could not connect to MySQL database at %s: %s\n",
            host.c_str(), mysql_error(mysqlInit));
        mysql_close(mysqlInit);
        return false;
    }

    LOG_DEBUG(logging, "Connected to MySQL database %s@%s:%s/%s", user.c_str(),
        host.c_str(), port_or_socket.c_str(), database.c_str());
    logging.info("MySQL client library: %s", mysql_get_client_info());
    logging.info("MySQL server ver: %s ", mysql_get_server_info(mMysql));
    ...
    {
        LOG_DEBUG(logging, "AUTOCOMMIT SUCCESSFULLY SET TO 1");
    }
    else
    {
        LOG_DEBUG(logging, "AUTOCOMMIT NOT SET TO 1");
    }
    /*-------------------------------------*/
    ...
    MYSQL_FIELD** pFields, uint64* pRowCount, uint32* pFieldCount)
{
    ...
    if (mysql_query(mMysql, sql))
    {
        ...
        logging.error("query ERROR: %s", mysql_error(mMysql));
        return false;
    }
    else
    {
        LOG_DEBUG(logging, "[%u ms] SQL: %s",
            WorldTimer::getMSTimeDiff(_s, WorldTimer::getMSTime()), sql);
    }
...
QueryResult* MySQLConnection::Query(const char* sql)
{
    ...
    QueryResultMysql* queryResult =
        new QueryResultMysql(result, fields, rowCount, fieldCount);
...
bool MySQLConnection::Execute(const char* sql)
{
...

    {
        ...
        {
            logging.error("SQL: %s", sql);
            logging.error("SQL ERROR: %s", mysql_error(mMysql));
            return false;
        }
        else
        {
...
unsigned long MySQLConnection::escape_string(
...
MySqlPreparedStatement::MySqlPreparedStatement(
    const std::string& fmt, SqlConnection& conn, MYSQL* mysql)
  : SqlPreparedStatement(fmt, conn), m_pMySQLConn(mysql), m_stmt(NULL),
    m_pInputArgs(NULL), m_pResult(NULL), m_pResultMetadata(NULL)
{
}
...
bool MySqlPreparedStatement::prepare()
{
    ...
    if (!m_stmt)
    {
        logging.error("SQL: mysql_stmt_init() failed ");
        return false;
    }
    ...
    {
        logging.error(
            "SQL: mysql_stmt_prepare() failed for '%s'", m_szFmt.c_str());
        logging.error("SQL ERROR: %s", mysql_stmt_error(m_stmt));
        return false;
    }
    ...
    {
        logging.error("SQL: no meta information for '%s'", m_szFmt.c_str());
        logging.error("SQL ERROR: %s", mysql_stmt_error(m_stmt));
        return false;
    }
...
void MySqlPreparedStatement::bind(const SqlStmtParameters& holder)
{
    ...
    if (m_nParams != static_cast<unsigned int>(holder.boundParams()))
    {
        assert(false);
        return;
    }
    ...
    SqlStmtParameters::ParameterContainer::const_iterator iter_last =
        _args.end();
    for (SqlStmtParameters::ParameterContainer::const_iterator iter =
             _args.begin();
         iter != iter_last; ++iter)
    {
        // bind parameter
    ...
    {
        logging.error("SQL ERROR: mysql_stmt_bind_param() failed\n");
        logging.error("SQL ERROR: %s", mysql_stmt_error(m_stmt));
    }
}
...
{
    assert(m_pInputArgs);
    assert(static_cast<unsigned int>(nIndex) < m_nParams);
...
bool MySqlPreparedStatement::execute()
{
    ...
    {
        logging.error("SQL: cannot execute '%s'", m_szFmt.c_str());
        logging.error("SQL ERROR: %s", mysql_stmt_error(m_stmt));
        return false;
    }
