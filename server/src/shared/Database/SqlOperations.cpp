...
bool SqlQuery::Execute(SqlConnection* conn)
{
    ...
    /// add the callback to the sql result queue of the thread it originated
    /// from
    m_queue->push(m_callback);
...
bool SqlQueryHolder::Execute(MaNGOS::IQueryCallback* callback,
    SqlDelayThread* thread, locked_queue* queue)
{
...
bool SqlQueryHolder::SetQuery(size_t index, const char* sql)
{
    ...
    {
        logging.error("Query index (" SIZEFMTD ") out of range (size: " SIZEFMTD
                      ") for query: %s",
            index, m_queries.size(), sql);
        return false;
    }

    if (m_queries[index].first != NULL)
    {
        logging.error("Attempt assign query to holder index (" SIZEFMTD
...
bool SqlQueryHolder::SetPQuery(size_t index, const char* format, ...)
{
    if (!format)
    {
        logging.error("Query (index: " SIZEFMTD ") is empty.", index);
        return false;
    }
...
QueryResult* SqlQueryHolder::GetResult(size_t index)
{
    ...
    {
        /// the query strings are freed on the first GetResult or in the
        /// destructor
        if (m_queries[index].first != NULL)
        {
...
{
    for (auto& elem : m_queries)
    {
        /// if the result was never used, free the resources
        /// results used already (getresult called) are expected to be deleted
        if (elem.first != NULL)
        {
            delete[](const_cast<char*>(elem.first));
            if (elem.second)
                delete elem.second;
        }
    }
