...
template <class T>
void SQLStorageLoaderBase<T>::convert_str_to_str(
    uint32 /*field_pos*/, char const* src, char*& dst)
{
    if (!src)
    {
        dst = new char[1];
...
template <class T>
template <class S>
void SQLStorageLoaderBase<T>::convert_to_str(
    uint32 /*field_pos*/, S /*src*/, char*& dst)
{
    dst = new char[1];
    *dst = 0;
}
...
template <class T>
template <class D>
void SQLStorageLoaderBase<T>::convert_from_str(
    uint32 /*field_pos*/, char const* /*src*/, D& dst)
{
    dst = 0;
}
...
template <class T>
template <class V>
void SQLStorageLoaderBase<T>::storeValue(
    V value, SQLStorage& store, char* p, uint32 x, uint32& offset)
{
...
template <class T>
void SQLStorageLoaderBase<T>::storeValue(
    char const* value, SQLStorage& store, char* p, uint32 x, uint32& offset)
{
    T* subclass = (static_cast<T*>(this));
    switch (store.dst_format[x])
    {
...
template <class T>
void SQLStorageLoaderBase<T>::Load(
    SQLStorage& store, bool error_at_empty /*= true*/)
{
    uint32 maxi;
    Field* fields;
    std::unique_ptr<QueryResult> result(WorldDatabase.PQuery(
        "SELECT MAX(%s) FROM %s", store.entry_field, store.table));
    if (!result)
    {
        logging.error("Error loading %s table (not exist?)\n", store.table);

        exit(1); // Stop server at loading non exited table or not accessable
                 // table
    }

    maxi = (*result)[0].GetUInt32() + 1;

    result.reset(WorldDatabase.PQuery("SELECT COUNT(*) FROM %s", store.table));
    if (result)
    {
        fields = result->Fetch();
    ...
    result.reset(WorldDatabase.PQuery("SELECT * FROM %s", store.table));

    if (!result)
    {
        if (error_at_empty)
            logging.error("%s table is empty!\n", store.table);
        else
            logging.info("%s table is empty!\n", store.table);
    ...
    if (store.iNumFields != result->GetFieldCount())
    {
        store.RecordCount = 0;
        logging.error(
            "Error in %s table, probably sql file format was updated (there "
            "should be %d fields in sql).\n",
    ...
    auto newIndex = new char* [maxi];
    memset(newIndex, 0, maxi * sizeof(char*));

    auto _data = new char[store.RecordCount * recordsize];
    uint32 count = 0;
    ...
    do
    {
        ...
        for (uint32 x = 0; x < store.iNumFields; x++)
            ...
            case FT_LOGIC:
                storeValue(
                    (bool)(fields[x].GetUInt32() > 0), store, p, x, offset);
                break;
            case FT_BYTE:
            ...
            case FT_STRING:
                storeValue(
                    (char const*)fields[x].GetString(), store, p, x, offset);
                break;
            case FT_NA:
            case FT_NA_BYTE:
                break;
