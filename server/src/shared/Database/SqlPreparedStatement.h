...
public:
    ...
    {
        assert(m_type == FIELD_DOUBLE);
        return m_binaryData.d;
    }
    const char* toStr() const
    {
        assert(m_type == FIELD_STRING);
    ...
    void* buff() const
    {
        return m_type == FIELD_STRING ? (void*)m_szStringData.c_str() :
                                        (void*)&m_binaryData;
    }
    ...
    size_t size() const
    {
        ...
        default:
            throw std::runtime_error(
                "unrecognized type of SqlStmtFieldType obtained");
        }
    }
...
public:
    ...
    // empty SQL statement parameters. In case nParams > 1 - reserve memory for
    // parameters
...
public:
    ...
    SqlStatement(const SqlStatement& index)
      : m_index(index.m_index), m_pDB(index.m_pDB), m_pParams(nullptr)
    {
        if (index.m_pParams)
    ...
    bool PExecute(T t, Args... args)
    {
        arg(t);
        return PExecute(args...);
    }
...
protected:
    // don't allow anyone except Database class to create static SqlStatement
    // objects
    friend class Database;
    SqlStatement(SqlStatementID index, Database& db)
      : m_index(std::move(index)), m_pDB(&db), m_pParams(nullptr)
    {
    }
...
protected:
    SqlPreparedStatement(std::string fmt, SqlConnection& conn)
      : m_nParams(0), m_nColumns(0), m_bIsQuery(false), m_bPrepared(false),
        m_szFmt(std::move(fmt)), m_pConn(conn)
    {
    }
