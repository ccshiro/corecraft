...
public:
    ...
    {
        logging.error("Attempted to %s in ByteBuffer (pos: " SIZEFMTD
...
public:
    ...
    // move
    ByteBuffer(ByteBuffer&& buf) noexcept : _rpos{0}, _wpos{0}
    {
        std::swap(_rpos, buf._rpos);
        std::swap(_wpos, buf._wpos);
        std::swap(_storage, buf._storage);
    }
    ...
    ByteBuffer& operator>>(std::string& value)
    {
        value.clear();
        while (
            rpos() < size()) // prevent crash at wrong string format in packet
        {
            char c = read<char>();
            if (c == 0)
                break;
