...
public:
    ...
    // Time is updated in World::_UpdateGameTime(), which is a single-threaded
    // scenario
    static MANGOS_DLL_SPEC time_t time_no_syscall() { return curr_sys_time; }

    // get time difference between two timestamps
    static inline uint32 getMSTimeDiff(
        const uint32& oldMSTime, const uint32& newMSTime)
    {
...
private:
    ...
    static MANGOS_DLL_SPEC time_t curr_sys_time;
    friend class World;
};
...
// Takes an exact time_t as opposed to the IntervalTimer which takes a diff
class SystemIntervalTimer
{
public:
    void SetInterval(time_t i)
    {
        interval_ = i;
        start_ = WorldTimer::time_no_syscall();
    }

    bool Passed() const
    {
        return start_ + interval_ < WorldTimer::time_no_syscall();
    }
    void Reset() { start_ = WorldTimer::time_no_syscall(); }
