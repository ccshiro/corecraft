...
enum ChatNotify
{
    ...
    // CHAT_YOU_CHANGED_NOTICE           = 0x02,             // "Changed
    // Channel: [%s]";
    ...
    CHAT_THROTTLED_NOTICE = 0x1F, //+ "[%s] The number of messages that can be
    // sent to this channel is limited, please wait
    // to send another message.";
    CHAT_NOT_IN_AREA_NOTICE = 0x20, //+ "[%s] You are not in the correct area
    // for this channel."; -- The user is trying
    // to send a chat to a zone specific channel,
    // and they're not physically in that zone.
    CHAT_NOT_IN_LFG_NOTICE = 0x21, //+ "[%s] You must be queued in looking for
    // group before joining this channel."; -- The
    // user must be in the looking for group
...
public:
    ...
    void Kick(ObjectGuid good, const char* badname)
    {
        KickOrBan(good, badname, false);
    }
    void Ban(ObjectGuid good, const char* badname)
    {
    ...
    void SetModerator(ObjectGuid p, const char* newname)
    {
        SetMode(p, newname, true, true);
    }
    void UnsetModerator(ObjectGuid p, const char* newname)
    {
        SetMode(p, newname, true, false);
    }
    void SetMute(ObjectGuid p, const char* newname)
    {
        SetMode(p, newname, false, true);
    }
    void UnsetMute(ObjectGuid p, const char* newname)
    {
...
private:
    ...
    void MakePlayerNotFound(
        WorldPacket* data, const std::string& name); //+ 0x09
    void MakeNotOwner(WorldPacket* data);            //? 0x0A
    void MakeChannelOwner(WorldPacket* data);        //? 0x0B
    void MakeModeChange(
        WorldPacket* data, ObjectGuid guid, uint8 oldflags);       //+ 0x0C
    ...
    void MakePlayerKicked(
        WorldPacket* data, ObjectGuid bad, ObjectGuid good); //? 0x12
    void MakeBanned(WorldPacket* data);                      //? 0x13
    void MakePlayerBanned(
        WorldPacket* data, ObjectGuid bad, ObjectGuid good); //? 0x14
    void MakePlayerUnbanned(
        WorldPacket* data, ObjectGuid bad, ObjectGuid good);            //? 0x15
