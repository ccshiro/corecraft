...
void LoadSkillExtraItemTable()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                logging.error(
                    "Skill specialization %u has nonexistent spell id in "
            ...
            {
                logging.error(
                    "Skill specialization %u have nonexistent required "
                    "specialization spell id %u in "
                    "`skill_extra_item_template`!",
            ...
            {
                logging.error(
                    "Skill specialization %u has too low additional create "
            ...
            {
                logging.error(
                    "Skill specialization %u has 0 max number of extra items "
                    "in `skill_extra_item_template`!",
                    spellId);
                continue;
            }

            SkillExtraItemEntry& skillExtraItemEntry =
                SkillExtraItemStore[spellId];
    ...
    logging.info("Loaded %u spell specialization definitions\n", count);
}
