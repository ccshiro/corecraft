...
enum PetSaveMode
{
    ...
    PET_SAVE_DISMISS_PET =
        102 // same effect as PET_SAVE_NOT_IN_SLOT, but keeps auras
};
...
// TODO: I think we can remove PetSpellState altogether; it is no longer needed
//       by Pet::_SaveSpells().
enum PetSpellState
{
...
struct PetSpell
{
    uint8 active; // use instead enum (not good use *uint8* limited enum in case
                  // when value in enum not possitive in *int8*)
...
enum PetTalk
{
    PET_TALK_ORDERED_SPELL_CAST = 0,
    PET_TALK_ORDERED_ATTACK = 1,
};
...
public:
    ...
    bool LoadPetFromDB(Player* owner, uint32 petentry = 0, uint32 pet_guid = 0,
        bool current = false);
    void SavePetToDB(PetSaveMode mode);
    void Unsummon(PetSaveMode mode, Unit* owner = nullptr);

    void SetDeathState(DeathState s) override; // overwrite virtual
                                               // Creature::SetDeathState and
                                               // Unit::SetDeathState
    ...
    void RegenerateAll(
        uint32 update_diff) override; // overwrite Creature::RegenerateAll
    ...
    void init_spell_bonus();
    int32 spell_bonus() { return spell_bonus_; }
    void spell_bonus(int32 damage) { spell_bonus_ = damage; }
    int32 owner_bonus_ap();

    // Some guardians have damage and stats modified by owner's stats
    void InitGuardianStatsFromOwner(Unit* owner);
    ...
    void _LoadSpells(const PetDbData* data);
    void _SaveSpells(PetDbData* data);
    void _LoadSpellCooldowns(const PetDbData* data);
    void _SaveSpellCooldowns(PetDbData* data);
    void _LoadAuras(const PetDbData* data, uint32 timediff, bool only_passive);
    void _SaveAuras(PetDbData* data);
    ...
    // overwrite Creature function for name localization back to WorldObject
    ...
    bool AIM_Initialize() override;

    void init_pet_template_data();

    loot_distributor* GetLootDistributor() const override
    {
        return m_lootDistributor;
    } // We need to override Creature::GetLootDistributor()
...
protected:
    ...
    PetType m_petType;
    int32 m_duration; // time until unsummon (used mostly for summoned guardians
                      // and not used for controlled pets)
    int32 m_loyaltyPoints;
    int32 spell_bonus_;
    uint64 m_auraUpdateMask;
    bool m_loading;

    const CreatureInfo* cinfo_;

    DeclinedName* m_declinedname;

    // Some pets have no need to be saved
    bool ShouldDBSavePet(uint32 entry = 0) const;
