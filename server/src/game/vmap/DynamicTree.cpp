...
struct DynamicTreeIntersectionCallback_WithLogger
{
    ...
    {
        LOG_DEBUG(logging, "Dynamic Intersection log");
    }
    bool operator()(const Ray& r, const GameObjectModel& obj, float& distance)
    {
        LOG_DEBUG(logging, "testing intersection with %s", obj.name.c_str());
        bool hit = obj.intersectRay(r, distance, true);
        if (hit)
        {
            did_hit = true;
            LOG_DEBUG(logging, "result: intersects");
        }
        return hit;
    }
    bool didHit() const { return did_hit; }
...
float DynamicMapTree::getHeight(
    float x, float y, float z, float maxSearchDist) const
{
    Vector3 v(x, y, z);
...
bool DynamicMapTree::checkCollision(float& out, float x1, float y1, float z1,
