...
WmoLiquid::WmoLiquid(
    uint32 width, uint32 height, const Vector3& corner, uint32 type)
...
GroupModel::GroupModel(const GroupModel& other)
  : iBound(other.iBound), iMogpFlags(other.iMogpFlags),
    ...
    triangles(other.triangles), meshTree(other.meshTree), iLiquid(nullptr)
{
    if (other.iLiquid)
        iLiquid = new WmoLiquid(*other.iLiquid);
}

void GroupModel::setMeshData(
    std::vector<Vector3>& vert, std::vector<MeshTriangle>& tri)
{
    vertices.swap(vert);
    triangles.swap(tri);
...
{
    GModelRayCallback(
        const std::vector<MeshTriangle>& tris, const std::vector<Vector3>& vert)
...
bool GroupModel::IntersectRay(
    const G3D::Ray& ray, float& distance, bool stopAtFirstHit) const
{
    if (!triangles.size())
        return false;
...
bool GroupModel::IsInsideObject(
    const Vector3& pos, const Vector3& down, float& z_dist) const
{
...
bool WorldModel::IntersectRay(
    const G3D::Ray& ray, float& distance, bool stopAtFirstHit) const
{
    // small M2 workaround, maybe better make separate class with virtual
    // intersection funcs
    // in any case, there's no need to use a bound tree if we only have one
    // submodel
...
public:
    ...
    void operator()(const Vector3& point, uint32 entry)
    {
        ...
        if (prims[entry].IsInsideObject(point, zVec, group_Z))
        {
            ...
            printf(
                "%10u %8X %7.3f,%7.3f,%7.3f | %7.3f,%7.3f,%7.3f | z=%f, "
                "p_z=%f\n",
                gm.GetWmoID(), gm.GetMogpFlags(), gm.GetBound().low().x,
                gm.GetBound().low().y, gm.GetBound().low().z,
                gm.GetBound().high().x, gm.GetBound().high().y,
                gm.GetBound().high().z, group_Z, point.z);
#endif
        }
        //}
