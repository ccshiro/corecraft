...
public:
    ...
    GroupModel(uint32 mogpFlags, uint32 groupWMOID, AABox bound)
      : iBound(std::move(bound)), iMogpFlags(mogpFlags),
        iGroupWMOID(groupWMOID), iLiquid(nullptr)
    {
    }
...
public:
    void getMeshData(std::vector<Vector3>& vertices,
        std::vector<MeshTriangle>& triangles, WmoLiquid*& liquid);
};
...
public:
    ...
    uint32 getRootWmoID() const { return RootWMOID; }
    const std::vector<GroupModel>& getGroupModels() const
    {
        return groupModels;
    }
