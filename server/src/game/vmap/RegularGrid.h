...
#define CELL_SIZE float(HGRID_MAP_SIZE / (float)CELL_NUMBER)
    ...
    void intersectRay(
        const Ray& ray, RayCallback& intersectCallback, float max_dist)
    {
        intersectRay(ray, intersectCallback, max_dist,
            ray.origin() + ray.direction() * max_dist);
    }

    template <typename RayCallback>
    void intersectRay(const Ray& ray, RayCallback& intersectCallback,
        float& max_dist, const Vector3& end)
    {
        ...
        // int Cycles =
        // std::max((int)ceilf(max_dist/tMaxX),(int)ceilf(max_dist/tMaxY));
        // int i = 0;
