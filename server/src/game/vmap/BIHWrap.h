...
class BIHWrap
{
    ...
    struct MDLCallback
    {
        ...
        uint32 objects_size_;

        MDLCallback(RayCallback& callback, const T* const* objects_array,
            uint32 objects_size)
        ...
        bool operator()(
            const Ray& ray, uint32 Idx, float& MaxDist, bool /*stopAtFirst*/)
        {
            if (Idx >= objects_size_)
                return false;
...
public:
    ...
    void intersectRay(
        const Ray& ray, RayCallback& intersectCallback, float& maxDist)
    {
        balance();
