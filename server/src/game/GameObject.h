...
struct GameObjectInfo
{
    ...
    uint32 GetCooldown()
        const // not triggering at detection target or use until coolodwn expire
    {
        switch (type)
        {
        case GAMEOBJECT_TYPE_TRAP:
...
enum GOState
{
    // Shiro: Doors seem to always be READY if closed and ACTIVE if open; can't
    // find any door that has a different default state (despite what these two
    // comments say...)
    GO_STATE_ACTIVE =
...
// ->GO_JUST_DEACTIVATED->GO_READY        -> ...
// For bobber:      GO_NOT_READY  ->GO_READY (close)->GO_ACTIVATED (open)
// ->GO_JUST_DEACTIVATED-><deleted>
// For door(closed):[GO_NOT_READY]->GO_READY (close)->GO_ACTIVATED (open)
// ->GO_JUST_DEACTIVATED->GO_READY(close) -> ...
// For door(open):  [GO_NOT_READY]->GO_READY (open) ->GO_ACTIVATED
...
public:
    ...
    bool Create(uint32 guidlow, uint32 name_id, Map* map, float x, float y,
        float z, float ang, float rotation0 = 0.0f, float rotation1 = 0.0f,
        float rotation2 = 0.0f, float rotation3 = 0.0f,
        uint32 animprogress = GO_ANIMPROGRESS_DEFAULT,
    ...
    bool HasStaticDBSpawnData()
        const; // listed in `gameobject` table and have fixed in DB guid
    ...
    void SetRitualTargetGuid(ObjectGuid guid);
    ...
    {
        m_respawnTime =
            respawn > 0 ? WorldTimer::time_no_syscall() + respawn : 0;
    ...
    // Functions spawn/remove gameobject with DB guid in all loaded map copies
    // (if point grid loaded in map)
    static void AddToRemoveListInMaps(
    ...
    void Use(Unit* user, bool bypass_no_interact = false,
        uint32 spell_misc_value = 0);

    LootState getLootState() const { return m_lootState; }
    void SetLootState(LootState s, Unit* user = nullptr);
...
protected:
    ...
    uint32 m_respawnDelayTime; // (secs) if 0 then current GO state no dependent
                               // from timer
    bool m_temporary;          // Deleted when despawned
    LootState m_lootState;
    bool m_spawnedByDefault;
    bool m_ritualStarted; // used to determine if enough particpants in a ritual
                          // is enough to start it
    uint32 m_ritualTime;  // used in rituals to properly set the ritual time,
                          // only ticks when ritual has started
    time_t m_cooldownTime; // used as internal reaction delay time store (not
                           // state change reaction).
    // For traps/goober this: spell casting cooldown, for doors/buttons: reset
    // time.
    ...
    ObjectGuid m_firstUser; // first GO user, in most used cases owner, but in
                            // some cases no, for example non-summoned multi-use
                            // GAMEOBJECT_TYPE_SUMMONING_RITUAL
    GuidsSet m_UniqueUsers; // all players who use item, some items activated
                            // after specific amount unique uses
...
private:
    ...
    void UpdateModel(); // updates model in case displayId was changed

    bool IsTrapVisibleToPlayer(
        const Player* p, const WorldObject* viewPoint) const;

    GameObjectAI* m_AI;

    time_t despawn_time_; // GO's that contain loot despawn after they've first
                          // been opened

    ObjectGuid ritual_target_;
};
#endif
