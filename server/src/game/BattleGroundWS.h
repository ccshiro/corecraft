...
enum BG_WS_FlagObjects
{
    BG_WS_FLAG_ALLIANCE_BASE =
        179830, // Alliance (blue) flag when it's in the base
    BG_WS_FLAG_HORDE_BASE = 179831, // Horde (red) flag when it's in the base
    BG_WS_FLAG_ALLIANCE_FIELD = 179785, // Alliance flag in the open field
    BG_WS_FLAG_HORDE_FIELD = 179786     // Horde flag in the open field
};
...
// These respawn timers are set in the database
/*enum BS_WS_BuffRespawnTimers
{
    BG_WS_SPEED_BUFF_TIMER         = 150,
    BG_WS_BERSERKER_TIMER          = 120,
    BG_WS_RESTORATION_TIMER        = 18,
};*/
...
public:
    /* Construction */
    BattleGroundWS(const battlefield::bracket& bracket);
    ~BattleGroundWS();
    ...
    /* BG Flags */
    ObjectGuid GetAllianceFlagPickerGuid() const
    {
        return m_FlagKeepers[BG_TEAM_ALLIANCE];
    }
    ObjectGuid GetHordeFlagPickerGuid() const
    {
    ...
    {
        return !m_FlagKeepers[BG_TEAM_ALLIANCE].IsEmpty();
    }
    bool IsHordeFlagPickedup() const
    {
        return !m_FlagKeepers[BG_TEAM_HORDE].IsEmpty();
    }
