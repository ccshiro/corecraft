...
Quest::Quest(Field* questRecord)
{
    ...
    for (auto& elem : RewItemId)
    {
        if (elem)
            ++m_rewitemscount;
    }

    for (auto& elem : RewChoiceItemId)
    {
        if (elem)
...
bool Quest::IsAllowedInRaid() const
{
    ...
    return sWorld::Instance()->getConfig(CONFIG_BOOL_QUEST_IGNORE_RAID);
}
...
uint32 Quest::GetCharTitleBitIndex() const
{
    ...
    CharTitlesEntry const* titleEntry =
        sCharTitlesStore.LookupEntry(CharTitleId);
