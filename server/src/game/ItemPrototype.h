...
enum ItemSpelltriggerType
{
    ...
    ITEM_SPELLTRIGGER_ON_STORE = 5, // casted at add item to inventory/equip,
                                    // applied aura removed at remove item, item
                                    // deleted at aura cancel/expire/etc
    ITEM_SPELLTRIGGER_LEARN_SPELL_ID = 6 // used in Item_template.spell_2 with
                                         // spell_id with SPELL_GENERIC_LEARN in
                                         // spell_1
};
...
enum ItemPrototypeFlags
{
    ...
    ITEM_FLAG_LOOTABLE = 0x00000004, // affect only non container items that can
                                     // be "open" for loot. It or lockid set
                                     // enable for client show "Right click to
                                     // open". See also ITEM_DYNFLAG_UNLOCKED
    ITEM_FLAG_UNK3 = 0x00000008,     // not used in pre-3.x
    ITEM_FLAG_UNK4 = 0x00000010,     // can't repeat old note: appears red icon
                                     // (like when item durability==0)
    ITEM_FLAG_INDESTRUCTIBLE = 0x00000020, // used for totem. Item can not be
                                           // destroyed, except by using spell
                                           // (item can be reagent for spell and
                                           // then allowed)
    ...
    ITEM_FLAG_IGNORE_BAG_SPACE =
        0x00000400, // ignore bag space at new item creation?
    ...
    ITEM_FLAG_PROSPECTABLE = 0x00040000, // item can have prospecting loot (in
                                         // fact some items expected have empty
                                         // loot)
    ITEM_FLAG_UNIQUE_EQUIPPED = 0x00080000,
    ITEM_FLAG_UNK20 = 0x00100000,
    ITEM_FLAG_USEABLE_IN_ARENA = 0x00200000,
    ITEM_FLAG_THROWABLE =
        0x00400000, // Only items of ITEM_SUBCLASS_WEAPON_THROWN have it but not
                    // all, so can't be used as in game check
...
const uint32 MaxItemSubclassValues[MAX_ITEM_CLASS] = {
    ...
    MAX_ITEM_SUBCLASS_WEAPON, MAX_ITEM_SUBCLASS_GEM, MAX_ITEM_SUBCLASS_ARMOR,
    MAX_ITEM_SUBCLASS_REAGENT, MAX_ITEM_SUBCLASS_PROJECTILE,
    MAX_ITEM_SUBCLASS_TRADE_GOODS, MAX_ITEM_SUBCLASS_GENERIC,
    MAX_ITEM_SUBCLASS_RECIPE, MAX_ITEM_SUBCLASS_MONEY, MAX_ITEM_SUBCLASS_QUIVER,
    MAX_ITEM_SUBCLASS_QUEST, MAX_ITEM_SUBCLASS_KEY, MAX_ITEM_SUBCLASS_PERMANENT,
    MAX_ITEM_SUBCLASS_JUNK};

inline uint8 ItemSubClassToDurabilityMultiplierId(
    uint32 ItemClass, uint32 ItemSubClass)
{
    switch (ItemClass)
    {
...
enum ItemExtraFlags
{
    ITEM_EXTRA_NON_CONSUMABLE = 0x01, // use as additional flag to
                                      // spellcharges_N negative values, item
                                      // not expire at no chanrges
    ITEM_EXTRA_REAL_TIME_DURATION =
        0x02, // if set and have Duration time, then offline time included in
              // counting, if not set then counted only in game time

    ITEM_EXTRA_ALL =
        0x03 // all used flags, used for check DB data (mask all above flags)
};
...
struct ItemPrototype
{
    ...
    uint32 GetSkill() const
    {
        const static uint32 item_weapon_skills[MAX_ITEM_SUBCLASS_WEAPON] = {
            ...
            SKILL_STAVES, 0, 0, SKILL_UNARMED, 0, SKILL_DAGGERS, SKILL_THROWN,
            SKILL_ASSASSINATION, SKILL_CROSSBOWS, SKILL_WANDS, SKILL_FISHING};
