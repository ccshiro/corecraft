...
{
    GameEventData()
      : start(1), end(0), occurence(0), length(0), holiday_id(HOLIDAY_NONE)
    {
    }
    time_t start;
    time_t end;
...
struct GameEventMail
{
    ...
    GameEventMail(uint32 _raceMask, uint32 _quest, uint32 _mailTemplateId,
        uint32 _senderEntry)
      : raceMask(_raceMask), questId(_quest), mailTemplateId(_mailTemplateId),
...
public:
    ...
    GameEventCreatureData const* GetCreatureUpdateDataForActiveEvent(
...
protected:
    ...
    typedef std::pair<GameEventCreatureDataPerGuidMap::const_iterator,
        GameEventCreatureDataPerGuidMap::const_iterator>
        GameEventCreatureDataPerGuidBounds;
    ...
    GameEventCreatureDataMap
        mGameEventCreatureData; // events size, only positive event case
    ...
    GameEventIdMap
        mGameEventSpawnPoolIds; // events size, only positive event case
    GameEventDataMap mGameEvent;
    ActiveEvents m_ActiveEvents;
    bool m_IsGameEventsInit;
};

#define sGameEventMgr MaNGOS::UnlockedSingleton<GameEventMgr>
