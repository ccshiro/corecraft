...
BattleGroundAB::BattleGroundAB(const battlefield::bracket& bracket)
  : specification_(battlefield::arathi_basin, bracket)
{
    SetMapId(529);
    SetTypeID(BATTLEGROUND_AB);
    SetArenaorBGType(false);
    SetMinPlayersPerTeam(8);
    SetMaxPlayersPerTeam(15);
    SetMinPlayers(16);
    SetMaxPlayers(30);
    SetName("Arathi Basin");
    SetTeamStartLoc(ALLIANCE, 1313.9f, 1310.74f, -9.01043, 3.40156f);
    SetTeamStartLoc(HORDE, 684.014f, 681.22f, -12.9159f, 0.263892f);
    SetLevelRange(10, 70);
    ...
    // Stuff that was in BattleGroundAB::Reset previously:
    ...
    m_HonorTics = (isBGWeekend) ? BG_AB_ABBGWeekendHonorTicks :
                                  BG_AB_NotABBGWeekendHonorTicks;
    m_ReputationTics = (isBGWeekend) ? BG_AB_ABBGWeekendReputationTicks :
    ...
    // End of stuff that was in BattleGroundAB::Reset previously
}
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        for (int node = 0; node < BG_AB_NODES_MAX; ++node)
        {
            ...
            if (m_NodeTimers[node])
            {
                ...
                else
                {
                    ...
                    _CreateBanner(
                        node, BG_AB_NODE_TYPE_OCCUPIED, teamIndex, true);
                    ...
                    if (teamIndex == 0)
                    {
                        SendMessage2ToAll(LANG_BG_AB_NODE_TAKEN,
                            CHAT_MSG_BG_SYSTEM_ALLIANCE, nullptr, LANG_BG_ALLY,
                            _GetNodeNameId(node));
                    ...
                    {
                        SendMessage2ToAll(LANG_BG_AB_NODE_TAKEN,
                            CHAT_MSG_BG_SYSTEM_HORDE, nullptr, LANG_BG_HORDE,
                            _GetNodeNameId(node));
        ...
        for (int team = 0; team < BG_TEAMS_COUNT; ++team)
        {
            ...
            if (m_lastTick[team] > BG_AB_TickIntervals[points])
            {
                ...
                if (team == BG_TEAM_ALLIANCE)
                    UpdateWorldState(
                        BG_AB_OP_RESOURCES_ALLY, m_TeamScores[team]);
                if (team == BG_TEAM_HORDE)
                    UpdateWorldState(
                        BG_AB_OP_RESOURCES_HORDE, m_TeamScores[team]);
            }
        }
...
void BattleGroundAB::StartingEventCloseDoors()
{
    ...
    for (int i = 0; i < BG_AB_NODES_MAX * 3; ++i)
        SpawnBGObject(
            m_BgObjects[BG_AB_OBJECT_SPEEDBUFF_STABLES + i], RESPAWN_ONE_WEEK);
}
...
void BattleGroundAB::StartingEventOpenDoors()
{
    for (int i = 0; i < BG_AB_NODES_MAX; ++i)
    {
        ...
        SpawnBGObject(
            m_BgObjects[BG_AB_OBJECT_SPEEDBUFF_STABLES + buff + i * 3],
            RESPAWN_IMMEDIATELY);
    }
...
void BattleGroundAB::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    // break;
    default:
        // logging.error("WARNING: Unhandled AreaTrigger in
...
void BattleGroundAB::_CreateBanner(
    uint8 node, uint8 type, uint8 teamIndex, bool delay)
{
    // Just put it into the queue
    if (delay)
    {
        // FIXME: Timer cannot be set to 0 or it will never trigger.
        // But there should be no timer for the flags to burn up.
        m_BannerTimers[node].timer = 1;
...
void BattleGroundAB::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    ...
    for (uint8 node = 0; node < BG_AB_NODES_MAX; ++node)
        FillInitialWorldState(
            data, count, BG_AB_OP_NODEICONS[node], m_Nodes[node] == 0);

    // Node occupied states
    for (uint8 node = 0; node < BG_AB_NODES_MAX; ++node)
        for (uint8 i = 1; i < BG_AB_NODES_MAX; ++i)
            FillInitialWorldState(data, count,
                BG_AB_OP_NODESTATES[node] + plusArray[i], m_Nodes[node] == i);

    // How many bases each team owns
    uint8 ally = 0, horde = 0;
    for (auto& elem : m_Nodes)
        if (elem == BG_AB_NODE_STATUS_ALLY_OCCUPIED)
            ++ally;
        else if (elem == BG_AB_NODE_STATUS_HORDE_OCCUPIED)
            ++horde;
    ...
    // Team scores
    FillInitialWorldState(
        data, count, BG_AB_OP_RESOURCES_MAX, BG_AB_MAX_TEAM_SCORE);
    FillInitialWorldState(data, count, BG_AB_OP_RESOURCES_WARNING,
        BG_AB_WARNING_NEAR_VICTORY_SCORE);
    FillInitialWorldState(
        data, count, BG_AB_OP_RESOURCES_ALLY, m_TeamScores[BG_TEAM_ALLIANCE]);
    FillInitialWorldState(
        data, count, BG_AB_OP_RESOURCES_HORDE, m_TeamScores[BG_TEAM_HORDE]);
...
void BattleGroundAB::EventPlayerClickedOnFlag(
    Player* source, GameObject* target_obj)
{
    ...
    if (m_Nodes[node] == BG_AB_NODE_TYPE_NEUTRAL)
    {
        ...
        if (teamIndex == 0)
            ...
            SendMessage2ToAll(LANG_BG_AB_NODE_CLAIMED, CHAT_MSG_BG_SYSTEM_HORDE,
                source, _GetNodeNameId(node), LANG_BG_HORDE);
             ...
             (m_Nodes[node] == BG_AB_NODE_STATUS_HORDE_CONTESTED))
    {
        ...
        else
        {
            ...
            m_NodeTimers[node] = 0;
            _NodeOccupied(
                node, (teamIndex == BG_TEAM_ALLIANCE) ? ALLIANCE : HORDE);
    ...
    if (m_Nodes[node] >= BG_AB_NODE_TYPE_OCCUPIED)
    {
        if (teamIndex == BG_TEAM_ALLIANCE)
            ...
            SendMessage2ToAll(LANG_BG_AB_NODE_TAKEN, CHAT_MSG_BG_SYSTEM_HORDE,
                nullptr, LANG_BG_HORDE, _GetNodeNameId(node));
    }
...
bool BattleGroundAB::SetupBattleGround()
{
    ...
    for (int i = 0; i < BG_AB_NODES_MAX; ++i)
    {
        if (!AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i, Buff_Entries[0],
                ...
                cos(BG_AB_BuffPositions[i][3] / 2), RESPAWN_ONE_WEEK) ||
            ...
            !AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i + 2,
                ...
                sin(BG_AB_BuffPositions[i][3] / 2),
                cos(BG_AB_BuffPositions[i][3] / 2), RESPAWN_ONE_WEEK))
            logging.error("BatteGroundAB: Failed to spawn buff object!");
    }
...
WorldSafeLocsEntry const* BattleGroundAB::GetClosestGraveYard(Player* player)
{
    ...
    if (!nodes.empty())
    {
        ...
        for (auto& node : nodes)
        {
            WorldSafeLocsEntry const* entry =
                sWorldSafeLocsStore.LookupEntry(BG_AB_GraveyardIds[node]);
            if (!entry)
                continue;
    ...
    if (!good_entry)
        good_entry =
            sWorldSafeLocsStore.LookupEntry(BG_AB_GraveyardIds[teamIndex + 5]);

    return good_entry;
}

void BattleGroundAB::UpdatePlayerScore(
