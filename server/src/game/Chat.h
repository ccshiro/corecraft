...
public:
    const char* Name;
    uint32
        SecurityLevel; // function pointer required correct align (use uint32)
    bool AllowConsole;
...
public:
    ...
    static void FillMessageData(WorldPacket* data, WorldSession* session,
        uint8 type, uint32 language, const char* channelName,
        ObjectGuid targetGuid, const char* message, Unit* speaker);

    static void FillMessageData(WorldPacket* data, WorldSession* session,
        uint8 type, uint32 language, ObjectGuid targetGuid, const char* message)
    {
        FillMessageData(data, session, type, language, nullptr, targetGuid,
            message, nullptr);
    }

    static void FillMessageData(WorldPacket* data, WorldSession* session,
        uint8 type, uint32 language, const char* message)
    {
        FillMessageData(data, session, type, language, nullptr, ObjectGuid(),
            message, nullptr);
    }

    void FillSystemMessageData(WorldPacket* data, const char* message)
    {
        FillMessageData(data, m_session, CHAT_MSG_SYSTEM, LANG_UNIVERSAL,
            ObjectGuid(), message);
    }
...
protected:
    ...
    bool HasLowerSecurity(
        Player* target, ObjectGuid guid = ObjectGuid(), bool strong = false);
    bool HasLowerSecurityAccount(
        WorldSession* target, uint32 account, bool strong = false);

    void SendGlobalSysMessage(const char* str);

    bool SetDataForCommandInTable(ChatCommand* table, const char* text,
        uint32 security, std::string const& help);
    void ExecuteCommand(const char* text);
    void LogCommand(char const* fullcmd);

    bool ShowHelpForCommand(ChatCommand* table, const char* cmd);
    bool ShowHelpForSubCommands(ChatCommand* table, char const* cmd);
    ChatCommandSearchResult FindCommand(ChatCommand* table, char const*& text,
        ChatCommand*& command, ChatCommand** parentCommand = nullptr,
        std::string* cmdNamePtr = nullptr, bool allAvailable = false,
    ...
    bool HandleBanWaveAddCommand(char* args);
    bool HandleBanWavePrintCommand(char* args);
    bool HandleBanWaveRemoveCommand(char* args);
    bool HandleBanWaveRunCommand(char* args);
    bool HandleBanWaveSearchCommand(char* args);
    ...
    bool HandleDebugBattlefieldQueueCommand(char* args);
    ...
    bool HandleDebugItemDisplay(char* args);
    bool HandleDebugItemDurabilityCommand(char* args);
    bool HandleDebugModItemValueCommand(char* args);
    bool HandleDebugModValueCommand(char* args);
    bool HandleDebugPvPCommand(char* args);
    bool HandleDebugSamplingMapCommand(char* args);
    ...
    bool HandleDebugThreadedMaps(char* args);
    bool HandleDebugThreatlist(char* args);
    bool HandleDebugUpdateWorldStateCommand(char* args);
    bool HandleDebugWardenCommand(char* args);
    bool HandleDebugWmolistCommand(char* args);
    bool HandleDebugLoSCommand(char* args);
    bool HandleDebugSendResetFailCommand(char* args);
    bool HandleDebugSendDifficultyCommand(char* args);

    // .debug pathing command table
    bool HandleDebugPathingAdtCommand(char* args);
    bool HandleDebugPathingPathCommand(char* args);
    bool HandleDebugPathingPositionCommand(char* args);
    bool HandleDebugPathingWmoCommand(char* args);

    // .debug play command table
    bool HandleDebugPlayCinematicCommand(char* args);
    bool HandleDebugPlaySoundCommand(char* args);

    bool HandleDebugSendAuctionHouseError(char* args);
    ...
    bool HandleInstanceResetLimitCommand(char* args);
    ...
    bool HandleLookupTplocCommand(char* args);
    ...
    // Arena Commands
    // Team subcommand
    bool HandleArenaTeamCreateCommand(char* args);
    bool HandleArenaTeamListCommand(char* args);
    bool HandleArenaTeamRatingCommand(char* args);

    // Teleport commands
    bool HandleTpCommand(char* args);
    bool HandleSummonCommand(char* args);
    bool HandleTplocAddCommand(char* args);
    bool HandleTplocDelCommand(char* args);

    //-----------------------Npc Commands-----------------------
    bool HandleNpcAddCommand(char* args);
    bool HandleNpcAddMoveCommand(char* args);
    bool HandleNpcAddVendorItemCommand(char* args);
    bool HandleNpcAggroLinkCommand(char* args);
    bool HandleNpcAIInfoCommand(char* args);
    bool HandleNpcAllowMovementCommand(char* args);
    bool HandleNpcBossLinkCommand(char* args);
    ...
    bool HandleNpcLeashCommand(char* args);
    ...
    bool HandlePetCommand(char* args);
    ...
    bool HandleSpyCommand(char* args);
    ...
    bool HandleMobConvertCommand(char* args);
    bool HandleMovegensCommand(char* args);
    bool HandleSplinemakeCommand(char* args);
    bool HandleSplineplayCommand(char* args);
    ...
    bool HandleMaxProfCommand(char* args);
    ...
    bool HandleNpcGroupCreateCommand(char* args);
    bool HandleNpcGroupDeleteCommand(char* args);
    bool HandleNpcGroupListCommand(char* args);
    bool HandleNpcGroupRenameCommand(char* args);
    bool HandleNpcGroupAddCommand(char* args);
    bool HandleNpcGroupRemoveCommand(char* args);
    bool HandleNpcGroupLeaderCommand(char* args);
    bool HandleNpcGroupInfoCommand(char* args);
    bool HandleNpcGroupFlagListCommand(char* args);
    bool HandleNpcGroupFlagAddCommand(char* args);
    bool HandleNpcGroupFlagRemoveCommand(char* args);
    bool HandleNpcGroupShowCommand(char* args);
    bool HandleNpcGroupUnshowCommand(char* args);
    bool HandleNpcGroupGoCommand(char* args);
    bool HandleNpcGroupWpAddCommand(char* args);
    bool HandleNpcGroupWpMoveCommand(char* args);
    bool HandleNpcGroupWpRemoveCommand(char* args);
    bool HandleNpcGroupWpShowCommand(char* args);
    bool HandleNpcGroupWpUnshowCommand(char* args);
    bool HandleNpcGroupWpEditCommand(char* args);

    //-----------------------Smart AI Commands-----------------------
    bool HandleSmartAIAddWp(char* args);
    bool HandleSmartAIAddGwp(char* args);

    //----------------------Ticket Chat Handler----------------------
    bool HandleTicketCheckin(char* args);
    bool HandleTicketCheckout(char* args);
    bool HandleTicketComment(char* args);
    bool HandleTicketPnote(char* args);
    bool HandleTicketPong(char* args);
    bool HandleTicketResolve(char* args);
    bool HandleTicketSay(char* args);
    ...
    // extraction different type params from args string, all functions update
    // (char** args) to first unparsed tail symbol at return
    ...
    // literal string (until whitespace and not started from "['|), any or 'lit'
    // if provided
    char* ExtractQuotedOrLiteralArg(char** args, bool asis = false);
    bool ExtractOnOff(char** args, bool& value);
    char* ExtractLinkArg(char** args, char const* const* linkTypes = nullptr,
        int* foundIdx = nullptr, char** keyPair = nullptr,
        char** somethingPair = nullptr);
    // shift-link like arg (with aditional info if need)
    char* ExtractArg(char** args,
        bool asis = false); // any name/number/quote/shift-link strings
    char* ExtractOptNotLastArg(char** args); // extract
                                             // name/number/quote/shift-link arg
                                             // only if more data in args for
                                             // parse

    char* ExtractKeyFromLink(
        char** text, char const* linkType, char** something1 = nullptr);
    char* ExtractKeyFromLink(char** text, char const* const* linkTypes,
        int* found_idx = nullptr, char** something1 = nullptr);
    bool ExtractUint32KeyFromLink(
        char** text, char const* linkType, uint32& value);

    uint32 ExtractAccountId(char** args, std::string* accountName = nullptr,
        Player** targetIfNullArg = nullptr);
    uint32 ExtractSpellIdFromLink(char** text);
    ObjectGuid ExtractGuidFromLink(char** text);
    GameTele const* ExtractGameTeleFromLink(char** text);
    bool ExtractLocationFromLink(
        char** text, uint32& mapid, float& x, float& y, float& z);
    bool ExtractRaceMask(
        char** text, uint32& raceMask, char const** maskName = nullptr);
    std::string ExtractPlayerNameFromLink(char** text);
    bool ExtractPlayerTarget(char** args, Player** player,
        ObjectGuid* player_guid = nullptr, std::string* player_name = nullptr);
    // select by arg (name/link) or in-game selection online/offline player
    // Parses a string of type "10g15s5c", "5g" or "-10s" and returns resulting
    // copper
    bool ExtractCopper(
        char** args, int32& copper, bool send_usage_on_failure = true);
    ...
    bool ShowAccountListHelper(QueryResult* result, uint32* limit = nullptr,
        bool title = true, bool error = true);
    void ShowFactionListHelper(FactionEntry const* factionEntry,
        LocaleConstant loc, FactionState const* repState = nullptr,
        Player* target = nullptr);
    void ShowItemListHelper(
        uint32 itemId, int loc_idx, Player* target = nullptr);
    void ShowQuestListHelper(
        uint32 questId, int32 loc_idx, Player* target = nullptr);
    bool ShowPlayerListHelper(QueryResult* result, uint32* limit = nullptr,
        bool title = true, bool error = true);
    void ShowSpellListHelper(
    ...
    bool GetDeletedCharacterInfoList(
        DeletedInfoList& foundList, std::string searchString = "");
    std::string GenerateDeletedCharacterGUIDsWhereStr(
...
private:
    WorldSession*
        m_session; // != NULL for chat command call and NULL for CLI command
...
class cli_cmd_handler : public ChatHandler
{
public:
    explicit cli_cmd_handler(cli_command& cmd) : command_(cmd) {}

    // Override following function from ChatHandler
    ...
    void handle() { ParseCommands(command_.command()); }

private:
    cli_command& command_;
};
