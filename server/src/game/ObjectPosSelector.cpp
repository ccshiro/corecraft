...
ObjectPosSelector::ObjectPosSelector(
    float x, float y, float dist, float searcher_size)
  : m_centerX(x), m_centerY(y), m_searcherDist(dist),
    m_searcherSize(searcher_size)
{
    // if size == 0, m_anglestep will become 0 -> freeze
    if (m_searcherSize == 0.0f)
        m_searcherSize = DEFAULT_BOUNDING_RADIUS;
...
/**
 * Add used area (circle) near target object excluded from possible searcher
 *position
 *
 *
 * @param size  Size of used circle
 * @param angle Angle of used circle center point from target-searcher line
 * @param dist  Distance from target object center point to used circle center
 *point
 *
 * Used circles data stored as projections to searcher dist size circle as angle
 *coordinate and half angle size
 */
void ObjectPosSelector::AddUsedArea(float size, float angle, float dist)
{
    ...
    float sr_angle = acos(
        (m_searcherDist * m_searcherDist + dist * dist - sr_dist * sr_dist) /
        (2 * m_searcherDist * dist));
 ...
 *
 * @param usedArea Used circle as projection to searcher distance circle in
 *angles form
 * @param side     Side of used circle
 * @param angle    Checked angle
 *
 * @return true, if used circle not intercepted with searcher circle in terms
 *projection angles
 */
bool ObjectPosSelector::CheckAngle(
    UsedArea const& usedArea, UsedAreaSide side, float angle) const
{
...
/**
 * Check searcher circle not intercepting with used circle at side (only start
 *angle provided)
 *
 * @param side     Side of used circle
 ...
 *
 * @return true, if 0.0f angle with m_searcher_halfangle*2 angle size not
 *intercept with used circles
 */
bool ObjectPosSelector::CheckOriginalAngle() const
{
    // check first left/right used angles if exists
    return (m_UsedAreaLists[USED_POS_PLUS].empty() ||
               CheckAngle(*m_UsedAreaLists[USED_POS_PLUS].begin(),
                   USED_POS_PLUS, 0.0f)) &&
           (m_UsedAreaLists[USED_POS_MINUS].empty() ||
               CheckAngle(*m_UsedAreaLists[USED_POS_MINUS].begin(),
                   USED_POS_MINUS, 0.0f));
}

/**
 * Initialize data for search angles starting from first possible angle at both
 * sides
 */
...
void ObjectPosSelector::InitializeAngle(UsedAreaSide side)
{
    ...
    // if another side not alow use 0.0f angle calculate possible value in
    // 0..m_searcherHalfSize range
...
void ObjectPosSelector::UpdateNextAreaStart(UsedAreaSide side)
{
    ...
    {
        m_nextUsedAreaStart[side] = m_nextUsedAreaItr[side]->first -
                                    m_nextUsedAreaItr[side]->second +
                                    m_searcherHalfSize;
        return;
    }
    ...
    {
        m_nextUsedAreaStart[side] =
            M_PI_F + (M_PI_F - lastArea.first - lastArea.second) +
            m_searcherHalfSize;
        return;
    }

    // last area and fail find any used area at another side, prepare fake data
    // as stopper
    m_nextUsedAreaStart[side] = M_PI_F + m_searcherHalfSize + 0.01f;
}
...
bool ObjectPosSelector::NextAngle(float& angle)
{
    ...
    for (;;)
    {
        ...
        if (m_stepAngle[USED_POS_PLUS] < M_PI_F &&
            m_stepAngle[USED_POS_PLUS] <= m_stepAngle[USED_POS_MINUS])
        {
...
bool ObjectPosSelector::NextSideAngle(UsedAreaSide side, float& angle)
{
    ...
    while (m_stepAngle[side] <= M_PI_F &&
           m_stepAngle[side] + m_searcherHalfSize >= m_nextUsedAreaStart[side])
    {
        // no used area for pass
        if (m_nextUsedAreaItr[side] == m_UsedAreaLists[side].end())
        {
            m_stepAngle[side] =
                M_PI_F + m_searcherHalfSize; // prevent continue search at side
            return false;
        }

        // angle set at first possible pos after passed m_nextUsedAreaItr
        m_stepAngle[side] =
            m_nextUsedAreaItr[side]->first + m_nextUsedAreaItr[side]->second;
...
/**
 * Find next angle in used area, that used if no angle found in free area with
 *LoS
 *
 * @param angle    Return at success found angle
 *
...
{
    if (m_nextUsedAreaItr[USED_POS_PLUS] ==
            m_UsedAreaLists[USED_POS_PLUS].end() &&
        m_nextUsedAreaItr[USED_POS_MINUS] ==
            m_UsedAreaLists[USED_POS_MINUS].end())
        return false;
    ...
    if (m_nextUsedAreaItr[USED_POS_PLUS] !=
        ...
        (m_nextUsedAreaItr[USED_POS_MINUS] ==
                m_UsedAreaLists[USED_POS_MINUS].end() ||
            m_nextUsedAreaItr[USED_POS_PLUS]->first <=
                m_nextUsedAreaItr[USED_POS_MINUS]->first))
    {
    ...
    {
        angle =
            m_nextUsedAreaItr[USED_POS_MINUS]->first * SignOf(USED_POS_MINUS);
        ++m_nextUsedAreaItr[USED_POS_MINUS];
    }
