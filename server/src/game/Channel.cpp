...
Channel::Channel(std::string name, uint32 channel_id)
  : m_announce(true), m_moderate(false), m_name(std::move(name)), m_flags(0),
    m_channelId(channel_id)
{
    // set special flags if built-in channel
    auto ch = sChatChannelsStore.LookupEntry(channel_id);
...
void Channel::Join(ObjectGuid p, const char* pass)
{
    ...
    Player* plr = sObjectMgr::Instance()->GetPlayer(p);

    if (plr)
    {
        if (HasFlag(CHANNEL_FLAG_LFG) && !plr->CanJoinLFGChannel())
        {
            MakeNotInLfg(&data);
            SendToOne(&data, p);
            return;
    ...
    if (m_announce &&
        (!plr || plr->GetSession()->GetSecurity() < SEC_POWER_GM ||
...
void Channel::Leave(ObjectGuid p, bool send)
{
    ...
    else
    {
        ...
        if (changeowner)
        {
            ObjectGuid newowner = !m_players.empty() ?
                                      m_players.begin()->second.player :
                                      ObjectGuid();
            SetOwner(newowner);
        }
    }
...
void Channel::KickOrBan(ObjectGuid good, const char* badname, bool ban)
{
    ...
    Player* gplr = sObjectMgr::Instance()->GetPlayer(good);
    if (gplr)
    ...
    else if (!m_players[good].IsModerator() && sec < SEC_POWER_GM)
    {
        WorldPacket data;
    ...
    {
        Player* bad = sObjectMgr::Instance()->GetPlayer(badname);
        if (bad == nullptr || !IsOn(bad->GetObjectGuid()))
        {
            WorldPacket data;
            MakePlayerNotFound(&data, badname);
            SendToOne(&data, good);
        }
        else if (sec < SEC_POWER_GM && bad->GetObjectGuid() == m_ownerGuid &&
                 good != m_ownerGuid)
        {
            WorldPacket data;
            MakeNotOwner(&data);
...
void Channel::UnBan(ObjectGuid good, const char* badname)
{
    ...
    {
        Player* bad = sObjectMgr::Instance()->GetPlayer(badname);
        if (bad == nullptr || !IsBanned(bad->GetObjectGuid()))
        {
            WorldPacket data;
            MakePlayerNotFound(&data, badname);
            SendToOne(&data, good);
...
void Channel::Password(ObjectGuid p, const char* pass)
{
    ...
    else if (!m_players[p].IsModerator() && sec < SEC_POWER_GM)
    {
        WorldPacket data;
        MakeNotModerator(&data);
        SendToOne(&data, p);
...
void Channel::SetMode(ObjectGuid p, const char* p2n, bool mod, bool set)
{
    ...
    {
        Player* newp = sObjectMgr::Instance()->GetPlayer(p2n);
        if (!newp)
        {
            WorldPacket data;
        ...
        // team
        if ((plr->GetSession()->GetSecurity() < SEC_POWER_GM ||
                newp->GetSession()->GetSecurity() < SEC_POWER_GM) &&
...
void Channel::SetOwner(ObjectGuid p, const char* newname)
{
    ...
    if (sec < SEC_POWER_GM && p != m_ownerGuid)
    {
        WorldPacket data;
        MakeNotOwner(&data);
        SendToOne(&data, p);
    ...
    Player* newp = sObjectMgr::Instance()->GetPlayer(newname);
    if (newp == nullptr || !IsOn(newp->GetObjectGuid()))
    {
        WorldPacket data;
...
void Channel::List(Player* player)
{
    ...
    else
    {
        WorldPacket data(SMSG_CHANNEL_LIST,
            1 + (GetName().size() + 1) + 1 + 4 + m_players.size() * (8 + 1));
        ...
        for (PlayerList::const_iterator i = m_players.begin();
             i != m_players.end(); ++i)
        {
            Player* plr = sObjectMgr::Instance()->GetPlayer(i->first);

            // PLAYER can't see MODERATOR, GAME MASTER, ADMINISTRATOR characters
            // MODERATOR, GAME MASTER, ADMINISTRATOR can see all
            if (plr &&
                (player->GetSession()->GetSecurity() > SEC_PLAYER ||
                    plr->GetSession()->GetSecurity() <= gmLevelInWhoList) &&
...
void Channel::Say(ObjectGuid p, const char* what, uint32 lang)
{
    ...
    else if (m_moderate && !m_players[p].IsModerator() && sec < SEC_POWER_GM)
    {
        WorldPacket data;
        MakeNotModerator(&data);
        SendToOne(&data, p);
    ...
    else
    {
        ...
        WorldPacket data(SMSG_MESSAGECHAT,
            1 + 4 + 8 + 4 + m_name.size() + 1 + 8 + 4 + messageLength + 1);
...
void Channel::SetOwner(ObjectGuid guid, bool exclaim)
{
    if (m_ownerGuid)
    {
        ...
        auto p_itr = m_players.find(m_ownerGuid);
...
{
    for (PlayerList::const_iterator i = m_players.begin(); i != m_players.end();
         ++i)
        if (Player* plr = sObjectMgr::Instance()->GetPlayer(i->first))
...
void Channel::SendToAllButOne(WorldPacket* data, ObjectGuid who)
{
    for (auto& p : m_players)
    {
        if (p.first == who)
            continue;
        if (Player* plr = sObjectMgr::Instance()->GetPlayer(p.first))
            plr->GetSession()->send_packet(data);
    }
}
...
void Channel::MakeChannelOwner(WorldPacket* data)
{
    ...
    if (!sObjectMgr::Instance()->GetPlayerNameByGUID(m_ownerGuid, name) ||
        name.empty())
...
// done 0x12
void Channel::MakePlayerKicked(
    WorldPacket* data, ObjectGuid bad, ObjectGuid good)
{
...
// done 0x15
void Channel::MakePlayerUnbanned(
    WorldPacket* data, ObjectGuid bad, ObjectGuid good)
{
...
void Channel::JoinNotify(ObjectGuid guid)
{
    ...
    if (IsConstant())
        data.initialize(
            SMSG_USERLIST_ADD, 8 + 1 + 1 + 4 + GetName().size() + 1);
    else
        data.initialize(
            SMSG_USERLIST_UPDATE, 8 + 1 + 1 + 4 + GetName().size() + 1);
