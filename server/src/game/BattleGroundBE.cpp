...
BattleGroundBE::BattleGroundBE(const battlefield::specification& spec)
  : specification_(spec), end_cond_check_timer_(0)
{
    ...
    SetMapId(562);
    SetTypeID(BATTLEGROUND_BE);
    SetMinPlayersPerTeam(1);
    SetMaxPlayersPerTeam(5);
    SetMinPlayers(2);
    SetMaxPlayers(10);
    SetName("Blade's Edge Arena");
    SetTeamStartLoc(ALLIANCE, 6292.66f, 288.579f, 4.95929f, 4.068f);
    SetTeamStartLoc(HORDE, 6184.98f, 236.011f, 4.97687f, 0.844f);
    SetLevelRange(20, 70);
...
void BattleGroundBE::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    FillInitialWorldState(
        data, count, 0x9f1, GetAlivePlayersCountByTeam(ALLIANCE));
    FillInitialWorldState(
        data, count, 0x9f0, GetAlivePlayersCountByTeam(HORDE));
...
void BattleGroundBE::UpdatePlayerScore(
