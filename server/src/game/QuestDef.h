...
enum QuestFailedReasons
{
    ...
    INVALIDREASON_DAILY_QUESTS_REMAINING =
        26, // You have already completed 10 daily quests today.
...
enum QuestFlags
{
    ...
    QUEST_FLAGS_PARTY_ACCEPT =
        0x00000002, // If player in party, all players that can accept this
                    // quest will receive confirmation box to accept quest
                    // CMSG_QUEST_CONFIRM_ACCEPT/SMSG_QUEST_CONFIRM_ACCEPT
    QUEST_FLAGS_EXPLORATION = 0x00000004, // Not used currently
    QUEST_FLAGS_SHARABLE = 0x00000008, // Can be shared: Player::CanShareQuest()
    // QUEST_FLAGS_NONE2        = 0x00000010,                // Not used
    // currently
    ...
    QUEST_FLAGS_UNK2 = 0x00000100, // Not used currently: _DELIVER_MORE Quest
                                   // needs more than normal _q-item_ drops from
                                   // mobs
    ...
    QUEST_FLAGS_TBC_RACES = 0x00000800, // Not used currently: Blood elf/Draenei
                                        // starting zone quests
    QUEST_FLAGS_DAILY = 0x00001000,     // Daily quest. Can be done once a day.
    // Quests reset at regular intervals for all
    // players.
};

enum QuestSpecialFlags
{
    // Mangos flags for set SpecialFlags in DB if required but used only at
    // server
    QUEST_SPECIAL_FLAG_REPEATABLE = 0x001, // |1 in SpecialFlags from DB
    QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT =
        0x002, // |2 in SpecialFlags from DB (if required area explore, spell
               // SPELL_EFFECT_QUEST_COMPLETE casting, table `*_script` command
               // SCRIPT_COMMAND_QUEST_EXPLORED use, set from script DLL)
    QUEST_SPECIAL_FLAG_SCRIPT_COMPLETED =
        0x04, // Has ReqCreature, but completed with script
    QUEST_SPECIAL_FLAG_NO_TALK_TO_CREDIT =
        0x08, // No credit from Player::TalkedToCreature
...
#define QUEST_SPECIAL_FLAG_DB_ALLOWED                                          \
    (QUEST_SPECIAL_FLAG_REPEATABLE | QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT | \
        QUEST_SPECIAL_FLAG_SCRIPT_COMPLETED |                                  \
        QUEST_SPECIAL_FLAG_NO_TALK_TO_CREDIT)
...
// (cached) quest details,
// all base quest information, and any utility functions such as generating the
// amount of
// xp to give
class Quest
{
...
public:
    ...
    uint32 GetCharTitleBitIndex() const; // in 2.x in different from 3.x in some
                                         // quest packets used bit index insed
                                         // id
    ...
    int32
        ReqCreatureOrGOId[QUEST_OBJECTIVES_COUNT]; // >0 Creature <0 Gameobject
...
{
    QuestStatusData()
      : m_status(QUEST_STATUS_NONE), m_rewarded(false), m_explored(false),
