...
public:
    ...
    WorldSession(uint32 id, connection_ptr sock, AccountTypes sec,
        uint8 expansion, time_t mute_time, LocaleConstant locale);
    ~WorldSession();

    // Translate incoming client time to server time
    void translate_movement_timestamp(MovementInfo& movementInfo);
    uint32 client_tick_count;
    uint32 server_tick_count;
    ...
    void send_packet(WorldPacket&& packet);
    void send_packet(const WorldPacket* packet);
    void SendNotification(const char* format, ...) ATTR_PRINTF(2, 3);
    void SendNotification(int32 string_id, ...);
    void SendPetNameInvalid(
        uint32 error, const std::string& name, DeclinedName* declinedName);
    void SendPartyResult(
        PartyOperation operation, const std::string& member, PartyResult res);
    ...
    void invalidate_warden_dynamic(WardenDynamicCheck check)
    {
        if (m_Warden)
            m_Warden->invalidate_dynamic(check);
    }
    ...
    // Disconnect: player was disconnected due to his own circumstances
    void DisconnectPlayer();
    // Kick: player was forcefully removed, i.e. by shutdown or GM kick
    void KickPlayer();

    void QueuePacket(WorldPacket&& new_packet);

    bool Update(PacketFilter& updater, uint32 diff);
    ...
    static void SendNameQueryOpcodeFromDBCallBack(
    ...
    uint32 GetTutorialInt(uint32 intId) { return m_Tutorials[intId]; }
    ...
    void SendAuctionCommandResult(AuctionEntry* auc, AuctionAction Action,
        AuctionError ErrorCode, InventoryResult invError = EQUIP_ERR_OK);
    ...
    void BuildListAuctionItems(std::vector<AuctionEntry*> const& auctions,
        WorldPacket& data, std::wstring const& searchedname, uint32 listfrom,
        uint32 levelmin, uint32 levelmax, uint32 usable, uint32 inventoryType,
        uint32 itemClass, uint32 itemSubClass, uint32 quality, uint32& count,
        uint32& totalcount, bool isFull);

    AuctionHouseEntry const* GetCheckedAuctionHouseForAuctioneer(
        ObjectGuid guid);

    // Item Enchantment
    void SendEnchantmentLog(ObjectGuid targetGuid, ObjectGuid casterGuid,
    ...
    // Looking For Group data
    bool lfg_auto_join;
    bool lfg_auto_invite;
    ...
    uint32 getDialogStatus(
        Player* pPlayer, Object* questgiver, uint32 defstatus);

public:                                        // opcodes handlers
    void Handle_NULL(WorldPacket& recvPacket); // not used
    void Handle_EarlyProccess(WorldPacket&
            recvPacket); // just mark packets processed in WorldSocket::OnRead
    void Handle_ServerSide(WorldPacket&
            recvPacket); // sever side only, can't be accepted from client
    ...
    void HandlePlayerRelog(Player* player);
    ...
    static void HandleAddFriendOpcodeCallBack(
        QueryResult* result, uint32 accountId, std::string friendNote);
    void HandleDelFriendOpcode(WorldPacket& recvPacket);
    void HandleAddIgnoreOpcode(WorldPacket& recvPacket);
    static void HandleAddIgnoreOpcodeCallBack(
        QueryResult* result, uint32 accountId);
    ...
    void AuctionBind(
        uint32 price, AuctionEntry* auction, Player* pl, Player* auction_owner);
    ...
    bool CanInteractWithQuestGiver(const ObjectGuid& guid);
    ...
    void HandleGuildBankMoneyAvailable(WorldPacket& recv_data);
    ...
    void clear_saved_facing() { saved_facing_.clear(); }

private:
    // private trade methods
    void moveItems(Item* myItems[], Item* hisItems[]);
    bool VerifyMovementInfo(
    ...
    std::mutex recv_queue_mutex_; // must be locked to access recv_queue_
    std::list<WorldPacket>
        recv_queue_; // order: newest (front) to oldest (back)

    //! This stores the time the user started to sit at the character select
    // screen
    time_t char_select_start_t_;

    //! This stores the timeout value for someone idling at the character select
    // screen (after how long to d/c them).
    int32 char_select_timeout_;

    // Warden
    Warden* m_Warden;

    // Save facing packets, and only distribute one per update (they client
    // spams these,
    // and we can receive 5-10 of them per update if the player is turning with
    // his mouse).
    WorldPacket saved_facing_;
    bool force_facing_update_;

    // Timer of when to remove player that skipped due logout process
    time_t altf4_timer_;
    bool kicked_; // True if player was kicked and should DC instantly
};
#endif
/// @}
