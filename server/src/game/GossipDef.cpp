...
{
    m_gItems.reserve(16); // can be set for max from most often sizes to speedup
                          // push_back and less memory use
    m_gMenuId = 0;
}
...
void GossipMenu::AddMenuItem(uint8 Icon, const std::string& Message,
    uint32 dtSender, uint32 optionId, uint32 id, const std::string& BoxMessage,
...
void GossipMenu::AddGossipMenuItemData(
    int32 action_menu, uint32 action_poi, uint32 action_script)
{
...
{
    AddMenuItem(Icon, Message, 0, 0, 0, "", 0, Coded);
}
    ...
    uint32 dtAction, char const* BoxMessage, uint32 BoxMoney, bool Coded)
{
    AddMenuItem(Icon, std::string(Message ? Message : ""), dtSender, dtAction,
        0, std::string(BoxMessage ? BoxMessage : ""), BoxMoney, Coded);
}

void GossipMenu::AddMenuItem(uint8 Icon, int32 itemText, uint32 dtSender,
    uint32 dtAction, int32 boxText, uint32 BoxMoney, bool Coded)
{
    uint32 loc_idx = m_session->GetSessionDbLocaleIndex();

    char const* item_text =
        itemText ? sObjectMgr::Instance()->GetMangosString(itemText, loc_idx) :
                   "";
    char const* box_text =
        boxText ? sObjectMgr::Instance()->GetMangosString(boxText, loc_idx) :
                  "";

    AddMenuItem(Icon, std::string(item_text), dtSender, dtAction, 0,
        std::string(box_text), BoxMoney, Coded);
}
...
uint32 GossipMenu::MenuItemId(unsigned int ItemId)
{
    if (ItemId >= m_gItems.size())
        return 0;

    return m_gItems[ItemId].m_id;
}
...
uint32 PlayerMenu::GossipOptionId(unsigned int Selection)
{
    return mGossipMenu.MenuItemId(Selection);
}
...
void PlayerMenu::SendGossipMenu(uint32 TitleTextId, ObjectGuid objectGuid)
{
    ...
    for (uint32 iI = 0; iI < mGossipMenu.MenuItemCount(); ++iI)
    {
        ...
        data << gItem.m_gBoxMessage; // accept text (related to money) pop up
                                     // box, 2.0.3, max 0x800
    }
    ...
    for (uint32 iI = 0; iI < mQuestMenu.MenuItemCount(); ++iI)
    {
        ...
        Quest const* pQuest = sObjectMgr::Instance()->GetQuestTemplate(questID);
    ...
    GetMenuSession()->send_packet(std::move(data));
}
...
{
    PointOfInterest const* poi =
        sObjectMgr::Instance()->GetPointOfInterest(poi_id);
    if (!poi)
    {
        logging.error(
            "Requested send nonexistent POI (Id: %u), ignore.", poi_id);
        return;
    }
    ...
    int loc_idx = GetMenuSession()->GetSessionDbLocaleIndex();
    if (loc_idx >= 0)
        if (PointOfInterestLocale const* pl =
                sObjectMgr::Instance()->GetPointOfInterestLocale(poi_id))
            if (pl->IconName.size() > size_t(loc_idx) &&
                !pl->IconName[loc_idx].empty())
...
{
    GossipText const* pGossip = sObjectMgr::Instance()->GetGossipText(textID);
    ...
    {
        std::string Text_0[MAX_GOSSIP_TEXT_OPTIONS],
            Text_1[MAX_GOSSIP_TEXT_OPTIONS];
        ...
        sObjectMgr::Instance()->GetNpcTextLocaleStringsAll(
            textID, loc_idx, &Text_0, &Text_1);
...
QuestMenu::QuestMenu()
{
    m_qItems.reserve(16); // can be set for max from most often sizes to speedup
                          // push_back and less memory use
}
...
void PlayerMenu::SendQuestGiverQuestList(
    QEmote eEmote, const std::string& Title, ObjectGuid npcGUID)
{
...
void PlayerMenu::SendQuestGiverQuestDetails(
    Quest const* pQuest, ObjectGuid guid, bool ActivateAccept)
{
    std::string Title = pQuest->GetTitle();
    ...
    if (loc_idx >= 0)
    {
        if (QuestLocale const* ql =
                sObjectMgr::Instance()->GetQuestLocale(pQuest->GetQuestId()))
        {
            if (ql->Title.size() > (size_t)loc_idx &&
                !ql->Title[loc_idx].empty())
                Title = ql->Title[loc_idx];
            if (ql->Details.size() > (size_t)loc_idx &&
                !ql->Details[loc_idx].empty())
                Details = ql->Details[loc_idx];
            if (ql->Objectives.size() > (size_t)loc_idx &&
                !ql->Objectives[loc_idx].empty())
    ...
    data << uint32(10 * MaNGOS::Honor::hk_honor_at_level(
                            GetMenuSession()->GetPlayer()->getLevel(),
                            pQuest->GetRewHonorableKills()));
    data << uint32(pQuest->GetRewSpell());     // reward spell, this spell will
                                               // display (icon) (casted if
                                               // RewSpellCast==0)
    data << uint32(pQuest->GetRewSpellCast()); // casted spell
    data << uint32(pQuest->GetCharTitleBitIndex()); // CharTitle, new 2.4.0,
                                                    // player gets this title
                                                    // (bit index from
                                                    // CharTitles)
    ...
    for (uint32 i = 0; i < QUEST_EMOTE_COUNT; ++i)
    {
        ...
        data << uint32(
            pQuest->DetailsEmoteDelay[i]); // DetailsEmoteDelay (in ms)
    }
...
void PlayerMenu::SendQuestQueryResponse(Quest const* pQuest)
{
    ...
    if (loc_idx >= 0)
    {
                ...
                sObjectMgr::Instance()->GetQuestLocale(pQuest->GetQuestId()))
        {
            ...
            if (ql->Objectives.size() > (size_t)loc_idx &&
                !ql->Objectives[loc_idx].empty())
                Objectives = ql->Objectives[loc_idx];
            if (ql->EndText.size() > (size_t)loc_idx &&
                !ql->EndText[loc_idx].empty())
                EndText = ql->EndText[loc_idx];

            for (int i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
                if (ql->ObjectiveText[i].size() > (size_t)loc_idx &&
                    !ql->ObjectiveText[i][loc_idx].empty())
    ...
    data << uint32(pQuest->GetQuestMethod()); // Accepted values: 0, 1 or 2.
                                              // 0==IsAutoComplete() (skip
                                              // objectives/details)
    data << int32(pQuest->GetQuestLevel()); // may be -1, static data, in other
                                            // cases must be used dynamic level:
                                            // Player::GetQuestLevelForPlayer (0
                                            // is not known, but assuming this
                                            // is no longer valid for quest
                                            // intended for client)
    data << uint32(
        pQuest->GetZoneOrSort()); // zone or sort to display in quest log

    data << uint32(pQuest->GetType());             // quest type
    data << uint32(pQuest->GetSuggestedPlayers()); // suggested players count

    data << uint32(pQuest->GetRepObjectiveFaction()); // shown in quest log as
                                                      // part of quest objective
    data << uint32(pQuest->GetRepObjectiveValue());   // shown in quest log as
                                                      // part of quest objective

    data << uint32(0); // RequiredOpositeRepFaction
    data << uint32(0); // RequiredOpositeRepValue, required faction value with
                       // another (oposite) faction (objective)

    data << uint32(pQuest->GetNextQuestInChain()); // client will request this
    ...
    if (pQuest->HasQuestFlag(QUEST_FLAGS_HIDDEN_REWARDS))
        ...
        data << uint32(
            pQuest->GetRewOrReqMoney()); // reward money (below max lvl)

    data << uint32(
        pQuest->GetRewMoneyMaxLevel());    // used in XP calculation at client
    data << uint32(pQuest->GetRewSpell()); // reward spell, this spell will
                                           // display (icon) (casted if
                                           // RewSpellCast==0)
    data << uint32(pQuest->GetRewSpellCast()); // casted spell
    ...
    data << uint32(MaNGOS::Honor::hk_honor_at_level(
        GetMenuSession()->GetPlayer()->getLevel(),
        pQuest->GetRewHonorableKills()));
    data << uint32(pQuest->GetSrcItemId());   // source item id
    data << uint32(pQuest->GetQuestFlags());  // quest flags
    data << uint32(pQuest->GetCharTitleId()); // CharTitleId, new 2.4.0, player
                                              // gets this title (id from
                                              // CharTitles)
...
void PlayerMenu::SendQuestGiverOfferReward(
    Quest const* pQuest, ObjectGuid npcGUID, bool EnableNext)
{
    std::string Title = pQuest->GetTitle();
    ...
    if (loc_idx >= 0)
    {
                ...
                sObjectMgr::Instance()->GetQuestLocale(pQuest->GetQuestId()))
        {
            if (ql->Title.size() > (size_t)loc_idx &&
                !ql->Title[loc_idx].empty())
                Title = ql->Title[loc_idx];
            if (ql->OfferRewardText.size() > (size_t)loc_idx &&
                !ql->OfferRewardText[loc_idx].empty())
...
void PlayerMenu::SendQuestGiverRequestItems(Quest const* pQuest,
    ObjectGuid npcGUID, bool Completable, bool CloseOnCancel)
{
    ...
    if (loc_idx >= 0)
    {
                ...
                sObjectMgr::Instance()->GetQuestLocale(pQuest->GetQuestId()))
        {
            if (ql->Title.size() > (size_t)loc_idx &&
                !ql->Title[loc_idx].empty())
                Title = ql->Title[loc_idx];
            if (ql->RequestItemsText.size() > (size_t)loc_idx &&
                !ql->RequestItemsText[loc_idx].empty())
    ...
    // Required Money
    data << uint32(
        pQuest->GetRewOrReqMoney() < 0 ? -pQuest->GetRewOrReqMoney() : 0);
