...
ChannelMgr* channelMgr(Team team)
{
    if (sWorld::Instance()->getConfig(
        ...
        return MaNGOS::UnlockedSingleton<
            AllianceChannelMgr>::Instance(); // cross-faction

    if (team == ALLIANCE)
        return MaNGOS::UnlockedSingleton<AllianceChannelMgr>::Instance();
    if (team == HORDE)
        return MaNGOS::UnlockedSingleton<HordeChannelMgr>::Instance();
