...
enum BattleGroundTimeIntervals
{
    ...
    RESPAWN_ONE_WEEK = 604800,       // secs
    RESPAWN_IMMEDIATELY = 0,         // secs
    BUFF_RESPAWN_TIME = 180,         // secs
    ARENA_SPAWN_BUFF_OBJECTS = 90,   // secs
};
...
enum BattleGroundBuffObjects
{
    ...
    BG_OBJECTID_SHADOW_SIGHT_ONE = 184663,
    BG_OBJECTID_SHADOW_SIGHT_TWO = 184664
};

const uint32 Buff_Entries[3] = {BG_OBJECTID_SPEEDBUFF_ENTRY,
    BG_OBJECTID_REGENBUFF_ENTRY, BG_OBJECTID_BERSERKERBUFF_ENTRY};
...
enum BattleGroundBracketId // bracketId for level ranges
{
    ...
    BG_BRACKET_ID_FIRST = 0, // brackets start from specific BG min level and
                             // each include 10 levels range
    BG_BRACKET_ID_LAST = 6,  // so for start level 10 will be 10-19, 20-29, ...
    // all greater max bg level included in last breaket
};
...
public:
    BattleGroundScore()
      : KillingBlows(0), Deaths(0), HonorableKills(0), BonusHonor(0),
        DamageDone(0), HealingDone(0)
    {
    }
    virtual ~BattleGroundScore() {} // virtual destructor is used when deleting
                                    // score from scores map
...
*/
class MANGOS_DLL_SPEC BattleGround
{
...
public:
    ...
    virtual void Update(uint32 diff); // must be implemented in BG subclass of
                                      // BG specific update code, but must in
                                      // begginning call parent version
    ...
    virtual bool SpawnedByDefault(Creature* /*c*/) { return true; }
    ...
    uint32 GetTimeElapsed() const { return m_TimeElapsed; }
    ...
    // FIXME: Add invite is completely different from IncreaseInvitedCount
    // the latter should be renamed because it actually counts how many players
    // belong to this bg
    bool add_invite(ObjectGuid guid);
    bool remove_invite(ObjectGuid guid);
    Team invitee_team(ObjectGuid guid) const;
    ...
    // Means we can delete the BattleGround
    ...
    void GetTeamStartLoc(
        Team team, float& X, float& Y, float& Z, float& O) const
    {
    ...
    virtual void FillInitialWorldStates(
        WorldPacket& /*data*/, uint32& /*count*/)
    {
    }
    void SendPacketToTeam(Team team, WorldPacket* packet,
        Player* sender = nullptr, bool self = true);
    ...
    void RewardReputationToTeam(
        uint32 faction_id, uint32 Reputation, Team team);
    ...
    void SendMessageToAll(
        int32 entry, ChatMsg type, Player const* source = nullptr);
    void SendYellToAll(int32 entry, uint32 language, ObjectGuid guid);
    void PSendMessageToAll(
        int32 entry, ChatMsg type, Player const* source, ...);

    // specialized version with 2 string id args
    void SendMessage2ToAll(int32 entry, ChatMsg type, Player const* source,
        int32 strId1 = 0, int32 strId2 = 0);
    void SendYell2ToAll(
    ...
    static BattleGroundTeamIndex GetTeamIndexByTeamId(Team team)
    {
    ...
    uint32 GetAlivePlayersCountByTeam(
        Team team) const; // used in arenas to correctly handle death in spirit
                          // of redemption / last stand etc. (killer = killed)
                          // cases
    ...
    void SetArenaTeamRatingChangeForTeam(Team team, int32 RatingChange)
    {
        m_ArenaTeamRatingChanges[GetTeamIndexByTeamId(team)] = RatingChange;
    }
    int32 GetArenaTeamRatingChangeForTeam(Team team) const
    {
    ...
    virtual void EventPlayerClickedOnFlag(
        Player* /*player*/, GameObject* /*target_obj*/)
    {
    }
    ...
    void RemovePlayerAtLeave(ObjectGuid guid, bool Transport);
    ...
    void SpawnEvent(
        uint8 event1, uint8 event2, bool spawn, bool despawn_other = true);
    ...
    void HandleTriggerBuff(ObjectGuid go_guid, Player* took_buff = nullptr);
    ...
    bool AddObject(uint32 type, uint32 entry, float x, float y, float z,
        float o, float rotation0, float rotation1, float rotation2,
        float rotation3, uint32 respawnTime = 0);
    ...
    {
        return team ? ((team == ALLIANCE) ? HORDE : ALLIANCE) : TEAM_NONE;
    }
    static BattleGroundTeamIndex GetOtherTeamIndex(
        BattleGroundTeamIndex teamIdx)
    {
        return teamIdx == BG_TEAM_ALLIANCE ? BG_TEAM_HORDE : BG_TEAM_ALLIANCE;
    }
    ...
    // door-events are automaticly added - but _ALL_ other must be in this
    // vector
    std::map<uint8, uint8> m_ActiveEvents;

    virtual battlefield::specification get_specification() const = 0;
    battlefield::bracket get_bracket() const
    {
        return get_specification().get_bracket();
    }
...
private:
    ...
    bool m_ArenaBuffSpawned; // true if shadow sight buffs have been spawned
    int32 m_EndTime; // it is set to 120000 when bg is ending and it decreases
                     // itself
    ArenaType m_ArenaType;    // 2=2v2, 3=3v3, 5=5v5
    bool m_InBGFreeSlotQueue; // used to make sure that BG is only once inserted
                              // into the
    ...
    // map the invited player to his team so we can expire his invite when he is
    // offline
    std::map<ObjectGuid, Team> invitee_teams_;
    // This is needed so we can get the team of players that have left the
    // battleground. FIXME: Fix m_Players
    std::map<ObjectGuid, Team> player_teams_;
     ...
     * faction */
    /* Invited counters will be changed only when removing already invited
     * player from queue, removing player from battleground and inviting player
     * to BG */
...
inline void FillInitialWorldState(
    ByteBuffer& data, uint32& count, uint32 state, uint32 value)
{
    data << uint32(state);
    data << uint32(value);
    ++count;
...
inline void FillInitialWorldState(
    ByteBuffer& data, uint32& count, uint32 state, bool value)
{
    data << uint32(state);
...
inline void FillInitialWorldState(
    ByteBuffer& data, uint32& count, WorldStatePair const* array)
{
