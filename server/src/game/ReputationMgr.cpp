...
int32 ReputationMgr::GetReputation(uint32 faction_id) const
{
    ...
    {
        logging.error(
            "ReputationMgr::GetReputation: Can't get reputation of %s for "
            "unknown faction (faction id) #%u.",
...
void ReputationMgr::ApplyForceReaction(
    uint32 faction_id, ReputationRank rank, bool apply)
{
    if (apply)
...
{
    WorldPacket data(
        SMSG_SET_FORCED_REACTIONS, 4 + m_forcedReactions.size() * (4 + 4));
...
void ReputationMgr::SendState(FactionState const* faction)
{
    ...
    for (auto& elem : m_factions)
    {
        if (elem.second.needSend)
        {
            elem.second.needSend = false;
            if (elem.second.ReputationListID != faction->ReputationListID)
            {
                data << (uint32)elem.second.ReputationListID;
                data << (uint32)elem.second.Standing;
    ...
    FactionEntry const* factionEntry, int32 standing, bool incremental)
{
    ...
    if (const RepSpilloverTemplate* repTemplate =
            sObjectMgr::Instance()->GetRepSpilloverTemplate(factionEntry->ID))
    {
    ...
    auto faction = m_factions.find(factionEntry->reputationListID);
...
{
    auto itr = m_factions.find(repListID);
    if (itr == m_factions.end())
        return;
...
void ReputationMgr::SetInactive(FactionState* faction, bool inactive)
{
    ...
    if (inactive && ((faction->Flags & (FACTION_FLAG_INVISIBLE_FORCED |
                                           FACTION_FLAG_HIDDEN)) ||
                        !(faction->Flags & FACTION_FLAG_VISIBLE)))
        return;
...
bool ReputationMgr::IsAtWar(uint32 faction_id) const
{
    ...
    {
        logging.error(
            "ReputationMgr::IsAtWar: Can't get AtWar flag of %s for unknown "
...
void ReputationMgr::LoadFromDB(QueryResult* result)
{
    ...
    // faction,standing,flags FROM character_reputation WHERE guid =
    // '%u'",GetGUIDLow());
    ...
    if (result)
    {
        do
        {
            ...
            FactionEntry const* factionEntry =
                sFactionStore.LookupEntry(fields[0].GetUInt32());
            if (factionEntry && (factionEntry->reputationListID >= 0))
            {
                ...
                if (dbFactionFlags & FACTION_FLAG_AT_WAR) // DB at war
                    SetAtWar(faction, true); // have internal checks for
                ...
                ForcedReactions::const_iterator forceItr =
                    m_forcedReactions.find(factionEntry->ID);
...
void ReputationMgr::SaveToDB()
{
    ...
    SqlStatement stmtDel = CharacterDatabase.CreateStatement(delRep,
        "DELETE FROM character_reputation WHERE guid = ? AND faction=?");
    SqlStatement stmtIns = CharacterDatabase.CreateStatement(insRep,
        "INSERT INTO character_reputation (guid,faction,standing,flags) VALUES "
        "(?, ?, ?, ?)");

    for (auto& elem : m_factions)
    {
        if (elem.second.needSave)
        {
            stmtDel.PExecute(m_player->GetGUIDLow(), elem.second.ID);
            stmtIns.PExecute(m_player->GetGUIDLow(), elem.second.ID,
                elem.second.Standing, elem.second.Flags);
            elem.second.needSave = false;
        }
    }
}
