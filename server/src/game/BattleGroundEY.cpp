...
BattleGroundEY::BattleGroundEY(const battlefield::bracket& bracket)
  : specification_(battlefield::eye_of_the_storm, bracket)
{
    SetMapId(566);
    SetTypeID(BATTLEGROUND_EY);
    SetArenaorBGType(false);
    SetMinPlayersPerTeam(8);
    SetMaxPlayersPerTeam(15);
    SetMinPlayers(16);
    SetMaxPlayers(30);
    SetName("Eye of the Storm");
    SetTeamStartLoc(ALLIANCE, 2523.69f, 1596.6f, 1269.35f, 3.40156f);
    SetTeamStartLoc(HORDE, 1807.74f, 1539.42f, 1267.63f, 0.263892f);
    SetLevelRange(61, 70);
    ...
    // Stuff that was in BattleGroundEY::Reset previously:
    ...
    m_HonorTics = (isBGWeekend) ? BG_EY_EYWeekendHonorTicks :
    ...
    // End of stuff that was in BattleGroundEY::Reset previously
}
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (m_PointAddingTimer <= 0)
        {
            ...
            if (m_TeamPointsCount[BG_TEAM_ALLIANCE] > 0)
                AddPoints(ALLIANCE,
                    BG_EY_TickPoints[m_TeamPointsCount[BG_TEAM_ALLIANCE] - 1]);
            if (m_TeamPointsCount[BG_TEAM_HORDE] > 0)
                AddPoints(HORDE,
                    BG_EY_TickPoints[m_TeamPointsCount[BG_TEAM_HORDE] - 1]);
        }
        ...
        if (m_TowerCapCheckTimer <= 0)
        {
            ...
            /*I used this order of calls, because although we will check if one
              player is in gameobject's distance 2 times
              but we can count of players on current point in
...
void BattleGroundEY::StartingEventOpenDoors()
{
    ...
    for (uint32 i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        ...
        SpawnBGObject(
            m_BgObjects[BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + buff + i * 3],
            RESPAWN_IMMEDIATELY);
    }
}
    ...
    for (uint8 i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        ...
        while (j < m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS].size())
        {
            Player* plr = sObjectMgr::Instance()->GetPlayer(
                m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS][j]);
            if (!plr)
            {
                logging.error(
                    "BattleGroundEY:CheckSomeoneJoinedPoint: %s not found!",
                    m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS][j]
                        .GetString()
                        .c_str());
                ++j;
            ...
            if (plr->CanCaptureTowerPoint() &&
                plr->IsWithinDist3d(BG_EY_NodePositions[i][0],
                    BG_EY_NodePositions[i][1], BG_EY_NodePositions[i][2],
                    BG_EY_POINT_RADIUS))
            {
                // player joined point!
                // show progress bar
                UpdateWorldStateForPlayer(PROGRESS_BAR_PERCENT_GREY,
                    BG_EY_PROGRESS_BAR_PERCENT_GREY, plr);
                UpdateWorldStateForPlayer(
                    PROGRESS_BAR_STATUS, m_PointBarStatus[i], plr);
                UpdateWorldStateForPlayer(
                    PROGRESS_BAR_SHOW, BG_EY_PROGRESS_BAR_SHOW, plr);
                // add player to point
                m_PlayersNearPoint[i].push_back(
                    m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS][j]);
                // remove player from "free space"
                m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS].erase(
                    m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS].begin() +
                    j);
            }
            else
                ++j;
...
void BattleGroundEY::CheckSomeoneLeftPoint()
{
    ...
    for (auto& elem : m_CurrentPointPlayersCount)
        elem = 0;
    for (uint8 i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        uint8 j = 0;
        ...
        {
            Player* plr =
                sObjectMgr::Instance()->GetPlayer(m_PlayersNearPoint[i][j]);
            if (!plr)
            {
                logging.error(
                    "BattleGroundEY:CheckSomeoneLeftPoint %s not found!",
                    m_PlayersNearPoint[i][j].GetString().c_str());
                // move nonexistent player to "free space" - this will cause
                // many error showing in log, but it is a very important bug
                m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS].push_back(
                    m_PlayersNearPoint[i][j]);
            ...
            if (!plr->CanCaptureTowerPoint() ||
                !plr->IsWithinDist3d(BG_EY_NodePositions[i][0],
                    BG_EY_NodePositions[i][1], BG_EY_NodePositions[i][2],
                    BG_EY_POINT_RADIUS))
            // move player out of point (add him to players that are out of
            // points
            {
                m_PlayersNearPoint[BG_EY_PLAYERS_OUT_OF_POINTS].push_back(
                    m_PlayersNearPoint[i][j]);
                m_PlayersNearPoint[i].erase(m_PlayersNearPoint[i].begin() + j);
                UpdateWorldStateForPlayer(
                    PROGRESS_BAR_SHOW, BG_EY_PROGRESS_BAR_DONT_SHOW, plr);
            }
            else
            {
                // player is neat flag, so update count:
                m_CurrentPointPlayersCount[2 * i + GetTeamIndexByTeamId(
                                                       plr->GetTeam())]++;
                ++j;
            }
        }
...
void BattleGroundEY::UpdatePointStatuses()
{
    for (uint8 point = 0; point < BG_EY_NODES_MAX; ++point)
    {
        ...
        m_PointBarStatus[point] +=
            (m_CurrentPointPlayersCount[2 * point] -
                    m_CurrentPointPlayersCount[2 * point + 1] <
        ...
        {
            if (Player* plr = sObjectMgr::Instance()->GetPlayer(
                    m_PlayersNearPoint[point][i]))
            {
                UpdateWorldStateForPlayer(
                    PROGRESS_BAR_STATUS, m_PointBarStatus[point], plr);
                // if point owner changed we must evoke event!
                if (pointOwnerTeamId != m_PointOwnedByTeam[point])
                {
                    // point was uncontrolled and player is from team which
                    // captured point
                    if (m_PointState[point] == EY_POINT_STATE_UNCONTROLLED &&
                        plr->GetTeam() == pointOwnerTeamId)
                        EventTeamCapturedPoint(plr, point);

                    // point was under control and player isn't from team which
                    // controlled it
                    if (m_PointState[point] == EY_POINT_UNDER_CONTROL &&
                        plr->GetTeam() != m_PointOwnedByTeam[point])
...
void BattleGroundEY::UpdatePointsIcons(Team team, uint32 Point)
{
    ...
    if (m_PointState[Point] == EY_POINT_UNDER_CONTROL)
    {
        ...
        if (team == ALLIANCE)
            UpdateWorldState(
                PointsIconStruct[Point].WorldStateAllianceControlledIndex, 1);
        else
            UpdateWorldState(
                PointsIconStruct[Point].WorldStateHordeControlledIndex, 1);
    }
    else
    {
        if (team == ALLIANCE)
            UpdateWorldState(
                PointsIconStruct[Point].WorldStateAllianceControlledIndex, 0);
        else
            UpdateWorldState(
                PointsIconStruct[Point].WorldStateHordeControlledIndex, 0);
...
void BattleGroundEY::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    case TR_BLOOD_ELF_POINT:
        if (m_PointState[BG_EY_NODE_BLOOD_ELF] == EY_POINT_UNDER_CONTROL &&
            m_PointOwnedByTeam[BG_EY_NODE_BLOOD_ELF] == Source->GetTeam())
            if (m_FlagState && GetFlagPickerGuid() == Source->GetObjectGuid())
                EventPlayerCapturedFlagEY(Source, BG_EY_NODE_BLOOD_ELF);
        break;
    case TR_FEL_REAVER_POINT:
        if (m_PointState[BG_EY_NODE_FEL_REAVER] == EY_POINT_UNDER_CONTROL &&
            m_PointOwnedByTeam[BG_EY_NODE_FEL_REAVER] == Source->GetTeam())
            if (m_FlagState && GetFlagPickerGuid() == Source->GetObjectGuid())
                EventPlayerCapturedFlagEY(Source, BG_EY_NODE_FEL_REAVER);
        break;
    case TR_MAGE_TOWER_POINT:
        if (m_PointState[BG_EY_NODE_MAGE_TOWER] == EY_POINT_UNDER_CONTROL &&
            m_PointOwnedByTeam[BG_EY_NODE_MAGE_TOWER] == Source->GetTeam())
            if (m_FlagState && GetFlagPickerGuid() == Source->GetObjectGuid())
                EventPlayerCapturedFlagEY(Source, BG_EY_NODE_MAGE_TOWER);
        break;
    case TR_DRAENEI_RUINS_POINT:
        if (m_PointState[BG_EY_NODE_DRAENEI_RUINS] == EY_POINT_UNDER_CONTROL &&
            m_PointOwnedByTeam[BG_EY_NODE_DRAENEI_RUINS] == Source->GetTeam())
            if (m_FlagState && GetFlagPickerGuid() == Source->GetObjectGuid())
                EventPlayerCapturedFlagEY(Source, BG_EY_NODE_DRAENEI_RUINS);
        break;
    case 4512:
    case 4515:
    case 4517:
...
bool BattleGroundEY::SetupBattleGround()
{
    ...
    for (int i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        AreaTriggerEntry const* at =
            sAreaTriggerStore.LookupEntry(m_Points_Trigger[i]);
        if (!at)
        {
            logging.error(
                "BattleGroundEY: Unknown trigger: %u", m_Points_Trigger[i]);
            continue;
        }
        if (!AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3,
                Buff_Entries[0], at->x, at->y, at->z, 0.907571f, 0, 0,
                0.438371f, 0.898794f, RESPAWN_ONE_WEEK) ||
            !AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3 + 1,
                Buff_Entries[1], at->x, at->y, at->z, 0.907571f, 0, 0,
                0.438371f, 0.898794f, RESPAWN_ONE_WEEK) ||
            !AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3 + 2,
                Buff_Entries[2], at->x, at->y, at->z, 0.907571f, 0, 0,
                0.438371f, 0.898794f, RESPAWN_ONE_WEEK))
            logging.error("BattleGroundEY: Cannot spawn buff");
    }
...
void BattleGroundEY::RespawnFlagAfterDrop()
{
    ...
    if (obj)
        ...
        logging.error("BattleGroundEY: Unknown dropped flag: %s",
            GetDroppedFlagGuid().GetString().c_str());
...
void BattleGroundEY::EventPlayerDroppedFlag(Player* Source)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
    {
        // if not running, do not cast things at the dropper player, neither
        // send unnecessary messages
        // just take off the aura
        if (IsFlagPickedup() && GetFlagPickerGuid() == Source->GetObjectGuid())
        {
            ClearFlagPicker();
            Source->remove_auras(BG_EY_NETHERSTORM_FLAG_SPELL);
        }
        return;
    }
    ...
    UpdateWorldState(
        NETHERSTORM_FLAG_STATE_HORDE, BG_EY_FLAG_STATE_WAIT_RESPAWN);
    UpdateWorldState(
        NETHERSTORM_FLAG_STATE_ALLIANCE, BG_EY_FLAG_STATE_WAIT_RESPAWN);

    if (Source->GetTeam() == ALLIANCE)
        SendMessageToAll(
            LANG_BG_EY_DROPPED_FLAG, CHAT_MSG_BG_SYSTEM_ALLIANCE, nullptr);
    else
        SendMessageToAll(
            LANG_BG_EY_DROPPED_FLAG, CHAT_MSG_BG_SYSTEM_HORDE, nullptr);
}

void BattleGroundEY::EventPlayerClickedOnFlag(
    Player* Source, GameObject* target_obj)
{
    if (GetStatus() != STATUS_IN_PROGRESS || IsFlagPickedup() ||
    ...
    {
        UpdateWorldState(
            NETHERSTORM_FLAG_STATE_ALLIANCE, BG_EY_FLAG_STATE_ON_PLAYER);
    ...
    if (Source->GetTeam() == ALLIANCE)
        PSendMessageToAll(LANG_BG_EY_HAS_TAKEN_FLAG,
            CHAT_MSG_BG_SYSTEM_ALLIANCE, nullptr, Source->GetName());
    else
        PSendMessageToAll(LANG_BG_EY_HAS_TAKEN_FLAG, CHAT_MSG_BG_SYSTEM_HORDE,
            nullptr, Source->GetName());
}
...
void BattleGroundEY::EventTeamLostPoint(Player* Source, uint32 Point)
{
    ...
    if (team == ALLIANCE)
        SendMessageToAll(LoosingPointTypes[Point].MessageIdAlliance,
            CHAT_MSG_BG_SYSTEM_ALLIANCE, Source);
    else
        SendMessageToAll(LoosingPointTypes[Point].MessageIdHorde,
            CHAT_MSG_BG_SYSTEM_HORDE, Source);
...
void BattleGroundEY::EventTeamCapturedPoint(Player* Source, uint32 Point)
{
    ...
    if (team == ALLIANCE)
        SendMessageToAll(CapturingPointTypes[Point].MessageIdAlliance,
            CHAT_MSG_BG_SYSTEM_ALLIANCE, Source);
    else
        SendMessageToAll(CapturingPointTypes[Point].MessageIdHorde,
            CHAT_MSG_BG_SYSTEM_HORDE, Source);
...
void BattleGroundEY::EventPlayerCapturedFlagEY(Player* Source, BG_EY_Nodes node)
{
    if (GetStatus() != STATUS_IN_PROGRESS ||
        GetFlagPickerGuid() != Source->GetObjectGuid())
        return;
    ...
    if (Source->GetTeam() == ALLIANCE)
    {
        ...
        SendMessageToAll(
            LANG_BG_EY_CAPTURED_FLAG_A, CHAT_MSG_BG_SYSTEM_ALLIANCE, Source);
    }
    else
    {
        team_id = BG_TEAM_HORDE;
        SendMessageToAll(
            LANG_BG_EY_CAPTURED_FLAG_H, CHAT_MSG_BG_SYSTEM_HORDE, Source);
    }

    if (m_TeamPointsCount[team_id] > 0)
        AddPoints(Source->GetTeam(),
            BG_EY_FlagPoints[m_TeamPointsCount[team_id] - 1]);

    UpdatePlayerScore(Source, SCORE_FLAG_CAPTURES, 1);
}

void BattleGroundEY::UpdatePlayerScore(
...
void BattleGroundEY::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    FillInitialWorldState(
        data, count, EY_HORDE_BASE, m_TeamPointsCount[BG_TEAM_HORDE]);
    FillInitialWorldState(
        data, count, EY_ALLIANCE_BASE, m_TeamPointsCount[BG_TEAM_ALLIANCE]);
    ...
    FillInitialWorldState(data, count, DRAENEI_RUINS_HORDE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_DRAENEI_RUINS] == HORDE &&
            m_PointState[BG_EY_NODE_DRAENEI_RUINS] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, DRAENEI_RUINS_ALLIANCE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_DRAENEI_RUINS] == ALLIANCE &&
            m_PointState[BG_EY_NODE_DRAENEI_RUINS] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, DRAENEI_RUINS_UNCONTROL,
        m_PointState[BG_EY_NODE_DRAENEI_RUINS] != EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, MAGE_TOWER_ALLIANCE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_MAGE_TOWER] == ALLIANCE &&
            m_PointState[BG_EY_NODE_MAGE_TOWER] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, MAGE_TOWER_HORDE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_MAGE_TOWER] == HORDE &&
            m_PointState[BG_EY_NODE_MAGE_TOWER] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, MAGE_TOWER_UNCONTROL,
        m_PointState[BG_EY_NODE_MAGE_TOWER] != EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, FEL_REAVER_HORDE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_FEL_REAVER] == HORDE &&
            m_PointState[BG_EY_NODE_FEL_REAVER] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, FEL_REAVER_ALLIANCE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_FEL_REAVER] == ALLIANCE &&
            m_PointState[BG_EY_NODE_FEL_REAVER] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, FEL_REAVER_UNCONTROL,
        m_PointState[BG_EY_NODE_FEL_REAVER] != EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, BLOOD_ELF_HORDE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_BLOOD_ELF] == HORDE &&
            m_PointState[BG_EY_NODE_BLOOD_ELF] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, BLOOD_ELF_ALLIANCE_CONTROL,
        m_PointOwnedByTeam[BG_EY_NODE_BLOOD_ELF] == ALLIANCE &&
            m_PointState[BG_EY_NODE_BLOOD_ELF] == EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(data, count, BLOOD_ELF_UNCONTROL,
        m_PointState[BG_EY_NODE_BLOOD_ELF] != EY_POINT_UNDER_CONTROL);
    FillInitialWorldState(
        data, count, NETHERSTORM_FLAG, m_FlagState == BG_EY_FLAG_STATE_ON_BASE);
...
WorldSafeLocsEntry const* BattleGroundEY::GetClosestGraveYard(Player* player)
{
    ...
    {
        logging.error(
            "BattleGroundEY: Not found the main team graveyard. Graveyard "
            "system isn't working!");
        return nullptr;
    }

    float plr_x = player->GetX();
    float plr_y = player->GetY();
    float plr_z = player->GetZ();

    distance = (entry->x - plr_x) * (entry->x - plr_x) +
               (entry->y - plr_y) * (entry->y - plr_y) +
               (entry->z - plr_z) * (entry->z - plr_z);
    nearestDistance = distance;

    for (uint8 i = 0; i < BG_EY_NODES_MAX; ++i)
    {
        if (m_PointOwnedByTeam[i] == player->GetTeam() &&
            m_PointState[i] == EY_POINT_UNDER_CONTROL)
        {
            entry = sWorldSafeLocsStore.LookupEntry(
                CapturingPointTypes[i].GraveYardId);
            if (!entry)
                logging.error("BattleGroundEY: Not found graveyard: %u",
                    CapturingPointTypes[i].GraveYardId);
            else
            {
                distance = (entry->x - plr_x) * (entry->x - plr_x) +
