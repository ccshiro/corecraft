...
void WorldSession::SendNameQueryOpcodeFromDB(ObjectGuid guid)
{
    CharacterDatabase.AsyncPQuery(
        &WorldSession::SendNameQueryOpcodeFromDBCallBack, GetAccountId(),
        !sWorld::Instance()->getConfig(CONFIG_BOOL_DECLINED_NAMES_USED) ?
            ...
            "FROM characters LEFT JOIN character_declinedname ON "
            "characters.guid = character_declinedname.guid WHERE "
            "characters.guid = '%u'",
        guid.GetCounter());
}
...
void WorldSession::HandleNameQueryOpcode(WorldPacket& recv_data)
{
    ...
    Player* pChar = sObjectMgr::Instance()->GetPlayer(guid);
...
void WorldSession::HandleCreatureQueryOpcode(WorldPacket& recv_data)
{
    ...
    if (ci)
    {
        ...
        data << uint32(
            ci->PetSpellDataId); // Id from CreatureSpellData.dbc    wdbField12
    ...
    if (info)
    {
        ...
        if (loc_idx >= 0)
        {
            GameObjectLocale const* gl =
                sObjectMgr::Instance()->GetGameObjectLocale(entryID);
            if (gl)
            {
                if (gl->Name.size() > size_t(loc_idx) &&
                    !gl->Name[loc_idx].empty())
                    Name = gl->Name[loc_idx];
                if (gl->CastBarCaption.size() > size_t(loc_idx) &&
                    !gl->CastBarCaption[loc_idx].empty())
    ...
    if (corpsemapid != _player->GetMapId())
    {
        ...
        if (MapEntry const* corpseMapEntry = sMapStore.LookupEntry(corpsemapid))
        {
            if (corpseMapEntry->IsDungeon() &&
                corpseMapEntry->ghost_entrance_map >= 0)
            {
                // if corpse map have entrance
                if (TerrainInfo const* entranceMap =
                        sTerrainMgr::Instance()->LoadTerrain(
                            corpseMapEntry->ghost_entrance_map))
                {
...
void WorldSession::HandlePageTextQueryOpcode(WorldPacket& recv_data)
{
    ...
    while (pageID)
    {
        ...
        else
        {
            ...
            if (loc_idx >= 0)
            {
                PageTextLocale const* pl =
                    sObjectMgr::Instance()->GetPageTextLocale(pageID);
                if (pl)
                {
                    if (pl->Text.size() > size_t(loc_idx) &&
                        !pl->Text[loc_idx].empty())
...
void WorldSession::SendQueryTimeResponse()
{
    ...
    data << uint32(WorldTimer::time_no_syscall());
    data << uint32(sWorld::Instance()->GetNextDailyQuestsResetTime() -
