...
#define RACEMASK_ALL_PLAYABLE                                  \
    ((1 << (RACE_HUMAN - 1)) | (1 << (RACE_ORC - 1)) |         \
        ...
        (1 << (RACE_UNDEAD - 1)) | (1 << (RACE_TAUREN - 1)) |  \
        (1 << (RACE_GNOME - 1)) | (1 << (RACE_TROLL - 1)) |    \
        (1 << (RACE_BLOODELF - 1)) | (1 << (RACE_DRAENEI - 1)))

// for most cases batter use ChrRace data for team check as more safe, but when
// need full mask of team can be use this defines.
...
#define RACEMASK_HORDE                                       \
    ((1 << (RACE_ORC - 1)) | (1 << (RACE_UNDEAD - 1)) |      \
        (1 << (RACE_TAUREN - 1)) | (1 << (RACE_TROLL - 1)) | \
        (1 << (RACE_BLOODELF - 1)))
...
enum Classes
{
    ...
    CLASS_ROGUE = 4,
    CLASS_PRIEST = 5,
    // CLASS_DEATH_KNIGHT  = 6,                              // not listed in
    // DBC, will be in 3.0
    CLASS_SHAMAN = 7,
    CLASS_MAGE = 8,
    CLASS_WARLOCK = 9,
...
#define CLASSMASK_ALL_CREATURES                                \
    ((1 << (CLASS_WARRIOR - 1)) | (1 << (CLASS_PALADIN - 1)) | \
        (1 << (CLASS_ROGUE - 1)) | (1 << (CLASS_MAGE - 1)))
...
#define SPELL_SCHOOL_MASK_MAGIC                              \
    (SPELL_SCHOOL_MASK_HOLY | SPELL_SCHOOL_MASK_FIRE |       \
...
enum SpellAttributes
{
    ...
    SPELL_ATTR_TRADESPELL = 0x00000020,      // 5 trade spells, will be added by
    // client to a sublist of profession
    // spell
    SPELL_ATTR_PASSIVE = 0x00000040, // 6 Passive spell
    SPELL_ATTR_UNK7 = 0x00000080,    // 7
    SPELL_ATTR_UNK8 =
        0x00000100, // 8 hide created item in tooltip (for effect=24)
    ...
    SPELL_ATTR_DAYTIME_ONLY =
        0x00001000, // 12 only useable at daytime, not set in 2.4.2
    SPELL_ATTR_NIGHT_ONLY =
        0x00002000, // 13 only useable at night, not set in 2.4.2
    SPELL_ATTR_INDOORS_ONLY =
        0x00004000, // 14 only useable indoors, not set in 2.4.2
    ...
    SPELL_ATTR_LEVEL_DAMAGE_CALCULATION =
        0x00080000, // 19 spelldamage depends on caster level
    SPELL_ATTR_STOP_ATTACK_TARGET = 0x00100000, // 20 Stop attack after use this
                                                // spell (and not begin attack
                                                // if use)
    SPELL_ATTR_IMPOSSIBLE_DODGE_PARRY_BLOCK =
        0x00200000, // 21 Cannot be dodged/parried/blocked
    ...
    SPELL_ATTR_DISABLED_WHILE_ACTIVE =
        0x02000000, // 25 Activate and start cooldown after aura fade or remove
                    // summoned creature or go
    ...
    SPELL_ATTR_UNAFFECTED_BY_INVULNERABILITY =
        0x20000000, // 29 unaffected by invulnerability (hmm possible not...)
    SPELL_ATTR_AFFECTED_BY_HEARTBEAT =
        0x40000000, // 30 Affected by hearbeat resist checks
...
enum SpellAttributesEx
{
    ...
    SPELL_ATTR_EX_CHANNEL_TRACK_TARGET = 0x00004000, // 14 Client tracks target
    SPELL_ATTR_EX_DISPEL_AURAS_ON_IMMUNITY =
        0x00008000, // 15 remove auras on immunity
    SPELL_ATTR_EX_UNAFFECTED_BY_SCHOOL_IMMUNE =
        0x00010000, // 16 unaffected by school immunity
    SPELL_ATTR_EX_NOT_AUTOCASTABLE =
        0x00020000, // 17 These pet abilites cannot be toggled to be auto-casted
    SPELL_ATTR_EX_UNK18 = 0x00040000,            // 18
    SPELL_ATTR_EX_CANT_TARGET_SELF = 0x00080000, // 19 Cannot target self
                                                 // (usually in conjunction with
                                                 // friendly AoE spells)
    SPELL_ATTR_EX_REQ_TARGET_COMBO_POINTS =
        0x00100000,                   // 20 Req combo points on target
    SPELL_ATTR_EX_UNK21 = 0x00200000, // 21
    SPELL_ATTR_EX_REQ_COMBO_POINTS = 0x00400000, // 22 Use combo points (in 4.x
                                                 // not required combo point
                                                 // target selected)
...
enum SpellAttributesEx2
{
    SPELL_ATTR_EX2_CAN_TARGET_DEAD =
        0x00000001,                   // 0 Can target dead unit or corpse
    SPELL_ATTR_EX2_UNK1 = 0x00000002, // 1
    SPELL_ATTR_EX2_CANT_REFLECTED =
        0x00000004, // 2 ? used for detect can or not spell reflected // do not
                    // need LOS (e.g. 18220 since 3.3.3)
    SPELL_ATTR_EX2_UNK3 = 0x00000008, // 3 auto targeting? (e.g. fishing skill
                                      // enhancement items since 3.3.3)
    ...
    SPELL_ATTR_EX2_UNK17 = 0x00020000, // 17 suspend weapon timer instead of
                                       // resetting it, (?Hunters Shot and
                                       // Stings only have this flag?)
    SPELL_ATTR_EX2_UNK18 =
        0x00040000, // 18 Only Revive pet - possible req dead pet
    SPELL_ATTR_EX2_NOT_NEED_SHAPESHIFT =
        0x00080000, // 19 does not necessary need shapeshift (pre-3.x not have
                    // passive spells with this attribute)
    SPELL_ATTR_EX2_UNK20 = 0x00100000, // 20
    SPELL_ATTR_EX2_DAMAGE_REDUCED_SHIELD =
        0x00200000, // 21 for ice blocks, pala immunity buffs, priest absorb
                    // shields, but used also for other spells -> not sure!
    SPELL_ATTR_EX2_UNK22 = 0x00400000, // 22
    SPELL_ATTR_EX2_UNK23 =
        0x00800000, // 23 Only mage Arcane Concentration have this flag
    ...
    SPELL_ATTR_EX2_FOOD_BUFF =
        0x80000000, // 31 Food or Drink Buff (like Well Fed)
};

enum SpellAttributesEx3
{
    SPELL_ATTR_EX3_DONT_AFFECT_COMBAT = 0x00000001, // 0 Does not affect combat
    SPELL_ATTR_EX3_UNK1 = 0x00000002,               // 1
    SPELL_ATTR_EX3_UNK2 = 0x00000004,               // 2
    SPELL_ATTR_EX3_UNK3 = 0x00000008,               // 3
    SPELL_ATTR_EX3_UNK4 =
        0x00000010, // 4 Druid Rebirth only this spell have this flag
    SPELL_ATTR_EX3_UNK5 = 0x00000020, // 5
    SPELL_ATTR_EX3_UNK6 = 0x00000040, // 6
    SPELL_ATTR_EX3_SEPARATE_STACKS =
        0x00000080, // 7 create a separate (de)buff stack for each caster
    SPELL_ATTR_EX3_ONLY_TARGET_PLAYERS = 0x00000100, // 8 Spell can only target
                                                     // players (used for AoE
                                                     // target selection)
    ...
    SPELL_ATTR_EX3_BATTLEGROUND =
        0x00000800, // 11 Can casted only on battleground
    SPELL_ATTR_EX3_CAST_ON_DEAD = 0x00001000, // 12 target is a dead player (not
                                              // every spell has this flag)
    SPELL_ATTR_EX3_UNK13 = 0x00002000,        // 13
    SPELL_ATTR_EX3_UNK14 =
        0x00004000, // 14 "Honorless Target" only this spells have this flag
    SPELL_ATTR_EX3_RANGED_ATT_TIMER = 0x00008000, // 15 uses ranged attack timer
    SPELL_ATTR_EX3_UNK16 =
        0x00010000, // 16 no triggers effects that trigger on casting a spell??
    SPELL_ATTR_EX3_NO_INITIAL_AGGRO =
        0x00020000, // 17 Causes no aggro if not missed
    SPELL_ATTR_EX3_CANT_MISS =
        0x00040000,                    // 18 Spell should always hit its target
    ...
    SPELL_ATTR_EX3_NO_DAMAGE_BONUS =
        0x20000000, // 29 Spell does not scale with bonus damage effects
...
{
    SPELL_ATTR_EX4_NOT_RESISTABLE = 0x00000001, // Cannot be resisted
    SPELL_ATTR_EX4_UNK1 = 0x00000002,           // 1 proc on finishing move?
    SPELL_ATTR_EX4_UNK2 = 0x00000004,           // 2
    SPELL_ATTR_EX4_UNK3 = 0x00000008,           // 3
    SPELL_ATTR_EX4_UNK4 =
        0x00000010, // 4 This will no longer cause guards to attack on use??
    SPELL_ATTR_EX4_UNK5 = 0x00000020,          // 5
    SPELL_ATTR_EX4_NOT_STEALABLE = 0x00000040, // 6 although such auras might be
                                               // dispellable, they cannot be
                                               // stolen
    SPELL_ATTR_EX4_UNK7 = 0x00000080,          // 7
    SPELL_ATTR_EX4_DOT_ADD_REMAINING_DAMAGE =
        0x00000100, // 8 When overwriting a DoT with the same DoT, the damage
                    // from the last DoT is added to the new (Mage's Ignite)
    SPELL_ATTR_EX4_UNK9 = 0x00000200, // 9
    SPELL_ATTR_EX4_SPELL_VS_EXTEND_COST =
        0x00000400,                    // 10 Rogue Shiv have this flag
    ...
    SPELL_ATTR_EX4_NO_PUSHBACK_OR_CC_BREAKAGE =
        0x00004000, // 14 Spells that cause damage but do not cause pushback or
                    // break CC (_might_ also indicate it can't proc spells
                    // (molten armor))
    ...
    SPELL_ATTR_EX4_UNK20 =
        0x00100000, // 20 do not give "more powerful spell" error message
    ...
    SPELL_ATTR_EX4_CAST_ONLY_IN_OUTLAND =
        0x04000000,                    // 26 Can only be used in Outland.
...
enum SpellAttributesEx5
{
    ...
    SPELL_ATTR_EX5_UNK2 =
        0x00000004, // 2 removed at enter arena (e.g. 31850 since 3.3.3)
    ...
    SPELL_ATTR_EX5_SINGLE_TARGET_SPELL =
        0x00000020, // 5 Only one target can be apply at a time
    ...
    SPELL_ATTR_EX5_START_PERIODIC_AT_APPLY =
        0x00000200,                    // 9  begin periodic tick at aura apply
    SPELL_ATTR_EX5_UNK10 = 0x00000400, // 10
    SPELL_ATTR_EX5_UNK11 = 0x00000800, // 11
    SPELL_ATTR_EX5_UNK12 = 0x00001000, // 12
    SPELL_ATTR_EX5_APPLIES_HASTE_BONUS = 0x00002000, // 13 channeled - haste
                                                     // affects duration (e.g.
                                                     // 8050 since 3.3.3)
    ...
    SPELL_ATTR_EX5_USABLE_WHILE_CONFUSED =
        0x00040000, // 18 usable while confused
    SPELL_ATTR_EX5_DONT_TARGET_WHILE_CASTING =
        0x00080000, // 19 Creature casting this spell should not target or turn
                    // towards its target when casting
    SPELL_ATTR_EX5_UNK20 = 0x00100000, // 20
    SPELL_ATTR_EX5_UNK21 = 0x00200000, // 21
    SPELL_ATTR_EX5_UNK22 = 0x00400000, // 22
    SPELL_ATTR_EX5_DOT_IGNORE_RESILIENCE =
        0x00800000,                    // 23 DoT not affected by resilience
    ...
    SPELL_ATTR_EX5_UNK31 =
        0x80000000, // 31 Forces all nearby enemies to focus attacks caster
};

enum SpellAttributesEx6
{
    SPELL_ATTR_EX6_UNK0 = 0x00000001, // 0 Only Move spell have this flag
    SPELL_ATTR_EX6_ONLY_IN_ARENA =
        0x00000002, // 1 only usable in arena, not used in 3.2.0a and early
    ...
    SPELL_ATTR_EX6_CANT_TARGET_CROWD_CONTROLLED =
        0x00000100, // 8 Spell cannot target crowd controlled targets (used for
                    // bouncing & AoE spells only -- in TBC, only for avenger's
                    // shield)
    SPELL_ATTR_EX6_UNK9 = 0x00000200,  // 9
    SPELL_ATTR_EX6_UNK10 = 0x00000400, // 10
    SPELL_ATTR_EX6_NOT_IN_RAID_INSTANCE =
        0x00000800, // 11 not usable in raid instance
    SPELL_ATTR_EX6_UNK12 =
        0x00001000, // 12 for auras SPELL_AURA_TRACK_CREATURES,
                    // SPELL_AURA_TRACK_RESOURCES and SPELL_AURA_TRACK_STEALTHED
                    // select non-stacking tracking spells
    ...
    SPELL_ATTR_EX6_UNK29 = 0x20000000, // 29 do not apply damage mods (usually
                                       // in cases where it has already been
                                       // applied)
...
{
    SPELL_ATTR_CUSTOM_RESIST_OVER_60 =
        0x00000001, // Chance to resist spell increases over level 60
    SPELL_ATTR_CUSTOM_CANT_TARGET_SELF =
        0x00000002, // SPELL_ATTR_EX_CANT_TARGET_SELF was multi-purpose before,
                    // all spells that only had the property of not being able
                    // to click on and then use yourself as main target have
                    // been moved to this attribute
    SPELL_ATTR_CUSTOM_ALWAYS_CASTABLE =
        0x00000004, // Ignores buff-stacking rules in Spell::CheckCast;
                    // application of auras can still fail; but the spell will
                    // always go off
    SPELL_ATTR_CUSTOM_CASTABLE_WHILE_DEAD = 0x00000008, // Spell can be casted
                                                        // (and do damage) while
                                                        // the caster is dead
    SPELL_ATTR_CUSTOM_IGNORE_BUFF_STACKING =
        0x00000010, // Ignores buff-stacking against other spell auras, same
                    // effect as if the spell had SPELL_ATTR_PASSIVE
    SPELL_ATTR_CUSTOM_RESILIENCE_CRIT_PROC =
        0x00000020, // Spell procs from resilience as well, see
                    // Player::DoResilienceCritProc
    SPELL_ATTR_CUSTOM_FIZZLE_OVER_60 =
        0x00000040, // Chance to fizzle over level 60
    SPELL_ATTR_CUSTOM_POSITIVE =
        0x00000080, // Opposite of SPELL_ATTR_EX_NEGATIVE
    SPELL_ATTR_CUSTOM_IGNORE_ALL_BUFF_STACK =
        0x00000100, // Ignore ALL buff-stacking rules (not the same as 0x10)
    SPELL_ATTR_CUSTOM_DAZE_EFFECT =
        0x00000200, // The spell is a "daze" effect (such as dazed, blast wave,
                    // aftermath, blade twisting, etc.)
    SPELL_ATTR_CUSTOM_REFRESH_MODIFIERS =
        0x00000400, // Modifiers will be recalculated when a stacking buff is
                    // re-applied (at the time of writing only Lifebloom is
                    // known to have this effect, and it received it in Patch
                    // 2.3.2)
    SPELL_ATTR_CUSTOM_DONT_USE_VISIBLE_AURA_SLOT =
        0x00000800, // Aura does not get sent to client
    SPELL_ATTR_CUSTOM_DONT_SAVE_AURA = 0x00001000, // Aura is not saved to the
                                                   // database, and will always
                                                   // disappear on relog
    SPELL_ATTR_CUSTOM_NO_SPELL_QUEUE = 0x00002000, // Spell is NOT added to
                                                   // target's spell queue, and
                                                   // IS processed right away
    SPELL_ATTR_CUSTOM_FORCE_SPELL_QUEUE =
        0x00004000, // Spell IS added to target's spell queue, and is NOT
                    // processed right away
    SPELL_ATTR_CUSTOM_DISABLE_ON_TRANSPORT =
        0x00008000, // Spell cannot be used while on a transport
    SPELL_ATTR_CUSTOM_CANNOT_TRIGGER_PROCS =
        0x00010000, // Spell can never trigger other spells/procs
    // AoE (Persist Area Aura) spell does not trigger an initial proc/unstealth
    SPELL_ATTR_CUSTOM_NO_INITIAL_BLAST = 0x00020000,
    // Spell is blockable despite having no damage component
    SPELL_ATTR_CUSTOM_BLOCKABLE = 0x00040000,
    SPELL_ATTR_CUSTOM_DONT_BREAK_TARGET_STEALTH = 0x00080000,
    SPELL_ATTR_CUSTOM_CAST_FROM_BEHIND = 0x00100000,
    SPELL_ATTR_CUSTOM_TARGET_IN_FRONT = 0x00200000,
    // Panic fear starts moving randomly in fear instead of away from caster
    SPELL_ATTR_CUSTOM_PANIC_FEAR = 0x00400000,
    // TARGET_IN_FRONT_OF_CASTER becomes behind caster,
    // TARGET_LARGE_FRONTAL_CONE becomes backwards cone, etc
    SPELL_ATTR_CUSTOM_REVERSE_DIRECTION = 0x00800000,
    // Physical spell is not affected by target's armor reduction
    SPELL_ATTR_CUSTOM_IGNORES_ARMOR = 0x01000000,
    SPELL_ATTR_CUSTOM_TELEPORT_NO_LEAVE_COMBAT = 0x02000000,
    SPELL_ATTR_CUSTOM_360_BOUNCE = 0x04000000,
    // mod_charm and aoe_charm spells that can turn targets into healers
    SPELL_ATTR_CUSTOM_CHARM_HEALERS_POSSIBLE = 0x08000000,
    // While this spell aura is on the target it's blacklisted in the
    // ThreatManager
    SPELL_ATTR_CUSTOM_AURA_CAUSES_BLACKLIST = 0x10000000,
};

enum SpellAttributesCustom1
{
    // Ignores all Line of Sight checks
    SPELL_ATTR_CUSTOM1_IGNORES_LOS = 0x00000001,
    // AoE spell still checks LoS to all targets despite being triggered
    SPELL_ATTR_CUSTOM1_AOE_FORCE_LOS = 0x00000002,
    // Positive aura is removed from NPC when evading
    SPELL_ATTR_CUSTOM1_CANCEL_ON_EVADE = 0x00000004,
};
...
enum Language
{
    ...
    LANG_ADDON = 0xFFFFFFFF // used by addons, in 2.4.0 not exit, replaced by
                            // messagetype?
};
...
enum Team
{
    TEAM_NONE = 0, // used when team value unknown or not set, 0 is also meaning
                   // that can be used !team check
    HORDE = 67,
    ALLIANCE = 469,
};
...
enum SpellCastResult
{
    ...
    SPELL_CAST_SERVERSIDE_SPECIAL_CHARGE =
        0xFE,            // custom value, don't send to client
    SPELL_CAST_OK = 0xFF // custom value, don't send to client
};
...
#define IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK              \
    ((1 << (MECHANIC_CHARM - 1)) | (1 << (MECHANIC_DISORIENTED - 1)) |   \
        (1 << (MECHANIC_FEAR - 1)) | (1 << (MECHANIC_ROOT - 1)) |        \
        (1 << (MECHANIC_PACIFY - 1)) | (1 << (MECHANIC_SLEEP - 1)) |     \
        (1 << (MECHANIC_SNARE - 1)) | (1 << (MECHANIC_STUN - 1)) |       \
        (1 << (MECHANIC_FREEZE - 1)) | (1 << (MECHANIC_KNOCKOUT - 1)) |  \
        (1 << (MECHANIC_POLYMORPH - 1)) | (1 << (MECHANIC_BANISH - 1)) | \
        (1 << (MECHANIC_SHACKLE - 1)) | (1 << (MECHANIC_TURN - 1)) |     \
        (1 << (MECHANIC_HORROR - 1)) | (1 << (MECHANIC_DAZE - 1)) |      \
        (1 << (MECHANIC_SAPPED - 1)))
...
// type can't cast to it,
// some spell_effects that related to ImmuneToEffect<effect>(only this effect in
// the spell) can't cast to it,
...
enum Targets
{
    TARGET_NONE = 0,
    TARGET_SELF = 1,
    TARGET_RANDOM_ENEMY_CHAIN_IN_AREA = 2, // only one spell has that, but
                                           // regardless, it's a target type
                                           // after all
    TARGET_RANDOM_FRIEND_CHAIN_IN_AREA = 3,
    TARGET_RANDOM_FRIEND_CHAIN_IN_AREA2 =
        4, // Not sure how this and the above differ, might be that one cares
           // about being in a party and the other doesn't
    TARGET_PET = 5,
    ...
    TARGET_ALL_FRIENDLY_UNITS_AROUND_CASTER =
        30, // select friendly for caster object faction (in different original
            // caster faction) in TargetB used only with
            // TARGET_ALL_AROUND_CASTER and in self casting range in TargetA
    ...
    TARGET_AREAEFFECT_GO_AROUND_DEST =
        52, // gameobject around destination, select by spell_script_target
    ...
    TARGET_BEHIND_VICTIM = 65, // used in teleport behind spells, caster/target
    ...
    TARGET_POINT_AT_NORTH = 78, // 78-85 possible _COORDINATES at radius with
                                // pi/4 step around target in unknown order, N?
    TARGET_POINT_AT_SOUTH = 79, // S?
    TARGET_POINT_AT_EAST = 80,  // 80/81 must be symmetric from line
    // caster->target, E (base at 82/83, 84/85 order)
    // ?
    TARGET_POINT_AT_WEST = 81, // 80/81 must be symmetric from line
                               // caster->target, W (base at 82/83, 84/85 order)
                               // ?
...
#define MAX_GAMEOBJECT_TYPE \
    35 // sending to client this or greater value can crash client.
...
enum GameObjectFlags
{
    ...
    GO_FLAG_NO_INTERACT = 0x00000010, // players cannot interact with this go
                                      // (often need to remove flag in event)
...
enum CreatureTypeFlags
{
    ...
    CREATURE_TYPEFLAGS_GHOST_VISIBLE =
        0x00000002, // Creatures which can _also_ be seen when player is a
                    // ghost, used in CanInteract function by client, can't be
                    // attacked
    CREATURE_TYPEFLAGS_BOSS_NPC = 0x00000004, // "BOSS" flag for tooltips
    CREATURE_TYPEFLAGS_UNK4 = 0x00000008,
    CREATURE_TYPEFLAGS_UNK5 = 0x00000010, // controls something in client
                                          // tooltip related to creature faction
    CREATURE_TYPEFLAGS_UNK6 = 0x00000020, // may be sound related
    CREATURE_TYPEFLAGS_UNK7 =
        0x00000040, // may be related to attackable / not attackable creatures
                    // with spells, used together with
                    // lua_IsHelpfulSpell/lua_IsHarmfulSpell
    CREATURE_TYPEFLAGS_UNK8 = 0x00000080, // has something to do with unit
                                          // interaction / quest status requests
    ...
    CREATURE_TYPEFLAGS_UNK12 =
        0x00000800, // related to possibility to cast spells while mounted
    CREATURE_TYPEFLAGS_CAN_ASSIST = 0x00001000, // Can aid any player (and
                                                // group) in combat. Typically
                                                // seen for escorting NPC's
    CREATURE_TYPEFLAGS_UNK14 =
        0x00002000, // checked from calls in Lua_PetHasActionBar
    CREATURE_TYPEFLAGS_UNK15 = 0x00004000, // Lua_UnitGUID, client does guid_low
                                           // &= 0xFF000000 if this flag is set
    CREATURE_TYPEFLAGS_ENGINEERLOOT = 0x00008000, // Can be looted by engineer

    CREATURE_TYPEFLAGS_PARTY_MEMBER =
        0x04000000, //! Creature can be targeted by spells that require target
                    // to be in caster's party/raid (NOTE: Port from TC, is this
                    // valid in 2.4.3?)
};
...
enum DiminishingGroup
{
    ...
    DIMINISHING_TRIGGER_STUN, // By aura proced stuns, usualy chance on hit
                              // talents
    DIMINISHING_SLEEP,
    DIMINISHING_CONTROL_ROOT, // Immobilizing effects from casted spells
    DIMINISHING_TRIGGER_ROOT, // Immobilizing effects from triggered spells like
                              // Frostbite
    ...
    DIMINISHING_POLYMORPH_KNOCKOUT, // Also with Polymorph and Sap, all Knockout
                                    // mechanics are here
    DIMINISHING_BANISH,
    // Other
...
enum InstanceResetMethod
{
    ...
    INSTANCE_RESET_RESPAWN_DELAY // called from reset scheduler for request
                                 // reset at map unload when map loaded at reset
                                 // attempt for normal dungeon difficulty
};

enum InstanceResetFailMsg
{
    INSTANCE_RESET_FAIL_INSIDE =
        0,                       // There are players still inside the instance
    INSTANCE_RESET_FAIL_OFFLINE, // There are players offline in your party
    INSTANCE_RESET_FAIL_ZONING, // There are players in your party attempting to
...
{
    return type == ARENA_TYPE_2v2 || type == ARENA_TYPE_3v3 ||
           type == ARENA_TYPE_5v5;
}
...
enum TradeStatus
{
    ...
    TRADE_STATUS_TRADE_CANCELLED = 3,
    TRADE_STATUS_TRADE_ACCEPT = 4,
    TRADE_STATUS_TRADE_UNACCEPT = 5,
    TRADE_STATUS_NO_TARGET = 6,
    TRADE_STATUS_BACK_TO_TRADE = 7,
    TRADE_STATUS_TRADE_COMPLETE = 8,
    // 9?
    TRADE_STATUS_TARGET_TOO_FAR = 10,
...
// Some Unit actions should be updated in server-wide batches
enum class BatchUpdates : int
{
    spells = 0, // Only used for spell queue atm
...
#define EXPECTED_MANGOSD_CLIENT_BUILD \
    {                                 \
        8606, 0                       \
    }

// max supported expansion level in mangosd
// NOTE: not set it more that supported by targeted client version with all
// expansions installed
// account with expansion > client supported will rejected at connection by
// client
...
// distance that chased target can move before TargetedMovementGeneratorMedium
// draws a new path
#define CHASE_MOVE_RECALC_POS_DIST 1.5f

// We cannot go lower than this update time. If the config value is less than
// this, we use this value.
#define MIN_MAP_UPDATE_DELAY 100

// creature_template.entry of waypoint wisp
#define VISUAL_WAYPOINT 1

// Default aggro distance for same leveled NPCs
#define NPC_DEFAULT_AGGRO_DIST 18.0f

// Default chain aggro distance
#define NPC_DEFAULT_CHAIN_DIST 10.0f
