 ...
 *
 * @file MassMailMgr.h
 * This file contains the the headers needed for MaNGOS to handle mass mails
 *send in safe and perfomence not affecting way.
 *
 */
...
/**
 * A class to represent the mail send factory to multiple (often all existing)
 *characters.
 *
...
public: // modifiers
     ...
     *
     * @param mailProto     prepared mail for clone and send to characters, will
     *deleted in result call.
     * @param raceMask      mask of races that must receive mail.
     *
     * Note: this function safe to be called from Map::Update content/etc, real
     *data add will executed in next tick after query results ready
     */
    void AddMassMailTask(
        MailDraft* mailProto, MailSender sender, uint32 raceMask);
     ...
     * @param mailProto     prepared mail for clone and send to characters, will
     *deleted in result call
     * @param queryStr      SQL query for get guid list of receivers, first
     *field in query result must be uint32 low guids list.
     *
     * Note: this function safe to be called from Map::Update content/etc, real
     *data add will executed in next tick after query results ready
     */
    void AddMassMailTask(
        MailDraft* mailProto, MailSender sender, char const* queryStr);
...
private:
    /// Mass mail task store mail prototype and receivers list who not get mail
    /// yet
    struct MassMail
    {
        explicit MassMail(MailDraft* mailProto, MailSender sender)
          : m_protoMail(mailProto), m_sender(std::move(sender))
        {
            assert(mailProto);
        }

        MassMail(MassMail const& massmail)
          : m_protoMail(std::move(const_cast<MassMail&>(massmail).m_protoMail)),
            m_sender(massmail.m_sender)
        {
        }

        /// m_protoMail is owned by MassMail, so at copy original MassMail field
        /// set to NULL
...
#define sMassMailMgr MaNGOS::UnlockedSingleton<MassMailMgr>
