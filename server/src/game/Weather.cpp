...
{
    m_timer.SetInterval(
        sWorld::Instance()->getConfig(CONFIG_UINT32_INTERVAL_CHANGEWEATHER));
    m_type = WEATHER_TYPE_FINE;
    m_grade = 0;

    LOG_DEBUG(logging,
        "WORLD: Starting weather system for zone %u (change every %u minutes).",
        m_zone, (m_timer.GetInterval() / (MINUTE * IN_MILLISECONDS)));
}
...
bool Weather::ReGenerate()
{
    ...
    time_t gtime = WorldTimer::time_no_syscall();
    ...
    LOG_DEBUG(logging, "Generating a change in %s weather for zone %u.",
    ...
    uint32 rnd = urand(0, 99);
    if (chance1 != 0 && rnd <= chance1)
        m_type = WEATHER_TYPE_RAIN;
    else if (chance2 != 0 && rnd <= chance2)
        m_type = WEATHER_TYPE_SNOW;
    else if (chance3 != 0 && rnd <= chance3)
...
{
    Player* player = sWorld::Instance()->FindPlayerInZone(m_zone);
    if (!player)
        return false;
...
///- Log the event
    ...
    LOG_DEBUG(logging, "Change the weather of zone %u to %s.", m_zone, wthstr);
#endif
