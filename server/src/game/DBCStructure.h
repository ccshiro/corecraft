...
struct AreaTableEntry
{
    ...
    uint32
        LiquidTypeOverride; // 29       m_liquidTypeID override for water type
                            // 30-32    unknown/unused
                            // 33       m_minElevation
...
{
    uint32 Index;                                           //          m_ID
categoryId (sent in RealmList packet)
...
/*struct Cfg_ConfigsEntry
{
    uint32 Id;                                              //          m_ID
    uint32 Type;                                            //
m_realmType (sent in RealmList packet)
    uint32 IsPvp;                                           //
m_playerKillingAllowed
...
{
    // uint32 Id;                                            // 0        m_ID
    uint32 RaceClassGender; // 1        m_raceID m_classID m_sexID m_outfitID
                            // (UNIT_FIELD_BYTES_0 & 0x00FFFFFF) comparable (0
                            // byte = race, 1 byte = class, 2 byte = gender)
    int32 ItemId[MAX_OUTFIT_ITEMS]; // 2-13     m_ItemID
    // int32 ItemDisplayId[MAX_OUTFIT_ITEMS];                // 14-25
    // m_DisplayItemID not required at server side
    // int32 ItemInventorySlot[MAX_OUTFIT_ITEMS];            // 26-37
    // m_InventoryType not required at server side
    // uint32 Unknown1;                                      // 38, unique
    // values (index-like with gaps ordered in other way as ids)
    // uint32 Unknown2;                                      // 39
    // uint32 Unknown3;                                      // 40
};
...
struct CharTitlesEntry
{
    ...
    // m_name1_lang
    // 35 string flags
    uint32 bit_index; // 36       m_mask_ID used in PLAYER_CHOSEN_TITLE and
                      // 1<<index in PLAYER__FIELD_KNOWN_TITLES
};
...
struct ChrClassesEntry
{
    ...
    // m_name_female_lang
    // 37 string flags
    // char*       nameNeutralGender[16];                    // 38-53
    // m_name_male_lang
    // 54 string flags
    // 55       m_filename
    uint32 spellfamily; // 56       m_spellClassSet
    // uint32 flags2;                                        // 57       m_flags
    // (0x08 HasRelicSlot)
};
...
struct ChrRacesEntry
{
    ...
    char* name[16];           // 14-29    m_name_lang used for DBC language
                              // detection/selection
                              // 30 string flags
    // char*       nameFemale[16];                           // 31-46
    // m_name_female_lang
    // 47 string flags
    // char*       nameNeutralGender[16];                    // 48-63
    // m_name_male_lang
    // 64 string flags
...
struct CreatureDisplayInfoEntry
{
    ...
    // CreatureDisplayInfoExtraEntry::DisplayExtraId
...
{
    uint32 DisplayExtraId; // 0        m_ID
                           // CreatureDisplayInfoEntry::m_extendedDisplayInfoID
    ...
    // m_NPCItemDisplay equipped static items
    // EQUIPMENT_SLOT_HEAD..EQUIPMENT_SLOT_HANDS, client show its by self
    // uint32    CanEquip;                                   // 19       m_flags
    // 0..1 Can equip additional things when used for players
    // char*                                                 // 20
    // m_BakeName CreatureDisplayExtra-*.blp
};
...
struct EmotesEntry
{
    ...
    uint32
        EmoteType; // 4        m_EmoteSpecProc (determine how emote are shown)
...
struct FactionTemplateEntry
{
    ...
    uint32 ourMask; // 3 if mask set (see FactionMasks) then faction included in
                    // masked team
    uint32 friendlyMask; // 4 if mask set (see FactionMasks) then faction
                         // friendly to masked team
    uint32 hostileMask; // 5 if mask set (see FactionMasks) then faction hostile
                        // to masked team
    ...
    bool IsFriendlyTo(FactionTemplateEntry const& entry) const
    {
        if (entry.faction)
        {
            for (auto& elem : enemyFaction)
                if (elem == entry.faction)
                    return false;
            for (auto& elem : friendFaction)
                if (elem == entry.faction)
                    return true;
        }
...
struct ItemClassEntry
{
    ...
    // uint32   unk1;                                        // 1
    // uint32   unk2;                                        // 2        only
    // weapon have 1 in field, other 0
...
//    ItemPrototype::CondExtendedCost
//    uint32      itemextendedcostentry;                      //
//    ItemPrototype::ExtendedCost
//    uint32      arenaseason;                                // arena season
//    number(1-4)
//};
...
{
    uint32 Id;       // 0
    uint32 LiquidId; // 1        1: Coilfang Raid - Water; 23: Water; 29: Ocean;
                     // 35: Magma; 41: Slime; 47: Naxxramas - Slime; 65: Hyjal
                     // Past - Water.
...
struct MapEntry
{
    ...
    // uint32 isPvP;                                         // 3        m_PVP 0
    // or 1 for battlegrounds (not arenas)
    char* name[16];     // 4-19     m_MapName_lang
                        // 20 string flags
                        // 21-23 unused (something PvPZone related - levels?)
                        // 24-26
    uint32 linked_zone; // 27       m_areaTableID
    // char*     hordeIntro[16];                             // 28-43
    // m_MapDescription0_lang
    // 44 string flags
    // char*     allianceIntro[16];                          // 45-60
    // m_MapDescription1_lang
    // 61 string flags
    uint32 multimap_id; // 62       m_LoadingScreenID (LoadingScreens.dbc)
                        // 63-64 not used
    // float   BattlefieldMapIconScale;                      // 65
    // m_minimapIconScale
    // chat*     unknownText1                                // 66-81 unknown
    // empty text fields, possible normal Intro text.
    // 82 text flags
    // chat*     heroicIntroText                             // 83-98 heroic
    // mode requirement text
    // 99 text flags
    // chat*     unknownText2                                // 100-115 unknown
    // empty text fields
    // 116 text flags
    int32 ghost_entrance_map; // 117      m_corpseMapID map_id of entrance map
                              // in ghost mode (continent always and in most
                              // cases = normal entrance)
    float ghost_entrance_x; // 118      m_corpseX entrance x coordinate in ghost
                            // mode  (in most cases = normal entrance)
    float ghost_entrance_y; // 119      m_corpseY entrance y coordinate in ghost
                            // mode  (in most cases = normal entrance)
    uint32 resetTimeRaid;   // 120
    uint32 resetTimeHeroic; // 121
                            // 122      all 0
    // uint32  timeOfDayOverride;                            // 123
    // m_timeOfDayOverride
    ...
    {
        return !IsDungeon() || MapID == 209 || MapID == 269 ||
               MapID == 309 || // TanarisInstance, CavernsOfTime, Zul'gurub
               MapID == 509 || MapID == 534 ||
               MapID == 560 || // AhnQiraj, HyjalPast, HillsbradPast
...
struct SkillLineAbilityEntry
{
    ...
    uint32 racemask;  // 3
    uint32 classmask; // 4
    // uint32    racemaskNot;                                // 5 always 0 in
    // 2.4.2
    // uint32    classmaskNot;                               // 6 always 0 in
    // 2.4.2
...
struct ClassFamilyMask
{
    ...
    {
        return Empty() ? nullptr : this;
    } // for allow normal use in if(mask)
    ...
    uint64 operator&(uint64 mask) const // possible will removed at finish
                                        // convertion code use IsFitToFamilyMask
    {
        return Flags & mask;
    }
...
struct SpellEntry
{
    ...
    uint32 SpellSpecific; // 2        enum SpellSpecific
    ...
    int32
        EquippedItemInventoryTypeMask; // 64       m_equippedItemInvTypes (mask)
    ...
    float EffectRealPointsPerLevel
        [MAX_EFFECT_INDEX]; // 77-79    m_effectRealPointsPerLevel
    int32 EffectBasePoints[MAX_EFFECT_INDEX]; // 80-82    m_effectBasePoints
                                              // (don't must be used in
                                              // spell/auras explicitly, must be
                                              // used cached
    ...
    uint32 EffectRadiusIndex[MAX_EFFECT_INDEX]; // 92-94    m_effectRadiusIndex
                                                // - spellradius.dbc
    ...
    // m_spellPriority not used
    char* SpellName[16]; // 127-142  m_name_lang
    // uint32    SpellNameFlag;                              // 143
    // m_name_flag not used
    char* Rank[16]; // 144-159  m_nameSubtext_lang
    // uint32    RankFlags;                                  // 160
    // m_nameSubtext_flag not used
    // char*     Description[16];                            // 161-176
    // m_description_lang not used
    // uint32    DescriptionFlags;                           // 177
    // m_description_flag not used
    // char*     ToolTip[16];                                // 178-193
    // m_auraDescription_lang not used
    // uint32    ToolTipFlags;                               // 194
    // m_auraDescription_flag not used
    ...
    // uint32    StanceBarOrder;                             // 205
    // m_stanceBarOrder not used
    float DmgMultiplier[MAX_EFFECT_INDEX]; // 206-208  m_effectChainAmplitude
    // uint32    MinFactionId;                               // 209
    // m_minFactionID not used
    // uint32    MinReputation;                              // 210
    // m_minReputation not used
    // uint32    RequiredAuraVision;                         // 211
    // m_requiredAuraVision not used
    uint32 TotemCategory
        [MAX_SPELL_TOTEM_CATEGORIES]; // 212-213  m_requiredTotemCategoryID
    uint32 AreaId;                    // 214
    uint32 SchoolMask;                // 215      m_schoolMask
    uint32 aoe_cap;                   // 216      Damage cap for aoe spells
    uint32 AttributesCustom;          // 217      Custom Spell Attributes
    uint32 AttributesCustom1;         // 218      Custom Spell Attributes
    float bounce_radius;              // 219      Chain Bounce Radius
    // helpers
    ...
    {
        return AttributesEx5 & attribute;
    }
    inline bool HasAttribute(SpellAttributesEx6 attribute) const
    {
        return AttributesEx6 & attribute;
    }
    inline bool HasAttribute(SpellAttributesCustom attribute) const
    {
        return AttributesCustom & attribute;
    }
    inline bool HasAttribute(SpellAttributesCustom1 attribute) const
    {
        return AttributesCustom1 & attribute;
    }
    inline bool HasEffect(SpellEffects effect) const
    {
        return (
            Effect[0] == effect || Effect[1] == effect || Effect[2] == effect);
    }
    inline bool HasApplyAuraName(uint32 aura) const
    {
        return (EffectApplyAuraName[0] == aura ||
                EffectApplyAuraName[1] == aura ||
                EffectApplyAuraName[2] == aura);
    }
    inline bool HasTargetType(uint32 targetA, uint32 targetB = 0) const
    {
        for (int i = 0; i < 3; ++i)
            if (EffectImplicitTargetA[i] == targetA &&
                (!targetB || targetB == EffectImplicitTargetB[i]))
                return true;
        return false;
    }
    inline int GetIndexForEffect(SpellEffects effect) const
    {
        for (int i = 0; i < 3; ++i)
            if (Effect[i] == effect)
                return i;
    ...
    inline int GetIndexForAura(uint32 aura) const
    {
        for (int i = 0; i < 3; ++i)
...
struct SpellRangeEntry
{
    ...
    // char*   ShortName[16];                                // 21-36
    // m_displayNameShort_lang
...
{
    uint32 ID; // 0        m_ID
    // uint32 buttonPosition;                                // 1
    // m_bonusActionBar
    // char*  Name[16];                                      // 2-17 m_name_lang
    // uint32 NameFlags;                                     // 18 string flags
    uint32 flags1;      // 19       m_flags
    int32 creatureType; // 20       m_creatureType <=0 humanoid, other normal
                        // creature types
    // uint32 unk1;                                          // 21
    // m_attackIconID
    ...
    // (but all 0)
    // uint32 unk3;                                          // 25 unused always
    // 0
    // uint32 unk4;                                          // 26 unused always
    // 0
...
struct SummonPropertiesEntry
{
    ...
    uint32 Slot; // 4,       m_slot if title = UNITNAME_SUMMON_TITLE_TOTEM, its
                 // actual slot (0-6).
...
struct TalentEntry
{
    ...
    // uint32  needAddInSpellBook;                           // 19       m_flags
    // also need disable higest ranks on reset talent tree
...
struct TotemCategoryEntry
{
    ...
    // 17 string flags
    uint32
        categoryType; // 18       m_totemCategoryType (one for specialization)
    uint32 categoryMask; // 19       m_totemCategoryMask (compatibility mask for
                         // same type: different for totems, compatible from
                         // high to low for rods)
};
...
struct WorldMapAreaEntry
{
    ...
    int32 virtual_map_id; // 8        m_displayMapID -1 (map_id have correct
                          // map) other: virtual map where zone show (map_id -
                          // where zone in fact internally)
};
...
{
    uint32    ID;                                           // 0        m_ID
    //uint32    worldMapAreaId;                             // 1
m_mapAreaID (WorldMapArea.dbc)
