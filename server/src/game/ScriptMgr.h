...
enum ScriptCommand // resSource, resTarget are the resulting Source/ Target
                   // after buddy search is done
{
    SCRIPT_COMMAND_TALK = 0, // resSource = WorldObject, resTarget = Unit/none
    // datalong1 (see enum ChatType for supported CHAT_TYPE_'s), datalong2 =
    // language
    // dataint = text entry from db_script_string -table. dataint2-4 optional
    // for random selected texts.
    SCRIPT_COMMAND_EMOTE = 1, // resSource = Unit, resTarget = Unit/none
                              // datalong1 = emote_id
    SCRIPT_COMMAND_FIELD_SET =
        2, // source = any, datalong3 = field_id, datalong2 = value
    SCRIPT_COMMAND_MOVE_TO =
        3, // resSource = Creature, datalong2 = travel_speed*100, x/y/z
    // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL: teleport unit to position
    SCRIPT_COMMAND_FLAG_SET =
        4, // source = any, datalong3 = field_id, datalong2 = bitmask
    SCRIPT_COMMAND_FLAG_REMOVE =
        5, // source = any, datalong3 = field_id, datalong2 = bitmask
    SCRIPT_COMMAND_TELEPORT_TO =
        6, // source or target with Player, datalong2 = map_id, x/y/z
    SCRIPT_COMMAND_QUEST_EXPLORED =
        7, // one from source or target must be Player, another GO/Creature,
           // datalong3=quest_id, datalong2=distance or 0
    SCRIPT_COMMAND_KILL_CREDIT = 8, // source or target with Player, datalong3 =
                                    // creature entry, datalong2 = bool
                                    // (0=personal credit, 1=group credit)
    SCRIPT_COMMAND_RESPAWN_GAMEOBJECT =
        9, // source = any, datalong=db_guid, datalong2=despawn_delay
    SCRIPT_COMMAND_TEMP_SUMMON_CREATURE =
        10, // source = any, datalong=creature entry, datalong2=despawn_delay
            // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL = summon active
    SCRIPT_COMMAND_OPEN_DOOR =
        11, // datalong=db_guid (or not provided), datalong2=reset_delay
    SCRIPT_COMMAND_CLOSE_DOOR =
        12, // datalong=db_guid (or not provided), datalong2=reset_delay
    SCRIPT_COMMAND_ACTIVATE_OBJECT = 13, // source = unit, target=GO, datalong =
                                         // custom anim id, if using custom anim
    SCRIPT_COMMAND_REMOVE_AURA = 14, // resSource = Unit, datalong = spell_id
    SCRIPT_COMMAND_CAST_SPELL =
        15, // resSource = Unit, cast spell at resTarget = Unit
            // datalong=spellid
            // data_flags &  SCRIPT_FLAG_COMMAND_ADDITIONAL = cast triggered
    SCRIPT_COMMAND_PLAY_SOUND =
        16, // resSource = WorldObject, target=any/player, datalong (sound_id),
            // datalong2 (bitmask: 0/1=anyone/target, 0/2=with distance
            // dependent, so 1|2 = 3 is target with distance dependent)
    SCRIPT_COMMAND_CREATE_ITEM = 17, // source or target must be player,
                                     // datalong3 = item entry, datalong2 =
                                     // amount
    SCRIPT_COMMAND_DESPAWN_SELF =
        18, // resSource = Creature, datalong = despawn delay
    SCRIPT_COMMAND_PLAY_MOVIE =
        19, // target can only be a player, datalog = movie id
    SCRIPT_COMMAND_MOVEMENT = 20,         // resSource = Creature. datalong =
                                          // MovementType (0:idle, 1:random or
                                          // 2:waypoint)
    ...
    SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL =
        23, // resSource = Creature, datalong=creature entry/modelid
    // data_flags &  SCRIPT_FLAG_COMMAND_ADDITIONAL = use datalong value as
    // modelid explicit
    SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL =
        24, // resSource = Creature, datalong=creature entry/modelid
    // data_flags &  SCRIPT_FLAG_COMMAND_ADDITIONAL = use datalong value as
    // modelid explicit
...
{                                       // default: s/b -> t
    ...
    SCRIPT_FLAG_REVERSE_DIRECTION =
        0x02, // t* -> s* (* result after previous flag is evaluated)
    SCRIPT_FLAG_SOURCE_TARGETS_SELF =
        0x04, // s* -> s* (* result after previous flag is evaluated)
...
{
public:
    ScriptAction(const char* _table, Map* _map, ObjectGuid _sourceGuid,
        ObjectGuid _targetGuid, ObjectGuid _ownerGuid,
        ScriptInfo const* _script)
      : m_sourceGuid(std::move(_sourceGuid)),
        m_targetGuid(std::move(_targetGuid)),
        m_ownerGuid(std::move(_ownerGuid)), m_table(_table), m_script(_script),
        m_map(_map)
    {
    }
...
private:
    ...
    // Helper functions
    bool GetScriptCommandObject(
        const ObjectGuid guid, bool includeItem, Object*& resultObject);
    bool GetScriptProcessTargets(WorldObject* pOrigSource,
    ...
    Player* GetPlayerTargetOrSourceAndLog(
...
public:
    ...
    bool IsScriptLibraryLoaded() const { return script_library_; }

    uint32 IncreaseScheduledScriptsCount()
    {
        std::lock_guard<std::mutex> g(schedule_mutex_);
    ...
    bool OnEffectDummy(Unit* pCaster, uint32 spellId, SpellEffectIndex effIndex,
        Creature* pTarget);
    bool OnEffectDummy(Unit* pCaster, uint32 spellId, SpellEffectIndex effIndex,
        GameObject* pTarget);
    bool OnEffectDummy(Unit* pCaster, uint32 spellId, SpellEffectIndex effIndex,
...
private:
    ...
    MaNGOS::library* script_library_;

    // Counter for amount of active scripts. (FIXME: This value should make use
    // of boost::atomic)
    ...
    bool(MANGOS_IMPORT* m_pOnGOGossipSelect)(
        Player*, GameObject*, uint32, uint32);
    bool(MANGOS_IMPORT* m_pOnGossipSelectWithCode)(
        Player*, Creature*, uint32, uint32, const char*);
    bool(MANGOS_IMPORT* m_pOnGOGossipSelectWithCode)(
        Player*, GameObject*, uint32, uint32, const char*);
    ...
    bool(MANGOS_IMPORT* m_pOnGOQuestRewarded)(
        Player*, GameObject*, Quest const*);
    ...
    bool(MANGOS_IMPORT* m_pOnEffectDummyCreature)(
        Unit*, uint32, SpellEffectIndex, Creature*);
    bool(MANGOS_IMPORT* m_pOnEffectDummyGO)(
        Unit*, uint32, SpellEffectIndex, GameObject*);
    bool(MANGOS_IMPORT* m_pOnEffectDummyItem)(
        Unit*, uint32, SpellEffectIndex, Item*);
    bool(MANGOS_IMPORT* m_pOnAuraDummy)(Aura const*, bool);
    bool(MANGOS_IMPORT* m_pOnConditionCheck)(
        const Condition* condition, ConditionSourceInfo& sourceInfo);
};

// Starters for events
bool StartEvents_Event(Map* map, uint32 id, Object* source, Object* target,
    bool isStart = true, Unit* forwardToPvp = nullptr);

#define sScriptMgr MaNGOS::UnlockedSingleton<ScriptMgr>
