...
enum AuraRemoveMode
{
    ...
    AURA_REMOVE_BY_DELETE, // use for speedup and prevent unexpected effects at
                           // player logout/pet unsummon (must be used _only_
                           // after save), delete.
...
// internal helper
struct ReapplyAffectedPassiveAurasHelper; // sort off

class MANGOS_DLL_SPEC AuraHolder
{
public:
    AuraHolder(const SpellEntry* spellproto, Unit* target, WorldObject* caster,
        Item* castItem, const SpellEntry* triggeredByProto);

    // If false, aura needs to be dropped/cannot be applied to target
    bool valid_in_current_state(Unit* caster);

    void AddAura(Aura* aura, SpellEffectIndex index);
    void ApplyAuraModifiers(bool apply, bool real = false);
    void _AddAuraHolder();
    void _RemoveAuraHolder();
    void HandleSpellSpecificBoosts(bool apply);
    void CleanupTriggeredSpells();

    uint32 GetStackAmount() const { return m_stackAmount; }
    void SetStackAmount(uint32 stackAmount, bool transfer = false);
    // return true if last charge dropped
    bool ModStackAmount(int32 num, bool transfer = false);

    Aura* GetAura(SpellEffectIndex index) const
    {
        return m_enabled_auras[index];
    }

    uint32 GetId() const { return m_spellProto->Id; }
    SpellEntry const* GetSpellProto() const { return m_spellProto; }
    // returns: SpellEntry of aura that triggered this one; can return nullptr
    const SpellEntry* GetTriggeredByProto() const { return m_triggeredByProto; }
    ...
    bool IsWeaponBuffCoexistableWith(AuraHolder const* ref) const;
    ...
    void SetPreferredAuraSlot(uint8 slot) { m_prefAuraSlot = slot; }
    ...
    void SetLoadedState(ObjectGuid const& casterGUID,
        ObjectGuid const& itemGUID, uint32 stackAmount, uint32 charges,
    ...
    // See comment for saved_hp_value_ below
    std::pair<uint32, uint32> GetSavedHpMp() const
    {
        return std::make_pair(saved_hp_, saved_mp_);
    }
    void SetSavedHpMp(const std::pair<uint32, uint32>& p)
    {
        saved_hp_ = p.first;
        saved_mp_ = p.second;
    }

    AuraHolder* overwriting_aura() const { return overwritten_by_; }
    void overwriting_aura(AuraHolder* holder) { overwritten_by_ = holder; }

    void SetDiminishingGroup(DiminishingGroup group)
    {
        m_diminishingGroup = group;
    }
    DiminishingGroup GetDiminishingGroup() const { return m_diminishingGroup; }

    void SetBeforeStunFacing(float f) { m_stunFace = f; }
    float GetBeforeStunFacing() const { return m_stunFace; }

    ~AuraHolder();

private:
    Aura* m_enabled_auras[MAX_EFFECT_INDEX]; // Currently enabled auras
    std::vector<Aura*> m_total_auras; // All auras we ever had enabled (includes
                                      // m_enabled_auras)

    void UpdateAuraApplication();  // called at charges or stack changes
    uint8 first_free_slot() const; // finds first free aura slot for this holder

    Unit* m_target;
    ObjectGuid m_casterGuid;
    ObjectGuid m_castItemGuid; // it is NOT safe to keep a pointer to the item
                               // because it may get deleted
    time_t m_applyTime;

    SpellEntry const* m_spellProto;
    const SpellEntry* m_triggeredByProto;

    uint8 m_auraSlot;     // Aura slot on unit (for show in client)
    uint8 m_prefAuraSlot; // Prefer this aura slot when selecting
    uint8 m_auraLevel; // Aura level (store caster level for correct show level
                       // dep amount)
    ...
    uint32 m_breakChance;

    uint32 saved_hp_; // HACK: hp & mp is saved before colliding auras are
                      // removed, in case a stamina/intellect-buff
    uint32 saved_mp_; // is overwritten, we don't want the full loss of current
                      // health/mana

    AuraHolder* overwritten_by_; // This is a pointer to the AuraHolder that
                                 // removed us due to stacking rules
    // (only applicable if remove mode is AURA_REMOVE_BY_STACK)

    float m_stunFace;

    DiminishingGroup m_diminishingGroup;

    friend class Aura;
    void DisableAura(SpellEffectIndex index);
};

typedef void (Aura::*pAuraHandler)(bool Apply, bool Real);
// Real == true at aura add/remove
// Real == false at aura mod unapply/reapply; when adding/removing dependent
// aura/item/stat mods
//
// Code in aura handler can be guarded by if(Real) check if it should execution
// only at real add/remove of aura
//
// MAIN RULE: Code MUST NOT be guarded by if(Real) check if it modifies any
// stats
//      (percent auras, stats mods, etc)
// Second rule: Code must be guarded by if(Real) check if it modifies object
// state (start/stop attack, send packets to client, etc)
//
// Other case choice: each code line moved under if(Real) check is mangos
// speedup,
//      each setting object update field code line moved under if(Real) check is
//      significant mangos speedup, and less server->client data sends
//      each packet sending code moved under if(Real) check is _large_ mangos
//      speedup, and lot less server->client data sends

class MANGOS_DLL_SPEC Aura
{
    friend struct ReapplyAffectedPassiveAurasHelper;
    friend Aura* CreateAura(SpellEntry const* spellproto, SpellEffectIndex eff,
        int32* currentBasePoints, AuraHolder* holder, Unit* target,
        Unit* caster, Item* castItem);
...
public:
    ...
    {
        // aura not have immediate effect at add/remove and handled by ID in
        // other code place
    }
    ...
    void HandleModAoeCharm(bool apply, bool Real);
    ...
    // removeauraholder: sort of hacky solution to signifiy aura is being
    // disabled from within Unit::RemoveAuraHolder
    void Disable(AuraRemoveMode mode = AURA_REMOVE_BY_DEFAULT,
        bool removeauraholder = false);
    ...
    {
        return GetHolder()->GetAuraMaxDuration();
    }
    ...
    uint32 GetAuraMaxTicks() const
    {
        ...
        return maxDuration > 0 && m_modifier.periodictime > 0 ?
                   maxDuration / m_modifier.periodictime :
                   0;
    }
    void ResetAuraTicks() { m_periodicTick = 0; }
    ...
    void SetLoadedState(int32 damage, uint32 periodicTime)
    {
        ...
        if (uint32 maxticks = GetAuraMaxTicks())
            m_periodicTick =
                maxticks - GetAuraDuration() / m_modifier.periodictime;
    }
    ...
    // more limited that used in future versions (spell_affect table based
    // only), so need be careful with backporting uses
    bool isAffectedOnSpell(SpellEntry const* spell) const;
    bool CanProcFrom(SpellEntry const* spell, uint32 EventProcEx, uint32 procEx,
        bool active, bool useClassMask) const;

    AuraHolder* GetHolder() { return m_spellAuraHolder; }
    const AuraHolder* GetHolder() const { return m_spellAuraHolder; }

    bool HasMechanic(uint32 mechanic) const;

protected:
    Aura(SpellEntry const* spellproto, SpellEffectIndex eff,
        int32* currentBasePoints, AuraHolder* holder, Unit* target,
        Unit* caster = nullptr, Item* castItem = nullptr);
    ...
    int32 m_currentBasePoints; // cache SpellEntry::CalculateSimpleValue and use
                               // for set custom base points
    int32 m_periodicTimer;     // Timer for periodic auras
    uint32 m_periodicTick; // Tick count pass (including current if use in tick
                           // code) from aura apply, used for some tick count
                           // dependent aura effects
    ...
    AuraHolder* const m_spellAuraHolder;
...
{
public:
    AreaAura(SpellEntry const* spellproto, SpellEffectIndex eff,
        int32* currentBasePoints, AuraHolder* holder, Unit* target,
        Unit* caster = nullptr, Item* castItem = nullptr);
    ~AreaAura();
...
private:
    ...
    ShortIntervalTimer new_targets_search_timer_;
};

class MANGOS_DLL_SPEC PersistentAreaAura : public Aura
{
public:
    PersistentAreaAura(SpellEntry const* spellproto, SpellEffectIndex eff,
        int32* currentBasePoints, AuraHolder* holder, Unit* target,
        Unit* caster = nullptr, Item* castItem = nullptr);
    ~PersistentAreaAura();
...
protected:
    SingleEnemyTargetAura(SpellEntry const* spellproto, SpellEffectIndex eff,
        int32* currentBasePoints, AuraHolder* holder, Unit* target,
        Unit* caster = nullptr, Item* castItem = nullptr);
...
AuraHolder* CreateAuraHolder(const SpellEntry* spellproto, Unit* target,
    WorldObject* caster, Item* castItem = nullptr,
    const SpellEntry* triggeredByProto = nullptr);
#endif
