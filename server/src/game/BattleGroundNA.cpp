...
BattleGroundNA::BattleGroundNA(const battlefield::specification& spec)
  : specification_(spec), end_cond_check_timer_(0)
{
    ...
    SetMapId(559);
    SetTypeID(BATTLEGROUND_NA);
    SetMinPlayersPerTeam(1);
    SetMaxPlayersPerTeam(5);
    SetMinPlayers(2);
    SetMaxPlayers(10);
    SetName("Nagrand Arena");
    SetTeamStartLoc(ALLIANCE, 4027.6f, 2972.78f, 12.0723f, 5.191f);
    SetTeamStartLoc(HORDE, 4085.45f, 2866.83f, 12.4005f, 2.0617f);
    SetLevelRange(20, 70);
...
void BattleGroundNA::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    if (!killer)
    {
        logging.error("BattleGroundNA: Killer player not found");
        return;
    }
...
void BattleGroundNA::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    FillInitialWorldState(
        data, count, 0xa0f, GetAlivePlayersCountByTeam(ALLIANCE));
    FillInitialWorldState(
        data, count, 0xa10, GetAlivePlayersCountByTeam(HORDE));
