...
#define GOSSIP_MAX_MENU_ITEMS \
    64 // client supported items unknown, but provided number must be enough
...
enum Gossip_Option
{
    ...
    GOSSIP_OPTION_UNLEARNPETSKILLS = 17, // UNIT_NPC_FLAG_TRAINER
                                         // (16) (bonus option for
                                         // GOSSIP_OPTION_TRAINER)
    GOSSIP_OPTION_MAX
};
...
enum GossipOptionIcon
{
    ...
    GOSSIP_ICON_CHAT_11 =
        11, // This and below are most the same visual as GOSSIP_ICON_CHAT
...
struct GossipMenuItem
{
    ...
    uint32 m_gSender;
    uint32 m_gOptionId; // gossip_menu_option.option_id
    uint32 m_id;        // gossip_menu_option.id
...
public:
    ...
    void AddMenuItem(
        uint8 Icon, const std::string& Message, bool Coded = false);
    void AddMenuItem(uint8 Icon, const std::string& Message, uint32 dtSender,
        uint32 optionId, uint32 id, const std::string& BoxMessage,
        uint32 BoxMoney, bool Coded = false);
    ...
    void AddMenuItem(uint8 Icon, char const* Message, uint32 dtSender,
        uint32 action, char const* BoxMessage, uint32 BoxMoney,
        bool Coded = false);

    void AddMenuItem(uint8 Icon, int32 itemText, uint32 dtSender, uint32 action,
        int32 boxText, uint32 BoxMoney, bool Coded = false);

    void SetMenuId(uint32 menu_id) { m_gMenuId = menu_id; }
    uint32 GetMenuId() { return m_gMenuId; }

    void AddGossipMenuItemData(
    ...
    // returns: gossip_menu_option.id
    uint32 MenuItemId(unsigned int ItemId);
...
public:
    ...
    uint32 GossipOptionId(unsigned int Selection);
    ...
    void SendQuestGiverQuestList(
        QEmote eEmote, const std::string& Title, ObjectGuid npcGUID);

    void SendQuestQueryResponse(Quest const* pQuest);
    void SendQuestGiverQuestDetails(
        Quest const* pQuest, ObjectGuid npcGUID, bool ActivateAccept);

    void SendQuestGiverOfferReward(
        Quest const* pQuest, ObjectGuid npcGUID, bool EnbleNext);
    void SendQuestGiverRequestItems(Quest const* pQuest, ObjectGuid npcGUID,
