...
SplineBase::EvaluationMethtod SplineBase::evaluators[SplineBase::ModesEnd] = {
    &SplineBase::EvaluateLinear, &SplineBase::EvaluateCatmullRom,
...
SplineBase::SegLenghtMethtod SplineBase::seglengths[SplineBase::ModesEnd] = {
    &SplineBase::SegLengthLinear, &SplineBase::SegLengthCatmullRom,
...
SplineBase::InitMethtod SplineBase::initializers[SplineBase::ModesEnd] = {
    ...
    &SplineBase::InitCatmullRom, &SplineBase::InitBezier3,
...
using G3D::Matrix4;
static const Matrix4 s_catmullRomCoeffs(-0.5f, 1.5f, -1.5f, 0.5f, 1.f, -2.5f,
    2.f, -0.5f, -0.5f, 0.f, 0.5f, 0.f, 0.f, 1.f, 0.f, 0.f);

static const Matrix4 s_Bezier3Coeffs(-1.f, 3.f, -3.f, 1.f, 3.f, -6.f, 3.f, 0.f,
...
inline void C_Evaluate(
    const Vector3* vertice, float t, const Matrix4& matr, Vector3& result)
{
...
inline void C_Evaluate_Derivative(
    const Vector3* vertice, float t, const Matrix4& matr, Vector3& result)
{
...
void SplineBase::EvaluateLinear(
    index_type index, float u, Vector3& result) const
{
    assert(index >= index_lo && index < index_hi);
    result = points[index] + (points[index + 1] - points[index]) * u;
}

void SplineBase::EvaluateCatmullRom(
    index_type index, float t, Vector3& result) const
{
    assert(index >= index_lo && index < index_hi);
    C_Evaluate(&points[index - 1], t, s_catmullRomCoeffs, result);
}

void SplineBase::EvaluateBezier3(
    index_type index, float t, Vector3& result) const
{
    index *= 3u;
    assert(index >= index_lo && index < index_hi);
...
void SplineBase::EvaluateDerivativeLinear(
    index_type index, float, Vector3& result) const
{
    assert(index >= index_lo && index < index_hi);
    result = points[index + 1] - points[index];
}

void SplineBase::EvaluateDerivativeCatmullRom(
    index_type index, float t, Vector3& result) const
{
    assert(index >= index_lo && index < index_hi);
...
void SplineBase::EvaluateDerivativeBezier3(
    index_type index, float t, Vector3& result) const
{
    index *= 3u;
    assert(index >= index_lo && index < index_hi);
...
float SplineBase::SegLengthCatmullRom(index_type index) const
{
    ...
    {
        C_Evaluate(p, float(i) / float(STEPS_PER_SEGMENT), s_catmullRomCoeffs,
            nextPos);
...
void SplineBase::init_cyclic_spline(const Vector3* controls, index_type count,
    EvaluationMode m, index_type cyclic_point)
{
    m_mode = m;
    cyclic = true;
...
void SplineBase::InitLinear(const Vector3* controls, index_type count,
    bool cyclic, index_type cyclic_point)
{
    assert(count >= 2);
...
void SplineBase::InitCatmullRom(const Vector3* controls, index_type count,
    bool cyclic, index_type cyclic_point)
{
...
void SplineBase::InitBezier3(const Vector3* controls, index_type count,
    bool /*cyclic*/, index_type /*cyclic_point*/)
{
