...
public:
     ...
     * @param fix_facing - if true it will call SetFacing with
     * ComputePosition()'s facing, or unit.GetO()
     */
    int32 Launch();

    /* Final pass of initialization that launches spline to stop movement. */
    void Stop();
    ...
    /* Initializes simple A to B mition, A is current unit's position, B is
     * destination
     */
     ...
     * ILisener will notify that pointId + N done
     ...
     */
    void SetCyclic();
    /* Makes movement catmull rom. Disabled by default
     */
    void SetCatmullRom() { args.flags |= MoveSplineFlag::Flying; }
...
inline void MoveSplineInit::MovebyPath(
    const PointsArray& controls, int32 path_offset)
{
