...
protected:
    ...
    typedef void (SplineBase::*EvaluationMethtod)(
        index_type, float, Vector3&) const;
    ...
    typedef void (SplineBase::*InitMethtod)(
        const Vector3*, index_type, bool, index_type);
...
public:
    explicit SplineBase()
      : index_lo(0), index_hi(0), m_mode(UninitializedMode), cyclic(false)
    {
    }
    ...
    void init_cyclic_spline(const Vector3* controls, index_type count,
...
#ifndef NDEBUG
        length_type prev_length = 0;
#endif
        length_type new_length = 0;
        while (i < index_hi)
        {
...
#include "spline.impl.h" // sort off
