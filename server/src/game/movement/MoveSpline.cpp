...
extern float computeFallElevation(
    float time_passed, bool isSafeFall, float start_velocy);
...
Location MoveSpline::ComputePosition() const
{
    ...
    if ((splineflags.done || spline.length() <= 1) && splineflags.isFacing())
    {
...
void MoveSpline::computeFallElevation(float& el) const
{
    float z_now = spline.getPoint(spline.first()).z -
                  movement::computeFallElevation(MSToSec(time_passed));
...
struct FallInitializer
{
    ...
    {
        return movement::computeFallTime(
                   start_elevation - s.getPoint(i + 1).z, false) *
               1000.f;
    }
};
...
struct CommonInitializer
{
    CommonInitializer(float _velocity)
      : velocityInv(1000.f / _velocity), time(minimal_duration)
    {
    }
    float velocityInv;
    int32 time;
...
{
    const SplineBase::EvaluationMode modes[2] = {
        SplineBase::ModeLinear, SplineBase::ModeCatmullrom};
    if (args.flags.cyclic)
    {
        ...
        spline.init_cyclic_spline(&args.path[0], args.path.size(),
            modes[args.flags.isSmooth()], cyclic_point);
    }
    else
    {
        spline.init_spline(
    ...
    // TODO: what to do in such cases? problem is in input data (all points are
    // at same coords)
...
void MoveSpline::Initialize(const MoveSplineInitArgs& args)
{
    ...
    // just return if it's a stop spline
    if (args.flags.done)
    {
        spline.clear();
        return;
...
MoveSpline::MoveSpline()
  : m_Id(0), time_passed(0), point_Idx(0), point_Idx_offset(0), transport(false)
{
...
#define CHECK(exp)                                                         \
    ...
    {                                                                      \
        logging.error(                                                     \
            "MoveSplineInitArgs::Validate: expression '%s' failed", #exp); \
        return false;                                                      \
    }
    CHECK(path.size() > 1);
    CHECK(velocity > 0.f);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size() - 1; ++i)
        {
            ...
            if (fabs(offset.x) >= MAX_OFFSET || fabs(offset.y) >= MAX_OFFSET ||
                fabs(offset.z) >= MAX_OFFSET)
            {
                logging.error(
