...
void WorldSession::HandleInspectArenaTeamsOpcode(WorldPacket& recv_data)
{
    ObjectGuid guid;
    recv_data >> guid;
    LOG_DEBUG(logging, "Inspect Arena stats %s", guid.GetString().c_str());

    if (Player* plr = sObjectMgr::Instance()->GetPlayer(guid))
    {
        for (uint8 i = 0; i < MAX_ARENA_SLOT; ++i)
        {
            ...
            {
                if (ArenaTeam* at =
                        sObjectMgr::Instance()->GetArenaTeamById(a_id))
...
void WorldSession::HandleArenaTeamInviteOpcode(WorldPacket& recv_data)
{
    ...
    LOG_DEBUG(logging, "Player %s Invited %s to Join his ArenaTeam",
...
{
    ArenaTeam* at = sObjectMgr::Instance()->GetArenaTeamById(
    ...
    if (!sWorld::Instance()->getConfig(
            CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
        _player->GetTeam() !=
            sObjectMgr::Instance()->GetPlayerTeamByGUID(at->GetCaptainGuid()))
    {
    ...
    // event
    at->BroadcastEvent(ERR_ARENA_TEAM_JOIN_SS, _player->GetObjectGuid(),
        _player->GetName(), at->GetName().c_str());
}
...
void WorldSession::HandleArenaTeamLeaveOpcode(WorldPacket& recv_data)
{
    ...
    ArenaTeam* at = sObjectMgr::Instance()->GetArenaTeamById(ArenaTeamId);
    if (!at)
        return;
    ...
    if (!at->DelMember(_player->GetObjectGuid()))
        return;

    // event
    at->BroadcastEvent(ERR_ARENA_TEAM_LEAVE_SS, _player->GetObjectGuid(),
        _player->GetName(), at->GetName().c_str());
}
...
void WorldSession::HandleArenaTeamDisbandOpcode(WorldPacket& recv_data)
{
    ...
    if (ArenaTeam* at = sObjectMgr::Instance()->GetArenaTeamById(ArenaTeamId))
    {
...
void WorldSession::HandleArenaTeamRemoveOpcode(WorldPacket& recv_data)
{
    ...
    // event
    at->BroadcastEvent(ERR_ARENA_TEAM_REMOVE_SSS, name.c_str(),
        at->GetName().c_str(), _player->GetName());
}
...
void WorldSession::HandleArenaTeamLeaderOpcode(WorldPacket& recv_data)
{
    ...
    // event
    at->BroadcastEvent(ERR_ARENA_TEAM_LEADER_CHANGED_SSS, _player->GetName(),
        name.c_str(), at->GetName().c_str());
}

void WorldSession::SendArenaTeamCommandResult(uint32 team_action,
    const std::string& team, const std::string& player, uint32 error_id)
{
...
{
    WorldPacket data(
        SMSG_ARENA_ERROR, 4 + 1); // 886 - You are not in a %uv%u arena team
    uint32 unk = 0;
...
team"
+ERR_ARENA_TEAM_NAME_INVALID "That name contains invalid characters, please
enter a new name"
+ERR_ARENA_TEAM_NAME_EXISTS_S "There is already an arena team named \"%s\""
+ERR_ARENA_TEAM_LEADER_LEAVE_S "You must promote a new team captain using
/teamcaptain before leaving the team"
