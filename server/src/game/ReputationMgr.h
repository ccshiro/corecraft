...
enum FactionFlags
{
    ...
    FACTION_FLAG_HIDDEN = 0x04, // hidden faction from reputation pane in client
                                // (player can gain reputation, but this update
                                // not sent to client)
    ...
    FACTION_FLAG_PEACE_FORCED = 0x10, // always overwrite FACTION_FLAG_AT_WAR,
                                      // used for prevent war with own team
                                      // factions
...
typedef std::pair<FactionStateList::const_iterator,
    FactionStateList::const_iterator> FactionStateListPair;
...
public: // accessors
    ...
    {
        auto repItr = m_factions.find(id);
    ...
    {
        auto forceItr = m_forcedReactions.find(factionTemplateEntry->faction);
        return forceItr != m_forcedReactions.end() ? &forceItr->second :
                                                     nullptr;
    }
