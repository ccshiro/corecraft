...
public:
    ...
    // Called when a player successfully enters the instance (after really added
    // to map)
    ...
    // Called when a player leaves the instance (before really removed from map
    // (or possibly world))
    ...
    ObjectGuid GetGuid(uint32 dataIdx)
    {
    ...
    virtual bool CheckConditionCriteriaMeet(
        Player const* source, uint32 map_id, uint32 instance_condition_id);

    // Overridable callback to pass instance events from core to script
    virtual void OnEvent(const std::string&) {}
};
