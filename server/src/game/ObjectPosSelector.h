...
struct ObjectPosSelector
{
    typedef std::multimap<float, float>
        UsedAreaList; // angle pos -> angle offset
    ...
    float m_centerX;
    float m_centerY;
    float m_searcherDist; // distance for searching pos (including searcher size
                          // and target object size)
    float m_searcherSize; // searcher object radius
    float m_searcherHalfSize; // angle size/2 of searcher object (at dist
                              // distance)

    UsedAreaList m_UsedAreaLists[2]; // list left/right side used angles (with
                                     // angle size)

    UsedAreaList::const_iterator
        m_nextUsedAreaItr[2]; // next used used areas for check at left/right
                              // side, possible angles selected in range
                              // m_smallStepAngle..m_nextUsedAreaItr
    float m_nextUsedAreaStart[2]; // cached angle for next used area from
                                  // m_nextUsedAreaItr or another side

    float m_stepAngle[2]; // current checked angle position at sides (less
                          // m_nextUsedArea), positive value
};
#endif
