...
#define BG_AV_MAX_NODE_DISTANCE \
    25 // distance in which players are still counted in range of a banner (for
       // alliance towers this is calculated from the center of the tower)
...
#define BG_AV_KILL_GET_COMMANDER \
    1 // for a safely returned wingcommander TODO implement it
...
enum BG_AV_Sounds
{
    ...
    BG_AV_SOUND_ALLIANCE_GOOD = 8173, // if something good happens for the team:
                                      // wins(maybe only through killing the
                                      // boss), captures mine or grave, destroys
                                      // tower and defends grave
    BG_AV_SOUND_HORDE_GOOD = 8213,
    BG_AV_SOUND_BOTH_TOWER_DEFEND = 8192,

    BG_AV_SOUND_ALLIANCE_CAPTAIN = 8232, // gets called when someone attacks
                                         // them and at the beginning after 3min
                                         // + rand(x) * 10sec (maybe buff)
...
#define BG_AV_MINE_BOSSES 46 // + mineid gives actual event id
#define BG_AV_MINE_BOSSES_NORTH 46
#define BG_AV_MINE_BOSSES_SOUTH 47
#define BG_AV_CAPTAIN_A 48
#define BG_AV_CAPTAIN_H 49
#define BG_AV_MINE_EVENT 50 // + mineid gives actual event id
#define BG_AV_MINE_EVENT_NORTH 50
#define BG_AV_MINE_EVENT_SOUTH 51
#define BG_AV_MINE_PATROLS 70 // + mineid gives actual event id
#define BG_AV_MINE_PATROL_N 70
#define BG_AV_MINE_PATROL_S 71
#define BG_AV_MINE_SUPPLIES 72 // + mineid gives actual event id
#define BG_AV_MINE_SUPPLIES_N 72
#define BG_AV_MINE_SUPPLIES_S 73
...
#define BG_AV_LAND_MINES_A 65
#define BG_AV_LAND_MINES_H 66
...
const uint32 BG_AV_GraveyardIds[9] = {BG_AV_GRAVE_STORM_AID,
    BG_AV_GRAVE_STORM_GRAVE, BG_AV_GRAVE_STONE_GRAVE, BG_AV_GRAVE_SNOWFALL,
    BG_AV_GRAVE_ICE_GRAVE, BG_AV_GRAVE_FROSTWOLF, BG_AV_GRAVE_FROST_HUT,
    BG_AV_GRAVE_MAIN_ALLIANCE, BG_AV_GRAVE_MAIN_HORDE};
...
// BattleGroundAVTeamIndex -> BattleGroundTeamIndex cast safe and array with
// BG_TEAMS_COUNT elements must checked != BG_AV_TEAM_NEUTRAL before used
...
enum BattleGroundAVInstData
{
    // Armor Scraps turn-ins
    ALLIANCE_SOLDIERS_UPGRADE_LEVEL =
        0, // How many times soldiers have been upgraded for the alliance
    ALLIANCE_ARMOR_SCRAPS =
        1, // How many armor scraps the alliance has collected
    HORDE_SOLDIERS_UPGRADE_LEVEL =
        2, // How many times soldiers have been upgraded for the horde
    HORDE_ARMOR_SCRAPS = 3, // How many armor scraps the horde has collected

    // Ground Assault turn-ins
    ALLIANCE_TOTAL_SUPPLIES = 4,
    HORDE_TOTAL_SUPPLIES = 5,

    // Cavalry turn-ins
    ALLIANCE_TAMED_ALTERAC_RAMS = 6,
    ALLIANCE_COLLECTED_WOLF_HIDES = 7,
    HORDE_TAMED_FROSTWOLVES = 8,
    HORDE_COLLECTED_RAM_HIDES = 9,

    // Wing Commander turn-ins
    ALLIANCE_SLIDORE_TURNINS = 10, // 90 needed
    HORDE_GUSE_TURNINS = 11,
    ALLIANCE_VIPORE_TURNINS = 12, // 60 needed
    HORDE_JEZTOR_TURNINS = 13,
    ALLIANCE_ICHMAN_TURNINS = 14, // 30 needed
    HORDE_MULVERICK_TURNINS = 15,

    STORM_CRYSTAL_COUNT = 16,
    SOLDIER_BLOOD_COUNT = 17,

    ALLY_WING_COMMANDER_BONUS_HONOR = 18, // Smart AI sets this to one when a WC
                                          // arrives, script then gives bonus
                                          // honor and sets to 0
    HORDE_WING_COMMANDER_BONUS_HONOR = 19, // --- " ----
...
enum BG_AV_QuestIds
{
    ...
    BG_AV_QUEST_A_COMMANDER1 =
        6942, // Wing Commander, 90 items for aerial assault
    BG_AV_QUEST_H_COMMANDER1 = 6825,
    BG_AV_QUEST_A_COMMANDER2 =
        6941, // Wing Commander, 60 items for aerial assault
    BG_AV_QUEST_H_COMMANDER2 = 6826,
    BG_AV_QUEST_A_COMMANDER3 =
        6943, // Wing Commander, 30 items for aerial assault
    ...
    BG_AV_QUEST_A_BEGIN_ATTACK = 6846,
    BG_AV_QUEST_H_LAUNCH_ATTACK = 6901,
};
...
public:
    BattleGroundAVScore()
      : GraveyardsAssaulted(0), GraveyardsDefended(0), TowersAssaulted(0),
        TowersDefended(0), SecondaryObjectives(0){};
...
class MANGOS_DLL_SPEC BattleGroundAV : public BattleGround
{
    friend class BattleGroundMgr;

public:
    BattleGroundAV(const battlefield::bracket& bracket);
    ~BattleGroundAV();
    ...
    virtual void FillInitialWorldStates(
        WorldPacket& data, uint32& count) override;
    ...
    virtual void EventPlayerClickedOnFlag(
        Player* source, GameObject* target_obj) override;
    ...
    void DoArmorScrapsUpgrade(BattleGroundAVTeamIndex team);

    bool SpawnedByDefault(Creature* c) override;
...
private:
    ...
    void CompleteMineQuest(uint8 mine, Team team);
    ...
    uint32 custom_honor_timer_;
    std::map<ObjectGuid, time_t> custom_honor_; // time_t: last timestamp when
                                                // custom honor and marks where
                                                // given out to player
};
