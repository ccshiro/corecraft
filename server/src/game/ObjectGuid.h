...
enum TypeMask
{
    ...
    // used combinations in Player::GetObjectByTypeMask (TYPEMASK_UNIT case
    // ignore players in call)
    ...
    TYPEMASK_CREATURE_GAMEOBJECT_PLAYER_OR_ITEM =
        TYPEMASK_UNIT | TYPEMASK_GAMEOBJECT | TYPEMASK_ITEM | TYPEMASK_PLAYER,

    TYPEMASK_WORLDOBJECT = TYPEMASK_UNIT | TYPEMASK_PLAYER |
                           TYPEMASK_GAMEOBJECT |
...
public: // constructors
    ...
    ObjectGuid(HighGuid hi, uint32 entry, uint32 counter)
      : m_guid(
            counter ?
                uint64(counter) | (uint64(entry) << 24) | (uint64(hi) << 48) :
                0)
    {
    }
    ObjectGuid(HighGuid hi, uint32 counter)
      : m_guid(counter ? uint64(counter) | (uint64(hi) << 48) : 0)
    {
    }
...
public: // accessors
    ...
    {
        return HasEntry() ?
                   uint32((m_guid >> 24) & UI64LIT(0x0000000000FFFFFF)) :
                   0;
    }
    uint32 GetCounter() const
    {
        return HasEntry() ? uint32(m_guid & UI64LIT(0x0000000000FFFFFF)) :
    ...
    static TypeID GetTypeId(HighGuid high)
    {
        ...
        case HIGHGUID_ITEM:
            return TYPEID_ITEM;
        // case HIGHGUID_CONTAINER:    return TYPEID_CONTAINER;
        // HIGHGUID_CONTAINER==HIGHGUID_ITEM currently
        case HIGHGUID_UNIT:
            return TYPEID_UNIT;
        case HIGHGUID_PET:
            return TYPEID_UNIT;
...
public: // constructors
    explicit PackedGuid() : m_packedGuid(PACKED_GUID_MIN_BUFFER_SIZE)
    {
        m_packedGuid.appendPackGUID(0);
    }
    explicit PackedGuid(uint64 const& guid)
      : m_packedGuid(PACKED_GUID_MIN_BUFFER_SIZE)
    {
...
// std::hash specialization for ObjectGuid
namespace std
{
template <>
