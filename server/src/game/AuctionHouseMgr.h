...
struct AuctionEntry
{
    ...
    uint32 buyout;
    time_t expireTime;
    uint32 bidder;  // current bidder player lowguid, can be 0 if bid generated
                    // by server, use 'bid'!=0 for check bid existance
    ...
    // -1,0,+1 order result
    int CompareAuctionEntry(
        uint32 column, const AuctionEntry* auc, Player* viewPlayer) const;

    bool UpdateBid(uint32 newbid,
        Player* newbidder = nullptr); // true if normal bid, false if buyout,
                                      // bidder==NULL for generated bid
};
...
public:
    ...
    typedef std::pair<AuctionEntryMap::const_iterator,
        AuctionEntryMap::const_iterator> AuctionEntryMapBounds;
    ...
    size_t AuctionCount(uint32 low_guid) const;
    ...
    void BuildListBidderItems(
        WorldPacket& data, Player* player, uint32& count, uint32& totalcount);
    void BuildListOwnerItems(
        WorldPacket& data, Player* player, uint32& count, uint32& totalcount);

    AuctionEntry* AddAuction(AuctionHouseEntry const* auctionHouseEntry,
        Item* newItem, uint32 etime, uint32 bid, uint32 buyout = 0,
        uint32 deposit = 0, Player* pl = nullptr);
...
{
public:
    AuctionSorter(AuctionSorter const& sorter)
      : m_sort(sorter.m_sort), m_viewPlayer(sorter.m_viewPlayer)
    {
    }
    AuctionSorter(uint8* sort, Player* viewPlayer)
      : m_sort(sort), m_viewPlayer(viewPlayer)
    {
    }
...
public:
    ...
    static uint32 GetAuctionDeposit(
...
#define sAuctionMgr MaNGOS::UnlockedSingleton<AuctionHouseMgr>
