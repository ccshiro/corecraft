...
void LoadRandomEnchantmentsTable()
{
    ...
    if (result)
    {
        ...
        logging.info("Loaded %u Item Enchantment definitions\n", count);
    }
    else
    {
        logging.error(
...
uint32 GetItemEnchantMod(uint32 entry)
{
    ...
    {
        logging.error(
            "Item RandomProperty / RandomSuffix id #%u used in `item_template` "
            "but it doesn't have records in `item_enchantment_template` table.",
            entry);
        return 0;
    }
