...
void WorldSession::HandleJoinChannelOpcode(WorldPacket& recvPacket)
{
    ...
    if (channel_id)
    {
        auto chan = sChatChannelsStore.LookupEntry(channel_id);
        if (!chan)
            return;

        if (!_player->CanJoinConstantChannel(chan, _player->GetZoneId()))
            return;
    }
...
void WorldSession::HandleGetChannelMemberCountOpcode(WorldPacket& recvPacket)
{
    ...
    if (ChannelMgr* cMgr = channelMgr(_player->GetTeam()))
    {
        if (Channel* chn = cMgr->GetChannel(channelname, _player, false))
        {
            WorldPacket data(
                SMSG_CHANNEL_MEMBER_COUNT, chn->GetName().size() + 1 + 1 + 4);
            data << chn->GetName();
