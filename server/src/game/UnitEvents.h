...
enum UnitThreatEventType
{
    ...
    // A target got ignored or unignored
    UEV_THREAT_REF_IGNORED_STATUS = 1 << 7,
};
...
public:
    ...
    ThreatRefStatusChangeEvent(
        uint32 pType, HostileReference* pHostileReference, float pValue)
      : UnitBaseEvent(pType)
    {
        iHostileReference = pHostileReference;
        iFValue = pValue;
    ...
    ThreatRefStatusChangeEvent(
        uint32 pType, HostileReference* pHostileReference, bool pValue)
      : UnitBaseEvent(pType)
    {
        iHostileReference = pHostileReference;
        iBValue = pValue;
