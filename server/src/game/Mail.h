...
#define MAIL_BODY_ITEM_TEMPLATE \
    8383 ///< - plain letter, A Dusty Unsent Letter: 889
...
enum MailCheckMask
{
    ...
    MAIL_CHECK_MASK_RETURNED =
        0x02, /// This mail was returned. No allow return mail.
    MAIL_CHECK_MASK_COPIED =
        0x04, /// This mail was copied. No allow make item copy from mail text.
...
public: // Constructors
    MailSender()
      : m_messageType(MAIL_NORMAL), m_senderId(0),
     ...
     * @param sender_guidlow_or_entry The lower part of the GUID of the player
     *sending
     *                                                                this mail,
     *or the Entry of the non-player object.
    ...
    MailSender(
        Object* sender, MailStationery stationery = MAIL_STATIONERY_DEFAULT);
    MailSender(AuctionEntry* sender);

public: // Accessors
    /// The Messagetype of this MailSender.
    MailMessageType GetMailMessageType() const { return m_messageType; }
    /// The GUID of the player represented by this MailSender, or the Entry of
...
private:
    ...
    MailSender(MailMessageType messageType, uint64 wrong_guid,
        MailStationery stationery = MAIL_STATIONERY_DEFAULT);
...
{
public: // Constructors
    explicit MailReceiver(ObjectGuid receiver_guid)
      : m_receiver(nullptr), m_receiver_guid(std::move(receiver_guid))
    {
    }
...
public: // Accessors
     ...
     *
     * @returns the low part of the GUID of the player associated with this
     *MailReciever
     */
...
public: // Constructors
         ...
         */
    MailDraft()
      : m_mailTemplateId(0), m_mailTemplateItemsNeed(false), m_bodyId(0),
        m_money(0), m_COD(0)
    {
    }
    /**
    ...
    MailDraft(std::string subject, uint32 itemTextId = 0)
      : m_mailTemplateId(0), m_mailTemplateItemsNeed(false),
        m_subject(std::move(subject)), m_bodyId(itemTextId), m_money(0),
        m_COD(0)
    {
    }
    /**
...
public: // modifiers
    // this two modifiers expected to be applied in normal case to blank draft
    // and exclusively, It DON'T must overwrite already set itemTextId, in other
    // cases it will work and with mixed cases but this will be not normal way
    // use.
...
public: // finishers
    void SendReturnToSender(
        uint32 sender_acc, ObjectGuid sender_guid, ObjectGuid receiver_guid);
    void SendMailTo(MailReceiver const& receiver, MailSender const& sender,
        MailCheckMask checked = MAIL_CHECK_MASK_NONE, uint32 deliver_delay = 0);

private:
    MailDraft(MailDraft const&); // trap decl, no body, mail draft must cloned
                                 // only explicitly...
    MailDraft& operator=(MailDraft const&); // trap decl, no body, ...because
                                            // items clone is high price
                                            // operation

    void deleteIncludedItems(bool inDB = false);
    bool prepareItems(Player* receiver); ///< called from SendMailTo for
...
struct Mail
{
    ...
    /// the LowGUID of the player that sent this mail, or creature low guid, or
    /// other id
    uint32 sender;
    ...
    uint32 itemTextId;
    /// flag mark mail that already has items, or already generate none items
    /// for template
    bool has_items;
    /// A vector containing Information about the items in this mail.
    MailItemInfoVec items;
    /// A vector containing Information about the items that where already take
    /// from this mail.
     ...
     * to the caller to make sure there is still room for more items in the
     *mail.
     *
     ...
     * @returns true if the item was removed, or false if no item with that GUID
     *was found.
     *
     */
    ...
    /*
     * Checks whether a mail contains items (including case none items generated
     *from template already) or not.
     * HasItems() checks whether the mail contains items or not.
     *
     * @returns true if the mail contains items or template items already
     *generated possible none, false otherwise.
     *
     */
    bool HasItems() const { return has_items; }

    /*
     * Generate items for template if items not genereated before (receiver has
     *been offline, has_items == false)
     *
     */
    void prepareTemplateItems(Player* receiver); ///< called from _LoadMails for
                                                 /// generate mailTemplateBase
                                                 /// items not generated for
                                                 /// offline player
};
