...
AccountOpResult AccountMgr::DeleteAccount(uint32 accid)
{
    ...
    result.reset(CharacterDatabase.PQuery(
        "SELECT guid FROM characters WHERE account='%u'", accid));
    if (result)
    {
        do
        {
            ...
            Player::DeleteFromDB(
                guid, accid, false); // no need to update realm characters
        } while (result->NextRow());
    }

    // table realm specific but common for all characters of account for realm
    CharacterDatabase.PExecute(
        "DELETE FROM character_tutorial WHERE account = '%u'", accid);
    ...
    bool res =
        ...
        LoginDatabase.PExecute(
            "DELETE FROM realmcharacters WHERE acctid='%u'", accid);
...
AccountOpResult AccountMgr::ChangeUsername(
    uint32 accid, std::string new_uname, std::string new_passwd)
{
    std::unique_ptr<QueryResult> result(
        LoginDatabase.PQuery("SELECT 1 FROM account WHERE id='%u'", accid));
    if (!result)
    ...
    if (!LoginDatabase.PExecute(
            "UPDATE account SET v='0',s='0',username='%s',sha_pass_hash='%s' "
            "WHERE id='%u'",
...
{
    std::unique_ptr<QueryResult> result(LoginDatabase.PQuery(
        "SELECT gmlevel FROM account WHERE id = '%u'", acc_id));
    if (result)
    {
...
uint32 AccountMgr::GetCharactersCount(uint32 acc_id)
{
    ...
    std::unique_ptr<QueryResult> result(CharacterDatabase.PQuery(
        "SELECT COUNT(guid) FROM characters WHERE account = '%u'", acc_id));
    if (result)
    {
        Field* fields = result->Fetch();
...
bool AccountMgr::CheckPassword(uint32 accid, std::string passwd)
{
    ...
    std::unique_ptr<QueryResult> result(LoginDatabase.PQuery(
        "SELECT 1 FROM account WHERE id='%u' AND sha_pass_hash='%s'", accid,
...
bool AccountMgr::normalizeString(std::string& utf8str)
{
    ...
    std::transform(
        &wstr_buf[0], wstr_buf + wstr_len, &wstr_buf[0], wcharToUpperOnlyLatin);

    return WStrToUtf8(wstr_buf, wstr_len, utf8str);
}

std::string AccountMgr::CalculateShaPassHash(
