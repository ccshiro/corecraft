...
BattleGroundRL::BattleGroundRL(const battlefield::specification& spec)
  : specification_(spec), end_cond_check_timer_(0)
{
    // Arena specific stuff
    SetArenaType(spec.arena_size());
    SetRated(spec.rated());
    SetArenaorBGType(true);

    SetMapId(572);
    SetTypeID(BATTLEGROUND_RL);
    SetMinPlayersPerTeam(1);
    SetMaxPlayersPerTeam(5);
    SetMinPlayers(2);
    SetMaxPlayers(10);
    SetName("Ruins of Lordaeron");
    SetTeamStartLoc(ALLIANCE, 1277.87f, 1744.9f, 32.5f, 4.865544f);
    SetTeamStartLoc(HORDE, 1295.13f, 1586.44f, 32.5f, 1.61399f);
    SetLevelRange(20, 70);

    // If debug mode is enabled the BattleGround constructor has already set up
    // the correct values
...
void BattleGroundRL::Update(uint32 diff)
{
    ...
    {
        if (end_cond_check_timer_ <= diff)
        {
            CheckArenaWinConditions();
            end_cond_check_timer_ = 2000;
        }
        else
            end_cond_check_timer_ -= diff;
    }
}
...
void BattleGroundRL::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    if (!killer)
    {
        logging.error("Killer player not found");
        return;
    }
...
{
    player->TeleportTo(GetMapId(), 1285.810547f, 1667.896851f, 39.957642f,
        player->GetO(), false);
    return true;
}
...
void BattleGroundRL::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    FillInitialWorldState(
        data, count, 0xbb8, GetAlivePlayersCountByTeam(ALLIANCE));
    FillInitialWorldState(
        data, count, 0xbb9, GetAlivePlayersCountByTeam(HORDE));
