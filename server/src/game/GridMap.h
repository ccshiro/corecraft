...
public:
    ...
    GridMapLiquidStatus getLiquidStatus(float x, float y, float z,
        uint8 ReqLiquidType, GridMapLiquidData* data = nullptr);
};
...
private:
    ...
    std::mutex mutex_;
    long m_count; // FIXME: Should be making use of boost::atomic
};
...
#define INVALID_HEIGHT_VALUE \
    -200000.0f // for return, must be equal to VMAP_INVALID_HEIGHT_VALUE, check
               // value for unknown height is VMAP_INVALID_HEIGHT
...
#define DEFAULT_WATER_SEARCH \
    50.0f // default search distance to case detection water level

// class for sharing and managin GridMap objects
class MANGOS_DLL_SPEC TerrainInfo : public Referencable
{
public:
    ...
    float GetWaterLevel(
        float x, float y, float z, float* pGround = nullptr) const;
    float GetWaterOrGroundLevel(float x, float y, float z,
        float* pGround = nullptr, bool swim = false) const;
    bool IsInWater(
        float x, float y, float z, GridMapLiquidData* data = nullptr) const;
    bool IsUnderWater(float x, float y, float z) const;

    GridMapLiquidStatus getLiquidStatus(float x, float y, float z,
        uint8 ReqLiquidType, bool check_vmap = true,
        GridMapLiquidData* data = nullptr) const;

    uint16 GetAreaFlag(
        float x, float y, float z, bool* isOutdoors = nullptr) const;
    uint8 GetTerrainType(float x, float y) const;

    uint32 GetAreaId(float x, float y, float z) const;
    uint32 GetZoneId(float x, float y, float z) const;
    void GetZoneAndAreaId(
...
private:
    ...
    GridMap* LoadMapAndVMap(uint32 x, uint32 y);

    const uint32 m_mapId;

    GridMap* m_GridMaps[MAP_DATA_FMT_CELL_COUNT][MAP_DATA_FMT_CELL_COUNT];

    std::mutex mutex_;
};

// class for managing TerrainData object and all sort of geometry querying
// operations
class MANGOS_DLL_DECL TerrainManager
{
...
public:
    ...
    // Loads all Map, VMap and MMap data into memory (may take several minutes,
    // only use this on startup!)
    void LoadAll();
    // Unloads all data loaded
    void UnloadAll();

    uint16 GetAreaFlag(uint32 mapid, float x, float y, float z) const
    {
        TerrainInfo* pData =
            const_cast<TerrainManager*>(this)->LoadTerrain(mapid);
        return pData->GetAreaFlag(x, y, z);
    }
    uint32 GetAreaId(uint32 mapid, float x, float y, float z) const
    {
        return TerrainManager::GetAreaIdByAreaFlag(
            GetAreaFlag(mapid, x, y, z), mapid);
    }
    uint32 GetZoneId(uint32 mapid, float x, float y, float z) const
    {
        return TerrainManager::GetZoneIdByAreaFlag(
            GetAreaFlag(mapid, x, y, z), mapid);
    }
    void GetZoneAndAreaId(
        uint32& zoneid, uint32& areaid, uint32 mapid, float x, float y, float z)
    {
        TerrainManager::GetZoneAndAreaIdByAreaFlag(
            zoneid, areaid, GetAreaFlag(mapid, x, y, z), mapid);
    }

    static uint32 GetAreaIdByAreaFlag(uint16 areaflag, uint32 map_id);
    static uint32 GetZoneIdByAreaFlag(uint16 areaflag, uint32 map_id);
    static void GetZoneAndAreaIdByAreaFlag(
...
#define sTerrainMgr MaNGOS::Singleton<TerrainManager>

// Loads all VMap and MMap data into memory
void LoadAllVmapAndMMapData();
