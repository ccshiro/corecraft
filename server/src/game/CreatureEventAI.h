...
enum EventAI_Type
{
    ...
    EVENT_T_AURA = 23, // Param1 = SpellID, Param2 = Number of time stacked,
                       // Param3/4 Repeat Min/Max
    EVENT_T_TARGET_AURA = 24, // Param1 = SpellID, Param2 = Number of time
                              // stacked, Param3/4 Repeat Min/Max
    ...
    EVENT_T_MISSING_AURA = 27, // Param1 = SpellID, Param2 = Number of time
                               // stacked expected, Param3/4 Repeat Min/Max
    EVENT_T_TARGET_MISSING_AURA = 28, // Param1 = SpellID, Param2 = Number of
                                      // time stacked expected, Param3/4 Repeat
                                      // Min/Max
...
enum EventAI_ActionType
{
    ...
    ACTION_T_TEXT = 1, // TextId1, optionally -TextId2, optionally -TextId3(if
                       // -TextId2 exist). If more than just -TextId1 is
                       // defined, randomize. Negative values.
    ACTION_T_SET_FACTION = 2,             // FactionId (or 0 for default)
    ACTION_T_MORPH_TO_ENTRY_OR_MODEL = 3, // Creature_template entry(param1) OR
                                          // ModelId (param2) (or 0 for both to
                                          // demorph)
    ...
    ACTION_T_RANDOM_SOUND = 9,  // SoundId1, SoundId2, SoundId3 (-1 in any field
                                // means no output if randomed that field)
    ACTION_T_RANDOM_EMOTE = 10, // EmoteId1, EmoteId2, EmoteId3 (-1 in any field
                                // means no output if randomed that field)
    ...
    ACTION_T_CAST_EVENT =
        16, // QuestID, SpellId, Target - must be removed as hack?
    ...
    ACTION_T_AUTO_ATTACK = 20, // AllowAttackState (0 = stop attack, anything
                               // else means continue attacking)
    ACTION_T_COMBAT_MOVEMENT = 21, // AllowCombatMovement (0 = stop combat based
                                   // movement, anything else continue
                                   // attacking)
    ...
    ACTION_T_SET_INVINCIBILITY_HP_LEVEL =
        42, // MinHpValue, format(0-flat,1-percent from max health)
    ACTION_T_MOUNT_TO_ENTRY_OR_MODEL = 43, // Creature_template entry(param1) OR
                                           // ModelId (param2) (or 0 for both to
                                           // unmount)
    ACTION_T_COMBAT_LOS_MOVEMENT = 44,     // 0 = don't move in LOS when casting
    // fails; anything else is DO move in LOS
    ACTION_T_BEHAVIOR_TOGGLE = 45, // 0 = disable, everything else is enable
    ACTION_T_BEHAVIOR_CHANGE =
        46, // Change the behavior to something different (behavior IDs tba)
    ACTION_T_END,
};
...
enum Target
{
    ...
    TARGET_T_HOSTILE_SECOND_AGGRO, // Second highest aggro (generaly used for
                                   // cleaves and some special attacks)
    TARGET_T_HOSTILE_LAST_AGGRO, // Dead last on aggro (no idea what this could
                                 // be used for)
    ...
    TARGET_T_ACTION_INVOKER, // Unit who caused this Event to occur (only works
                             // for EVENT_T_AGGRO, EVENT_T_KILL, EVENT_T_DEATH,
                             // EVENT_T_SPELLHIT, EVENT_T_OOC_LOS,
                             // EVENT_T_FRIENDLY_HP, EVENT_T_FRIENDLY_IS_CC,
                             // EVENT_T_FRIENDLY_MISSING_BUFF)

    // Hostile targets (including pets)
    TARGET_T_HOSTILE_WPET,              // Current target (can be a pet)
    TARGET_T_HOSTILE_WPET_SECOND_AGGRO, // Second highest aggro (generaly used
                                        // for cleaves and some special attacks)
    TARGET_T_HOSTILE_WPET_LAST_AGGRO,   // Dead last on aggro (no idea what this
                                        // could be used for)
    ...
    TARGET_T_FRIENDLY_RANDOM, // Any random friendly target within 30 yards.
...
enum EventFlags
{
    ...
    EFLAG_RANDOM_ACTION = 0x20, // Event only execute one from existed actions
                                // instead each action.
...
struct CreatureEventAI_Action
{
    ...
    union
    {
        ...
        struct
        {
            ...
            uint32 factionFlags; // flags will restore default faction at evade
                                 // and/or respawn
        } set_faction;
        ...
        struct
        {
            int32 soundId1; // (-1 in any field means no output if randomed that
                            // field)
            int32 soundId2;
            int32 soundId3;
        } random_sound;
        ...
        struct
        {
            int32 emoteId1; // (-1 in any field means no output if randomed that
                            // field)
            int32 emoteId2;
            int32 emoteId3;
        } random_emote;
        ...
        // ACTION_T_SET_UNIT_FLAG                           = 18,  // value
        // provided mask bits that will be set
        // ACTION_T_REMOVE_UNIT_FLAG                        = 19,  // value
        // provided mask bits that will be clear
        struct
        {
            uint32 value;
            uint32 target;
        ...
        struct
        {
            uint32 state; // 0 = stop attack, anything else means continue
                          // attacking
        } auto_attack;
        // ACTION_T_COMBAT_MOVEMENT                         = 21
        struct
        {
            uint32 state; // 0 = stop combat based movement, anything else
                          // continue attacking
            uint32 melee; // if set: at stop send melee combat stop if in
                          // combat, use for terminate melee fighting state for
                          // switch to ranged
        } combat_movement;
        ...
        {
            uint32
                creatureId; // set one from fields (or 0 for both to dismount)
            uint32 modelId;
        } mount;
        // ACTION_T_COMBAT_LOS_MOVEMENT                     = 44
        struct
        {
            uint32 state; // 0 = don't move into LOS if spellcast fails
        } combat_los_movement;
        // ACTION_T_BEHAVIOR_TOGGLE                         = 45
        struct
        {
            uint32 state; // 0 = disable, !0 = enable
        } behavior_toggle;
        // ACTION_T_BEHAVIOR_CHANGE                         = 46
        struct
        {
            uint32 behavior; // 0 1 2 3 probably, warrior paladin mage rogue
        } behavior_change;
        // RAW
        struct
        {
...
struct CreatureEventAI_Event
{
    ...
    union
    {
        ...
        {
            uint32 spellId;
            uint32 schoolMask; // -1 (==0xffffffff) is ok value for full mask,
                               // or must be more limited mask like (0 < 1) = 1
                               // for normal/physical school
            uint32 repeatMin;
            uint32 repeatMax;
        } spell_hit;
        // EVENT_T_RANGE                                    = 9
...
typedef std::unordered_map<uint32, CreatureEventAI_Event_Vec>
    CreatureEventAI_Event_Map;
...
// EventSummon_Map
typedef std::unordered_map<uint32, CreatureEventAI_Summon>
    CreatureEventAI_Summon_Map;

struct CreatureEventAIHolder
{
    CreatureEventAIHolder(CreatureEventAI_Event p)
      : Event(std::move(p)), Time(0), Enabled(true)
    {
    }
...
protected:
    ...
    CreatureEventAIList m_CreatureEventAIList; // Holder for events (stores
                                               // enabled, time, and eventid)

    uint8 m_Phase;                // Current phase, max 32 phases
    bool m_CombatMovementEnabled; // If we allow targeted movment gen (movement
                                  // twoards top threat)
    bool m_CombatLOSMovementEnabled; // If we allow moving into line of sight
                                     // when casting
    bool m_MeleeEnabled;             // If we allow melee auto attack
    float m_AttackDistance;          // Distance to attack from
    float m_AttackAngle;             // Angle of attack
    uint32
        m_InvinceabilityHpLevel; // Minimal health level allowed at damage apply

    BehavioralAI m_BehavioralAI;
    bool m_BehavioralAIIsMovingUs; // True if we should not move ourselves
};
