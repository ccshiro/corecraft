...
OutdoorPvPZM::OutdoorPvPZM()
  : OutdoorPvP(), m_graveyardOwner(TEAM_NONE),
    ...
    m_towersAlliance(0), m_towersHorde(0)
{
    // init world states
...
void OutdoorPvPZM::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    ...
    {
        FillInitialWorldState(
            data, count, m_towerWorldState[i], WORLD_STATE_ADD);
...
void OutdoorPvPZM::HandlePlayerEnterZone(Player* player, bool isMainZone)
{
    ...
    // have the aura
    player->remove_auras(SPELL_TWIN_SPIRE_BLESSING);
...
void OutdoorPvPZM::HandleCreatureCreate(Creature* creature)
{
    ...
    case NPC_PVP_BEAM_RED:
        if (creature->GetY() < 7000.0f) // East Beam
            m_beamTowerRed[0] = creature->GetObjectGuid();
        else if (creature->GetY() < 7300.0f) // Center Beam
    ...
    for (uint8 i = 0; i < MAX_ZM_TOWERS; ++i)
    {
        if (zangarmarshTowers[i] == go->GetEntry())
        {
            for (uint8 j = 0; j < 4; ++j)
            {
                ...
                {
                    // prevent processing if the owner did not change (happens
                    // if progress event is called after contest event)
                    if (zangarmarshTowerEvents[i][j].team != m_towerOwner[i])
                    {
                        if (zangarmarshTowerEvents[i][j].defenseMessage)
                            sWorld::Instance()->SendDefenseMessage(
                                ZONE_ID_ZANGARMARSH,
                                zangarmarshTowerEvents[i][j].defenseMessage);

                        return ProcessCaptureEvent(go, i,
                            zangarmarshTowerEvents[i][j].team,
...
bool OutdoorPvPZM::ProcessCaptureEvent(GameObject* go, uint32 towerId,
    Team team, uint32 newWorldState, uint32 newMapState)
{
    if (team == ALLIANCE)
    {
        // update counter
        SetBeaconArtKit(go->GetMap(), ALLIANCE, towerId);
        ++m_towersAlliance;
        ...
        if (m_towersAlliance == MAX_ZM_TOWERS)
        {
            ...
            // sends another
            sWorld::Instance()->SendDefenseMessage(
                ZONE_ID_ZANGARMARSH, LANG_OPVP_ZM_CAPTURE_BOTH_BEACONS_A);
    ...
    {
        // update counter
        SetBeaconArtKit(go->GetMap(), HORDE, towerId);
        ++m_towersHorde;
        ...
        if (m_towersHorde == MAX_ZM_TOWERS)
        {
            ...
            // sends another
            sWorld::Instance()->SendDefenseMessage(
                ZONE_ID_ZANGARMARSH, LANG_OPVP_ZM_CAPTURE_BOTH_BEACONS_H);
    ...
    {
        SetBeaconArtKit(go->GetMap(), TEAM_NONE, towerId);
...
void OutdoorPvPZM::UpdateScoutState(Team team, bool spawned)
{
    if (team == ALLIANCE)
    {
        ...
        m_scoutWorldStateAlliance = spawned ?
        ...
        if (spawned)
            sWorld::Instance()->SendDefenseMessage(
                ZONE_ID_ZANGARMARSH, LANG_OPVP_ZM_SPAWN_FIELD_SCOUT_A);
    }
    else
    {
        SendUpdateWorldState(m_scoutWorldStateHorde, WORLD_STATE_REMOVE);
        m_scoutWorldStateHorde = spawned ? WORLD_STATE_ZM_FLAG_READY_HORDE :
                                           WORLD_STATE_ZM_FLAG_NOT_READY_HORDE;
        SendUpdateWorldState(m_scoutWorldStateHorde, WORLD_STATE_ADD);

        if (spawned)
            sWorld::Instance()->SendDefenseMessage(
                ZONE_ID_ZANGARMARSH, LANG_OPVP_ZM_SPAWN_FIELD_SCOUT_H);
    }
}
...
bool OutdoorPvPZM::HandleGameObjectUse(Player* player, GameObject* go)
{
    ...
    if (!player->has_aura(SPELL_BATTLE_STANDARD_HORDE) &&
        !player->has_aura(SPELL_BATTLE_STANDARD_ALLIANCE))
        return false;

    if (team == ALLIANCE && m_towersAlliance != 2)
        return false;
    else if (team == HORDE && m_towersHorde != 2)
        return false;
    ...
    SetBeaconArtKit(go->GetMap(), TEAM_NONE, -1);

    if (team == ALLIANCE)
    {
        // change banners
        RespawnGO(go, m_graveyardBannerAlliance, true);
        SetBeaconArtKit(go->GetMap(), ALLIANCE, -1);
        ...
        player->remove_auras(SPELL_BATTLE_STANDARD_ALLIANCE);

        // send defense message
        sWorld::Instance()->SendDefenseMessage(
            ZONE_ID_ZANGARMARSH, LANG_OPVP_ZM_CAPTURE_GRAVEYARD_A);
    }
    else
    {
        // change banners
        RespawnGO(go, m_graveyardBannerHorde, true);
        SetBeaconArtKit(go->GetMap(), HORDE, -1);
        ...
        // remove player flag aura
        player->remove_auras(SPELL_BATTLE_STANDARD_HORDE);

        // send defense message
        sWorld::Instance()->SendDefenseMessage(
            ZONE_ID_ZANGARMARSH, LANG_OPVP_ZM_CAPTURE_GRAVEYARD_H);
    }

    // change the graveyard link
    sObjectMgr::Instance()->SetGraveYardLinkTeam(
        GRAVEYARD_ID_TWIN_SPIRE, GRAVEYARD_ZONE_TWIN_SPIRE, team);
...
//{
//    if (spellId == SPELL_BATTLE_STANDARD_HORDE || spellId ==
//    SPELL_BATTLE_STANDARD_ALLIANCE)
//    {
//        // ToDo: implement this when the scout DB conditions are implemented
//        // The scouts gossip options should check a DB condition if the gossip
//        is pvp available
//        // The idea is to set the Outdoor PvP condition to false on flag take
//        - this will allow only one player to use the flag
//        // on flag drop the condition can be set back to true if necessary, so
//        the players can retake the flag
//        return true;
//    }
//
//    return false;
...
void OutdoorPvPZM::SetBeaconArtKit(Map* map, Team team, int type)
{
    UnitVisibility alliance, horde;
    if (team == ALLIANCE)
    {
        alliance = VISIBILITY_ON;
        horde = VISIBILITY_OFF;
    }
    else if (team == HORDE)
    {
        alliance = VISIBILITY_OFF;
        horde = VISIBILITY_ON;
    }
    else
    {
    ...
    {
        // Type is a tower id
        if (Creature* creature = map->GetCreature(m_beamTowerBlue[type]))
            creature->SetVisibility(alliance);
        if (Creature* creature = map->GetCreature(m_beamTowerRed[type]))
            creature->SetVisibility(horde);
    }
    else
    {
        // Type is the graveyard
        if (Creature* creature = map->GetCreature(m_beamGraveyardBlue))
            creature->SetVisibility(alliance);
        if (Creature* creature = map->GetCreature(m_beamGraveyardRed))
            creature->SetVisibility(horde);
    }
}

Team OutdoorPvPZM::GetOwningTeam() const
{
    if (m_scoutWorldStateAlliance == WORLD_STATE_ZM_FLAG_READY_ALLIANCE)
        return ALLIANCE;
    else if (m_scoutWorldStateHorde == WORLD_STATE_ZM_FLAG_READY_HORDE)
        return HORDE;
    return TEAM_NONE;
}
