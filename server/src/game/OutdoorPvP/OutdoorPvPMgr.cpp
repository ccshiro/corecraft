...
#define LOAD_OPVP_ZONE(a)                                                    \
    if (sWorld::Instance()->getConfig(CONFIG_BOOL_OUTDOORPVP_##a##_ENABLED)) \
    {                                                                        \
...
void OutdoorPvPMgr::InitOutdoorPvP()
{
    ...
    logging.info("Loaded %u Outdoor PvP zones\n", counter);
}
...
void OutdoorPvPMgr::Update(uint32 diff)
{
    ...
    for (auto& elem : m_scripts)
        if (elem)
            elem->Update(m_updateTimer.GetCurrent());
...
/**
   ...
   @param   default value being returned if no saved value for the capture point
   was found
 */
float OutdoorPvPMgr::GetCapturePointSliderValue(
    uint32 entry, float defaultValue)
{
    CapturePointSliderMap::const_iterator itr =
        m_capturePointSlider.find(entry);
