...
OutdoorPvPEP::OutdoorPvPEP()
  : OutdoorPvP(), m_towersAlliance(0), m_towersHorde(0)
{
    ...
    sObjectMgr::Instance()->SetGraveYardLinkTeam(
        GRAVEYARD_ID_EASTERN_PLAGUE, GRAVEYARD_ZONE_EASTERN_PLAGUE, TEAM_NONE);
}

void OutdoorPvPEP::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    FillInitialWorldState(
        data, count, WORLD_STATE_EP_TOWER_COUNT_ALLIANCE, m_towersAlliance);
    FillInitialWorldState(
        data, count, WORLD_STATE_EP_TOWER_COUNT_HORDE, m_towersHorde);

    for (auto& elem : m_towerWorldState)
...
void OutdoorPvPEP::HandlePlayerEnterZone(Player* player, bool isMainZone)
{
    ...
    for (auto& plaguelandsTowerBuff : plaguelandsTowerBuffs)
        player->remove_auras(player->GetTeam() == ALLIANCE ?
                                 plaguelandsTowerBuff.spellIdAlliance :
                                 plaguelandsTowerBuff.spellIdHorde);
    ...
    case ALLIANCE:
        if (m_towersAlliance > 0)
            player->CastSpell(player,
                plaguelandsTowerBuffs[m_towersAlliance - 1].spellIdAlliance,
                true);
        break;
    case HORDE:
        if (m_towersHorde > 0)
            player->CastSpell(player,
                plaguelandsTowerBuffs[m_towersHorde - 1].spellIdHorde, true);
        break;
    default:
        break;
    }
...
void OutdoorPvPEP::HandleCreatureCreate(Creature* creature)
{
    ...
    case NPC_SPECTRAL_FLIGHT_MASTER:
        ...
        creature->setFaction(m_towerOwner[TOWER_ID_PLAGUEWOOD] == ALLIANCE ?
                                 FACTION_FLIGHT_MASTER_ALLIANCE :
                                 FACTION_FLIGHT_MASTER_HORDE);
        creature->CastSpell(creature,
            m_towerOwner[TOWER_ID_PLAGUEWOOD] == ALLIANCE ?
                SPELL_SPIRIT_PARTICLES_BLUE :
                SPELL_SPIRIT_PARTICLES_RED,
            true);
        break;
...
void OutdoorPvPEP::HandleGameObjectCreate(GameObject* go)
{
    ...
    case GO_TOWER_BANNER:
        // sort banners
        if (go->IsWithinDist2d(plaguelandsTowerLocations[TOWER_ID_NORTHPASS][0],
                plaguelandsTowerLocations[TOWER_ID_NORTHPASS][1], 50.0f))
            InitBanner(go, TOWER_ID_NORTHPASS);
        else if (go->IsWithinDist2d(
                     plaguelandsTowerLocations[TOWER_ID_CROWNGUARD][0],
                     plaguelandsTowerLocations[TOWER_ID_CROWNGUARD][1], 50.0f))
            InitBanner(go, TOWER_ID_CROWNGUARD);
        else if (go->IsWithinDist2d(
                     plaguelandsTowerLocations[TOWER_ID_EASTWALL][0],
                     plaguelandsTowerLocations[TOWER_ID_EASTWALL][1], 50.0f))
            InitBanner(go, TOWER_ID_EASTWALL);
        else if (go->IsWithinDist2d(
                     plaguelandsTowerLocations[TOWER_ID_PLAGUEWOOD][0],
                     plaguelandsTowerLocations[TOWER_ID_PLAGUEWOOD][1], 50.0f))
            InitBanner(go, TOWER_ID_PLAGUEWOOD);
        break;
...
void OutdoorPvPEP::HandleObjectiveComplete(
                    ...
                    if (plaguelandsTowerEvents[i][j].team != m_towerOwner[i])
                    {
                        if (plaguelandsTowerEvents[i][j].defenseMessage)
                            sWorld::Instance()->SendDefenseMessage(
                                ZONE_ID_EASTERN_PLAGUELANDS,
                                plaguelandsTowerEvents[i][j].defenseMessage);

                        return ProcessCaptureEvent(go, i,
                            plaguelandsTowerEvents[i][j].team,
                            plaguelandsTowerEvents[i][j].worldState);
                    }
...
bool OutdoorPvPEP::ProcessCaptureEvent(
    GameObject* go, uint32 towerId, Team team, uint32 newWorldState)
{
    if (team == ALLIANCE)
    {
        // update banner
        for (std::list<ObjectGuid>::const_iterator itr =
                 m_towerBanners[towerId].begin();
             itr != m_towerBanners[towerId].end(); ++itr)
            SetBannerVisual(
                go, (*itr), CAPTURE_ARTKIT_ALLIANCE, CAPTURE_ANIM_ALLIANCE);

        // update counter
        ++m_towersAlliance;
        SendUpdateWorldState(
            WORLD_STATE_EP_TOWER_COUNT_ALLIANCE, m_towersAlliance);
    ...
    else if (team == HORDE)
    {
        ...
        for (std::list<ObjectGuid>::const_iterator itr =
             ...
             itr != m_towerBanners[towerId].end(); ++itr)
            SetBannerVisual(
                go, (*itr), CAPTURE_ARTKIT_HORDE, CAPTURE_ANIM_HORDE);
    ...
    else
    {
        ...
        for (std::list<ObjectGuid>::const_iterator itr =
             ...
             itr != m_towerBanners[towerId].end(); ++itr)
            SetBannerVisual(
                go, (*itr), CAPTURE_ARTKIT_NEUTRAL, CAPTURE_ANIM_NEUTRAL);
        ...
        if (m_towerOwner[towerId] == ALLIANCE)
        {
            ...
            if (m_towersAlliance == 0)
                BuffTeam(
                    ALLIANCE, plaguelandsTowerBuffs[0].spellIdAlliance, true);
        }
        else
        {
            // update counter
            --m_towersHorde;
            SendUpdateWorldState(
                WORLD_STATE_EP_TOWER_COUNT_HORDE, m_towersHorde);
    ...
    if (team != TEAM_NONE)
    {
        ...
        case TOWER_ID_NORTHPASS:
            RespawnGO(go, team == ALLIANCE ? m_lordaeronShrineAlliance :
                                             m_lordaeronShrineHorde,
                true);
            break;
        case TOWER_ID_CROWNGUARD:
            sObjectMgr::Instance()->SetGraveYardLinkTeam(
                GRAVEYARD_ID_EASTERN_PLAGUE, GRAVEYARD_ZONE_EASTERN_PLAGUE,
                team);
            break;
    ...
    else
    {
        ...
        {
        case TOWER_ID_NORTHPASS:
            RespawnGO(go, m_towerOwner[TOWER_ID_NORTHPASS] == ALLIANCE ?
...
bool OutdoorPvPEP::HandleGameObjectUse(Player* /*player*/, GameObject* go)
{
    ...
    if (go->GetEntry() == GO_LORDAERON_SHRINE_ALLIANCE ||
...
// Handle the unsummon of the spectral flight master when the Plaguewood tower
// is lost
...
{
    for (std::list<ObjectGuid>::const_iterator itr = m_soldiers.begin();
         itr != m_soldiers.end(); ++itr)
    {
