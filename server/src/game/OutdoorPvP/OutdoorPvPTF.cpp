...
OutdoorPvPTF::OutdoorPvPTF()
  : OutdoorPvP(), m_zoneWorldState(WORLD_STATE_TF_TOWERS_CONTROLLED),
    m_zoneOwner(TEAM_NONE),
    // m_zoneUpdateTimer(TIMER_TF_UPDATE_TIME),
    m_zoneLockTimer(0), m_towersAlliance(0), m_towersHorde(0)
{
...
void OutdoorPvPTF::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    ...
    if (m_zoneWorldState == WORLD_STATE_TF_TOWERS_CONTROLLED)
    {
        FillInitialWorldState(
            data, count, WORLD_STATE_TF_TOWER_COUNT_H, m_towersHorde);
        FillInitialWorldState(
            data, count, WORLD_STATE_TF_TOWER_COUNT_A, m_towersAlliance);
...
void OutdoorPvPTF::HandlePlayerEnterZone(Player* player, bool isMainZone)
{
    ...
    // relog
    player->remove_auras(SPELL_AUCHINDOUN_BLESSING);
...
void OutdoorPvPTF::HandleObjectiveComplete(
    uint32 eventId, std::list<Player*> players, Team team)
{
    for (auto& terokkarTowerEvent : terokkarTowerEvents)
    {
        for (auto& elem : terokkarTowerEvent)
        {
            if (elem.eventEntry == eventId)
            {
                for (auto& player : players)
                {
                    if ((player) && (player)->GetTeam() == team)
                        (player)->AreaExploredOrEventHappens(
                            team == ALLIANCE ?
                                QUEST_SPIRITS_OF_AUCHINDOUM_ALLIANCE :
                ...
                if (terokkarTowerEvents[i][j].eventEntry == eventId)
                {
                    ...
                    if (terokkarTowerEvents[i][j].team != m_towerOwner[i])
                    {
                        sWorld::Instance()->SendDefenseMessage(
                            ZONE_ID_TEROKKAR_FOREST,
                            terokkarTowerEvents[i][j].defenseMessage);

                        return ProcessCaptureEvent(go, i,
                            terokkarTowerEvents[i][j].team,
                            terokkarTowerEvents[i][j].worldState);
                    }
...
bool OutdoorPvPTF::ProcessCaptureEvent(
    ...
    else
    {
        ...
        if (m_towerOwner[towerId] == ALLIANCE)
        {
            ...
            SendUpdateWorldState(
                WORLD_STATE_TF_TOWER_COUNT_A, m_towersAlliance);
        }
        else
        {
            --m_towersHorde;
...
void OutdoorPvPTF::LockZone(
    GameObject* go, uint32 towerId, Team team, uint32 newWorldState)
{
    SendUpdateWorldState(m_zoneWorldState, WORLD_STATE_REMOVE);
    m_zoneWorldState = team == ALLIANCE ? WORLD_STATE_TF_LOCKED_ALLIANCE :
    ...
    sWorld::Instance()->SendDefenseMessage(ZONE_ID_TEROKKAR_FOREST,
        team == ALLIANCE ? LANG_OPVP_TF_CAPTURE_ALL_TOWERS_A :
                           LANG_OPVP_TF_CAPTURE_ALL_TOWERS_H);

    // remove tower states when zone has been captured and locked
    for (auto& elem : m_towerWorldState)
        SendUpdateWorldState(elem, WORLD_STATE_REMOVE);
...
void OutdoorPvPTF::UnlockZone()
{
    ...
    m_towerWorldState[4] = WORLD_STATE_TF_SOUTH_TOWER_NEUTRAL;
    for (auto& elem : m_towerWorldState)
        SendUpdateWorldState(elem, WORLD_STATE_ADD);
    ...
    {
        // Find player who is in main zone (Terokkar Forest) to get correct map
        // reference
        if (!itr->second)
            continue;
...
void OutdoorPvPTF::Update(uint32 diff)
{
    if (m_zoneLockTimer)
    {
        ...
        {
            // update timer - if OutdoorPvPMgr update timer interval needs to be
            // lowered replace this line with the commented-out ones below
            UpdateTimerWorldState();
...
void OutdoorPvPTF::UpdateTimerWorldState()
{
    ...
    SendUpdateWorldState(
        WORLD_STATE_TF_TIME_MIN_SECOND_DIGIT, minutesLeft - firstDigit * 10);
...
void OutdoorPvPTF::LockTowers(const WorldObject* objRef)
{
    for (uint8 i = 0; i < MAX_TF_TOWERS; ++i)
    {
        if (GameObject* go = objRef->GetMap()->GetGameObject(m_towerBanners[i]))
            ...
            sOutdoorPvPMgr::Instance()->SetCapturePointSlider(terokkarTowers[i],
...
void OutdoorPvPTF::ResetTowers(const WorldObject* objRef)
{
    for (uint8 i = 0; i < MAX_TF_TOWERS; ++i)
    {
        if (GameObject* go = objRef->GetMap()->GetGameObject(m_towerBanners[i]))
        {
            ...
            // visual update needed because banner still has artkit from
            // previous owner
            SetBannerVisual(go, CAPTURE_ARTKIT_NEUTRAL, CAPTURE_ANIM_NEUTRAL);
        }
        else
            ...
            sOutdoorPvPMgr::Instance()->SetCapturePointSlider(
                terokkarTowers[i], CAPTURE_SLIDER_MIDDLE);
    }
}

Team OutdoorPvPTF::GetOwningTeam() const
{
    if (m_zoneWorldState == WORLD_STATE_TF_LOCKED_ALLIANCE)
        return ALLIANCE;
    else if (m_zoneWorldState == WORLD_STATE_TF_LOCKED_HORDE)
        return HORDE;
    return TEAM_NONE;
}
