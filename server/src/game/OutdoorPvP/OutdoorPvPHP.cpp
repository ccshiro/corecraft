...
void OutdoorPvPHP::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    FillInitialWorldState(
        data, count, WORLD_STATE_HP_TOWER_COUNT_ALLIANCE, m_towersAlliance);
    FillInitialWorldState(
        data, count, WORLD_STATE_HP_TOWER_COUNT_HORDE, m_towersHorde);
    FillInitialWorldState(
        data, count, WORLD_STATE_HP_TOWER_DISPLAY_A, WORLD_STATE_ADD);
    FillInitialWorldState(
        data, count, WORLD_STATE_HP_TOWER_DISPLAY_H, WORLD_STATE_ADD);
...
void OutdoorPvPHP::SendRemoveWorldStates(Player* player)
{
    player->SendUpdateWorldState(
        WORLD_STATE_HP_TOWER_DISPLAY_A, WORLD_STATE_REMOVE);
    player->SendUpdateWorldState(
        WORLD_STATE_HP_TOWER_DISPLAY_H, WORLD_STATE_REMOVE);
...
void OutdoorPvPHP::HandlePlayerEnterZone(Player* player, bool isMainZone)
{
    ...
    player->remove_auras(player->GetTeam() == ALLIANCE ?
                             SPELL_HELLFIRE_SUPERIORITY_ALLIANCE :
...
void OutdoorPvPHP::HandleGameObjectCreate(GameObject* go)
{
    ...
    case GO_TOWER_BANNER_OVERLOOK:
        ...
        go->SetGoArtKit(
            GetBannerArtKit(m_towerOwner[0], GO_ARTKIT_OVERLOOK_ALLIANCE,
                GO_ARTKIT_OVERLOOK_HORDE, GO_ARTKIT_OVERLOOK_NEUTRAL));
        break;
    case GO_TOWER_BANNER_STADIUM:
        m_banners[1] = go->GetObjectGuid();
        go->SetGoArtKit(
            GetBannerArtKit(m_towerOwner[1], GO_ARTKIT_STADIUM_ALLIANCE,
                GO_ARTKIT_STADIUM_HORDE, GO_ARTKIT_STADIUM_NEUTRAL));
        break;
    case GO_TOWER_BANNER_BROKEN_HILL:
        m_banners[2] = go->GetObjectGuid();
        go->SetGoArtKit(
            GetBannerArtKit(m_towerOwner[2], GO_ARTKIT_BROKEN_HILL_ALLIANCE,
                GO_ARTKIT_BROKEN_HILL_HORDE, GO_ARTKIT_BROKEN_HILL_NEUTRAL));
        break;
...
void OutdoorPvPHP::HandleObjectiveComplete(
...
void OutdoorPvPHP::HandlePlayerKillInsideArea(Player* player)
{
    ...
    if (areaid == 3669 || areaid == 3670 || areaid == 3671)
        player->CastSpell(player, player->GetTeam() == ALLIANCE ?
                                      SPELL_HELLFIRE_TOWER_TOKEN_ALLIANCE :
                    ...
                    if (hellfireTowerEvents[i][j].team != m_towerOwner[i])
                    {
                        if (hellfireTowerEvents[i][j].defenseMessage)
                            sWorld::Instance()->SendDefenseMessage(
                                ZONE_ID_HELLFIRE_PENINSULA,
                                hellfireTowerEvents[i][j].defenseMessage);

                        return ProcessCaptureEvent(go, i,
                            hellfireTowerEvents[i][j].team,
                            hellfireTowerEvents[i][j].worldState,
                            hellfireTowerEvents[i][j].towerArtKit,
                            hellfireTowerEvents[i][j].towerAnim);
                    }
...
bool OutdoorPvPHP::ProcessCaptureEvent(GameObject* go, uint32 towerId,
    Team team, uint32 newWorldState, uint32 towerArtKit, uint32 towerAnim)
{
    ...
    if (team == ALLIANCE)
    {
        ...
        SendUpdateWorldState(
            WORLD_STATE_HP_TOWER_COUNT_ALLIANCE, m_towersAlliance);
    ...
    else
    {
        ...
        else
        {
            ...
            --m_towersHorde;
            SendUpdateWorldState(
                WORLD_STATE_HP_TOWER_COUNT_HORDE, m_towersHorde);
        }
    }
