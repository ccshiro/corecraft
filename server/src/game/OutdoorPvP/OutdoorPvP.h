...
public:
    ...
    virtual void HandleObjectiveComplete(
    ...
    // Called when a creature respawns
    ...
    bool AddCreature(uint32 entry, Team teamval, uint32 guidLow);

    // Returns the team that is currently in control of the Zone's Objective
    virtual Team GetOwningTeam() const { return TEAM_NONE; }
...
protected:
    ...
    // set banner visual
    void SetBannerVisual(const WorldObject* objRef, ObjectGuid goGuid,
        uint32 artKit, uint32 animId);
