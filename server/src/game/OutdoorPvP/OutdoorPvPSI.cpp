...
OutdoorPvPSI::OutdoorPvPSI()
  : OutdoorPvP(), m_resourcesAlliance(0), m_resourcesHorde(0),
...
void OutdoorPvPSI::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    FillInitialWorldState(
        data, count, WORLD_STATE_SI_GATHERED_A, m_resourcesAlliance);
    FillInitialWorldState(
        data, count, WORLD_STATE_SI_GATHERED_H, m_resourcesHorde);
    FillInitialWorldState(
        data, count, WORLD_STATE_SI_SILITHYST_MAX, MAX_SILITHYST);
}
...
void OutdoorPvPSI::HandlePlayerEnterZone(Player* player, bool isMainZone)
{
    ...
    // have the aura
    player->remove_auras(SPELL_CENARION_FAVOR);
...
bool OutdoorPvPSI::HandleAreaTrigger(Player* player, uint32 triggerId)
{
    ...
    case AREATRIGGER_SILITHUS_ALLIANCE:
        if (player->GetTeam() != ALLIANCE || !player->has_aura(SPELL_SILITHYST))
            return false;
        ...
        // handle the case when the faction has reached maximum resources
        // allowed
        if (m_resourcesAlliance == MAX_SILITHYST)
        {
            ...
            sWorld::Instance()->SendDefenseMessage(
                ZONE_ID_SILITHUS, LANG_OPVP_SI_CAPTURE_A);
        }

        // give quest credit if necessary
        if (player->GetQuestStatus(QUEST_SCOURING_DESERT_ALLIANCE) ==
            QUEST_STATUS_INCOMPLETE)
            player->KilledMonsterCredit(NPC_SILITHUS_DUST_QUEST_ALLIANCE);
        break;
    case AREATRIGGER_SILITHUS_HORDE:
        if (player->GetTeam() != HORDE || !player->has_aura(SPELL_SILITHYST))
            return false;
        ...
        if (m_resourcesHorde == MAX_SILITHYST)
        {
            ...
            SendUpdateWorldState(
                WORLD_STATE_SI_GATHERED_A, m_resourcesAlliance);
            ...
            // Send defense message
            sWorld::Instance()->SendDefenseMessage(
                ZONE_ID_SILITHUS, LANG_OPVP_SI_CAPTURE_H);
        }

        // give quest credit if necessary
        if (player->GetQuestStatus(QUEST_SCOURING_DESERT_HORDE) ==
            QUEST_STATUS_INCOMPLETE)
    ...
    player->remove_auras(SPELL_SILITHYST);
    ...
    player->GetReputationMgr().ModifyReputation(
        sFactionStore.LookupEntry(FACTION_CENARION_CIRCLE),
...
bool OutdoorPvPSI::HandleDropFlag(Player* player, uint32 spellId)
{
    ...
    {
    case ALLIANCE:
        if (player->IsWithinDist3d(silithusFlagDropLocations[0].x,
                silithusFlagDropLocations[0].y, silithusFlagDropLocations[0].z,
                5.0f))
            return false;
        break;
    case HORDE:
        if (player->IsWithinDist3d(silithusFlagDropLocations[1].x,
                silithusFlagDropLocations[1].y, silithusFlagDropLocations[1].z,
                5.0f))
            return false;
        break;
    default:
...
{
    if (go->GetEntry() == GO_SILITHYST_MOUND ||
        go->GetEntry() == GO_SILITHYST_GEYSER)
    {
