...
AuctionHouseMgr::~AuctionHouseMgr()
{
    for (ItemMap::const_iterator itr = mAitems.begin(); itr != mAitems.end();
         ++itr)
        delete itr->second;
}

AuctionHouseObject* AuctionHouseMgr::GetAuctionsMap(
...
uint32 AuctionHouseMgr::GetAuctionDeposit(
    AuctionHouseEntry const* entry, uint32 time, Item* pItem)
{
    float deposit = float(pItem->GetProto()->SellPrice * pItem->GetCount() *
                          (time / MIN_AUCTION_TIME));

    deposit = deposit * entry->depositPercent * 3.0f / 100.0f;

    float min_deposit =
        float(sWorld::Instance()->getConfig(CONFIG_UINT32_AUCTION_DEPOSIT_MIN));
    ...
    return uint32(
        deposit *
        sWorld::Instance()->getConfig(CONFIG_FLOAT_RATE_AUCTION_DEPOSIT));
}
...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction)
{
    ...
    Player* bidder = sObjectMgr::Instance()->GetPlayer(bidder_guid);

    uint32 bidder_accId = 0;

    ObjectGuid ownerGuid = ObjectGuid(HIGHGUID_PLAYER, auction->owner);
    // data for gm.log
    if (sWorld::Instance()->getConfig(CONFIG_BOOL_GM_LOG_TRADE))
    {
        ...
        {
            bidder_accId =
                sObjectMgr::Instance()->GetPlayerAccountIdByGUID(bidder_guid);
            bidder_security =
                bidder_accId ?
                    sAccountMgr::Instance()->GetSecurity(bidder_accId) :
                    SEC_PLAYER;
            ...
            if (bidder_security > SEC_PLAYER) // not do redundant DB requests
            {
                if (!sObjectMgr::Instance()->GetPlayerNameByGUID(
                    ...
                    bidder_name =
                        sObjectMgr::Instance()->GetMangosStringForDBCLocale(
                            LANG_UNKNOWN);
            }
        }
        ...
        if (bidder_security > SEC_PLAYER)
        {
            ...
            if (ownerGuid &&
                !sObjectMgr::Instance()->GetPlayerNameByGUID(
                    ownerGuid, owner_name))
                owner_name =
                    sObjectMgr::Instance()->GetMangosStringForDBCLocale(
                        LANG_UNKNOWN);

            uint32 owner_accid =
                sObjectMgr::Instance()->GetPlayerAccountIdByGUID(ownerGuid);

            gm_logger.info(
                "GM %s (Account: %u) won item in auction (Entry: %u Count: %u) "
                "and pay money: %u. Original owner %s (Account: %u)",
                bidder_name.c_str(), bidder_accId, auction->itemTemplate,
                auction->itemCount, auction->bid, owner_name.c_str(),
                owner_accid);
        }
    }
    else if (!bidder)
    ...
    if (bidder || bidder_accId)
    {
        ...
        msgAuctionWonSubject << auction->itemTemplate << ":"
                             << auction->itemRandomPropertyId << ":"
                             << AUCTION_WON;
        ...
        msgAuctionWonBody << std::dec << ":" << auction->bid << ":"
                          << auction->buyout;
        LOG_DEBUG(logging, "AuctionWon body string : %s",
        ...
        CharacterDatabase.PExecute(
            ...
            auction->bidder, auction->itemGuidLow);

        if (bidder)
            bidder->GetSession()->SendAuctionBidderNotification(auction, true);

        RemoveAItem(auction->itemGuidLow); // we have to remove the item, before
                                           // we delete it !!
        ...
        MailDraft(msgAuctionWonSubject.str(), msgAuctionWonBody.str())
            .AddItem(pItem)
            .SendMailTo(MailReceiver(bidder, bidder_guid), auction,
    ...
    {
        CharacterDatabase.PExecute(
            "DELETE FROM item_instance WHERE guid='%u'", auction->itemGuidLow);
        RemoveAItem(auction->itemGuidLow); // we have to remove the item, before
                                           // we delete it !!
...
void AuctionHouseMgr::SendAuctionSalePendingMail(AuctionEntry* auction)
{
    ...
    Player* owner = sObjectMgr::Instance()->GetPlayer(owner_guid);

    // owner exist (online or offline)
    if (owner ||
        (owner_guid &&
            sObjectMgr::Instance()->GetPlayerAccountIdByGUID(owner_guid)))
    {
        std::ostringstream msgAuctionSalePendingSubject;
        msgAuctionSalePendingSubject << auction->itemTemplate << ":"
                                     << auction->itemRandomPropertyId << ":"
        ...
        time_t distrTime = WorldTimer::time_no_syscall() + HOUR;
        ...
        LOG_DEBUG(logging, "AuctionSalePending body string : %s",
            msgAuctionSalePendingBody.str().c_str());

        MailDraft(
            msgAuctionSalePendingSubject.str(), msgAuctionSalePendingBody.str())
            .SendMailTo(MailReceiver(owner, owner_guid), auction,
                MAIL_CHECK_MASK_COPIED);
    }
}
...
void AuctionHouseMgr::SendAuctionSuccessfulMail(AuctionEntry* auction)
{
    ...
    if (owner || owner_accId)
    {
        ...
        msgAuctionSuccessfulSubject << auction->itemTemplate << ":"
                                    << auction->itemRandomPropertyId << ":"
                                    << AUCTION_SUCCESSFUL;
        ...
        LOG_DEBUG(logging, "AuctionSuccessful body string : %s",
        ...
        MailDraft(
            msgAuctionSuccessfulSubject.str(), auctionSuccessfulBody.str())
            .SetMoney(profit)
            .SendMailTo(MailReceiver(owner, owner_guid), auction,
                MAIL_CHECK_MASK_COPIED, HOUR);
    }
}
...
{ // return an item in auction to its owner by mail
    ...
    if (!pItem)
    {
        logging.error("Auction item (GUID: %u) not found, and lost.",
            auction->itemGuidLow);
        return;
    }
    ...
    if (owner || owner_accId)
    {
        ...
        subject << auction->itemTemplate << ":" << auction->itemRandomPropertyId
                << ":" << AUCTION_EXPIRED;
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    QueryResult* result = CharacterDatabase.Query(
        "SELECT data,itemguid,item_template FROM auction JOIN item_instance ON "
        "itemguid = guid");

    if (!result)
    {
        logging.info("Loaded 0 auction items\n");
        return;
    }
    ...
    do
    {
        ...
        {
            logging.error(
                "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u id: "
                "#%u) in auction, skipped.",
    ...
    logging.info("Loaded %u auction items\n", count);
}

void AuctionHouseMgr::LoadAuctions()
{
    QueryResult* result =
        CharacterDatabase.Query("SELECT COUNT(*) FROM auction");
    if (!result)
    {
        logging.info("Loaded 0 auctions. DB table `auction` is empty.\n");
        return;
    }
    ...
    result = CharacterDatabase.Query(
        "SELECT "
        "id,houseid,itemguid,item_template,item_count,item_randompropertyid,"
        "itemowner,buyoutprice,time,buyguid,lastbid,startbid,deposit FROM "
        "auction");
    if (!result)
    {
        logging.info("Loaded 0 auctions. DB table `auction` is empty.\n");
    ...
    do
    {
        ...
        if (auction->owner)
        {
            ...
            if (plWName.empty())
            {
                std::string plName;
                if (!sObjectMgr::Instance()->GetPlayerNameByGUID(
                        ObjectGuid(HIGHGUID_PLAYER, auction->owner), plName))
                    plName =
                        sObjectMgr::Instance()->GetMangosStringForDBCLocale(
                            LANG_UNKNOWN);
        ...
        // and item_template in fact (GetAItem will fail if problematic in
        // result check in AuctionHouseMgr::LoadAuctionItems)
        Item* pItem = GetAItem(auction->itemGuidLow);
        if (!pItem)
        {
            auction->DeleteFromDB();
            logging.error("Auction %u has not a existing item : %u, deleted",
                auction->Id, auction->itemGuidLow);
            delete auction;
            continue;
        }
            ...
            (auction->itemRandomPropertyId != pItem->GetItemRandomPropertyId()))
        {
            ...
            CharacterDatabase.PExecute(
                ...
                auction->itemTemplate, auction->itemCount,
        ...
        if (!auction->auctionHouseEntry)
        {
            ...
            if (auction->itemGuidLow)
            {
                ...
                MailDraft(msgAuctionCanceledOwner.str(), "") // TODO: fix body
                    .AddItem(pItem)
                    .SendMailTo(MailReceiver(ObjectGuid(
                                    HIGHGUID_PLAYER, auction->owner)),
    ...
    logging.info("Loaded %u auctions\n", AuctionCount);
}
...
uint32 AuctionHouseMgr::GetAuctionHouseTeam(AuctionHouseEntry const* house)
{
    ...
    // but player factions not have filled team field, and hard go from faction
    // value to faction_template value,
...
AuctionHouseEntry const* AuctionHouseMgr::GetAuctionHouseEntry(Unit* unit)
{
            ...
            CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
    {
        if (unit->GetTypeId() == TYPEID_UNIT)
        {
            ...
            // associated with auction house races.
            // but no easy way convert creature faction to player race faction
            // for specific city
        ...
        else
        {
            ...
            else
            {
                ...
                case HORDE:
                    ...
                    break;
                default:
                    break; // default value of houseid set above
                }
            }
        }
    }
...
{
    std::vector<AuctionEntry*> WonBids;
    ...
    for (auto itr = AuctionsMap.begin(); itr != AuctionsMap.end();)
    {
        if (curTime > itr->second->expireTime)
        {
            ...
            if (itr->second->bid)
                WonBids.push_back(itr->second);
            ///- cancel the auction if there was no bidder and clear the auction
            else
            {
                sAuctionMgr::Instance()->SendAuctionExpiredMail(itr->second);

                itr->second->DeleteFromDB();
                sAuctionMgr::Instance()->RemoveAItem(itr->second->itemGuidLow);
                delete itr->second;
    ...
    // NOTE: AuctionBidWinning() changes the AuctionsMap
    for (auto obj : WonBids)
        obj->AuctionBidWinning();
}

void AuctionHouseObject::BuildListBidderItems(
    WorldPacket& data, Player* player, uint32& count, uint32& totalcount)
{
...
void AuctionHouseObject::BuildListOwnerItems(
    WorldPacket& data, Player* player, uint32& count, uint32& totalcount)
{
    for (AuctionEntryMap::const_iterator itr = AuctionsMap.begin();
         itr != AuctionsMap.end(); ++itr)
...
size_t AuctionHouseObject::AuctionCount(uint32 low_guid) const
{
    size_t i = 0;
    for (const auto& elem : AuctionsMap)
    {
        if (elem.second->owner == low_guid)
            ++i;
    }
    return i;
}

int AuctionEntry::CompareAuctionEntry(
    uint32 column, const AuctionEntry* auc, Player* viewPlayer) const
{
    switch (column)
    {
    case 0: // level = 0
    {
        ItemPrototype const* itemProto1 =
            ObjectMgr::GetItemPrototype(itemTemplate);
        ItemPrototype const* itemProto2 =
            ObjectMgr::GetItemPrototype(auc->itemTemplate);
    ...
    case 5: // name = 5
    {
        ...
        sObjectMgr::Instance()->GetItemLocaleStrings(
            itemProto1->ItemId, loc_idx, &name1);

        std::string name2 = itemProto2->Name1;
        sObjectMgr::Instance()->GetItemLocaleStrings(
            itemProto2->ItemId, loc_idx, &name2);
...
bool AuctionSorter::operator()(
    const AuctionEntry* auc1, const AuctionEntry* auc2) const
{
    ...
    for (uint32 i = 0; i < MAX_AUCTION_SORT; ++i)
    {
        ...
        int res = auc1->CompareAuctionEntry(
            m_sort[i] & ~AUCTION_SORT_REVERSED, auc2, m_viewPlayer);
...
void WorldSession::BuildListAuctionItems(
    std::vector<AuctionEntry*> const& auctions, WorldPacket& data,
    std::wstring const& wsearchedname, uint32 listfrom, uint32 levelmin,
    uint32 levelmax, uint32 usable, uint32 inventoryType, uint32 itemClass,
    uint32 itemSubClass, uint32 quality, uint32& count, uint32& totalcount,
    bool isFull)
{
    ...
    {
        Item* item = sAuctionMgr::Instance()->GetAItem(Aentry->itemGuidLow);
        if (!item)
            continue;
        ...
        else
        {
            ...
            if (usable != 0x00 && _player->can_use_item(proto) != EQUIP_ERR_OK)
                continue;

            std::string name = proto->Name1;
            sObjectMgr::Instance()->GetItemLocaleStrings(
                proto->ItemId, loc_idx, &name);
...
AuctionEntry* AuctionHouseObject::AddAuction(
    AuctionHouseEntry const* auctionHouseEntry, Item* newItem, uint32 etime,
    uint32 bid, uint32 buyout, uint32 deposit, Player* pl /*= NULL*/)
{
    uint32 auction_time = uint32(
        etime * sWorld::Instance()->getConfig(CONFIG_FLOAT_RATE_AUCTION_TIME));

    if (pl)
    {
        // XXX: Mangos used to rely on this operation never failing because it
        // has been
        // checked previously. If that assumption is untrue we'd potentially
        // have a dupe here.
        // Preferably this would be redesigned to be less shit, but if a fail
        // condition, against
        // all odds, trigger we have no way of dealing with it, so we opt to
        // throw and end execution.
        inventory::transaction trans;
        trans.remove(newItem);
        if (!pl->storage().finalize(trans))
            throw std::runtime_error(
                "AuctionHouseObject::AddAuction: Transaction did not go "
                "through. This transaction should've been verified before we "
                "call this function.");
    }

    auto AH = new AuctionEntry;
    AH->Id = sObjectMgr::Instance()->GenerateAuctionID();
    ...
    AH->bid = 0;
    AH->buyout = buyout;
    AH->expireTime = WorldTimer::time_no_syscall() + auction_time;
    AH->deposit = deposit;
    AH->auctionHouseEntry = auctionHouseEntry;

    AddAuction(AH);

    sAuctionMgr::Instance()->AddAItem(newItem);
...
{
    Item* pItem = sAuctionMgr::Instance()->GetAItem(itemGuidLow);
    if (!pItem)
    {
        logging.error("auction to item, that doesn't exist !!!!");
        return false;
    }
    data << uint32(Id);
    ...
    data << uint32((expireTime - WorldTimer::time_no_syscall()) *
...
uint32 AuctionEntry::GetAuctionCut() const
{
    return uint32(auctionHouseEntry->cutPercent * bid *
                  sWorld::Instance()->getConfig(CONFIG_FLOAT_RATE_AUCTION_CUT) /
                  100.0f);
}
...
void AuctionEntry::SaveToDB() const
{
    ...
    CharacterDatabase.PExecute(
        ...
        "', '%u', '%u', '%u', '%u')",
        Id, auctionHouseEntry->houseId, itemGuidLow, itemTemplate, itemCount,
        itemRandomPropertyId, owner, buyout, (uint64)expireTime, bidder, bid,
        startbid, deposit);
}

void AuctionEntry::AuctionBidWinning(Player* newbidder)
{
    sAuctionMgr::Instance()->SendAuctionSalePendingMail(this);
    sAuctionMgr::Instance()->SendAuctionSuccessfulMail(this);
    sAuctionMgr::Instance()->SendAuctionWonMail(this);

    sAuctionMgr::Instance()->RemoveAItem(this->itemGuidLow);
    sAuctionMgr::Instance()
        ->GetAuctionsMap(this->auctionHouseEntry)
        ->RemoveAuction(this->Id);
...
{
    Player* auction_owner = owner ?
                                sObjectMgr::Instance()->GetPlayer(
                                    ObjectGuid(HIGHGUID_PLAYER, owner)) :
                                nullptr;
    ...
    if (newbidder && newbidder->GetGUIDLow() == bidder)
    {
        // XXX
        inventory::transaction trans;
        trans.remove(newbid - bid);
        newbidder->storage().finalize(trans);
    }
    else
    {
        if (newbidder)
    ...
    {
        if (auction_owner)
            auction_owner->GetSession()->SendAuctionOwnerNotification(
                this, false);
