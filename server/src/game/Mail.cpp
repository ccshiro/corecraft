...
MailReceiver::MailReceiver(Player* receiver)
  : m_receiver(receiver), m_receiver_guid(receiver->GetObjectGuid())
{
}
/**
...
MailDraft::MailDraft(std::string subject, std::string text)
  : m_mailTemplateId(0), m_mailTemplateItemsNeed(false),
    ...
    m_bodyId(!text.empty() ? sObjectMgr::Instance()->CreateItemText(text) : 0),
    m_money(0), m_COD(0)
{
}
...
bool MailDraft::prepareItems(Player* receiver)
{
    ...
    loot_recipient_mgr m(receiver);
    m.add_solo_tap(receiver);
    // can be empty
    mailLoot.fill_loot(m_mailTemplateId, LootTemplates_Mail, &m, true, true);

    uint32 max_slot = mailLoot.size();
    for (uint32 i = 0; m_items.size() < MAX_MAIL_ITEMS && i < max_slot; ++i)
    {
        if (LootItem* lootitem = mailLoot.get_slot_item(i))
        {
            if (!mailLoot.can_loot_item(lootitem, receiver->GetObjectGuid()))
                continue;

            if (Item* item = Item::CreateItem(
                    lootitem->itemid, lootitem->count, receiver))
            {
                item->db_save(); // save for prevent lost at next mail load, if
...
void MailDraft::CloneFrom(MailDraft const& draft)
{
    ...
    {
        std::string text = sObjectMgr::Instance()->GetItemText(bodyId);
        m_bodyId = sObjectMgr::Instance()->CreateItemText(text);
    }

    m_money = draft.GetMoney();
    m_COD = draft.GetCOD();

    for (const auto& elem : draft.m_items)
    {
        Item* item = elem.second;
 ...
 */
void MailDraft::SendReturnToSender(
    uint32 sender_acc, ObjectGuid sender_guid, ObjectGuid receiver_guid)
{
    Player* receiver = sObjectMgr::Instance()->GetPlayer(receiver_guid);

    uint32 rc_account = 0;
    if (!receiver)
        rc_account =
            sObjectMgr::Instance()->GetPlayerAccountIdByGUID(receiver_guid);
    ...
    if (!m_items.empty())
    {
        ...
        for (auto& elem : m_items)
        {
            ...
            // XXX:
            //
            item->db_save(); // item not in inventory and can be save standalone
    ...
    MailSender const& sender, MailCheckMask checked, uint32 deliver_delay)
{
    ...
    if (!pReceiver)
        pReceiverAccount = sObjectMgr::Instance()->GetPlayerAccountIdByGUID(
            receiver.GetPlayerGuid());
    ...
    uint32 mailId = sObjectMgr::Instance()->GenerateMailID();

    time_t deliver_time = WorldTimer::time_no_syscall() + deliver_delay;
    ...
    // mail from battlemaster (rewardmarks) should last only one day
             ...
             sBattleGroundMgr::Instance()->GetBattleMasterBG(
                 sender.GetSenderId()) != BATTLEGROUND_TYPE_NONE)
        expire_delay = DAY;
    ...
    CharacterDatabase.PExecute(
        ...
        receiver.GetPlayerGuid().GetCounter(), safe_subject.c_str(),
        GetBodyId(), (has_items ? 1 : 0), (uint64)expire_time,
    ...
    if (pReceiver)
    {
        ...
        {
            for (auto& elem : m_items)
                pReceiver->AddMItem(elem.second);
        }
    }
...
void Mail::prepareTemplateItems(Player* receiver)
{
    ...
    m.add_solo_tap(receiver);
    // can be empty
    mailLoot.fill_loot(mailTemplateId, LootTemplates_Mail, &m, true, true);

    CharacterDatabase.BeginTransaction();
    CharacterDatabase.PExecute(
        "UPDATE mail SET has_items = 1 WHERE id = %u", messageID);
        ...
        if (LootItem* lootitem = mailLoot.get_slot_item(i))
        {
                    ...
                    lootitem->itemid, lootitem->count, receiver))
            {
                ...
                CharacterDatabase.PExecute(
                    ...
                    "(mail_id,item_guid,item_template,receiver) VALUES ('%u', "
                    "'%u', '%u','%u')",
                    messageID, item->GetGUIDLow(), item->GetEntry(),
                    receiver->GetGUIDLow());
            }
        }
    }
