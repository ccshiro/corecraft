...
void WorldSession::HandleGroupInviteOpcode(WorldPacket& recv_data)
{
    ...
    Player* player =
        sObjectMgr::Instance()->GetPlayer(membername.c_str(), false);
...
void WorldSession::HandleGroupAcceptOpcode(WorldPacket& /*recv_data*/)
{
    ...
    {
        logging.error(
            "HandleGroupAcceptOpcode: %s tried to accept an invite to his own "
            "group",
    ...
    Player* leader = sObjectMgr::Instance()->GetPlayer(group->GetLeaderGuid());
...
void WorldSession::HandleGroupDeclineOpcode(WorldPacket& /*recv_data*/)
{
    ...
    data << GetPlayer()->GetName();
    leader->GetSession()->send_packet(std::move(data));
}
...
void WorldSession::HandleGroupUninviteGuidOpcode(WorldPacket& recv_data)
{
    ...
    {
        logging.error(
            "WorldSession::HandleGroupUninviteGuidOpcode: leader %s tried to "
            "uninvite himself from the group.",
            GetPlayer()->GetGuidStr().c_str());
        return;
    }
...
void WorldSession::HandleGroupUninviteOpcode(WorldPacket& recv_data)
{
    ...
    {
        logging.error(
            "WorldSession::HandleGroupUninviteOpcode: leader %s tried to "
            "uninvite himself from the group.",
            GetPlayer()->GetGuidStr().c_str());
        return;
    }
    ...
    {
        if (grp->RemoveMember(guid, 1) <= 1)
        {
            sObjectMgr::Instance()->RemoveGroup(grp);
            delete grp;
        }
        return;
    }
...
void WorldSession::HandleGroupSetLeaderOpcode(WorldPacket& recv_data)
{
    ...
    /** error handling **/
    if (!player || !group->IsLeader(GetPlayer()->GetObjectGuid()) ||
...
void WorldSession::HandleLootRoll(WorldPacket& recv_data)
{
    ...
    uint32 itemSlot;
    uint8 rollType;
    recv_data >> lootedTarget; // guid of the object that is looted
    recv_data >> itemSlot;
    recv_data >> rollType;
    ...
    if (itemSlot > std::numeric_limits<uint8>::max())
        return;

    WorldObject* obj = nullptr;
    obj = GetPlayer()->GetMap()->GetWorldObject(lootedTarget);
    if (!obj || !obj->GetLootDistributor())
        return;

    obj->GetLootDistributor()->attempt_place_roll(
        GetPlayer(), itemSlot, rollType);
}
...
void WorldSession::HandleMinimapPingOpcode(WorldPacket& recv_data)
{
    ...
    GetPlayer()->GetGroup()->BroadcastPacket(
        &data, true, -1, GetPlayer()->GetObjectGuid());
}
...
void WorldSession::HandleRandomRollOpcode(WorldPacket& recv_data)
{
    ...
    // LOG_DEBUG(logging,"ROLL: MIN: %u, MAX: %u, ROLL: %u", minimum, maximum,
    // roll);
...
void WorldSession::HandleGroupChangeSubGroupOpcode(WorldPacket& recv_data)
{
    ...
    // We get correct pointer here, no need to check if BG raid group
    Group* group = GetPlayer()->GetGroup();
    if (!group)
        return;
    ...
    {
        if (ObjectGuid guid =
                sObjectMgr::Instance()->GetPlayerGuidByName(name.c_str()))
...
void WorldSession::HandleGroupSwapSubGroupOpcode(WorldPacket& recv_data)
{
    ...
    if (!group->IsLeader(_player->GetObjectGuid()) &&
        !group->IsAssistant(_player->GetObjectGuid()))
        return;

    auto guid_one =
        sObjectMgr::Instance()->GetPlayerGuidByName(name_one.c_str());
    auto guid_two =
        sObjectMgr::Instance()->GetPlayerGuidByName(name_two.c_str());

    if (!guid_one || !guid_two)
        return;

    group->SwapMembers(guid_one, guid_two);
}
...
void WorldSession::HandlePartyAssignmentOpcode(WorldPacket& recv_data)
{
    ...
    LOG_DEBUG(logging, "MSG_PARTY_ASSIGNMENT");
...
void WorldSession::BuildPartyMemberStatsChangedPacket(
    Player* player, WorldPacket* data)
{
    ...
    *data << uint32(mask);
    ...
    if (mask & GROUP_UPDATE_FLAG_POSITION)
        *data << uint16(player->GetX()) << uint16(player->GetY());
...
void WorldSession::HandleRequestPartyMemberStatsOpcode(WorldPacket& recv_data)
{
    ...
    Pet* pet = player->IsInWorld() ? player->GetPet() : nullptr;

    WorldPacket data(
        SMSG_PARTY_MEMBER_STATS_FULL, 4 + 2 + 2 + 2 + 1 + 2 * 6 + 8 + 1 + 8);
    data << player->GetPackGUID();
    ...
    {
        WorldLocation& loc =
            player->GetTeleportDest(); // So take teleportation destination
        iZoneId = sTerrainMgr::Instance()->GetZoneId(
            loc.mapid, loc.coord_x, loc.coord_y, loc.coord_z);
        iCoordX = loc.coord_x;
        iCoordY = loc.coord_y;
    }
    else
    ...
    if (pet)
    {
        ...
        data << uint16(
            pet->GetPower(petpowertype)); // GROUP_UPDATE_FLAG_PET_CUR_POWER
        data << uint16(
            pet->GetMaxPower(petpowertype)); // GROUP_UPDATE_FLAG_PET_MAX_POWER
        ...
        data.put<uint64>(
            petMaskPos, petauramask); // GROUP_UPDATE_FLAG_PET_AURAS
    }
    else
    {
...
void WorldSession::HandleOptOutOfLootOpcode(WorldPacket& recv_data)
{
    ...
    if (!GetPlayer()) // needed because STATUS_AUTHED
    {
        if (unkn != 0)
            logging.error(
                "CMSG_GROUP_PASS_ON_LOOT value<>0 for not-loaded character!");
        return;
    }

    if (unkn != 0)
        logging.error("CMSG_GROUP_PASS_ON_LOOT: activation not implemented!");
}
