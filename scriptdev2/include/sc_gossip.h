...
inline bool ShowStable(Creature* c, Player* p)
{
    return c->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_STABLEMASTER) &&
           p->getClass() == CLASS_HUNTER;
}
...
// Coded
#define ADD_GOSSIP_ITEM(uiIcon, chrText, uiSender, uiOptionId) \
    PlayerTalkClass->GetGossipMenu().AddMenuItem(              \
        uiIcon, chrText, uiSender, uiOptionId, 0, "", 0)
#define ADD_GOSSIP_ITEM_ID(uiIcon, iTextId, uiSender, uiOptionId) \
    PlayerTalkClass->GetGossipMenu().AddMenuItem(                 \
        uiIcon, iTextId, uiSender, uiOptionId, 0, 0, 0)
#define ADD_GOSSIP_ITEM_EXTENDED(                                            \
    uiIcon, chrText, uiSender, uiOptionId, chrBoxMessage, uiBoxMoney, bCode) \
    PlayerTalkClass->GetGossipMenu().AddMenuItem(uiIcon, chrText, uiSender,  \
        uiOptionId, 0, chrBoxMessage, uiBoxMoney, bCode)

// This fuction Sends the current menu to show to client
// uiTextId - NPCTEXTID (uint32)
// guid - npc guid (ObjectGuid)
#define SEND_GOSSIP_MENU(uiTextId, guid) \
    PlayerTalkClass->SendGossipMenu(uiTextId, guid)
...
#define SEND_STABLELIST(a) GetSession()->SendStablePet(a)
