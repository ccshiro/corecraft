 ...
 */
void ScriptedInstance::DoUseDoorOrButton(
    ObjectGuid guid, uint32 uiWithRestoreTime, bool bUseAlternativeState)
{
    if (!guid)
        return;
    ...
    if (GameObject* pGo = instance->GetGameObject(guid))
    {
            ...
            pGo->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
        {
            ...
            logging.error(
                "SD2: Script call DoUseDoorOrButton, but gameobject entry %u "
                "is type %u.",
...
void ScriptedInstance::DoUseDoorOrButton(uint32 uiEntry,
    uint32 uiWithRestoreTime /*= 0*/, bool bUseAlternativeState /*= false*/)
{
    EntryGuidMap::iterator find = m_mGoEntryGuidStore.find(uiEntry);
    if (find != m_mGoEntryGuidStore.end())
        DoUseDoorOrButton(
            find->second, uiWithRestoreTime, bUseAlternativeState);
    else
        // Output log, possible reason is not added GO to storage, or not yet
        // loaded
        LOG_DEBUG(logging,
            "SD2: Script call DoUseDoorOrButton(by Entry), but no gameobject "
            "of entry %u was created yet, or it was not stored by script for "
            "map %u.",
            uiEntry, instance->GetId());
}
...
/**
   ...
   @param   uiTimeToDespawn (in seconds) Despawn the GO after this time, default
   is a minute
 */
void ScriptedInstance::DoRespawnGameObject(
    ObjectGuid guid, uint32 uiTimeToDespawn)
{
    if (!guid)
        return;
...
/// Function that uses a door or button that is stored in m_mGoEntryGuidStore
void ScriptedInstance::DoToggleGameObjectFlags(
    uint32 uiEntry, uint32 uiGOflags, bool bApply)
{
    EntryGuidMap::iterator find = m_mGoEntryGuidStore.find(uiEntry);
    if (find != m_mGoEntryGuidStore.end())
        ...
        // loaded
        LOG_DEBUG(logging,
            "SD2: Script call ToogleTameObjectFlags (by Entry), but no "
 ...
 */
void ScriptedInstance::DoToggleGameObjectFlags(
    ObjectGuid guid, uint32 uiGOflags, bool bApply)
{
    if (!guid)
        return;
...
void ScriptedInstance::DoRespawnGameObject(
    uint32 uiEntry, uint32 uiTimeToDespawn)
{
    EntryGuidMap::iterator find = m_mGoEntryGuidStore.find(uiEntry);
    if (find != m_mGoEntryGuidStore.end())
        ...
        // loaded;
        LOG_DEBUG(logging,
            "SD2: Script call DoRespawnGameObject(by Entry), but no gameobject "
            "of entry %u was created yet, or it was not stored by script for "
            "map %u.",
            uiEntry, instance->GetId());
}
...
void ScriptedInstance::DoUpdateWorldState(uint32 uiStateId, uint32 uiStateData)
{
    ...
    if (!lPlayers.isEmpty())
    {
        ...
        LOG_DEBUG(logging,
            "SD2: DoUpdateWorldState attempt send data but no players in map.");
}

/// Get the first found Player* (with requested properties) in the map. Can
/// return NULL.
Player* ScriptedInstance::GetPlayerInMap(
    bool bOnlyAlive /*=false*/, bool bCanBeGamemaster /*=true*/)
{
    Map::PlayerList const& lPlayers = instance->GetPlayers();
    ...
    for (const auto& lPlayer : lPlayers)
    {
        ...
        if (pPlayer && (!bOnlyAlive || pPlayer->isAlive()) &&
            (bCanBeGamemaster || !pPlayer->isGameMaster()))
            return pPlayer;
    }
...
std::list<Player*> ScriptedInstance::GetAllPlayersInMap(
    bool bOnlyAlive /*= false*/, bool bCanBeGamemaster /*= true*/)
{
    Map::PlayerList const& lPlayers = instance->GetPlayers();
...
/// m_mGoEntryGuidStore. Can return NULL
GameObject* ScriptedInstance::GetSingleGameObjectFromStorage(uint32 uiEntry)
{
    ...
    LOG_DEBUG(logging,
        "SD2: Script requested gameobject with entry %u, but no gameobject of "
        "this entry was created yet, or it was not stored by script for map "
        "%u.",
        uiEntry, instance->GetId());
...
// Make pCreature attack the nearest player in map
void ScriptedInstance::AttackNearestPlayer(Creature* pCreature)
{
    if (!pCreature || !pCreature->AI())
        return;

    std::list<Player*> players = GetAllPlayersInMap(true, false);
    ...
    if (players.size() > 0)
    {
        for (auto& player : players)
        {
            ...
            // Skip units with stealth & invisibility
            if (pTemp->HasAuraType(SPELL_AURA_MOD_STEALTH) ||
                pTemp->HasAuraType(SPELL_AURA_MOD_INVISIBILITY))
                continue;

            double dist = pCreature->GetDistance(pTemp);
            if (last_dist > dist)
            {
    ...
    if (nearestPlayer)
        pCreature->AI()->AttackStart(nearestPlayer);
}

/// Returns a pointer to a loaded Creature that was stored in
/// m_mGoEntryGuidStore. Can return NULL
Creature* ScriptedInstance::GetSingleCreatureFromStorage(
    uint32 uiEntry, bool bSkipDebugLog /*=false*/)
{
    EntryGuidMap::iterator find = m_mNpcEntryGuidStore.find(uiEntry);
    if (find != m_mNpcEntryGuidStore.end())
        return instance->GetAnyTypeCreature(find->second);

    // Output log, possible reason is not added GO to map, or not yet loaded;
    if (!bSkipDebugLog)
        LOG_DEBUG(logging,
            "SD2: Script requested creature with entry %u, but no npc of this "
...
DialogueHelper::DialogueHelper(DialogueEntry const* pDialogueArray)
  : m_pInstance(nullptr), m_pDialogueArray(pDialogueArray),
    m_pCurrentEntry(nullptr), m_pDialogueTwoSideArray(nullptr),
    m_pCurrentEntryTwoSide(nullptr), m_uiTimer(0), m_bIsFirstSide(true),
...
/**
   ...
   @param   pDialogueTwoSideArray The static const array of DialogueEntryTwoSide
   holding the information about the dialogue. This array MUST be terminated by
   {0,0,0,0,0}
 */
DialogueHelper::DialogueHelper(
    DialogueEntryTwoSide const* pDialogueTwoSideArray)
  : m_pInstance(nullptr), m_pDialogueArray(nullptr), m_pCurrentEntry(nullptr),
...
void DialogueHelper::StartNextDialogueText(int32 iTextEntry)
{
    ...
    {
        for (DialogueEntry const* pEntry = m_pDialogueArray; pEntry->iTextEntry;
             ++pEntry)
        {
    ...
    {
        for (DialogueEntryTwoSide const* pEntry = m_pDialogueTwoSideArray;
             pEntry->iTextEntry; ++pEntry)
        {
            if (pEntry->iTextEntry == iTextEntry)
            {
    ...
    {
        logging.error(
            "SD2: Script call DialogueHelper::StartNextDialogueText, but "
            "textEntry %i is not in provided dialogue (on map id %u)",
            iTextEntry, m_pInstance ? m_pInstance->instance->GetId() : 0);
        return;
    }
...
void DialogueHelper::DoNextDialogueStep()
{
    ...
    if ((m_pCurrentEntry && !m_pCurrentEntry->iTextEntry) ||
        (m_pCurrentEntryTwoSide && !m_pCurrentEntryTwoSide->iTextEntry))
    {
        m_uiTimer = 0;
        return;
    }
    ...
    {
        // Second Entries can be 0, if they are the entry from first side will
        // be taken
        uiSpeakerEntry =
            !m_bIsFirstSide && m_pCurrentEntryTwoSide->uiSayerEntryAlt ?
                m_pCurrentEntryTwoSide->uiSayerEntryAlt :
                m_pCurrentEntryTwoSide->uiSayerEntry;
        iTextEntry = !m_bIsFirstSide && m_pCurrentEntryTwoSide->iTextEntryAlt ?
                         m_pCurrentEntryTwoSide->iTextEntryAlt :
                         m_pCurrentEntryTwoSide->iTextEntry;
    ...
    if (uiSpeakerEntry && iTextEntry < 0)
    {
        ...
        if (m_pInstance && !pSpeaker) // Get Speaker from instance
        {
            if (m_bCanSimulate) // Simulate case
                m_pInstance->DoOrSimulateScriptTextForThisInstance(
                    iTextEntry, uiSpeakerEntry);
            else
                pSpeaker =
                    m_pInstance->GetSingleCreatureFromStorage(uiSpeakerEntry);
        }
    ...
    JustDidDialogueStep(m_pDialogueArray ? m_pCurrentEntry->iTextEntry :
                                           m_pCurrentEntryTwoSide->iTextEntry);
