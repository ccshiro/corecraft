...
guardAI::guardAI(Creature* pCreature)
  : ScriptedAI(pCreature), m_uiGlobalCooldown(0), m_uiBuffTimer(0)
{
}
...
void guardAI::UpdateAI(const uint32 uiDiff)
{
    ...
    if (m_creature->isAlive() && !m_creature->isInCombat())
    {
        ...
        {
            // Find a spell that targets friendly and applies an aura (these are
            // generally buffs)
            const SpellEntry* pSpellInfo = SelectSpell(m_creature, -1, -1,
        ...
        !m_creature->IsNonMeleeSpellCasted(false))
    {
        ...
        if (m_creature->CanReachWithMeleeAttack(m_creature->getVictim()))
        {
            ...
            if (m_creature->GetHealthPercent() < 30.0f)
                pSpellInfo =
                    SelectSpell(m_creature, -1, -1, SELECT_TARGET_ANY_FRIEND, 0,
                        0, 0, 0, SELECT_EFFECT_HEALING);

            // No healing spell available, select a hostile spell
            if (pSpellInfo)
                bHealing = true;
            else
                pSpellInfo = SelectSpell(m_creature->getVictim(), -1, -1,
