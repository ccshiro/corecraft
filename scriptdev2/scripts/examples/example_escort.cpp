...
struct MANGOS_DLL_DECL example_escortAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiWP) override
    {
        ...
        case 3:
            ...
            m_creature->SummonCreature(NPC_FELBOAR, m_creature->GetX() + 5.0f,
                m_creature->GetY() + 7.0f, m_creature->GetZ(), 0.0f,
    ...
    void JustDied(Unit* pKiller) override
    {
        ...
        // Fail quest for group - if you don't implement JustDied in your
        // script, this will automatically work
    ...
    void UpdateEscortAI(const uint32 uiDiff) override
    {
        ...
        if (m_creature->SelectHostileTarget() && m_creature->getVictim())
        {
            if (m_uiDeathCoilTimer < uiDiff)
            {
                ...
                m_creature->CastSpell(
                    m_creature->getVictim(), SPELL_DEATH_COIL, false);
                m_uiDeathCoilTimer = 4000;
            }
            else
        ...
        else
        {
            ...
            if (HasEscortState(STATE_ESCORT_ESCORTING))
            {
                ...
                {
                    if (m_creature->has_aura(SPELL_ELIXIR_OF_FORTITUDE))
                    {
                        DoScriptText(SAY_RAND_1, m_creature);
                        m_creature->CastSpell(
                            m_creature, SPELL_BLUE_FIREWORK, false);
                    }
                    else
                    {
                        DoScriptText(SAY_RAND_2, m_creature);
                        m_creature->CastSpell(
                            m_creature, SPELL_ELIXIR_OF_FORTITUDE, false);
                    }
...
bool GossipHello_example_escort(Player* pPlayer, Creature* pCreature)
{
    ...
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_2,
        GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_3,
...
bool GossipSelect_example_escort(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    example_escortAI* pEscortAI =
        dynamic_cast<example_escortAI*>(pCreature->AI());
