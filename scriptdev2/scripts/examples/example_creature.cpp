...
// **** Please modify whatever you'd like to as this script is only for
// developement ****

// **** Script Info ****
// This script is written in a way that it can be used for both friendly and
// hostile monsters
...
// **** Quick Info ****
// Functions with Handled Function marked above them are functions that are
// called automatically by the core
// Functions that are marked Custom Function are functions I've created to
// simplify code

enum
{
    // List of text id's. The text is stored in database, also in a localized
    // version
    // (if translation not exist for the textId, default english text will be
    // used)
    SAY_AGGRO = -1999900,
...
// If (and only if) a gossip must be handled within SD2, then it should be moved
// to SD2-database!
...
{
    // *** HANDLED FUNCTION ***
    // This is the constructor, called only once when the creature is first
    // created
    ...
    // *** HANDLED FUNCTION ***
    // Aggro is called when we enter combat, against an enemy, and haven't been
    // in combat before
    ...
    // *** HANDLED FUNCTION ***
    // Update AI is called Every single map update (roughly once every 100ms if
    // a player is within the grid)
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (!m_creature->getVictim())
        {
            ...
            if (m_uiSayTimer < uiDiff)
            {
                ...
                m_uiSayTimer =
                    45 * IN_MILLISECONDS; // Say something agian in 45 seconds
            }
            else
                m_uiSayTimer -= uiDiff;
        ...
        {
            // Cast spell two on self (AoE spell with only self-target) if we
            // can
            if (DoCastSpellIfCan(m_creature, SPELL_TWO) == CAST_OK)
                m_uiSpellTwoTimer = 37 * IN_MILLISECONDS; // Only Update Timer,
                                                          // if we could start
                                                          // casting
        }
        else
        ...
        if (m_uiPhase == 1)
        {
            ...
            {
                // Only switch phase and display phase-switich text, if out cast
                // was started sucessfull
        ...
        else if (m_uiPhase > 1)
        {
            ...
            if (m_uiBeserkTimer < uiDiff)
            {
                ...
                if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_BESERK) ==
                    CAST_OK)
                {
                    // Say our line if we cast
                    DoScriptText(
                        SAY_BESERK, m_creature, m_creature->getVictim());

                    // Cast our beserk spell agian in 12 seconds (if we didn't
                    // kill everyone)
...
// In this case as there is nothing special about this gossip dialogue, it
// should be moved to world-DB
...
// In this case here the faction change could be handled by world-DB gossip,
// hence it should be handled there!
bool GossipSelect_example_creature(
