...
SD%Complete: 70
SDComment: Undead side 90% implemented, event needs better implementation,
Barthildas relocation for reload case is missing, Baron Combat handling is
buggy.
...
instance_stratholme::instance_stratholme(Map* pMap)
  : ScriptedInstance(pMap), m_uiBaronRunTimer(0), m_uiBarthilasRunTimer(0),
    m_uiMindlessSummonTimer(0), m_uiSlaugtherSquareTimer(0), m_uiYellCounter(0),
    m_uiMindlessCount(0), m_uiPostboxesUsed(0), m_uiSilverHandKilled(0)
{
    Initialize();
}
...
void instance_stratholme::OnCreatureCreate(Creature* pCreature)
{
    ...
    case NPC_CRIMSON_CONJURER:
        ...
        if (pCreature->IsWithinDist2d(
                aTimmyLocation[1].m_fX, aTimmyLocation[1].m_fY, 40.0f))
...
void instance_stratholme::OnObjectCreate(GameObject* pGo)
{
    ...
    case GO_GAUNTLET_GATE1:
        // TODO
        // weird, but unless flag is set, client will not respond as expected.
        // DB bug?
...
void instance_stratholme::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_BARON_RUN:
        ...
        case IN_PROGRESS:
            if (m_auiEncounter[uiType] == IN_PROGRESS ||
                m_auiEncounter[uiType] == FAIL)
                break;

            DoOrSimulateScriptTextForThisInstance(
                SAY_ANNOUNCE_RUN_START, NPC_BARON);

            m_uiBaronRunTimer = 45 * MINUTE * IN_MILLISECONDS;
            LOG_DEBUG(
                logging, "SD2: Instance Stratholme: Baron run in progress.");
            break;
        case FAIL:
            // may add code to remove aura from players, but in theory the time
            // should be up already and removed.
            break;
        case DONE:
            m_uiBaronRunTimer = 0;
            break;
    ...
    case TYPE_PALLID:
        ...
        if (uiData == DONE)
        {
            DoSortZiggurats();
            DoUseDoorOrButton(
                m_zigguratStorage[uiType - TYPE_BARONESS].m_doorGuid);
        }
        if (uiData == SPECIAL)
    ...
    case TYPE_RAMSTEIN:
        if (uiData == SPECIAL)
        {
            if (m_auiEncounter[uiType] != SPECIAL &&
                m_auiEncounter[uiType] != DONE)
            {
                m_uiSlaugtherSquareTimer = 20000; // TODO - unknown, also
                                                  // possible that this is not
                                                  // the very correct place..
                DoUseDoorOrButton(GO_PORT_GAUNTLET);
            }

            uint32 uiCount = m_sAbomnationGUID.size();
            for (GUIDSet::iterator itr = m_sAbomnationGUID.begin();
                 itr != m_sAbomnationGUID.end();)
            {
            ...
            if (!uiCount)
            {
                // Old Comment: a bit itchy, it should close GO_ZIGGURAT_DOOR_4
                // door after 10 secs, but it doesn't. skipping it for now.
                ...
                if (Creature* pBaron = GetSingleCreatureFromStorage(NPC_BARON))
                {
                    ...
                    if (Creature* pRamstein = pBaron->SummonCreature(
                            NPC_RAMSTEIN, aStratholmeLocation[2].m_fX,
                            aStratholmeLocation[2].m_fY,
                            aStratholmeLocation[2].m_fZ,
                            aStratholmeLocation[2].m_fO,
                            TEMPSUMMON_DEAD_DESPAWN, 0))
                        pRamstein->movement_gens.push(
                            new movement::PointMovementGenerator(0,
                                aStratholmeLocation[3].m_fX,
                                aStratholmeLocation[3].m_fY,
                                aStratholmeLocation[3].m_fZ, true, false));

                    LOG_DEBUG(logging,
                        "SD2: Instance Stratholme - Slaugther event: Ramstein "
                        "spawned.");
                }
            }
            else
                LOG_DEBUG(logging,
                    "SD2: Instance Stratholme - Slaugther event: %u Abomnation "
                    "left to kill.",
                    uiCount);
        }
        // After fail aggroing Ramstein means wipe on Ramstein, so close door
        // again
        ...
        if (uiData == DONE)
        {
            ...
            if (Creature* pBaron = GetSingleCreatureFromStorage(NPC_BARON))
            {
                for (uint8 i = 0; i < 5; ++i)
                {
                    if (Creature* pTemp = pBaron->SummonCreature(
                            NPC_BLACK_GUARD, aStratholmeLocation[6].m_fX,
                            aStratholmeLocation[6].m_fY,
                            aStratholmeLocation[6].m_fZ,
                            aStratholmeLocation[6].m_fO,
                            TEMPSUMMON_DEAD_DESPAWN, 0))
                        m_luiGuardGUIDs.push_back(pTemp->GetObjectGuid());
                }

                LOG_DEBUG(logging,
                    "SD2: Instance Stratholme - Slaugther event: Summoned 5 "
                    "guards.");
            }
        }
        ...
        if (uiData == FAIL && m_auiEncounter[uiType] != FAIL)
        {
            ...
            for (const auto& elem : m_sAbomnationGUID)
            {
                Creature* pAbom = instance->GetCreature(elem);
                if (pAbom)
                    pAbom->movement_gens.remove_all(movement::gen::point);
            }
        }
        ...
        if (uiData == DONE)
        {
            if (m_auiEncounter[TYPE_BARON_RUN] == DONE)
            {
                ...
                for (const auto& player : players)
                {
                    ...
                    {
                        if (pPlayer->has_aura(SPELL_BARON_ULTIMATUM))
                            pPlayer->remove_auras(SPELL_BARON_ULTIMATUM);

                        if (pPlayer->GetQuestStatus(QUEST_DEAD_MAN_PLEA) ==
                ...
                if (Creature* pYsidaT =
                        GetSingleCreatureFromStorage(NPC_YSIDA_TRIGGER))
                {
                    if (Creature* pYsida = pYsidaT->SummonCreature(NPC_YSIDA,
                            pYsidaT->GetX(), pYsidaT->GetY(), pYsidaT->GetZ(),
                            pYsidaT->GetO(), TEMPSUMMON_TIMED_DESPAWN, 1800000))
                    {
                        DoScriptText(SAY_EPILOGUE, pYsida);
                        pYsida->movement_gens.push(
                            new movement::PointMovementGenerator(0,
                                aStratholmeLocation[7].m_fX,
                                aStratholmeLocation[7].m_fY,
                                aStratholmeLocation[7].m_fZ, true, false));
                    }
    ...
    case TYPE_BARTHILAS_RUN:
        if (uiData == IN_PROGRESS)
        {
            ...
            if (pBarthilas && pBarthilas->isAlive() &&
                !pBarthilas->isInCombat())
            {
                DoScriptText(SAY_WARN_BARON, pBarthilas);
                pBarthilas->movement_gens.push(
                    new movement::PointMovementGenerator(0,
                        aStratholmeLocation[0].m_fX,
                        aStratholmeLocation[0].m_fY,
                        aStratholmeLocation[0].m_fZ, true, true));
    ...
    case TYPE_TRUE_MASTERS:
        ...
        if (uiData == SPECIAL)
        {
            ...
            // When the 5th paladin is killed set data to DONE in order to give
            // the quest credit for the last paladin
...
void instance_stratholme::Load(const char* chrIn)
{
    ...
    // Special Treatment for the Ziggurat-Bosses, as otherwise the event
    // couldn't reload
    if (m_auiEncounter[TYPE_BARONESS] == DONE)
        m_auiEncounter[TYPE_BARONESS] = SPECIAL;
    if (m_auiEncounter[TYPE_NERUB] == DONE)
...
{
    return pFirst && pSecond && pFirst->GetZ() > pSecond->GetZ();
}
...
void instance_stratholme::DoSortZiggurats()
{
    ...
    for (GUIDList::const_iterator itr = m_luiAcolyteGUIDs.begin();
         itr != m_luiAcolyteGUIDs.end(); ++itr)
    {
    ...
    {
        // Sort the acolytes by height, and the one with the biggest height is
        // the announcer (a bit outside the map)
        lAcolytes.sort(sortByHeight);
        m_mNpcEntryGuidStore[NPC_THUZADIN_ACOLYTE] =
            (*lAcolytes.begin())->GetObjectGuid();
        lAcolytes.erase(lAcolytes.begin());
    }

    // Sort Acolytes
    for (std::list<Creature*>::iterator itr = lAcolytes.begin();
         itr != lAcolytes.end();)
    {
        bool bAlreadyIterated = false;
        for (auto& elem : m_zigguratStorage)
        {
            if (GameObject* pZigguratDoor =
                    instance->GetGameObject(elem.m_doorGuid))
            {
                if ((*itr)->isAlive() &&
                    (*itr)->IsWithinDistInMap(pZigguratDoor, 35.0f, false))
                {
                    elem.m_lZigguratAcolyteGuid.push_back(
                        (*itr)->GetObjectGuid());
    ...
    // In case some mobs have not been able to be sorted, store their GUIDs
    // again
    for (std::list<Creature*>::const_iterator itr = lAcolytes.begin();
         itr != lAcolytes.end(); ++itr)
        m_luiAcolyteGUIDs.push_back((*itr)->GetObjectGuid());

    // Sort Crystal
    for (GUIDList::iterator itr = m_luiCrystalGUIDs.begin();
         itr != m_luiCrystalGUIDs.end();)
    {
        ...
        for (auto& elem : m_zigguratStorage)
        {
                    ...
                    instance->GetGameObject(elem.m_doorGuid))
            {
                ...
                {
                    elem.m_crystalGuid = pCrystal->GetObjectGuid();
                    itr = m_luiCrystalGUIDs.erase(itr);
                    bAlreadyIterated = true;
                    break;
                }
...
void instance_stratholme::OnCreatureEnterCombat(Creature* pCreature)
{
    ...
    case NPC_RAMSTEIN:
        ...
        break;
    // TODO - uncomment when proper working within core! case NPC_BARON:
    // SetData(TYPE_BARON, IN_PROGRESS);    break;
...
void instance_stratholme::OnCreatureDeath(Creature* pCreature)
{
    ...
    case NPC_MINDLESS_UNDEAD:
        ...
        if (m_luiUndeadGUIDs.empty())
        {
            ...
            for (GUIDList::const_iterator itr = m_luiGuardGUIDs.begin();
                 itr != m_luiGuardGUIDs.end(); ++itr)
            {
                Creature* pGuard = instance->GetCreature(*itr);
                if (pGuard && pGuard->isAlive() && !pGuard->isInCombat())
                {
                    auto pos = pGuard->GetPointXYZ(
                        G3D::Vector3(aStratholmeLocation[5].m_fX,
                            aStratholmeLocation[5].m_fY,
                            aStratholmeLocation[5].m_fZ),
                        2 * M_PI_F * rand_norm_f(), 10.0f * rand_norm_f());
                    pGuard->movement_gens.push(
                        new movement::PointMovementGenerator(
                            0, pos.x, pos.y, pos.z, true, false));
                }
            }
        }
        break;
    ...
    case NPC_CRIMSON_GUARDSMAN:
    case NPC_CRIMSON_CONJURER:
        if (m_suiCrimsonLowGuids.find(pCreature->GetGUIDLow()) !=
            m_suiCrimsonLowGuids.end())
        {
            ...
            if (m_suiCrimsonLowGuids.empty())
                pCreature->SummonCreature(NPC_TIMMY_THE_CRUEL,
                    aTimmyLocation[0].m_fX, aTimmyLocation[0].m_fY,
                    aTimmyLocation[0].m_fZ, aTimmyLocation[0].m_fO,
                    TEMPSUMMON_DEAD_DESPAWN, 0);
        }
        break;
    }
...
void instance_stratholme::ThazudinAcolyteJustDied(Creature* pCreature)
{
    for (uint8 i = 0; i < MAX_ZIGGURATS; ++i)
    {
        ...
        m_zigguratStorage[i].m_lZigguratAcolyteGuid.remove(
            pCreature->GetObjectGuid());
        if (m_zigguratStorage[i].m_lZigguratAcolyteGuid.empty())
        {
            // A random zone yell after one is cleared
            int32 aAnnounceSay[MAX_ZIGGURATS] = {SAY_ANNOUNCE_ZIGGURAT_1,
                SAY_ANNOUNCE_ZIGGURAT_2, SAY_ANNOUNCE_ZIGGURAT_3};
            DoOrSimulateScriptTextForThisInstance(
                aAnnounceSay[i], NPC_THUZADIN_ACOLYTE);

            // Kill Crystal
            if (Creature* pCrystal =
                    instance->GetCreature(m_zigguratStorage[i].m_crystalGuid))
                pCrystal->DealDamage(pCrystal, pCrystal->GetHealth(), NULL,
...
void instance_stratholme::Update(uint32 uiDiff)
{
    if (m_uiBarthilasRunTimer)
    {
        if (m_uiBarthilasRunTimer <= uiDiff)
        {
            ...
            if (pBarthilas && pBarthilas->isAlive() &&
                !pBarthilas->isInCombat())
                pBarthilas->NearTeleportTo(aStratholmeLocation[1].m_fX,
                    aStratholmeLocation[1].m_fY, aStratholmeLocation[1].m_fZ,
                    aStratholmeLocation[1].m_fO);
    ...
    {
        if (m_uiYellCounter == 0 &&
            m_uiBaronRunTimer <= 10 * MINUTE * IN_MILLISECONDS)
        {
            DoOrSimulateScriptTextForThisInstance(
                SAY_ANNOUNCE_RUN_10_MIN, NPC_BARON);
            ++m_uiYellCounter;
        }
        else if (m_uiYellCounter == 1 &&
                 m_uiBaronRunTimer <= 5 * MINUTE * IN_MILLISECONDS)
        {
            DoOrSimulateScriptTextForThisInstance(
                SAY_ANNOUNCE_RUN_5_MIN, NPC_BARON);
            ++m_uiYellCounter;
        }
        ...
        if (m_uiBaronRunTimer <= uiDiff)
        {
            ...
            DoOrSimulateScriptTextForThisInstance(
                SAY_ANNOUNCE_RUN_FAIL, NPC_BARON);

            m_uiBaronRunTimer = 0;
            LOG_DEBUG(logging,
                "SD2: Instance Stratholme: Baron run event reached end. Event "
                "has state %u.",
    ...
    if (m_uiMindlessSummonTimer)
    {
        if (m_uiMindlessCount < 30)
        {
            if (m_uiMindlessSummonTimer <= uiDiff)
            {
                if (Creature* pBaron = GetSingleCreatureFromStorage(NPC_BARON))
                {
                    // Summon mindless skeletons and move them to random point
                    // in the center of the square
                    if (Creature* pTemp = pBaron->SummonCreature(
                            NPC_MINDLESS_UNDEAD, aStratholmeLocation[4].m_fX,
                            aStratholmeLocation[4].m_fY,
                            aStratholmeLocation[4].m_fZ,
                            aStratholmeLocation[4].m_fO,
                            TEMPSUMMON_DEAD_DESPAWN, 0))
                    {
                        auto pos = pBaron->GetPointXYZ(
                            G3D::Vector3(aStratholmeLocation[5].m_fX,
                                aStratholmeLocation[5].m_fY,
                                aStratholmeLocation[5].m_fZ),
                            2 * M_PI_F * rand_norm_f(), 20.0f * rand_norm_f());
                        pTemp->movement_gens.push(
                            new movement::PointMovementGenerator(
                                0, pos.x, pos.y, pos.z, true, false));
        ...
        if (m_uiSlaugtherSquareTimer <= uiDiff)
        {
            ...
            for (const auto& elem : m_sAbomnationGUID)
            {
                ...
                if (!pAbom || !pAbom->isAlive() ||
                    pAbom->movement_gens.has(movement::gen::point))
                    continue;
                ...
                {
                    if (GameObject* pDoor =
                            GetSingleGameObjectFromStorage(GO_PORT_SLAUGTHER))
                    {
                        auto pos = pAbom->GetPointXYZ(
                            G3D::Vector3(
                                pDoor->GetX(), pDoor->GetY(), pDoor->GetZ()),
                            2 * M_PI_F * rand_norm_f(), 10.0f * rand_norm_f());
                        pAbom->movement_gens.push(
                            new movement::PointMovementGenerator(
                                0, pos.x, pos.y, pos.z, true, false));
                    }
                }
