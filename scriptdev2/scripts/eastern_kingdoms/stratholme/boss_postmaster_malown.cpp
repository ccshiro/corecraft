...
// He should be spawned along with three other elites once the third postbox has
// been opened
...
struct MANGOS_DLL_DECL boss_postmaster_malownAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        {
            // Cast
            if (rand_norm_f() < 0.65f) // 65% chance to cast
            {
        ...
        {
            // Cast
            if (rand_norm_f() < 0.45f) // 45% chance to cast
            {
        ...
        {
            // Cast
            if (rand_norm_f() < 0.03f) // 3% chance to cast
            {
                DoCastSpellIfCan(
                    m_creature->getVictim(), SPELL_CURSEOFWEAKNESS);
            }
        ...
        {
            // Cast
            if (rand_norm_f() < 0.05f) // 5% chance to cast
            {
