...
SD%Complete: 80
SDComment: Timers; Not sure if we need to respawn dead npcs on evade; May need
additional adjustments / research
...
/*#####
# Additional:
# Although this is a working solution, the correct would be in addition to check
if Aurius is dead.
# Once player extinguish the eternal flame (cast spell 31497->start event 11206)
Aurius should become hostile.
...
static SilverHandAbilityStruct m_aSilverHandAbility[8] = {
    {NPC_GREGOR_THE_JUSTICIAR, SPELL_HAMMER_JUSTICE, TARGET_TYPE_RANDOM, 2000,
     15000},
    {NPC_GREGOR_THE_JUSTICIAR, SPELL_HAMMER_WRATH, TARGET_TYPE_RANDOM, 10000,
     15000},
    {NPC_GREGOR_THE_JUSTICIAR, SPELL_HOLY_SHOCK, TARGET_TYPE_RANDOM, 4000,
     7000},
    ...
    {NPC_AELMAR_THE_VANQUISHER, SPELL_JUDGEMENT, TARGET_TYPE_VICTIM, 4000,
     9000},
...
struct MANGOS_DLL_DECL boss_silver_hand_bossesAI : public ScriptedAI
{
    boss_silver_hand_bossesAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        {
            if (m_aSilverHandAbility[i].m_uiCreatureEntry ==
                m_creature->GetEntry())
    ...
    void Reset() override
    {
        for (auto& elem : m_mSpellTimers)
            elem.second = m_aSilverHandAbility[elem.first].m_uiInitialTimer;
    }
                ...
                if (pKiller->GetTypeId() == TYPEID_PLAYER)
                {
                    if (Creature* pCredit =
                            m_pInstance->GetSingleCreatureFromStorage(
                                NPC_PALADIN_QUEST_CREDIT))
                        ((Player*)pKiller)
                            ->KilledMonsterCredit(NPC_PALADIN_QUEST_CREDIT,
                                pCredit->GetObjectGuid());
                }
            }
        }
    }
    ...
    bool CanUseSpecialAbility(uint32 uiIndex)
    {
        ...
        case TARGET_TYPE_RANDOM:
            pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM,
                0, m_aSilverHandAbility[uiIndex].m_uiSpellId,
                SELECT_FLAG_IN_LOS);
            break;
        ...
        if (pTarget)
        {
            if (DoCastSpellIfCan(pTarget,
                    m_aSilverHandAbility[uiIndex].m_uiSpellId) == CAST_OK)
                return true;
        }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        for (auto& elem : m_mSpellTimers)
        {
            if (elem.second < uiDiff)
            {
                ...
                {
                    elem.second = m_aSilverHandAbility[elem.first].m_uiCooldown;
                    break;
                }
            }
            else
