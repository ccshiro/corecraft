...
enum
{
    ...
    SPELL_BALNAZZARTRANSFORM =
        17288, // restore full HP/mana, trigger spell Balnazzar Transform Stun
...
struct MANGOS_DLL_DECL boss_dathrohan_balnazzarAI : public ScriptedAI
{
    ...
    void JustDied(Unit* /*Victim*/) override
    {
        ...
        for (uint32 i = 0; i < uiCount; ++i)
            m_creature->SummonCreature(m_aSummonPoint[i].m_uiEntry,
                m_aSummonPoint[i].m_fX, m_aSummonPoint[i].m_fY,
                m_aSummonPoint[i].m_fZ, m_aSummonPoint[i].m_fOrient,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (!m_bTransformed)
        {
            ...
            {
                DoCastSpellIfCan(
                    m_creature->getVictim(), SPELL_CRUSADERSHAMMER);
            ...
            if (m_creature->GetHealthPercent() < 40.0f)
            {
                ...
                if (DoCastSpellIfCan(m_creature, SPELL_BALNAZZARTRANSFORM) ==
                    CAST_OK)
                {
