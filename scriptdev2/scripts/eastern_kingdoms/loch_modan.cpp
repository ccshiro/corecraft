...
SDName: Loch_Modan
SD%Complete: 100
SDComment: Quest support: 3181 (only to argue with pebblebitty to get to searing
gorge, before quest rewarded), 309
...
bool GossipHello_npc_mountaineer_pebblebitty(
    Player* pPlayer, Creature* pCreature)
{
    ...
    if (!pPlayer->GetQuestRewardStatus(3181) == 1)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
            "Open the gate please, i need to get to Searing Gorge",
...
bool GossipSelect_npc_mountaineer_pebblebitty(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    ...
    case GOSSIP_ACTION_INFO_DEF + 5:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
            "Ok, i'll try to remember that.", GOSSIP_SENDER_MAIN,
            GOSSIP_ACTION_INFO_DEF + 6);
        pPlayer->SEND_GOSSIP_MENU(1837, pCreature->GetObjectGuid());
        break;
    case GOSSIP_ACTION_INFO_DEF + 6:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A key? Ok!",
...
struct MANGOS_DLL_DECL npc_miranAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 19:
            ...
            m_creature->SummonCreature(NPC_DARK_IRON_DWARF,
                m_afAmbushSpawn[0].m_fX, m_afAmbushSpawn[0].m_fY,
                m_afAmbushSpawn[0].m_fZ, m_afAmbushSpawn[0].m_fO,
                TEMPSUMMON_CORPSE_TIMED_DESPAWN, 25000);
            m_creature->SummonCreature(NPC_DARK_IRON_DWARF,
                m_afAmbushSpawn[1].m_fX, m_afAmbushSpawn[1].m_fY,
                m_afAmbushSpawn[1].m_fZ, m_afAmbushSpawn[1].m_fO,
                TEMPSUMMON_CORPSE_TIMED_DESPAWN, 25000);
            break;
        case 23:
            ...
            if (Player* pPlayer = GetPlayerForEscort())
                pPlayer->GroupEventHappens(
                    QUEST_PROTECTING_THE_SHIPMENT, m_creature);
            break;
        }
    }
