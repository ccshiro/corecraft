...
enum
{
    ...
    SPELL_MANA_RAGE = 44320, // This spell triggers 44321, which changes scale
                             // and regens mana Requires an entry in
                             // spell_script_target
...
struct MANGOS_DLL_DECL boss_selin_fireheartAI : public ScriptedAI
{
    ...
    ObjectGuid m_crystalGuid; // This will help us create a pointer to the
                              // crystal we are draining. We store GUIDs, never
                              // units in case unit is deleted/offline (offline
                              // if player of course).
    ...
    void SelectNearestCrystal()
    {
        ...
        for (GUIDList::const_iterator itr = m_lCrystalGuids.begin();
             itr != m_lCrystalGuids.end(); ++itr)
        {
            ...
            if (pCrystal && pCrystal->isAlive())
            {
                // select nearest
                if (!pCrystalChosen ||
                    m_creature->GetDistanceOrder(
                        pCrystal, pCrystalChosen, false))
                {
                    m_crystalGuid = pCrystal->GetObjectGuid();
                    pCrystalChosen = pCrystal; // Store a copy of pCrystal so we
                                               // don't need to recreate a
                                               // pointer to closest crystal for
                                               // the movement and yell.
                }
            }
        }
        if (pCrystalChosen)
        {
            ...
            pCrystalChosen->CastSpell(
                pCrystalChosen, SPELL_FEL_CRYSTAL_COSMETIC, true);

            auto pos = pCrystalChosen->GetPoint(m_creature, CONTACT_DISTANCE);
    ...
    void ShatterRemainingCrystals()
    {
        ...
        {
            // Creature* pCrystal =
            // m_creature->GetMap()->GetCreature(FelCrystals[i]);
        ...
        if (m_pInstance)
        {
            ...
            if (m_lCrystalGuids.empty())
            {
                ...
                for (GUIDList::const_iterator itr = m_lCrystalGuids.begin();
                     itr != m_lCrystalGuids.end(); ++itr)
                    LOG_DEBUG(logging,
                        "SD2: Selin: Added Fel Crystal %s to list",
                        ObjectGuid(*itr).GetString().c_str());
            }
        }
    }
    ...
    void JustReachedHome() override
    {
        if (m_pInstance)
        {
                 ...
                 itr != m_lCrystalGuids.end(); ++itr)
            {
                ...
                {
                    if (!pCrystal->isAlive())
                        pCrystal->Respawn(); // Let MaNGOS handle setting death
                                             // state, etc.

                    // Only need to set unselectable flag. You can't attack
                    // unselectable units so non_attackable flag is not
                    // necessary here.
                    pCrystal->SetFlag(
    ...
    {
        if (uiType == movement::gen::point && uiPointId == 1)
        {
            Creature* pCrystalChosen =
                m_creature->GetMap()->GetCreature(m_crystalGuid);
            if (pCrystalChosen && pCrystalChosen->isAlive())
            {
                // Make the crystal attackable
                // We also remove NON_ATTACKABLE in case the database has it
                // set.
                pCrystalChosen->RemoveFlag(UNIT_FIELD_FLAGS,
            ...
            else
            {
                ...
                logging.error(
                    "SD2: Selin Fireheart unable to drain crystal as the "
                    "crystal is either dead or deleted..");
                m_bDrainingCrystal = false;
            }
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (!m_bDrainingCrystal)
        {
            ...
            if (uiMaxPowerMana && ((m_creature->GetPower(POWER_MANA) * 100 /
                                       uiMaxPowerMana) < 10))
            {
                if (m_uiDrainLifeTimer < uiDiff)
                {
                    ...
                    {
                        if (DoCastSpellIfCan(pTarget, SPELL_DRAIN_LIFE) ==
                            CAST_OK)
                ...
                if (!m_bIsRegularMode)
                {
                    if (m_uiDrainManaTimer < uiDiff)
                    {
                        ...
                        if (DoCastSpellIfCan(pTarget, SPELL_DRAIN_MANA) ==
                            CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(m_creature, SPELL_FEL_EXPLOSION) ==
                    CAST_OK)
        ...
        else // if m_bDrainingCrystal
        {
            if (m_bIsDraining)
            {
                if (m_uiEmpowerTimer < uiDiff)
                {
                    ...
                    Creature* CrystalChosen =
                        m_creature->GetMap()->GetCreature(m_crystalGuid);
                    if (CrystalChosen && CrystalChosen->isAlive())
                        // Use Deal Damage to kill it, not SetDeathState.
                        CrystalChosen->DealDamage(CrystalChosen,
                            CrystalChosen->GetHealth(), NULL, DIRECT_DAMAGE,
...
struct MANGOS_DLL_DECL mob_fel_crystalAI : public ScriptedAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        if (ScriptedInstance* pInstance =
                (ScriptedInstance*)m_creature->GetInstanceData())
        {
            Creature* pSelin =
                pInstance->GetSingleCreatureFromStorage(NPC_SELIN_FIREHEART);

            if (pSelin && pSelin->isAlive())
            {
                boss_selin_fireheartAI* pSelinAI =
                    dynamic_cast<boss_selin_fireheartAI*>(pSelin->AI());

                if (pSelinAI &&
                    pSelinAI->m_crystalGuid == m_creature->GetObjectGuid())
                {
                    // Set this to false if we are the creature that Selin is
                    // draining so his AI flows properly
                    ...
                    if (pSelin->getVictim())
                    {
                        ...
                        /* pSelin->GetMotionMaster()->MoveChase( */
                        /*     pSelin->getVictim()); */
                    }
                }
            }
