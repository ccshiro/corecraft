...
struct MANGOS_DLL_DECL boss_vexallusAI : public ScriptedAI
{
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        pSummoned->CastSpell(pSummoned, SPELL_ENERGY_BOLT, false, NULL, NULL,
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (!Enraged)
        {
            ...
            if (m_creature->GetHealthPercent() <=
                float(100 - INTERVAL_MODIFIER * IntervalHealthAmount))
            {
                // increase amount, unless we're at 10%, then we switch and
                // return
                ...
                if (m_bIsRegularMode)
                    m_creature->CastSpell(
                        m_creature, SPELL_SUMMON_PURE_ENERGY, true);
                else
                {
                    m_creature->CastSpell(
                        m_creature, SPELL_SUMMON_PURE_ENERGY1_H, true);
                    m_creature->CastSpell(
                        m_creature, SPELL_SUMMON_PURE_ENERGY2_H, true);
                }
            }
            ...
            if (ChainLightningTimer < diff)
            {
                if (Unit* target = m_creature->SelectAttackingTarget(
                    ...
                    DoCastSpellIfCan(target, m_bIsRegularMode ?
...
struct MANGOS_DLL_DECL mob_pure_energyAI : public ScriptedAI
{
    ...
    void JustDied(Unit* pKiller) override
    {
        if (m_creature->IsTemporarySummon())
        {
            ...
            {
                Creature* pVex = m_creature->GetMap()->GetCreature(
                ...
                if (Player* pPlayer =
                        pKiller->GetCharmerOrOwnerPlayerOrPlayerItself())
                    pPlayer->CastSpell(pPlayer, SPELL_ENERGY_FEEDBACK, true,
                        NULL, NULL, pVex->GetObjectGuid());
            }
        }
    }
