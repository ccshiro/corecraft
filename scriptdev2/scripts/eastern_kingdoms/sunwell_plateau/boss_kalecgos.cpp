...
enum
{
    ...
    SPELL_SPECTRAL_REALM_EJECT =
        44845, // pull the players ouside the spectral realm in case of wipe
...
static const uint32 aWildMagicSpells[6] = {
    44978, 45001, 45002, 45004, 45006, 45010};
...
struct MANGOS_DLL_DECL boss_kalecgosAI : public ScriptedAI
{
    ...
    void JustReachedHome() override
    {
        ...
        DoCastSpellIfCan(
            m_creature, SPELL_SPECTRAL_REALM_EJECT, CAST_TRIGGERED);
    ...
    void DamageTaken(Unit* /*pDoneBy*/, uint32& uiDamage) override
    {
        if (uiDamage > m_creature->GetHealth())
        {
            ...
            {
                if (DoCastSpellIfCan(
                        m_creature, SPELL_BANISH, CAST_TRIGGERED) == CAST_OK)
                    m_bIsBanished = true;
            }
            else
                DoStartOutro();
    ...
    {
        DoScriptText(
            urand(0, 1) ? SAY_EVIL_SLAY_1 : SAY_EVIL_SLAY_2, m_creature);
    }
    ...
    void DoStartOutro()
    {
        ...
        if (Creature* pSathrovarr =
                m_pInstance->GetSingleCreatureFromStorage(NPC_SATHROVARR))
        {
            // The teleport spell doesn't work right for this, so we need to
            // teleport him manually
            pSathrovarr->NearTeleportTo(1704.34f, 928.17f, 53.08f, 0);
            pSathrovarr->DealDamage(pSathrovarr, pSathrovarr->GetHealth(), NULL,
                DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false, false);
        }

        if (Creature* pKalec =
                m_pInstance->GetSingleCreatureFromStorage(NPC_KALECGOS_HUMAN))
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiExitTimer)
        {
            ...
            {
                auto pos =
                    m_creature->GetPoint2d(2 * M_PI_F * rand_norm_f(), 30.0f);
        ...
        if (!m_bIsEnraged && m_creature->GetHealthPercent() < 10.0f)
        {
            ...
            if (m_creature->has_aura(SPELL_CRAZED_RAGE))
                m_bIsEnraged = true;
            else
            {
        ...
        {
            if (DoCastSpellIfCan(
                    m_creature->getVictim(), SPELL_ARCANE_BUFFET) == CAST_OK)
            {
                if (!urand(0, 2))
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_FROST_BREATH) ==
                CAST_OK)
            {
                if (!urand(0, 1))
        ...
        if (m_uiWildMagicTimer < uiDiff)
        {
            ...
            {
                if (DoCastSpellIfCan(pTarget, aWildMagicSpells[urand(0, 5)]) ==
                    CAST_OK)
...
struct MANGOS_DLL_DECL boss_sathrovarrAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit* /*pDoneBy*/, uint32& uiDamage) override
    {
        if (uiDamage > m_creature->GetHealth())
        {
            ...
            if (DoCastSpellIfCan(m_creature, SPELL_BANISH, CAST_TRIGGERED) ==
                CAST_OK)
                m_bIsBanished = true;
            ...
            if (Creature* pKalecgos = m_pInstance->GetSingleCreatureFromStorage(
                    NPC_KALECGOS_DRAGON))
            {
                // Eject all players and set Kalec to uncorrupted
                pKalecgos->CastSpell(
                    pKalecgos, SPELL_SPECTRAL_REALM_EJECT, true);

                if (boss_kalecgosAI* pKalecgosAI =
                        dynamic_cast<boss_kalecgosAI*>(pKalecgos->AI()))
    ...
    {
        DoScriptText(
            urand(0, 1) ? SAY_SATH_SLAY_1 : SAY_SATH_SLAY_2, m_creature);

        // !!! Workaround which ejects the players from the spectral realm on
        // death !!!
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        ...
        if (pWho->GetTypeId() == TYPEID_PLAYER &&
            ...
            pWho->IsWithinDistInMap(m_creature, 75.0f))
        {
            if (!pWho->has_aura(SPELL_SPECTRAL_REALM_AURA))
            {
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiCorruptingStrikeTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature->getVictim(),
                    SPELL_CORRUPTING_STRIKE) == CAST_OK)
            {
                if (!urand(0, 1))
        ...
        if (m_uiCurseOfBoundlessAgonyTimer < uiDiff)
        {
            if (Unit* pTarget = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_RANDOM, 0))
            {
                if (DoCastSpellIfCan(pTarget, SPELL_CURSE_OF_BOUNDLESS_AGONY) ==
                    CAST_OK)
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(),
                    SPELL_SHADOW_BOLT_VOLLEY) == CAST_OK)
            {
                if (!urand(0, 1))
...
struct MANGOS_DLL_DECL boss_kalecgos_humanoidAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            // Cast on self because spell has target "all friendly units around
            // the caster"
        ...
        {
            if (DoCastSpellIfCan(
                    m_creature->getVictim(), SPELL_HEROIC_STRIKE) == CAST_OK)
