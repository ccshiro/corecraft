...
SD%Complete: 50
SDComment: Sinister Reflection need core and AI support; Orb of the Blue Fight
activation NYI; Shield Orb summoning NYI; Armageddon NYI; Offcombat yells NYI;
...
enum
{
    ...
    // phase 3
    SPELL_ARMAGEDDON =
        45921, // used from 50% hp - summons 25735 on target location
    ...
    NPC_ARMAGEDDON =
        25735, // uses spell 45914 followed by spell 45911, 45912, then by 45909
    NPC_BLUE_ORB_TARGET =
        25640, // dummy npc near gameobjects 187869, 188114, 188115, 188116
    ...
    SPELL_KALEC_TELEPORT =
        46473, // teleports and transforms Kalec in human form
    ...
    NPC_SOLDIER = 26259, // summoned in 2 waves before Velen. Should move into 2
                         // circle formations
...
static const DialogueEntry aPhaseDialogue[] = {
    {PHASE_DARKNESS, 0, 2000}, {EVENT_SWITCH_PHASE_2, 0, 17000},
    ...
    {PHASE_ARMAGEDDON, 0, 2000}, {EVENT_SWITCH_PHASE_3, 0, 14000},
    {SAY_KALECGOS_AWAKE_2, NPC_KALECGOS, 7000},
    {SAY_ANVEENA_LOST, NPC_ANVEENA, 7000}, {SAY_PHASE_4, NPC_KILJAEDEN, 6000},
    {EVENT_DRAGON_ORB, 0, 0}, // phase 3 transition end
    {PHASE_SACRIFICE, 0, 2000}, {EVENT_SWITCH_PHASE_4, 0, 5000},
...
static const DialogueEntry aOutroDialogue[] = {
    {NPC_KALECGOS, 0, 15000}, {SAY_KALECGOS_GOODBYE, NPC_KALECGOS, 40000},
    {NPC_BOSS_PORTAL, 0, 10000}, {POINT_SUMMON_SOLDIERS, 0, 18000},
    {NPC_VELEN, 0, 1000}, {NPC_LIADRIN, 0, 4000},
    {SAY_OUTRO_1, NPC_VELEN, 25000}, {SAY_OUTRO_2, NPC_VELEN, 15000},
    {SAY_OUTRO_3, NPC_VELEN, 13000}, {SPELL_CALL_ENTROPIUS, 0, 10000},
    {SAY_OUTRO_4, NPC_VELEN, 20000}, {POINT_MOVE_LIADRIN, 0, 5000},
    {SAY_OUTRO_5, NPC_LIADRIN, 10000}, {SAY_OUTRO_6, NPC_VELEN, 15000},
    {SAY_OUTRO_7, NPC_LIADRIN, 3000}, {SAY_OUTRO_8, NPC_VELEN, 4000},
    {SPELL_BLAZE_TO_LIGHT, 0, 13000}, {SAY_OUTRO_9, NPC_VELEN, 14000},
    {SAY_OUTRO_10, NPC_LIADRIN, 20000}, {SAY_OUTRO_11, NPC_VELEN, 8000},
    {SAY_OUTRO_12, NPC_VELEN, 4000}, {POINT_EVENT_EXIT, 0, 0}, {0, 0, 0},
};
...
struct MANGOS_DLL_DECL npc_kiljaeden_controllerAI : public ScriptedAI,
                                                    private DialogueHelper
{
    npc_kiljaeden_controllerAI(Creature* pCreature)
      : ScriptedAI(pCreature), DialogueHelper(aOutroDialogue)
    {
    ...
    void DoStartOutroDialogue() { StartNextDialogueText(NPC_KALECGOS); }
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case NPC_KALECGOS:
            if (Creature* pKalec =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_KALECGOS))
            {
                pKalec->CastSpell(pKalec, SPELL_KALEC_TELEPORT, true);
                pKalec->SetLevitate(false);
            }
            m_creature->SummonCreature(NPC_CORE_ENTROPIUS, m_creature->GetX(),
                m_creature->GetY(), 85.0f, 0, TEMPSUMMON_CORPSE_DESPAWN, 0);
            break;
        case NPC_BOSS_PORTAL:
            // ToDo: summon soldiers to the right
            m_creature->SummonCreature(NPC_BOSS_PORTAL, aOutroLocations[0].m_fX,
        ...
        case POINT_SUMMON_SOLDIERS:
            ...
            break;
        case NPC_VELEN:
            m_creature->SummonCreature(NPC_VELEN, aOutroLocations[1].m_fX,
                aOutroLocations[1].m_fY, aOutroLocations[1].m_fZ,
                aOutroLocations[1].m_fO, TEMPSUMMON_CORPSE_DESPAWN, 0);
            break;
        case NPC_LIADRIN:
            m_creature->SummonCreature(NPC_LIADRIN, aOutroLocations[2].m_fX,
                aOutroLocations[2].m_fY, aOutroLocations[2].m_fZ,
                aOutroLocations[2].m_fO, TEMPSUMMON_TIMED_DESPAWN,
                4 * MINUTE * IN_MILLISECONDS);
            break;
        case SPELL_CALL_ENTROPIUS:
            if (Creature* pVelen =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_VELEN))
                pVelen->CastSpell(pVelen, SPELL_CALL_ENTROPIUS, false);
            // Set point id = 1 for movement event
            if (Creature* pEntropius =
                    m_creature->GetMap()->GetCreature(m_EntropiusGuid))
            {
                pEntropius->SetWalk(false);
                /* pEntropius->GetMotionMaster()->MovePoint( */
                /*     1, m_creature->GetX(), m_creature->GetY(), 35.0f); */
            }
            break;
        case POINT_MOVE_LIADRIN:
            if (Creature* pLiadrin =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_LIADRIN))
                /* pLiadrin->GetMotionMaster()->MovePoint(0, */
                /*     aOutroLocations[4].m_fX, aOutroLocations[4].m_fY, */
                /*     aOutroLocations[4].m_fZ); */
                break;
        case SPELL_BLAZE_TO_LIGHT:
                    ...
                    m_creature->GetMap()->GetCreature(m_EntropiusGuid))
            {
                ...
                pEntropius->remove_auras(SPELL_ENTROPIUS_BODY);
                pEntropius->SetWalk(true);
                /* pEntropius->GetMotionMaster()->MovePoint(2,
                 * m_creature->GetX(), */
        ...
        case POINT_EVENT_EXIT:
            ...
            if (Creature* pVelen =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_VELEN))
                /* pVelen->GetMotionMaster()->MovePoint(1,
                 * aOutroLocations[1].m_fX, */
                /*     aOutroLocations[1].m_fY, aOutroLocations[1].m_fZ); */
                break;
        }
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        {
        case NPC_VELEN:
        /* pSummoned->GetMotionMaster()->MovePoint(0, aOutroLocations[3].m_fX,
         */
        ...
        Creature* pSummoned, movement::gen type, uint32 uiPointId) override
    {
        ...
        if (uiPointId == 1)
        {
            ...
            {
                // Interrupt Velen's casting when entropius has reached the
                // ground
                if (Creature* pVelen =
                        m_pInstance->GetSingleCreatureFromStorage(NPC_VELEN))
            ...
            {
                // Cast teleport and despawn Velen, the portal and Kalec;
                // Liadrin will despawn on timer
                pSummoned->CastSpell(pSummoned, SPELL_TELEPORT_VISUAL, true);
                pSummoned->ForcedDespawn(1000);

                // Note: portal should despawn only after all the soldiers have
                // reached this point and "teleported" outside
                if (Creature* pPortal =
                        m_creature->GetMap()->GetCreature(m_PortalGuid))
        ...
        {
            // When the purified Muru reaches the ground the sunwell ignites and
            // Muru despawns
...
struct MANGOS_DLL_DECL boss_kiljaedenAI : public ScriptedAI,
                                          private DialogueHelper
{
    boss_kiljaedenAI(Creature* pCreature)
      : ScriptedAI(pCreature), DialogueHelper(aPhaseDialogue)
    {
        m_pInstance = ((instance_sunwell_plateau*)pCreature->GetInstanceData());
        InitializeDialogueHelper(m_pInstance);
        Reset();
    ...
    void JustReachedHome() override
    {
        if (m_pInstance)
        {
            ...
            if (Creature* pKiljaedenController =
                    m_pInstance->GetSingleCreatureFromStorage(
                        NPC_KILJAEDEN_CONTROLLER))
                pKiljaedenController->CastSpell(
                    pKiljaedenController, SPELL_ANVEENA_DRAIN, true);

            // Respawn Anveena if necessary
            if (Creature* pAnveena =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_ANVEENA))
            {
                if (!pAnveena->isAlive())
                    pAnveena->Respawn();
            }
            ...
            if (Creature* pKalec = m_pInstance->GetSingleCreatureFromStorage(
                    NPC_KALECGOS, true))
        ...
        if (m_pInstance)
        {
            ...
            if (Creature* pKiljaedenController =
                    m_pInstance->GetSingleCreatureFromStorage(
                        NPC_KILJAEDEN_CONTROLLER))
            {
                if (npc_kiljaeden_controllerAI* pControllerAI =
                        dynamic_cast<npc_kiljaeden_controllerAI*>(
                            pKiljaedenController->AI()))
    ...
    void GetAIInformation(ChatHandler& reader) override
    {
        reader.PSendSysMessage(
            "Kil'jaeden is currently in phase %u", m_uiPhase);
    }
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case PHASE_SACRIFICE:
            if (DoCastSpellIfCan(m_creature, SPELL_SINISTER_REFLECTION,
                    CAST_INTERRUPT_PREVIOUS) == CAST_OK)
            {
                DoScriptText(irand(0, 1) ? SAY_REFLECTION_1 : SAY_REFLECTION_2,
                    m_creature);

                // In the 2nd and 3rd transition kill all drakes
                if (iEntry == PHASE_ARMAGEDDON || iEntry == PHASE_SACRIFICE)
                    DoCastSpellIfCan(
                        m_creature, SPELL_DESTROY_DRAKES, CAST_TRIGGERED);

                m_uiPhase = PHASE_TRANSITION;
                // Darkness of Souls needs the timer reseted
                m_uiDarknessOfSoulsTimer =
                    iEntry == PHASE_SACRIFICE ? 30000 : 45000;
            }
            break;
        ...
        case EVENT_DRAGON_ORB:
            if (Creature* pKalec =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_KALECGOS))
                DoScriptText(
                    irand(0, 1) ? SAY_KALECGOS_ORB_2 : SAY_KALECGOS_ORB_3,
                    pKalec);
        // no break
        ...
        case SAY_PHASE_5:
            if (Creature* pAnveena =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_ANVEENA))
            {
                pAnveena->remove_auras(SPELL_ANVEENA_PRISON);
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        case PHASE_TRANSITION:
            // Transition phase is handled in the dialogue helper; however we
            // don't want the spell timers to be decreased so we use a specific
            // phase
            break;
        case PHASE_SACRIFICE:
        ...
        case PHASE_ARMAGEDDON:
            ...
            if (m_uiArmageddonTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature, SPELL_ARMAGEDDON) == CAST_OK)
                    m_uiArmageddonTimer =
                        m_uiPhase == PHASE_SACRIFICE ? 20000 : 30000;
            }
            else
                m_uiArmageddonTimer -= uiDiff;

            // Go to next phase and start transition dialogue
            if (m_uiPhase == PHASE_ARMAGEDDON &&
                m_creature->GetHealthPercent() < 25.0f)
        ...
        case PHASE_DARKNESS:
            ...
            /* ToDo: Uncomment this spell when the Blue Dragonfight Orbs are
            implemented
            if (m_uiDarknessOfSoulsTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature, SPELL_DARKNESS_OF_SOULS) ==
            CAST_OK)
                    m_uiDarknessOfSoulsTimer = m_uiPhase == PHASE_SACRIFICE ?
            30000 : 45000;
            }
            else
            ...
            */
            ...
            // Go to next phase and start transition dialogue
            if (m_uiPhase == PHASE_DARKNESS &&
                m_creature->GetHealthPercent() < 55.0f)
        ...
        case PHASE_INFERNO:
            ...
            if (m_uiKalecSummonTimer)
            {
                ...
                {
                    m_creature->SummonCreature(NPC_KALECGOS, m_creature->GetX(),
                        m_creature->GetY(), 85.0f, 3.80f,
            ...
            if (m_uiLegionLightingTimer < uiDiff)
            {
                ...
                {
                    if (DoCastSpellIfCan(pTarget, SPELL_LEGION_LIGHTING) ==
                        CAST_OK)
                        m_uiLegionLightingTimer = urand(10000, 15000);
                }
            }
            ...
            {
                if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_SOUL_FLY) ==
                    CAST_OK)
            ...
            // Go to next phase and start transition dialogue
            if (m_uiPhase == PHASE_INFERNO &&
                m_creature->GetHealthPercent() < 85.0f)
...
bool EffectAuraDummy_spell_aura_dummy_darkness_of_souls(
    const Aura* pAura, bool bApply)
{
    // On Aura removal cast the explosion and yell
    // This is a special case when the dummy effect should be triggered at the
    // end of the channeling
    if (pAura->GetId() == SPELL_DARKNESS_OF_SOULS &&
        pAura->GetEffIndex() == EFFECT_INDEX_0 && !bApply)
    {
        if (Creature* pTarget = (Creature*)pAura->GetTarget())
        {
            pTarget->CastSpell(pTarget,
                pAura->GetSpellProto()->CalculateSimpleValue(EFFECT_INDEX_2),
                true);
...
void AddSC_boss_kiljaeden()
{
    ...
    pNewScript->pEffectAuraDummy =
        &EffectAuraDummy_spell_aura_dummy_darkness_of_souls;
