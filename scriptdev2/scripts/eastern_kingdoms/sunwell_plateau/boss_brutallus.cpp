...
enum
{
    ...
    SPELL_FLAME_RING =
        44873, // this spell should have a fire explosion when removed
    SPELL_CHARGE = 44884,
    SPELL_BREAK_ICE =
        46637, // Break the ice, open the door - dummy spell for 46638 and 47030
...
static const DialogueEntry aIntroDialogue[] = {
    {NPC_MADRIGOSA, 0, 6000}, {YELL_MADR_ICE_BARRIER, NPC_MADRIGOSA, 7000},
    {YELL_MADR_INTRO, NPC_MADRIGOSA, 7000}, {YELL_INTRO, NPC_BRUTALLUS, 6000},
    {SPELL_FROST_BREATH, 0, 6000}, {POINT_MOVE_ICE_BLOCK, 0, 5000},
    {YELL_MADR_ICE_BLOCK, NPC_MADRIGOSA, 5000}, {SPELL_FLAME_RING, 0, 7000},
    {YELL_INTRO_BREAK_ICE, NPC_BRUTALLUS, 1000}, {SPELL_FEL_FIREBALL, 0, 4000},
    {POINT_MOVE_GROUND, 0, 5000}, {YELL_MADR_TRAP, NPC_MADRIGOSA, 14000},
    {YELL_INTRO_CHARGE, NPC_BRUTALLUS, 10000},
    {YELL_INTRO_KILL_MADRIGOSA, NPC_BRUTALLUS, 8000},
    {YELL_INTRO_TAUNT, NPC_BRUTALLUS, 0}, {0, 0, 0},
};

struct MANGOS_DLL_DECL boss_brutallusAI : public ScriptedAI,
                                          private DialogueHelper
{
    boss_brutallusAI(Creature* pCreature)
      : ScriptedAI(pCreature), DialogueHelper(aIntroDialogue)
    {
    ...
    void KilledUnit(Unit* pVictim) override
    {
        ...
        {
        case 0:
            DoScriptText(YELL_KILL1, m_creature);
            break;
        case 1:
            DoScriptText(YELL_KILL2, m_creature);
            break;
        case 2:
            DoScriptText(YELL_KILL3, m_creature);
            break;
        }
    }
    ...
    void GetAIInformation(ChatHandler& reader) override
    {
        ...
        {
            if (m_pInstance->GetData(TYPE_BRUTALLUS) == SPECIAL)
                reader.PSendSysMessage("Brutallus intro event is currently %s",
                    m_bIsIntroInProgress ? "in progress" : "completed");
            else
                reader.PSendSysMessage("Brutallus intro event is currently %s",
                    m_pInstance->GetData(TYPE_BRUTALLUS) == NOT_STARTED ?
                        "not started" :
                        "completed");

            if (m_pInstance->GetData(TYPE_BRUTALLUS) != NOT_STARTED)
            {
                if (Creature* pMadrigosa =
                        m_pInstance->GetSingleCreatureFromStorage(
                            NPC_MADRIGOSA, true))
                    reader.PSendSysMessage(
                        "Madrigosa guid is %s and has %u health.",
                        pMadrigosa->GetObjectGuid().GetString().c_str(),
                        pMadrigosa->GetHealth());
            }
        }
    }
    ...
    void SummonedCreatureJustDied(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == NPC_MADRIGOSA)
            logging.error(
                "SD2: Npc %u, %s, died unexpectedly. Felmyst won't be summoned "
                "anymore.",
                pSummoned->GetEntry(), pSummoned->GetName());
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        if (pSummoned->GetEntry() == NPC_MADRIGOSA)
        {
            ...
            pSummoned->movement_gens.push(
                new movement::PointMovementGenerator(0, aMadrigosaFlyLoc[0],
                    aMadrigosaFlyLoc[1], aMadrigosaFlyLoc[2], false, true));
        }
        ...
        Creature* pSummoned, movement::gen type, uint32 uiPointId) override
    {
        if (type != movement::gen::point ||
            pSummoned->GetEntry() != NPC_MADRIGOSA)
            return;
    ...
    void SpellHitTarget(Unit* pTarget, const SpellEntry* pSpell) override
    {
        ...
        if (pTarget->GetEntry() == NPC_MADRIGOSA && pSpell->Id == SPELL_CHARGE)
        {
            ...
            pTarget->remove_auras_if([pTarget](AuraHolder* h)
                {
                    return !(h->IsPassive() && h->GetCaster() == pTarget &&
                               pTarget->player_or_pet()) &&
                           !h->IsDeathPersistent();
                });
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case NPC_MADRIGOSA:
            if (Creature* pTrigger = m_pInstance->GetSingleCreatureFromStorage(
                    NPC_FLIGHT_TRIGGER_LEFT))
                m_creature->SummonCreature(NPC_MADRIGOSA, pTrigger->GetX(),
                    pTrigger->GetY(), pTrigger->GetZ(), 0,
                    TEMPSUMMON_DEAD_DESPAWN, 0);
            m_bIsIntroInProgress = true;
            break;
        case YELL_MADR_ICE_BARRIER:
            if (Creature* pMadrigosa =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
        ...
        case YELL_MADR_INTRO:
            ...
            /*         m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
             */
            /*     pMadrigosa->GetMotionMaster()->MovePoint(POINT_MOVE_GROUND,
             */
            /*         aMadrigosaGroundLoc[0], aMadrigosaGroundLoc[1], */
            /*         aMadrigosaGroundLoc[2]); */
            break;
        case YELL_INTRO:
            if (Creature* pMadrigosa =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
        ...
        case POINT_MOVE_ICE_BLOCK:
            ...
            if (Creature* pMadrigosa =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
            {
                /* pMadrigosa->GetMotionMaster()->MovePoint(POINT_MOVE_ICE_BLOCK,
                 */
                /*     aMadrigosaFlyLoc[0], aMadrigosaFlyLoc[1], */
                /*     aMadrigosaFlyLoc[2]); */
                pMadrigosa->SetLevitate(true);
            }
            // Temporary! This will make Brutallus not follow Madrigosa through
            // the air until mmaps are implemented
        ...
        case YELL_INTRO_BREAK_ICE:
            m_creature->remove_auras(SPELL_FLAME_RING);
            break;
        ...
        case YELL_MADR_TRAP:
                    ...
                    m_pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
            {
                ...
                // Need to remove the fire aura after 4 sec so Madrigosa won't
                // die so soon
                pMadrigosa->remove_auras(SPELL_FEL_FIREBALL);
            }
            break;
        ...
        case YELL_INTRO_KILL_MADRIGOSA:
            // Face the players
            if (GameObject* pIceWall =
                    m_pInstance->GetSingleGameObjectFromStorage(GO_ICE_BARRIER))
    ...
    void DoStartIntro() { StartNextDialogueText(NPC_MADRIGOSA); }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_METEOR_SLASH) ==
                CAST_OK)
                m_uiSlashTimer = 11000;
        }
        else
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_STOMP) ==
                CAST_OK)
                m_uiStompTimer = 30000;
        }
        else
        ...
        if (m_uiBurnTimer < uiDiff)
        {
            if (Unit* pTarget = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_RANDOM, 0, SPELL_BURN, SELECT_FLAG_PLAYER))
            {
...
bool EffectAuraDummy_spell_aura_dummy_npc_brutallus_cloud(
    const Aura* pAura, bool bApply)
{
    // On Aura removal start Felmyst summon visuals
    if (pAura->GetId() == SPELL_BRUTALLUS_DEATH_CLOUD &&
        pAura->GetEffIndex() == EFFECT_INDEX_0 && !bApply)
    {
        if (Creature* pTarget = (Creature*)pAura->GetTarget())
        {
            if (ScriptedInstance* pInstance =
                    (ScriptedInstance*)pTarget->GetInstanceData())
            {
                if (Creature* pMadrigosa =
                        pInstance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
                {
                    pMadrigosa->CastSpell(
                        pMadrigosa, SPELL_FELBLAZE_PREVIZUAL, true);
...
{
    if (ScriptedInstance* pInstance =
            (ScriptedInstance*)pPlayer->GetInstanceData())
    {
        // this simply set encounter state, and trigger ice barrier become
        // active
        ...
        if (pInstance->GetData(TYPE_BRUTALLUS) == NOT_STARTED)
        {
            ...
            if (Creature* pBrutallus =
                    pInstance->GetSingleCreatureFromStorage(NPC_BRUTALLUS))
            {
                if (boss_brutallusAI* pBossAI =
                        dynamic_cast<boss_brutallusAI*>(pBrutallus->AI()))
...
void AddSC_boss_brutallus()
{
    ...
    pNewScript->pEffectAuraDummy =
        &EffectAuraDummy_spell_aura_dummy_npc_brutallus_cloud;
    pNewScript->RegisterSelf();
