...
enum
{
    ...
    SPELL_SOUL_SEVER =
        45918, // kills all charmed targets at wipe - script effect for 45917
...
struct MANGOS_DLL_DECL boss_felmystAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        if (!m_bHasTransformed)
        {
            if (pWho->GetTypeId() == TYPEID_PLAYER &&
                ...
                pWho->IsWithinDistInMap(m_creature, 100.0f))
            {
    ...
    void SpellHitTarget(Unit* pTarget, const SpellEntry* pSpell) override
    {
        if (pTarget->GetTypeId() == TYPEID_PLAYER &&
            pSpell->Id == SPELL_ENCAPSULATE_CHANNEL)
            pTarget->CastSpell(pTarget, SPELL_ENCAPSULATE, true, NULL, NULL,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiPhase == PHASE_GROUND)
        {
            ...
            {
                if (DoCastSpellIfCan(
                        m_creature->getVictim(), SPELL_CORROSION) == CAST_OK)
                    m_uiCorrosionTimer = 30000;
            }
            else
            ...
            if (m_uiEncapsulateTimer < uiDiff)
            {
                ...
                {
                    if (DoCastSpellIfCan(pTarget, SPELL_ENCAPSULATE_CHANNEL) ==
                        CAST_OK)
                        m_uiEncapsulateTimer = urand(30000, 40000);
                }
            }
