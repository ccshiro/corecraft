...
SDName: boss_muru
SD%Complete: 80
SDComment: Spell Negative Energy for Entropius needs core support; Summon
humanoids spells have some core issues; Dark Fiend, Singularity and Darkness
NPCs need eventAI support
...
enum
{
    ...
    SPELL_OPEN_PORTAL_PERIODIC =
        45994, // periodic spell which opens a portal at 30 secs; triggers 45976
    SPELL_OPEN_PORTAL = 45976,        // has muru portal as target
    SPELL_SUMMON_BERSERKER_1 = 46037, // humanoids summoned at 15 secs (3 on
                                      // each side) then after 60 secs
    ...
    SPELL_SUMMON_DARK_FIEND_1 =
        46000, // summons 8 dark fiends (25744); ToDo: script npc in eventAI
    ...
    // transition
    SPELL_OPEN_ALL_PORTALS = 46177, // dummy spell which opens all the portals
                                    // to begin the transition phase - has muru
                                    // portal as target
    SPELL_SUMMON_ENTROPIUS = 46217,
    SPELL_ENTROPIUS_SPAWN = 46223, // visual effect after spawn

    // entropius spells
    SPELL_NEGATIVE_ENERGY_ENT = 46284, // periodic aura spell; triggers 46289
                                       // which has script effect. Damage spell
                                       // is 46285 but it needs core support
    SPELL_SUMMON_BLACK_HOLE = 46282,   // 15 sec cooldown; summons 25855
    SPELL_SUMMON_DARKNESS = 46269,     // summons 25879 by missile

    // portal spells
    SPELL_SENTINEL_SUMMONER_VISUAL =
        45989, // hits the summoner, so it will summon the sentinel
...
struct MANGOS_DLL_DECL boss_muruAI : public ScriptedAI
{
    ...
    void Aggro(Unit* /*pWho*/) override
    {
        ...
        DoCastSpellIfCan(
            m_creature, SPELL_OPEN_PORTAL_PERIODIC, CAST_TRIGGERED);
    }
    ...
    void DamageTaken(Unit* /*pDoneBy*/, uint32& uiDamage) override
    {
        if (uiDamage > m_creature->GetHealth())
        {
            ...
            {
                // Start transition
                if (DoCastSpellIfCan(m_creature, SPELL_OPEN_ALL_PORTALS) ==
                    CAST_OK)
                {
                    // remove the auras
                    m_creature->remove_auras(SPELL_NEGATIVE_ENERGY);
                    m_creature->remove_auras(SPELL_OPEN_PORTAL_PERIODIC);
    ...
    void DoSummonHumanoids()
    {
        ...
        {
            DoCastSpellIfCan(
                m_creature, SPELL_SUMMON_BERSERKER_1, CAST_TRIGGERED);
            DoCastSpellIfCan(
                m_creature, SPELL_SUMMON_BERSERKER_2, CAST_TRIGGERED);
        }
...
struct MANGOS_DLL_DECL boss_entropiusAI : public ScriptedAI
{
    ...
    {
        for (GUIDList::const_iterator itr = m_lSummonedCreaturesList.begin();
             itr != m_lSummonedCreaturesList.end(); ++itr)
        {
    ...
    void JustReachedHome() override
    {
        if (m_pInstance)
        {
            ...
            // respawn muru
            if (Creature* pMuru =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_MURU))
                pMuru->Respawn();
        }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiBlackHoleTimer < uiDiff)
        {
            ...
            {
                if (DoCastSpellIfCan(pTarget, SPELL_SUMMON_BLACK_HOLE) ==
                    CAST_OK)
                    m_uiBlackHoleTimer = 15000;
            }
        }
...
struct MANGOS_DLL_DECL npc_portal_targetAI : public ScriptedAI
{
    npc_portal_targetAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        m_pInstance = ((instance_sunwell_plateau*)pCreature->GetInstanceData());
        Reset();
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == NPC_VOID_SENTINEL_SUMMONER)
            DoCastSpellIfCan(
                pSummoned, SPELL_SENTINEL_SUMMONER_VISUAL, CAST_TRIGGERED);
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiSentinelTimer)
        {
            ...
            {
                if (DoCastSpellIfCan(
                        m_creature, SPELL_SUMMON_SENTINEL_SUMMONER) == CAST_OK)
                    m_uiSentinelTimer = 0;
            }
            else
        ...
        if (m_uiTransformTimer)
        {
            if (m_uiTransformTimer <= uiDiff)
            {
                ...
                DoCastSpellIfCan(m_creature,
                    m_uiTransformCount % 2 ? SPELL_TRANSFORM_VISUAL_1 :
                                             SPELL_TRANSFORM_VISUAL_2,
                    CAST_TRIGGERED);
...
struct MANGOS_DLL_DECL npc_void_sentinel_summonerAI : public ScriptedAI
{
    npc_void_sentinel_summonerAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        m_pInstance = ((instance_sunwell_plateau*)pCreature->GetInstanceData());
        Reset();
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        if (pSummoned->GetEntry() == NPC_VOID_SENTINEL)
        {
            ...
            if (Creature* pMuru =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_MURU))
            {
                if (Unit* pTarget = pMuru->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0))
