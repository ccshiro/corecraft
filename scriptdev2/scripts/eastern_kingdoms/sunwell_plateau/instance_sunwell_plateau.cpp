...
static const DialogueEntry aFelmystOutroDialogue[] = {
    {NPC_KALECGOS, 0, 10000}, {SAY_KALECGOS_OUTRO, NPC_KALECGOS, 10000},
    {SPELL_OPEN_BACK_DOOR, 0, 0}, {0, 0, 0},
};

instance_sunwell_plateau::instance_sunwell_plateau(Map* pMap)
  : ScriptedInstance(pMap), DialogueHelper(aFelmystOutroDialogue),
    m_uiDeceiversKilled(0), m_uiSpectralRealmTimer(5000),
    m_uiKalecRespawnTimer(0), m_uiMuruBerserkTimer(0)
{
    Initialize();
}
...
void instance_sunwell_plateau::OnPlayerEnter(Player* pPlayer)
{
    ...
    if (m_auiEncounter[TYPE_BRUTALLUS] != DONE ||
        m_auiEncounter[TYPE_FELMYST] == DONE)
        return;
    ...
    pPlayer->SummonCreature(NPC_FELMYST, aMadrigosaGroundLoc[0],
        aMadrigosaGroundLoc[1], aMadrigosaGroundLoc[2], aMadrigosaGroundLoc[3],
    ...
    if (pCreature->GetEntry() == NPC_DECEIVER)
    {
        ...
        if (m_uiDeceiversKilled == MAX_DECEIVERS)
        {
            if (Creature* pController =
                    GetSingleCreatureFromStorage(NPC_KILJAEDEN_CONTROLLER))
            {
                if (Creature* pKiljaeden = pController->SummonCreature(
                        NPC_KILJAEDEN, pController->GetX(), pController->GetY(),
                        pController->GetZ(), pController->GetO(),
                        TEMPSUMMON_DEAD_DESPAWN, 0))
                    pKiljaeden->SetInCombatWithZone();

                pController->remove_auras(SPELL_ANVEENA_DRAIN);
            }
        }
    }
}
...
void instance_sunwell_plateau::OnObjectCreate(GameObject* pGo)
{
    ...
    case GO_FIRE_BARRIER:
        if (m_auiEncounter[TYPE_KALECGOS] == DONE &&
            m_auiEncounter[TYPE_BRUTALLUS] == DONE &&
            m_auiEncounter[TYPE_FELMYST] == DONE)
...
void instance_sunwell_plateau::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_KALECGOS:
        ...
        if (uiData == FAIL)
        {
            ...
            if (Creature* pKalecDragon =
                    GetSingleCreatureFromStorage(NPC_KALECGOS_DRAGON))
                pKalecDragon->ForcedDespawn();
            if (Creature* pKalecHuman =
                    GetSingleCreatureFromStorage(NPC_KALECGOS_HUMAN))
                pKalecHuman->ForcedDespawn();
            if (Creature* pSathrovarr =
                    GetSingleCreatureFromStorage(NPC_SATHROVARR))
...
void instance_sunwell_plateau::Update(uint32 uiDiff)
{
    ...
    // Muru berserk timer; needs to be done here because it involves two
    // distinct creatures
    if (m_auiEncounter[TYPE_MURU] == IN_PROGRESS)
    {
        if (m_uiMuruBerserkTimer <= uiDiff)
        {
            if (Creature* pEntrpius =
                    GetSingleCreatureFromStorage(NPC_ENTROPIUS, true))
...
void instance_sunwell_plateau::JustDidDialogueStep(int32 iEntry)
{
    ...
    case NPC_KALECGOS:
        if (Creature* pTrigger =
                GetSingleCreatureFromStorage(NPC_FLIGHT_TRIGGER_LEFT))
        {
            if (Creature* pKalec = pTrigger->SummonCreature(NPC_KALECGOS,
                    pTrigger->GetX(), pTrigger->GetY(), pTrigger->GetZ(), 0,
                    TEMPSUMMON_TIMED_DESPAWN, 1 * MINUTE * IN_MILLISECONDS))
            {
                pKalec->SetWalk(false);
                pKalec->SetLevitate(true);
                /* pKalec->GetMotionMaster()->MovePoint(0, aMadrigosaFlyLoc[0],
                 */
                /*     aMadrigosaFlyLoc[1], aMadrigosaFlyLoc[2]); */
            }
        }
        break;
...
bool AreaTrigger_at_sunwell_plateau(
    Player* pPlayer, AreaTriggerEntry const* pAt)
{
    if (pAt->id == AREATRIGGER_TWINS)
    {
        ...
        instance_sunwell_plateau* pInstance =
            (instance_sunwell_plateau*)pPlayer->GetInstanceData();
