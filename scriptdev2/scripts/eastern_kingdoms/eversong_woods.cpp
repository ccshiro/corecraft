...
const uint32 uiChallengerId[4] = {
    NPC_BLOODWRATH, NPC_LIGHTREND, NPC_SWIFTBLADE, NPC_SUNSTRIKER};

const int32 uiSayId[4] = {-1000319, -1000320, -1000321, -1000322};

float fChallengerLoc[4][4] = {{10110.667f, -6628.059f, 4.100f, 2.708f},
...
struct MANGOS_DLL_DECL npc_kelerun_bloodmournAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        for (auto& elem : m_aChallengerGuids)
            elem.Clear();
    ...
    void DoSpawnChallengers()
    {
        for (uint8 i = 0; i < MAX_CHALLENGER; ++i)
        {
            if (Creature* pCreature =
                    m_creature->SummonCreature(uiChallengerId[i],
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_bIsEventInProgress)
        {
            if (m_uiTimeOutTimer && m_uiTimeOutTimer < uiDiff)
            {
                if (!m_playerGuid)
                {
                    // player are expected to use GO within a minute, if not,
                    // event will fail.
                    Reset();
                    return;
                }
            ...
            {
                if (Creature* pChallenger = m_creature->GetMap()->GetCreature(
                        m_aChallengerGuids[m_uiChallengerCount]))
                {
            ...
            if (m_uiEngageTimer && m_uiEngageTimer < uiDiff)
            {
                ...
                if (pPlayer && pPlayer->isAlive())
                {
                    ...
                    {
                        DoScriptText(
                            uiSayId[m_uiChallengerCount], m_creature, pPlayer);
...
// easiest way is to expect database to respawn GO at quest accept
// (quest_start_script)
bool QuestAccept_npc_kelerun_bloodmourn(
    Player* /*pPlayer*/, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_SECOND_TRIAL)
    {
        if (npc_kelerun_bloodmournAI* pKelrunAI =
                dynamic_cast<npc_kelerun_bloodmournAI*>(pCreature->AI()))
...
bool GOUse_go_harbinger_second_trial(Player* pPlayer, GameObject* pGO)
{
    ...
    {
        if (Creature* pCreature =
                GetClosestCreatureWithEntry(pGO, NPC_KELERUN, 30.0f))
        {
            if (npc_kelerun_bloodmournAI* pKelrunAI =
                    dynamic_cast<npc_kelerun_bloodmournAI*>(pCreature->AI()))
...
#define GOSSIP_ITEM_SHOW                                                   \
    "Why... yes, of course. I've something to show you right inside this " \
    "building, Mr. Anvilward."
...
bool GossipHello_npc_prospector_anvilward(Player* pPlayer, Creature* pCreature)
{
    if (pPlayer->GetQuestStatus(QUEST_THE_DWARVEN_SPY) ==
        ...
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_MOMENT,
...
bool GossipSelect_npc_prospector_anvilward(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    switch (uiAction)
    {
    case GOSSIP_ACTION_INFO_DEF + 1:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_SHOW,
    ...
    case GOSSIP_ACTION_INFO_DEF + 2:
        ...
        if (npc_prospector_anvilwardAI* pEscortAI =
                dynamic_cast<npc_prospector_anvilwardAI*>(pCreature->AI()))
...
struct MANGOS_DLL_DECL npc_apprentice_mirvedaAI : public ScriptedAI
{
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
        ...
        if (pPlayer &&
            pPlayer->GetQuestStatus(QUEST_UNEXPECTED_RESULT) ==
                QUEST_STATUS_INCOMPLETE)
    ...
    void StartEvent(Player* pPlayer)
    {
        ...
        m_creature->SummonCreature(NPC_GHARSUL, 8745.0f, -7134.32f, 35.22f,
            0.0f, TEMPSUMMON_CORPSE_DESPAWN, 4000);
        m_creature->SummonCreature(NPC_ANGERSHADE, 8745.0f, -7134.32f, 35.22f,
            0.0f, TEMPSUMMON_CORPSE_DESPAWN, 4000);
        m_creature->SummonCreature(NPC_ANGERSHADE, 8745.0f, -7134.32f, 35.22f,
            0.0f, TEMPSUMMON_CORPSE_DESPAWN, 4000);
    }
...
bool QuestAccept_unexpected_results(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_UNEXPECTED_RESULT)
        if (npc_apprentice_mirvedaAI* pMirvedaAI =
                dynamic_cast<npc_apprentice_mirvedaAI*>(pCreature->AI()))
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiWaveTimer)
        {
            if (m_uiWaveTimer <= uiDiff)
            {
                ...
                {
                    for (uint8 i = 0; i < 3; ++i)
                        m_creature->SummonCreature(NPC_ENRAGED_WRAITH,
                            aSummonPos[i][0], aSummonPos[i][1],
                            aSummonPos[i][2], aSummonPos[i][3],
                            TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 5 * MINUTE);
                    m_uiWaveTimer = 29000;
                    m_bFirstWave = false;
                }
                else
        ...
        if (m_uiFinishTimer)
        {
            if (m_uiFinishTimer <= uiDiff)
            {
                ...
                if (m_creature->IsTemporarySummon())
                {
                    ...
                    if (Player* pPlayer = m_creature->GetMap()->GetPlayer(
                            pTemporary->GetSummonerGuid()))
