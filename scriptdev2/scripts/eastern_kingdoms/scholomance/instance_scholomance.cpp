...
instance_scholomance::instance_scholomance(Map* pMap)
  : ScriptedInstance(pMap), m_uiGandlingEvent(0)
{
    Initialize();
}
...
void instance_scholomance::Initialize()
{
    ...
    for (auto& aGandlingEvent : aGandlingEvents)
...
void instance_scholomance::OnCreatureCreate(Creature* pCreature)
{
    ...
    case NPC_DARKMASTER_GANDLING:
        m_mNpcEntryGuidStore[NPC_DARKMASTER_GANDLING] =
            pCreature->GetObjectGuid();
        break;
    case NPC_BONE_MINION:
        GandlingEventMap::iterator find =
            m_mGandlingData.find(m_uiGandlingEvent);
...
void instance_scholomance::SetData(uint32 uiType, uint32 uiData)
{
    ...
    {
    case TYPE_KIRTONOS:
        // This door is initially closed by DB-scripts, so only use it in case
        // of FAIL, DONE, or on aggro after wipe
    ...
    case TYPE_RAS_FROSTWHISPER:
        ...
        break;
    case TYPE_MALICIA: // TODO this code can be simplified, when it is known
                       // which event-ids correspond to which room
        m_auiEncounter[uiType] = uiData;
    ...
    case TYPE_GANDLING:
        m_auiEncounter[uiType] = uiData;
        // Close the door to main room, because the encounter will take place
        // only in the main hall and random around all the 6 rooms
...
void instance_scholomance::DoSpawnGandlingIfCan(bool bByPlayerEnter)
{
    ...
    if (m_auiEncounter[TYPE_MALICIA] == DONE &&
        m_auiEncounter[TYPE_THEOLEN] == DONE &&
        m_auiEncounter[TYPE_POLKELT] == DONE &&
        m_auiEncounter[TYPE_RAVENIAN] == DONE &&
        m_auiEncounter[TYPE_ALEXEI_BAROV] == DONE &&
        m_auiEncounter[TYPE_ILLUCIA_BAROV] == DONE)
    {
        if (Creature* pGandling = pPlayer->SummonCreature(
                NPC_DARKMASTER_GANDLING, aGandlingSpawnLocs[0].m_fX,
                aGandlingSpawnLocs[0].m_fY, aGandlingSpawnLocs[0].m_fZ,
                aGandlingSpawnLocs[0].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0))
        {
            if (!bByPlayerEnter)
...
void instance_scholomance::OnCreatureEnterCombat(Creature* pCreature)
{
    ...
    case NPC_BONE_MINION:
        for (auto& elem : m_mGandlingData)
        {
            // if there are no minions for a room, skip it
            if (!elem.second.m_sAddGuids.empty())
            {
                // set data to fail in case of player death
                if (elem.second.m_sAddGuids.find(pCreature->GetGUIDLow()) !=
                    elem.second.m_sAddGuids.end())
                {
                    HandlePortalEvent(elem.first, IN_PROGRESS);
                    break;
                }
            }
        }
        ...
        for (auto& elem : m_mGandlingData)
        {
            ...
            if (!elem.second.m_sAddGuids.empty())
            {
                ...
                if (elem.second.m_sAddGuids.find(pCreature->GetGUIDLow()) !=
                    elem.second.m_sAddGuids.end())
                {
                    HandlePortalEvent(elem.first, FAIL);
                    break;
                }
            }
        }
        ...
        for (auto& elem : m_mGandlingData)
        {
            ...
            if (!elem.second.m_sAddGuids.empty())
            {
                ...
                if (elem.second.m_sAddGuids.find(pCreature->GetGUIDLow()) !=
                    elem.second.m_sAddGuids.end())
                {
                    elem.second.m_sAddGuids.erase(pCreature->GetGUIDLow());

                    // if the current list is empty; set event id as done
                    if (elem.second.m_sAddGuids.empty())
                    {
                        HandlePortalEvent(elem.first, DONE);
                        break;
                    }
                }
            }
...
bool ProcessEventId_event_spell_gandling_shadow_portal(
    uint32 uiEventId, Object* pSource, Object* /*pTarget*/, bool /*bIsStart*/)
{
    if (pSource->GetTypeId() == TYPEID_UNIT)
    {
        if (instance_scholomance* pInstance =
                (instance_scholomance*)((Creature*)pSource)->GetInstanceData())
        {
            // Check if we are handling an event associated with the room events
            // of gandling
            for (auto& aGandlingEvent : aGandlingEvents)
            {
                if (uiEventId == aGandlingEvent)
                {
                    // Set data in progress for the current event and store
                    // current event
                    pInstance->HandlePortalEvent(uiEventId, SPECIAL);
                    // return false, to allow the DB-scripts to summon some
                    // NPCSs
                    return false;
                }
            }
...
void AddSC_instance_scholomance()
{
    ...
    pNewScript->pProcessEventId =
        &ProcessEventId_event_spell_gandling_shadow_portal;
    pNewScript->RegisterSelf();
}
