...
enum
{
    ...
    SPELL_SUMMON_BONE_MINIONS = 27687, // triggers 27690, 27691, 27692, 27693
                                       // (spells were removed after vanilla)
    SPELL_FRENZY = 8269,
};
...
struct MANGOS_DLL_DECL boss_kormokAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (DoCastSpellIfCan(m_creature, SPELL_SUMMON_BONE_MINIONS) ==
                CAST_OK)
        ...
        {
            if (DoCastSpellIfCan(m_creature, SPELL_SUMMON_BONE_MAGES) ==
                CAST_OK)
                m_bHasMages = true;
        }
