...
struct MANGOS_DLL_DECL boss_darkmaster_gandlingAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (DoCastSpellIfCan(
                    m_creature->getVictim(), SPELL_ARCANE_MISSILES) == CAST_OK)
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_CURSE) ==
                CAST_OK)
        ...
        // Teleporting Random Target to one of the six pre boss rooms and spawn
        // 3-4 skeletons near the gamer.
        // We will only telport if gandling has more than 3% of hp so teleported
        // gamers can always loot.
        if (m_creature->GetHealthPercent() > 3.0f)
        {
            if (m_uiTeleportTimer < uiDiff)
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOW_PORTAL,
                        SELECT_FLAG_PLAYER))
                {
                    if (DoCastSpellIfCan(pTarget, SPELL_SHADOW_PORTAL) ==
                        CAST_OK)
                    {
                        // remove threat
