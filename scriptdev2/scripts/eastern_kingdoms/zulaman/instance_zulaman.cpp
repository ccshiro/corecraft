...
instance_zulaman::instance_zulaman(Map* pMap)
  : ScriptedInstance(pMap), m_uiEventTimer(MINUTE * IN_MILLISECONDS),
    m_uiGongCount(0), m_uiBearEventPhase(0), m_bIsBearPhaseInProgress(false),

    m_uiEggsRemainingCount_Left(20), m_uiEggsRemainingCount_Right(20)
{
    Initialize();
}
...
void instance_zulaman::OnCreatureCreate(Creature* pCreature)
{
    ...
    case NPC_TANZAR:
        m_aEventNpcInfo[INDEX_NALORAKK].npGuid = pCreature->GetObjectGuid();
        break;
    case NPC_KRAZ:
        m_aEventNpcInfo[INDEX_JANALAI].npGuid = pCreature->GetObjectGuid();
        break;
    case NPC_ASHLI:
        m_aEventNpcInfo[INDEX_HALAZZI].npGuid = pCreature->GetObjectGuid();
        break;
    case NPC_HARKOR:
        m_aEventNpcInfo[INDEX_AKILZON].npGuid = pCreature->GetObjectGuid();
        break;
    ...
    case NPC_WARBRINGER:
    case NPC_AXETHROWER:
        if (pCreature->GetZ() > 10.0f && pCreature->GetZ() < 15.0f)
            m_aNalorakkEvent[0].sBearTrashGuidSet.insert(
                pCreature->GetObjectGuid());
        else if (pCreature->GetZ() > 25.0f && pCreature->GetZ() < 30.0f)
            m_aNalorakkEvent[1].sBearTrashGuidSet.insert(
                pCreature->GetObjectGuid());
        else if (pCreature->GetZ() > 40.0f && pCreature->GetZ() < 41.0f)
            m_aNalorakkEvent[2].sBearTrashGuidSet.insert(
                pCreature->GetObjectGuid());
        else if (pCreature->GetZ() > 41.0f)
            m_aNalorakkEvent[3].sBearTrashGuidSet.insert(
                pCreature->GetObjectGuid());
        break;
    }
}
...
void instance_zulaman::OnCreatureDeath(Creature* pCreature)
{
    ...
    case NPC_WARBRINGER:
    case NPC_AXETHROWER:
        if (m_aNalorakkEvent[m_uiBearEventPhase].sBearTrashGuidSet.find(
                pCreature->GetObjectGuid()) !=
            m_aNalorakkEvent[m_uiBearEventPhase].sBearTrashGuidSet.end())
        {
            ++m_aNalorakkEvent[m_uiBearEventPhase].uiTrashKilled;
            if (m_aNalorakkEvent[m_uiBearEventPhase].uiTrashKilled ==
                m_aNalorakkEvent[m_uiBearEventPhase].sBearTrashGuidSet.size())
            {
                if (Creature* pNalorakk =
                        GetSingleCreatureFromStorage(NPC_NALORAKK))
                {
                    ++m_uiBearEventPhase;
                    if (m_uiBearEventPhase == MAX_BEAR_WAVES)
                        pNalorakk->RemoveFlag(
                            UNIT_FIELD_FLAGS, UNIT_FLAG_PASSIVE);
                    else
                    {
                        pNalorakk->movement_gens.push(
                            new movement::PointMovementGenerator(1,
                                aBearEventInfo[m_uiBearEventPhase].fX,
                                aBearEventInfo[m_uiBearEventPhase].fY,
                                aBearEventInfo[m_uiBearEventPhase].fZ, true,
                                true));
                    }
                }
            }
...
void instance_zulaman::OnCreatureEvade(Creature* pCreature)
{
    ...
    case NPC_AXETHROWER:
        for (const auto& elem :
            m_aNalorakkEvent[m_uiBearEventPhase].sBearTrashGuidSet)
        {
            Creature* pTemp = instance->GetCreature(elem);
...
void instance_zulaman::OnObjectCreate(GameObject* pGo)
{
    ...
    case GO_MASSIVE_GATE:
        if (m_auiEncounter[TYPE_EVENT_RUN] == DONE ||
            m_auiEncounter[TYPE_EVENT_RUN] == FAIL)
...
{
    LOG_DEBUG(logging,
        "SD2: Instance Zulaman: SetData received for type %u with data %u",
        uiType, uiData);
    ...
    case TYPE_EVENT_RUN:
        ...
        if (uiData == IN_PROGRESS)
        {
            ...
            if (m_auiEncounter[TYPE_RUN_EVENT_TIME])
                SetData(
                    TYPE_RUN_EVENT_TIME, m_auiEncounter[TYPE_RUN_EVENT_TIME]);
            else
        ...
        if (uiData == FAIL)
        {
            ...
            for (auto& elem : m_aEventNpcInfo)
            {
                // Not yet rescued, so too late
                if (!elem.uiSavePosition)
                {
                    if (Creature* pCreature =
                            instance->GetCreature(elem.npGuid))
    ...
    case TYPE_JANALAI:
        if (uiData == FAIL)
        {
            ...
            for (GUIDList::const_iterator itr = m_lEggsGUIDList.begin();
                 itr != m_lEggsGUIDList.end(); ++itr)
            {
    ...
    default:
        logging.error(
            "SD2: Instance Zulaman: ERROR SetData = %u for type %u does not "
            "exist/not implemented.",
            uiType, uiData);
        return;
    }

    if (uiData == DONE && GetKilledPreBosses() == 4 &&
        (uiType == TYPE_AKILZON || uiType == TYPE_NALORAKK ||
            uiType == TYPE_JANALAI || uiType == TYPE_HALAZZI))
    {
        DoUseDoorOrButton(GO_HEXLORD_ENTRANCE);
        if (m_auiEncounter[TYPE_EVENT_RUN] == IN_PROGRESS)
...
void instance_zulaman::Load(const char* chrIn)
{
    ...
    if (m_auiEncounter[TYPE_RUN_EVENT_TIME] != 0 &&
        m_auiEncounter[TYPE_EVENT_RUN] != DONE &&
        m_auiEncounter[TYPE_EVENT_RUN] != FAIL)
...
void instance_zulaman::SendNextBearWave(Unit* pTarget)
{
        ...
        m_aNalorakkEvent[m_uiBearEventPhase].sBearTrashGuidSet)
    {
        ...
        if (pTemp && pTemp->isAlive())
        {
            ...
            // For the first wave we need to make them jump to the ground before
            // attacking
            if (!m_uiBearEventPhase)
            {
                // float fX, fY, fZ;
                // pTemp->GetRandomPoint(35.31f, 1412.24f, 2.04f, 3.0f, fX, fY,
                // fZ);
                // TODO: Why are they using MoveJump here? That's not a mangos
                // function
                // pTemp->GetMotionMaster()->MoveJump(fX, fY, fZ,
                // pTemp->GetSpeed(MOVE_RUN)*2, 5.0f);
            }
        }
    }
...
{
    return (m_auiEncounter[TYPE_AKILZON] == DONE ? 1 : 0) +
           (m_auiEncounter[TYPE_NALORAKK] == DONE ? 1 : 0) +
           (m_auiEncounter[TYPE_JANALAI] == DONE ? 1 : 0) +
           (m_auiEncounter[TYPE_HALAZZI] == DONE ? 1 : 0);
}
...
void instance_zulaman::DoTimeRunSay(RunEventSteps uiData)
{
    ...
    {
    case RUN_START:
        DoOrSimulateScriptTextForThisInstance(SAY_INST_BEGIN, NPC_MALACRASS);
        break;
    case RUN_FAIL:
        DoOrSimulateScriptTextForThisInstance(
            urand(0, 1) ? SAY_INST_SACRIF1 : SAY_INST_SACRIF2, NPC_MALACRASS);
        break;
    case RUN_DONE:
        DoOrSimulateScriptTextForThisInstance(SAY_INST_COMPLETE, NPC_MALACRASS);
        break;
    case RUN_PROGRESS:
        // This function is on progress called before the data is set to the
        // array
        switch (GetKilledPreBosses() + 1)
        {
        case 1:
            DoOrSimulateScriptTextForThisInstance(
                SAY_INST_PROGRESS_1, NPC_MALACRASS);
            break;
        case 2:
            DoOrSimulateScriptTextForThisInstance(
                SAY_INST_PROGRESS_2, NPC_MALACRASS);
            break;
        case 3:
            DoOrSimulateScriptTextForThisInstance(
                SAY_INST_PROGRESS_3, NPC_MALACRASS);
            break;
        }
        break;
    case RUN_FAIL_SOON:
        ...
        case 0:
            DoOrSimulateScriptTextForThisInstance(
                SAY_INST_WARN_1, NPC_MALACRASS);
            break;
        case 1:
            DoOrSimulateScriptTextForThisInstance(
                SAY_INST_WARN_2, NPC_MALACRASS);
            break;
        case 2:
            DoOrSimulateScriptTextForThisInstance(
                SAY_INST_WARN_3, NPC_MALACRASS);
            break;
        case 3:
            DoOrSimulateScriptTextForThisInstance(
                SAY_INST_WARN_4, NPC_MALACRASS);
            break;
        }
        break;
    }
...
void instance_zulaman::DoChestEvent(BossToChestIndex uiIndex)
{
    ...
    // related Chest:   m_aEventNpcInfo[uiIndex]        // Not yet stored,
    // because likely unneeded
}
...
void instance_zulaman::Update(uint32 uiDiff)
{
    if (m_auiEncounter[TYPE_EVENT_RUN] == IN_PROGRESS)
    {
        ...
        {
            if (m_auiEncounter[TYPE_RUN_EVENT_TIME] ==
                5) // TODO, verify 5min for warning texts
                DoTimeRunSay(RUN_FAIL_SOON);

            if (m_auiEncounter[TYPE_RUN_EVENT_TIME] == 0)
            {
                LOG_DEBUG(logging,
                    "SD2: Instance Zulaman: event time reach end, event "
                    "failed.");
            ...
            LOG_DEBUG(logging, "SD2: Instance Zulaman: minute decrease to %u.",
                m_auiEncounter[TYPE_RUN_EVENT_TIME]);
