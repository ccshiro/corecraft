...
SD%Complete: 70
SDComment: Contain adds and adds selection; Stolen abilities timers need
improvement
SDCategory: Zul'Aman
EndScriptData */
...
enum
{
     ...
     * The boss casts siphon soul right after he finishes the spirit bolts
     * channel, which takes 10 sec
     * The siphon soul is a channeled spell for 30 sec during which the boss
     * uses some class abilities of the target
     * Basically the boss casts a dummy spell which chooses a random target on
     * which it casts the actuall channel spell
     * The drain power spell acts as a enrage timer. It's cast each 30 seconds
     * after the boss' health is below 80%
     */
...
static PlayerAbilityStruct m_aMalacrassStolenAbility[][4] = {
    {// 0* shadow priest - exception: it seems that the priest has two specs. We
     // use this slot for the shadow priest
...
struct MANGOS_DLL_DECL boss_malacrassAI : public ScriptedAI
{
    ...
    void InitializeAdds()
    {
        ...
        if (m_lAddsEntryList.empty())
        {
            ...
            for (auto& elem : m_auiSpawnEntry)
                m_lAddsEntryList.push_back(urand(0, 1) ?
                                               elem.m_uiCreatureEntry :
                                               elem.m_uiCreatureEntryAlt);

            // summon mobs from the list
            for (auto& elem : m_lAddsEntryList)
            {
                if (Creature* pAdd = m_creature->SummonCreature((elem),
                        m_afAddPosX[j], ADD_POS_Y, ADD_POS_Z, ADD_ORIENT,
        ...
        else
        {
            ...
            {
                Creature* pAdd =
                    m_creature->GetMap()->GetCreature(m_aAddGuid[j]);

                // object already removed, not exist
                if (!pAdd)
                {
                    if ((pAdd = m_creature->SummonCreature((elem),
                             m_afAddPosX[j], ADD_POS_Y, ADD_POS_Z, ADD_ORIENT,
                             TEMPSUMMON_CORPSE_DESPAWN, 0)) != nullptr)
                        m_aAddGuid[j] = pAdd->GetObjectGuid();
                }
                ++j;
            }
    ...
    void SpellHitTarget(Unit* pTarget, const SpellEntry* pSpell) override
    {
        ...
        if (pTarget->GetTypeId() == TYPEID_PLAYER &&
            pSpell->Id == SPELL_SIPHON_SOUL)
        {
            m_uiPlayerClass = ((Player*)pTarget)->getClass();
            m_bCanUsePlayerSpell = true;

            // In case the player it's priest we can choose either a holy priest
            // or a shadow priest
            ...
            for (uint8 i = 0; i < m_uiMaxSpells; ++i)
                m_vPlayerSpellTimer.push_back(
                    m_aMalacrassStolenAbility[m_uiPlayerClass][i]
                        .m_uiInitialTimer);
        }
    }
    ...
    bool CanUseSpecialAbility(uint32 uiSpellIndex)
    {
        ...
        switch (m_aMalacrassStolenAbility[m_uiPlayerClass][uiSpellIndex]
                    .m_uiTargetType)
        {
        ...
        if (pTarget)
        {
            if (DoCastSpellIfCan(pTarget,
                    m_aMalacrassStolenAbility[m_uiPlayerClass][uiSpellIndex]
                        .m_uiSpellId,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (DoCastSpellIfCan(m_creature, SPELL_SIPHON_SOUL_DUMMY) ==
                CAST_OK)
            {
...
struct MANGOS_DLL_DECL boss_malacrass_addAI : public ScriptedAI
{
    ...
    void KilledUnit(Unit* pVictim) override
    {
        ...
        if (Creature* pMalacrass =
                m_pInstance->GetSingleCreatureFromStorage(NPC_MALACRASS))
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
                ...
                m_pInstance->GetSingleCreatureFromStorage(NPC_MALACRASS))
        {
            ...
            {
            case 0:
                DoScriptText(SAY_ADD_DIED1, pMalacrass);
                break;
            case 1:
                DoScriptText(SAY_ADD_DIED2, pMalacrass);
                break;
            case 2:
                DoScriptText(SAY_ADD_DIED3, pMalacrass);
                break;
            }
        }
    }
...
{
    mob_thurgAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            std::vector<Creature*> lTempList =
                DoFindFriendlyMissingBuff(50.0f, SPELL_BLOODLUST);
...
{
    mob_alyson_antilleAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiArcaneTorrentTimer < uiDiff)
        {
            if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                    uint32(0), SELECT_FLAG_PLAYER | SELECT_FLAG_IN_MELEE_RANGE))
            {
        ...
        {
            // this will fail if we previously was following target and pTarget
            // is now different than before
            if (Unit* pTarget =
                    DoSelectLowestHpFriendly(RANGE_FRIENDLY_TARGET * 2, 30000))
            {
                if (pTarget->IsWithinDistInMap(
                        m_creature, RANGE_FRIENDLY_TARGET))
                {
        ...
        if (m_uiDispelMagicTimer < uiDiff)
        {
            Unit* pTarget = NULL;
            std::vector<Creature*> lTempList =
                DoFindFriendlyCC(RANGE_FRIENDLY_TARGET);
...
{
    mob_gazakrothAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
    ...
    void Reset() override { m_uiFireboltTimer = 1000; }
...
{
    mob_lord_raadanAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiThunderclapTimer < uiDiff)
        {
            if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0,
                    SPELL_THUNDERCLAP, SELECT_FLAG_PLAYER))
            {
...
{
    mob_darkheartAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
    ...
    void Reset() override { m_uiPsychicWailTimer = 8000; }
...
{
    mob_slitherAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
    ...
    void Reset() override { m_uiVenomSpitTimer = 4000; }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (Unit* pVictim = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_RANDOM, 0))
...
{
    mob_fenstalkerAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
    ...
    void Reset() override { m_uiVolatileInfectionTimer = 15000; }
...
{
    mob_koraggAI(Creature* pCreature) : boss_malacrass_addAI(pCreature)
    {
        Reset();
    }
