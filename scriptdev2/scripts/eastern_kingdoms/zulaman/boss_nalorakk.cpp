...
struct MANGOS_DLL_DECL boss_nalorakkAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        ...
        if (pWho->GetTypeId() == TYPEID_PLAYER &&
            ...
            m_creature->IsWithinDistInMap(
                pWho, aBearEventInfo[m_uiCurrentWave].fAggroDist))
        {
    ...
    void MovementInform(movement::gen uiMotionType, uint32 uiPointId) override
    {
        ...
        if (uiPointId)
        {
            ...
            {
                // Set the instance data to fail on movement inform because we
                // are not moving the boss to home position
                if (m_pInstance)
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        // Spells for Troll Form (only to be casted if we NOT have bear phase
        // aura)
        if (!m_bIsInBearForm)
        {
            ...
            {
                if (DoCastSpellIfCan(
                        m_creature->getVictim(), SPELL_BRUTAL_SWIPE) == CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_MANGLE) ==
                    CAST_OK)
            ...
            if (m_uiSurgeTimer < uiDiff)
            {
                ...
                Unit* pTtarget = m_creature->SelectAttackingTarget(
            ...
            {
                if (DoCastSpellIfCan(m_creature, SPELL_BEAR_SHAPE,
        ...
        else
        {
            ...
            if (m_uiLaceratingSlashTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature->getVictim(),
                        SPELL_LACERATING_SLASH) == CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(
                        m_creature->getVictim(), SPELL_REND_FLESH) == CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(m_creature, SPELL_DEAFENING_ROAR) ==
                    CAST_OK)
                    m_uiDeafeningRoarTimer = urand(15000, 25000);
            }
            else
