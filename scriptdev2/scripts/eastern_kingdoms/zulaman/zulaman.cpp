...
entry from running this script
SDCategory: Zul'Aman
EndScriptData */
...
struct MANGOS_DLL_DECL npc_forest_frogAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* caster, const SpellEntry* spell) override
    {
        if (spell->Id == SPELL_REMOVE_AMANI_CURSE &&
            ...
            m_creature->GetEntry() == NPC_FOREST_FROG)
        {
...
struct MANGOS_DLL_DECL npc_harrison_jones_zaAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 1:
            ...
            m_pInstance->DoToggleGameObjectFlags(
                GO_STRANGE_GONG, GO_FLAG_NO_INTERACT, false);
        ...
        case 4:
            ...
            // TODO: Spawn group of Amani'shi Savage and make them run to
            // entrance
            break;
        }
    }
    ...
    void SetHoldState(bool bOnHold)
    {
        ...
        if (m_pInstance && m_pInstance->GetData(TYPE_EVENT_RUN) == SPECIAL &&
            m_creature->has_aura(SPELL_BANGING_THE_GONG))
            m_creature->remove_auras(SPELL_BANGING_THE_GONG);
    }
};
...
bool GossipHello_npc_harrison_jones_za(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pInstance && pInstance->GetData(TYPE_EVENT_RUN) == NOT_STARTED)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_BEGIN,
...
bool GossipSelect_npc_harrison_jones_za(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        if (npc_harrison_jones_zaAI* pHarrisonAI =
                dynamic_cast<npc_harrison_jones_zaAI*>(pCreature->AI()))
...
// Unsure how this Gong must work. Here we always return false to allow Mangos
// always process further.
bool GOUse_go_strange_gong(Player* /*pPlayer*/, GameObject* pGo)
{
    ...
    if (pInstance->GetData(TYPE_EVENT_RUN) == SPECIAL)
    {
        if (Creature* pCreature =
                pInstance->GetSingleCreatureFromStorage(NPC_HARRISON))
        {
            if (npc_harrison_jones_zaAI* pHarrisonAI =
                    dynamic_cast<npc_harrison_jones_zaAI*>(pCreature->AI()))
            ...
            logging.error("SD2: Instance Zulaman: go_strange_gong failed");
