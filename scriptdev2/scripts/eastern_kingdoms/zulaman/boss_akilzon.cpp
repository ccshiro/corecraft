...
struct MANGOS_DLL_DECL boss_akilzonAI : public ScriptedAI
{
    ...
    void DoSummonEagles()
    {
        for (uint32 i = 0; i < MAX_EAGLE_COUNT; ++i)
        {
            ...
            m_creature->SummonCreature(NPC_SOARING_EAGLE, pos.x, pos.y, pos.z,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiCallLightTimer < uiDiff)
        {
            m_creature->CastSpell(
                m_creature->getVictim(), SPELL_CALL_LIGHTNING, false);
            m_uiCallLightTimer = urand(15000, 25000);
        }
        else
...
struct MANGOS_DLL_DECL mob_soaring_eagleAI : public ScriptedAI
{
    ...
    void DoMoveToRandom()
    {
        ...
        if (Creature* pAzkil =
                m_pInstance->GetSingleCreatureFromStorage(NPC_AKILZON))
        {
            auto pos = pAzkil->GetPoint(
                2 * M_PI_F * rand_norm_f(), 30.0f * rand_norm_f());

            m_creature->movement_gens.push(new movement::PointMovementGenerator(
                POINT_ID_RANDOM, pos.x, pos.y, pos.z, true, true));
