...
enum
{
    ...
    // Eagle Form
    SPELL_ENERGY_STORM = 43983,    // enemy area aura, trigger 42577 on vortexes
                                   // which cast 43137 on targets
...
static const BossPhase aZuljinPhases[] = {
    {SPELL_SHAPE_OF_THE_BEAR, SAY_BEAR_TRANSFORM, EMOTE_BEAR_SPIRIT,
     NPC_BEAR_SPIRIT, PHASE_BEAR},
    {SPELL_SHAPE_OF_THE_EAGLE, SAY_EAGLE_TRANSFORM, EMOTE_EAGLE_SPIRIT,
     NPC_EAGLE_SPIRIT, PHASE_EAGLE},
    {SPELL_SHAPE_OF_THE_LYNX, SAY_LYNX_TRANSFORM, EMOTE_LYNX_SPIRIT,
     NPC_LYNX_SPIRIT, PHASE_LYNX},
    {SPELL_SHAPE_OF_THE_DRAGONHAWK, SAY_DRAGONHAWK_TRANSFORM,
     EMOTE_DRAGONHAWK_SPIRIT, NPC_DRAGONHAWK_SPIRIT, PHASE_DRAGONHAWK}};
...
struct MANGOS_DLL_DECL boss_zuljinAI : public ScriptedAI
{
    ...
    void JustReachedHome() override
    {
        ...
        // Reset all spirits
        for (auto& aZuljinPhase : aZuljinPhases)
        {
            if (Creature* pSpirit = m_pInstance->GetSingleCreatureFromStorage(
                    aZuljinPhase.uiSpiritId))
            {
    ...
    {
        for (GUIDList::const_iterator itr = m_lSummonsList.begin();
             itr != m_lSummonsList.end(); ++itr)
        {
    ...
    void MovementInform(movement::gen uiMotionType, uint32 uiPointId) override
    {
        ...
        // drain the spirit
        if (Creature* pSpirit = m_pInstance->GetSingleCreatureFromStorage(
                aZuljinPhases[m_uiPhase].uiSpiritId))
    ...
    void SpellHit(Unit* /*pCaster*/, const SpellEntry* pSpell) override
    {
        ...
        {
            DoCastSpellIfCan(
                m_creature, aZuljinPhases[m_uiPhase].uiSpiritSpellId);
            ...
            // In Eagle phase we just cast Energy storm and summon 4 Feather
            // cyclones; Boss doesn't move in this phase
            else
            {
                DoCastSpellIfCan(
                    m_creature, SPELL_ENERGY_STORM, CAST_TRIGGERED);

                // summon 4 vortexes
                for (uint8 i = 0; i < MAX_VORTEXES; ++i)
                    DoCastSpellIfCan(
                        m_creature, SPELL_SUMMON_CYCLONE, CAST_TRIGGERED);
            }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_creature->GetHealthPercent() < m_uiHealthCheck)
        {
            ...
            m_creature->movement_gens.push(new movement::PointMovementGenerator(
                POINT_ID_CENTER, fZuljinMoveLoc[0], fZuljinMoveLoc[1],
                fZuljinMoveLoc[2], true, false));

            // Despawn vortexes and remvoe the energy storm after eagle phase is
            // complete
            if (m_uiPhase == PHASE_EAGLE)
            {
                m_creature->remove_auras(SPELL_ENERGY_STORM);
                DoDespawnVortexes();
            }
            ...
            if (m_uiPhase != PHASE_TROLL)
            {
                if (m_creature->has_aura(
                        aZuljinPhases[m_uiPhase].uiSpiritSpellId))
        ...
        case PHASE_TROLL:
            ...
            if (m_uiGrievousThrowTimer < uiDiff)
            {
                ...
                {
                    if (DoCastSpellIfCan(pTarget, SPELL_GRIEVOUS_THROW) ==
                        CAST_OK)
        ...
        case PHASE_BEAR:
            ...
            {
                if (DoCastSpellIfCan(m_creature, SPELL_CREEPING_PARALYSIS) ==
                    CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(
                        m_creature->getVictim(), SPELL_OVERPOWER) == CAST_OK)
            ...
            break;
        case PHASE_EAGLE:
            // Nothing here; Spells casted just once at the beginning of the
            // phase;
            break;
        case PHASE_LYNX:
            // ToDo: Note for further development of this phase!
            // In this phase the boss used two melee spells:
            // SPELL_CLAW_RAGE_HASTE and SPELL_LYNX_RUSH_HASTE
            // However the way these spells are used and what are their
            // mechanics and timers is highly unknown
            break;
        case PHASE_DRAGONHAWK:
            ...
            {
                if (DoCastSpellIfCan(m_creature, SPELL_SUMMON_PILLAR) ==
                    CAST_OK)
            ...
            if (m_uiFlameBreathTimer < uiDiff)
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0))
                {
                    if (DoCastSpellIfCan(pTarget, SPELL_FLAME_BREATH) ==
                        CAST_OK)
                        m_uiFlameBreathTimer = 15000;
                }
            }
