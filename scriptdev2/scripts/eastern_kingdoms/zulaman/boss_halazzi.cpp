...
struct MANGOS_DLL_DECL boss_halazziAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        if (m_pInstance)
        {
            if (Creature* pSpiritLynx =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_SPIRIT_LYNX))
    ...
    void DoUpdateStats(const CreatureInfo* pInfo)
    {
        ...
        if (m_uiPhase == PHASE_SINGLE)
        {
            m_creature->SetHealth(
                m_creature->GetMaxHealth() / 4 * m_uiPhaseCounter);
            --m_uiPhaseCounter;
        }
    }
    ...
    void SpellHit(Unit* /*pCaster*/, const SpellEntry* pSpell) override
    {
        ...
        if (const CreatureInfo* pInfo =
                GetCreatureTemplateStore(pSpell->EffectMiscValue[0]))
            DoUpdateStats(pInfo);
            ...
            if (m_creature->GetHealthPercent() <= float(25 * m_uiPhaseCounter))
            {
                ...
                else
                {
                    ...
                    m_creature->CastSpell(
                        m_creature, SPELL_TRANSFIGURE_TO_TROLL, false);
                }
            }
        }
        else
        {
            Creature* pSpiritLynx =
                m_pInstance->GetSingleCreatureFromStorage(NPC_SPIRIT_LYNX);
                ...
                (pSpiritLynx && pSpiritLynx->GetHealthPercent() < 10.0f))
            {
                ...
                {
                case 3:
                    uiSpellId = SPELL_TRANSFORM_TO_LYNX_75;
                    break;
                case 2:
                    uiSpellId = SPELL_TRANSFORM_TO_LYNX_50;
                    break;
                case 1:
                    uiSpellId = SPELL_TRANSFORM_TO_LYNX_25;
                    break;
                }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiPhase == PHASE_FINAL || m_uiPhase == PHASE_SINGLE)
        {
            ...
            {
                DoScriptText(
                    urand(0, 1) ? SAY_SABERLASH1 : SAY_SABERLASH2, m_creature);
...
struct MANGOS_DLL_DECL boss_spirit_lynxAI : public ScriptedAI
{
    ...
    void KilledUnit(Unit* pVictim) override
    {
        ...
        if (Creature* pHalazzi =
                m_pInstance->GetSingleCreatureFromStorage(NPC_HALAZZI))
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiFrenzyTimer < uiDiff)
        {
            ...
            m_uiFrenzyTimer = urand(
                20000, 30000); // subsequent frenzys casted every 20-30 seconds
        }
        else
            m_uiFrenzyTimer -= uiDiff;
