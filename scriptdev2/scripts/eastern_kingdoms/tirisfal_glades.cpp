...
bool GOUse_go_mausoleum_door(Player* pPlayer, GameObject* /*pGo*/)
{
    ...
    if (GameObject* pTrigger =
            GetClosestGameObjectWithEntry(pPlayer, GO_TRIGGER, 30.0f))
    {
        pTrigger->SetGoState(GO_STATE_READY);
        pPlayer->SummonCreature(NPC_ULAG, 2390.26f, 336.47f, 40.01f, 2.26f,
...
bool GOUse_go_mausoleum_trigger(Player* pPlayer, GameObject* pGo)
{
    ...
    if (GameObject* pDoor =
            GetClosestGameObjectWithEntry(pPlayer, GO_DOOR, 30.0f))
    {
...
bool QuestAccept_npc_calvin_montague(
