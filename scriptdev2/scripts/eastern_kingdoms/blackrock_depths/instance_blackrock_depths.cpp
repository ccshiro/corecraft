...
instance_blackrock_depths::instance_blackrock_depths(Map* pMap)
  : ScriptedInstance(pMap), m_uiBarAleCount(0), m_uiCofferDoorsOpened(0),

    m_fArenaCenterX(0.0f), m_fArenaCenterY(0.0f), m_fArenaCenterZ(0.0f)
{
    Initialize();
}
...
void instance_blackrock_depths::OnCreatureCreate(Creature* pCreature)
{
    ...
    case NPC_WARBRINGER_CONST:
        ...
        if (std::abs(pCreature->GetZ() - aVaultPositions[2]) > 1.0f ||
            !pCreature->IsWithinDist2d(
                aVaultPositions[0], aVaultPositions[1], 20.0f))
            break;
        // Golems in Relict Vault need to have a stoned aura, set manually to
        // prevent reapply when reached home
...
void instance_blackrock_depths::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_VAULT:
        if (uiData == SPECIAL)
        {
            ...
            if (m_uiCofferDoorsOpened == MAX_RELIC_DOORS)
            {
                ...
                for (const auto& elem : m_sVaultNpcGuids)
                {
                    if ((pConstruct = instance->GetCreature(elem)) != nullptr)
                        pConstruct->remove_auras(SPELL_STONED);
                }
                ...
                // Summon doomgrip
                pConstruct->SummonCreature(NPC_WATCHER_DOOMGRIP,
                    aVaultPositions[0], aVaultPositions[1], aVaultPositions[2],
                    aVaultPositions[3], TEMPSUMMON_DEAD_DESPAWN, 0);
            }
    ...
    case TYPE_TOMB_OF_SEVEN:
        ...
        case FAIL:
            if (m_auiEncounter[3] ==
                IN_PROGRESS) // Prevent use more than one time
                DoUseDoorOrButton(GO_TOMB_ENTER);
            break;
        case DONE:
...
{
    if (GetData(TYPE_RING_OF_LAW) == IN_PROGRESS ||
        GetData(TYPE_RING_OF_LAW) == SPECIAL)
    {
        for (auto& aArenaNPC : aArenaNPCs)
        {
