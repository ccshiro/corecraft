...
{
    boss_emperor_dagran_thaurissanAI(Creature* pCreature)
    ...
    void JustDied(Unit* /*pVictim*/) override
    {
        ...
        if (Creature* pPrincess =
                m_pInstance->GetSingleCreatureFromStorage(NPC_PRINCESS))
        {
...
struct MANGOS_DLL_DECL boss_moira_bronzebeardAI : public ScriptedAI
{
    ...
    void JustReachedHome() override
    {
        if (m_pInstance)
        {
            if (Creature* pEmperor =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_EMPEROR))
            {
                // if evade, then check if he is alive. If not, start make
                // portal
                if (!pEmperor->isAlive())
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiHeal_Timer < uiDiff)
        {
            ...
            {
                if (pEmperor->isAlive() &&
                    pEmperor->GetHealthPercent() != 100.0f)
