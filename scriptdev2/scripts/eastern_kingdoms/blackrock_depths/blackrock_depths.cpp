...
bool GOUse_go_relic_coffer_door(Player* /*pPlayer*/, GameObject* pGo)
{
    if (ScriptedInstance* pInstance = (ScriptedInstance*)pGo->GetInstanceData())
    {
        ...
        if (pInstance->GetData(TYPE_VAULT) != DONE &&
            pInstance->GetData(TYPE_VAULT) != IN_PROGRESS)
 ...
 * Visual: Npc Grimstone should use some visual spell when appear/ disappear /
 * opening/ closing doors
 * Texts: The texts and their positions need confirmation
 * Event timer might also need adjustment
 * Quest-Event: This needs to be clearified - there is some suggestion, that
 * Theldren&Adds also might come as first wave.
 */
...
enum
{
    ...
    // 4 or 6 in total? 1+2+1 / 2+2+2 / 3+3. Depending on this, code should be
    // changed.
    MAX_MOB_AMOUNT = 4,
    MAX_THELDREN_ADDS = 4,
...
static const uint32 aGladiator[MAX_POSSIBLE_THELDREN_ADDS] = {NPC_LEFTY,
    NPC_ROTFANG, NPC_SNOKH, NPC_MALGEN, NPC_KORV, NPC_REZZNIK, NPC_VAJASHNI,
    NPC_VOLIDA};
static const uint32 aRingMob[] = {NPC_WORM, NPC_STINGER, NPC_SCREECHER,
    NPC_THUNDERSNOUT, NPC_CREEPER, NPC_BEETLE};
static const uint32 aRingBoss[] = {NPC_GOROSH, NPC_GRIZZLE, NPC_EVISCERATOR,
    NPC_OKTHOR, NPC_ANUBSHIAH, NPC_HEDRUM};
...
{
    if (instance_blackrock_depths* pInstance =
            (instance_blackrock_depths*)pPlayer->GetInstanceData())
    {
        ...
        pInstance->SetData(TYPE_RING_OF_LAW,
            pInstance->GetData(TYPE_RING_OF_LAW) == DATA_BANNER_BEFORE_EVENT ?
                SPECIAL :
                IN_PROGRESS);

        pPlayer->SummonCreature(NPC_GRIMSTONE,
            aSpawnPositions[POS_GRIMSTONE][0],
            aSpawnPositions[POS_GRIMSTONE][1],
            aSpawnPositions[POS_GRIMSTONE][2],
            aSpawnPositions[POS_GRIMSTONE][3], TEMPSUMMON_DEAD_DESPAWN, 0);
...
struct MANGOS_DLL_DECL npc_grimstoneAI : public npc_escortAI
{
    npc_grimstoneAI(Creature* pCreature) : npc_escortAI(pCreature)
    {
        ...
        uint8 uiCount = 0;
        for (uint8 i = 0;
             i < MAX_POSSIBLE_THELDREN_ADDS && uiCount < MAX_THELDREN_ADDS; ++i)
        {
            if (urand(0, 1) ||
                i >= MAX_POSSIBLE_THELDREN_ADDS - MAX_THELDREN_ADDS + uiCount)
            {
    ...
    void DoChallengeQuestCredit()
    {
        ...
        {
            Player* pPlayer = elem.getSource();
            if (pPlayer &&
                pPlayer->GetQuestStatus(QUEST_THE_CHALLENGE) ==
    ...
    void SummonRingMob(uint32 uiEntry, SpawnPosition uiPosition)
    {
        auto pos = m_creature->GetPointXYZ(
            G3D::Vector3(aSpawnPositions[uiPosition][0],
                aSpawnPositions[uiPosition][1], aSpawnPositions[uiPosition][2]),
            2 * M_PI_F * rand_norm_f(), 2.0f * rand_norm_f());
        m_creature->SummonCreature(
            uiEntry, pos.x, pos.y, pos.z, 0, TEMPSUMMON_DEAD_DESPAWN, 0);
    }
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 3: // Middle reached second time
            DoScriptText(urand(0, 1) ? SAY_SUMMON_BOSS_1 : SAY_SUMMON_BOSS_2,
                m_creature);
            break;
        ...
        case 5:
            if (m_pInstance)
            {
                ...
                LOG_DEBUG(logging,
                    "SD2: npc_grimstone: event reached end and set complete.");
            }
            break;
        }
    }
    ...
    void UpdateEscortAI(const uint32 uiDiff) override
    {
        ...
        if (m_pInstance->GetData(TYPE_RING_OF_LAW) == FAIL)
        {
            ...
            for (GUIDList::const_iterator itr = m_lSummonedGUIDList.begin();
                 itr != m_lSummonedGUIDList.end(); ++itr)
            {
        ...
        if (m_uiEventTimer)
        {
            if (m_uiEventTimer <= uiDiff)
            {
                ...
                case 0:
                    ...
                    // DoScriptText(-1000000, m_creature); // no more text on
                    // spawn
                    m_pInstance->DoUseDoorOrButton(GO_ARENA_4);
                    Start(false);
                ...
                case 7:
                    ...
                    m_pInstance->DoUseDoorOrButton(GO_ARENA_1);
                    // DoScriptText(-1000000, m_creature); // after killed the
                    // mobs, no say here
                    SetEscortPaused(false);
                    m_uiEventTimer = 0;
                    break;
                ...
                case 9:
                    // Summon Boss
                    m_creature->SetVisibility(VISIBILITY_OFF);
                    // If banner summoned after start, then summon Thelden after
                        ...
                        m_uiPhase == PHASE_MOBS)
                    {
                        ...
                        for (auto& elem : m_uiGladiatorId)
                            SummonRingMob(elem, POS_NORTH);
                    }
                    else
                    {
...
bool EffectDummyCreature_spell_banner_of_provocation(Unit* /*pCaster*/,
    uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
    ...
    {
        instance_blackrock_depths* pInstance =
            (instance_blackrock_depths*)pCreatureTarget->GetInstanceData();
        if (pInstance && pInstance->GetData(TYPE_RING_OF_LAW) != DONE &&
            pInstance->GetData(TYPE_RING_OF_LAW) != SPECIAL)
            pInstance->SetData(TYPE_RING_OF_LAW,
                pInstance->GetData(TYPE_RING_OF_LAW) == IN_PROGRESS ?
                    SPECIAL :
                    static_cast<EncounterState>(DATA_BANNER_BEFORE_EVENT));
...
#define GOSSIP_ITEM_KHARAN_7                                                  \
    "So you suspect that someone on the inside was involved? That they were " \
    "tipped off?"
...
bool GossipHello_npc_kharan_mighthammer(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pPlayer->GetQuestStatus(QUEST_WHAT_IS_GOING_ON) ==
        QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_KHARAN_1,
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

    if (pPlayer->GetQuestStatus(QUEST_KHARANS_TALE) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_KHARAN_2,
...
bool GossipHello_npc_lokhtos_darkbargainer(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pCreature->isVendor() &&
        pPlayer->GetReputationRank(FACTION_THORIUM_BROTHERHOOD) >= REP_FRIENDLY)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, GOSSIP_ITEM_SHOW_ACCESS,
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_GET_CONTRACT,
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    }
...
bool GossipSelect_npc_lokhtos_darkbargainer(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
        pPlayer->CastSpell(
            pPlayer, SPELL_CREATE_THORIUM_BROTHERHOOD_CONTRACT, false);
    }
...
struct MANGOS_DLL_DECL npc_rocknotAI : public npc_escortAI
{
    ...
    void UpdateEscortAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiBreakDoorTimer)
        {
            ...
            {
                DoGo(GO_BAR_DOOR, 2);
                DoGo(GO_BAR_KEG_TRAP,
                    0); // doesn't work very well, leaving code here for future
                // spell by trap has effect61, this indicate the bar go hostile

                if (Creature* pTmp =
                        m_pInstance->GetSingleCreatureFromStorage(NPC_PHALANX))
                    pTmp->setFaction(14);
...
bool QuestRewarded_npc_rocknot(
    Player* /*pPlayer*/, Creature* pCreature, Quest const* pQuest)
{
    ...
    if (pQuest->GetQuestId() == QUEST_ALE)
    {
        ...
        // keep track of amount in instance script, returns SPECIAL if amount ok
        // and event in progress
        if (pInstance->GetData(TYPE_BAR) == SPECIAL)
        {
            ...
            if (npc_rocknotAI* pEscortAI =
                    dynamic_cast<npc_rocknotAI*>(pCreature->AI()))
...
void AddSC_blackrock_depths()
{
    ...
    pNewScript->pEffectDummyNPC =
        &EffectDummyCreature_spell_banner_of_provocation;
    pNewScript->RegisterSelf();
