    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 2:
            if (m_creature->HasStealthAura())
                m_creature->remove_auras(SPELL_AURA_MOD_STEALTH);
        ...
        if (m_creature->GetHealthPercent() < 20.0f)
        {
            if (Player* pPlayer = GetPlayerForEscort())
            {
                pPlayer->GroupEventHappens(
                    QUEST_MISSING_DIPLO_PT11, m_creature);

                uiDamage = 0;

                m_creature->setFaction(
    ...
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_MISSING_DIPLO_PT11)
    {
        ...
        if (npc_tapoke_slim_jahnAI* pEscortAI =
                dynamic_cast<npc_tapoke_slim_jahnAI*>(pSlim->AI()))
