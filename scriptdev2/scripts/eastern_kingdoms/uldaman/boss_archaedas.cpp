...
SD%Complete: 60
SDComment: Need correct way to deal with awaken vault and guardian spells,
waiting for additions in mangos for them (target combination 22/7)
SDCategory: Uldaman
EndScriptData */
...
struct MANGOS_DLL_DECL boss_archaedasAI : public ScriptedAI
{
    ...
    void SpellHitTarget(Unit* pTarget, const SpellEntry* /*pSpell*/) override
    {
        ...
        {
            if (pTarget->has_aura(SPELL_STONED))
            {
                pTarget->remove_auras(SPELL_STONED);
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
            ...
            if (m_uiAwakeningTimer <= uiDiff)
            {
                ...
                case 2:
                    if (Player* pPlayer = m_creature->GetMap()->GetPlayer(
                            m_pInstance->GetGuid(DATA_EVENT_STARTER)))
                        AttackStart(pPlayer);
                    else
                        EnterEvadeMode();
                    break;
        ...
        if (!m_bDwarvesAwaken && m_creature->GetHealthPercent() >= 33.0f)
        {
            ...
            {
                if (Creature* pEarthen =
                        m_pInstance->GetClosestDwarfNotInCombat(
                            m_creature, PHASE_ARCHA_1))
                {
                    if (DoCastSpellIfCan(
                            pEarthen, SPELL_AWAKEN_EARTHEN_DWARF) == CAST_OK)
        ...
        {
            if (Creature* pGuard = m_pInstance->GetClosestDwarfNotInCombat(
                    m_creature, PHASE_ARCHA_2))
            {
                if (DoCastSpellIfCan(pGuard, SPELL_AWAKEN_EARTHEN_GUARDIAN) ==
                    CAST_OK)
                {
        ...
        {
            if (Creature* pWarder = m_pInstance->GetClosestDwarfNotInCombat(
                    m_creature, PHASE_ARCHA_3))
            {
                if (DoCastSpellIfCan(pWarder, SPELL_AWAKEN_VAULT_WARDER) ==
                    CAST_OK)
                {
