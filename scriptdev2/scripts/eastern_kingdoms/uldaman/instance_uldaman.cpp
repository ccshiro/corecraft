...
instance_uldaman::instance_uldaman(Map* pMap)
  : ScriptedInstance(pMap), m_uiKeeperCooldown(5000), m_uiStoneKeepersFallen(0)
{
    Initialize();
}
...
void instance_uldaman::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_ARCHAEDAS:
        if (uiData == FAIL)
        {
            for (GUIDList::const_iterator itr = m_lWardens.begin();
                 itr != m_lWardens.end(); ++itr)
            {
...
void instance_uldaman::DoResetKeeperEvent()
{
    ...
    for (auto& elem : m_mKeeperMap)
    {
        if (Creature* pKeeper = instance->GetCreature(elem.first))
        {
...
Creature* instance_uldaman::GetClosestDwarfNotInCombat(
    Creature* pSearcher, uint32 uiPhase)
{
         ...
         itr != m_lWardens.end(); ++itr)
    {
        ...
        if (pTemp && pTemp->isAlive() && !pTemp->getVictim())
        {
            ...
            {
            case PHASE_ARCHA_1:
                if (pTemp->GetEntry() != NPC_CUSTODIAN &&
                    pTemp->GetEntry() != NPC_HALLSHAPER)
                    continue;
                break;
            case PHASE_ARCHA_2:
            ...
            if (!m_mKeeperMap.empty())
            {
                for (auto& elem : m_mKeeperMap)
                {
                    ...
                    if (Creature* pKeeper = instance->GetCreature(elem.first))
                    {
                        ...
                        {
                            if (Player* pPlayer =
                                    pKeeper->GetMap()->GetPlayer(m_playerGuid))
                            {
                                // we should use group instead, event starter
                                // can be dead while group is still fighting
                                if (pPlayer->isAlive() &&
                                    !pPlayer->isInCombat())
                                {
                                    pKeeper->remove_auras(SPELL_STONED);
...
bool ProcessEventId_event_spell_altar_boss_aggro(
    uint32 uiEventId, Object* pSource, Object* /*pTarget*/, bool bIsStart)
{
    if (bIsStart && pSource->GetTypeId() == TYPEID_PLAYER)
    {
        if (instance_uldaman* pInstance =
                (instance_uldaman*)((Player*)pSource)->GetInstanceData())
        {
