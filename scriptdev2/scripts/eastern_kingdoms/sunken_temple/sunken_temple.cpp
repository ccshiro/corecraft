...
bool AreaTrigger_at_shade_of_eranikus(
    Player* pPlayer, AreaTriggerEntry const* /*pAt*/)
{
            ...
            (ScriptedInstance*)pPlayer->GetInstanceData())
    {
        ...
        if (pPlayer->GetQuestRewardStatus(QUEST_THE_CHARGE_OF_DRAGONFLIGHTS) &&
            ...
            pPlayer->GetQuestStatus(QUEST_ERANIKUS_TYRANT_OF_DREAMS) !=
                QUEST_STATUS_COMPLETE)
        {
            if (pInstance->GetData(TYPE_MALFURION) != DONE)
            {
                pPlayer->SummonCreature(NPC_MALFURION,
                    aSunkenTempleLocation[2].m_fX,
                    aSunkenTempleLocation[2].m_fY,
                    aSunkenTempleLocation[2].m_fZ,
                    aSunkenTempleLocation[2].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0);
    ...
    uint32 uiEventId, Object* pSource, Object* pTarget, bool /*bIsStart*/)
{
    if (pSource->GetTypeId() == TYPEID_PLAYER &&
        pTarget->GetTypeId() == TYPEID_GAMEOBJECT)
    {
        if (instance_sunken_temple* pInstance =
                (instance_sunken_temple*)((Player*)pSource)->GetInstanceData())
        {
            ...
            if (pInstance->ProcessStatueEvent(uiEventId))
            {
                ...
                if (GameObject* pLight =
                        GetClosestGameObjectWithEntry((GameObject*)pTarget,
                            GO_ATALAI_LIGHT, INTERACTION_DISTANCE))
                    pInstance->DoRespawnGameObject(
                        pLight->GetObjectGuid(), 30 * MINUTE);
            }
            else
            {
                // If the wrong statue was activated, then trigger trap
                // We don't know actually which trap goes to which statue so we
                // need to search for each
                if (GameObject* pTrap =
                        GetClosestGameObjectWithEntry((GameObject*)pTarget,
                            GO_ATALAI_TRAP_1, INTERACTION_DISTANCE))
                    pTrap->Use((Unit*)pSource);
                else if (GameObject* pTrap =
                             GetClosestGameObjectWithEntry((GameObject*)pTarget,
                                 GO_ATALAI_TRAP_2, INTERACTION_DISTANCE))
                    pTrap->Use((Unit*)pSource);
                else if (GameObject* pTrap =
                             GetClosestGameObjectWithEntry((GameObject*)pTarget,
                                 GO_ATALAI_TRAP_3, INTERACTION_DISTANCE))
                    pTrap->Use((Unit*)pSource);
            }
...
## event_avatar_of_hakkar
######*/
bool ProcessEventId_event_avatar_of_hakkar(uint32 /*uiEventId*/,
    Object* pSource, Object* /*pTarget*/, bool /*bIsStart*/)
{
    if (pSource->GetTypeId() == TYPEID_PLAYER)
    {
        if (instance_sunken_temple* pInstance =
...
{
    instance_sunken_temple* pInstance =
        (instance_sunken_temple*)pGo->GetInstanceData();
