...
instance_sunken_temple::instance_sunken_temple(Map* pMap)
  : ScriptedInstance(pMap), m_uiProtectorsRemaining(0), m_uiStatueCounter(0),
    m_uiFlameCounter(0), m_uiAvatarSummonTimer(0), m_uiSupressorTimer(0),
    m_bIsFirstHakkarWave(false), m_bCanSummonBloodkeeper(false)
{
    Initialize();
}
...
void instance_sunken_temple::SetData(uint32 uiType, uint32 uiData)
{
        ...
        if (uiData == DONE)
        {
            ...
            if (!m_uiProtectorsRemaining)
            {
                ...
                DoOrSimulateScriptTextForThisInstance(
                    SAY_JAMMALAN_INTRO, NPC_JAMMALAN);
            }
        }
        break;
    case TYPE_JAMMALAN:
    ...
    case TYPE_AVATAR:
        if (uiData == SPECIAL)
        {
            ...
            Creature* pShade =
                GetSingleCreatureFromStorage(NPC_SHADE_OF_HAKKAR);
            if (!pShade)
                return;
            ...
            {
            // Yells on each flame
            // TODO It might be possible that these yells should be ordered
            // randomly, however this is the seen state
            case 1:
            ...
            // Summon timer is confusing random; timers were: 13, 39 and 52
            // secs;
        ...
        if (uiData == IN_PROGRESS)
        {
            ...
            if (Creature* pShade = pPlayer->SummonCreature(NPC_SHADE_OF_HAKKAR,
                    aSunkenTempleLocation[1].m_fX,
                    aSunkenTempleLocation[1].m_fY,
                    aSunkenTempleLocation[1].m_fZ,
                    aSunkenTempleLocation[1].m_fO, TEMPSUMMON_MANUAL_DESPAWN,
                    0))
            {
                m_mNpcEntryGuidStore[NPC_SHADE_OF_HAKKAR] =
                    pShade->GetObjectGuid();
                pShade->SetRespawnDelay(DAY);
            }

            // Respawn circles
            for (GUIDVector::const_iterator itr = m_vuiCircleGUIDs.begin();
                 itr != m_vuiCircleGUIDs.end(); ++itr)
                DoRespawnGameObject(*itr, 30 * MINUTE);
        }
        else if (uiData == FAIL)
        {
            // In case of wipe during the summoning ritual the shade is
            // despawned
            // The trash mobs stay in place, they are not despawned; the avatar
            // is not sure if it's despawned or not but most likely he'll stay
            // in place

            // Despawn the shade and the avatar if needed -- TODO, avatar
            // really?
            if (Creature* pShade =
                    GetSingleCreatureFromStorage(NPC_SHADE_OF_HAKKAR))
...
void instance_sunken_temple::DoSpawnAtalarionIfCan()
{
    ...
    pPlayer->SummonCreature(NPC_ATALARION, aSunkenTempleLocation[0].m_fX,
        aSunkenTempleLocation[0].m_fY, aSunkenTempleLocation[0].m_fZ,
        aSunkenTempleLocation[0].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0);

    // Spawn the idol of Hakkar
    DoRespawnGameObject(GO_IDOL_OF_HAKKAR, 30 * MINUTE);

    // Spawn the big green lights
    for (GUIDList::const_iterator itr = m_luiBigLightGUIDs.begin();
         itr != m_luiBigLightGUIDs.end(); ++itr)
        DoRespawnGameObject(*itr, 30 * MINUTE);
}
...
{
    for (GUIDList::const_iterator itr = m_luiFlameGUIDs.begin();
         itr != m_luiFlameGUIDs.end(); ++itr)
...
void instance_sunken_temple::Update(uint32 uiDiff)
{
            ...
            if (m_bIsFirstHakkarWave) // First wave summoned
            {
                     ...
                     itr != m_vuiCircleGUIDs.end(); ++itr)
                {
                    if (GameObject* pCircle = instance->GetGameObject(*itr))
                        pShade->SummonCreature(NPC_HAKKARI_MINION,
                            pCircle->GetX(), pCircle->GetY(), pCircle->GetZ(),
                            0, TEMPSUMMON_DEAD_DESPAWN, 0);
                }

                // Summon Bloodkeeper at random circle
                if (GameObject* pCircle =
                        instance->GetGameObject(m_vuiCircleGUIDs[urand(
                            0, m_vuiCircleGUIDs.size() - 1)]))
                    pShade->SummonCreature(NPC_BLOODKEEPER, pCircle->GetX(),
                        pCircle->GetY(), pCircle->GetZ(), 0,
    ...
    if (m_uiSupressorTimer)
    {
        if (m_uiSupressorTimer <= uiDiff)
        {
            ...
            pShade->SummonCreature(NPC_SUPPRESSOR,
                aHakkariDoorLocations[uiSummonLoc].m_fX,
                aHakkariDoorLocations[uiSummonLoc].m_fY,
                aHakkariDoorLocations[uiSummonLoc].m_fZ, 0,
                TEMPSUMMON_DEAD_DESPAWN, 0);
