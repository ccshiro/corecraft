...
enum
{
    ...
    NPC_SHADE_OF_HAKKAR = 8440, // Shade of Hakkar appears when the event
                                // starts; will despawn when avatar of hakkar is
                                // summoned
    NPC_BLOODKEEPER = 8438, // Spawned rarely and contains the hakkari blood ->
                            // used to extinguish the flames
    NPC_HAKKARI_MINION = 8437, // Npc randomly spawned during the event = trash
    NPC_SUPPRESSOR =
        8497, // Npc summoned at one of the two doors and moves to the boss;

    NPC_MALFURION = 15362,

    GO_ALTAR_OF_HAKKAR = 148836, // Used in order to show the player the order
                                 // of the statue activation
    GO_IDOL_OF_HAKKAR =
        148838, // Appears when atalarion is summoned; this was removed in 4.0.1

    GO_ATALAI_LIGHT =
        148883, // Green light, activates when the correct statue is chosen
    GO_ATALAI_LIGHT_BIG = 148937, // Big light, used at the altar event

    GO_ATALAI_TRAP_1 =
        177484, // Trapps triggered if the wrong statue is activated
    GO_ATALAI_TRAP_2 = 177485, // The traps are spawned in DB randomly around
                               // the statues (we don't know exactly which
                               // statue has which trap)
    ...
    GO_EVIL_CIRCLE = 148998, // Objects used at the avatar event. they are
                             // spawned when the event starts, and the mobs are
                             // summon atop of them
    ...
    SPELL_SUMMON_AVATAR =
        12639, // Cast by the shade of hakkar, updates entry to avatar
...
static const uint32 m_aAtalaiStatueEvents[MAX_STATUES] = {EVENT_ID_STATUE_1,
    EVENT_ID_STATUE_2, EVENT_ID_STATUE_3, EVENT_ID_STATUE_4, EVENT_ID_STATUE_5,
    EVENT_ID_STATUE_6};
...
static const SummonLocations aHakkariDoorLocations[2] = {
    {-420.629f, 276.682f, -90.827f, 0}, {-512.015f, 276.134f, -90.827f, 0}};
