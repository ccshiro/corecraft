...
struct MANGOS_DLL_DECL npc_galen_goodwardAI : public npc_escortAI
{
    ...
    void Aggro(Unit* pWho) override
    {
        if (HasEscortState(STATE_ESCORT_ESCORTING))
            DoScriptText(urand(0, 1) ? SAY_ATTACKED_1 : SAY_ATTACKED_2,
                m_creature, pWho);
    }
    ...
    void WaypointStart(uint32 uiPointId) override
    {
        ...
        case 0:
        {
            ...
            if (m_galensCageGuid)
                ...
                pCage = GetClosestGameObjectWithEntry(
                    m_creature, GO_GALENS_CAGE, INTERACTION_DISTANCE);
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 0:
            if (GameObject* pCage =
                    m_creature->GetMap()->GetGameObject(m_galensCageGuid))
...
bool QuestAccept_npc_galen_goodward(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_GALENS_ESCAPE)
    {
        if (npc_galen_goodwardAI* pEscortAI =
                dynamic_cast<npc_galen_goodwardAI*>(pCreature->AI()))
        {
