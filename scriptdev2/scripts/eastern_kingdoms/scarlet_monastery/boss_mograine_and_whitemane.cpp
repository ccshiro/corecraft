...
struct MANGOS_DLL_DECL boss_scarlet_commander_mograineAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        if (Creature* pWhitemane =
                m_pInstance->GetSingleCreatureFromStorage(NPC_WHITEMANE))
        {
    ...
    void DamageTaken(Unit* /*pDoneBy*/, uint32& uiDamage) override
    {
                ...
                m_pInstance->GetSingleCreatureFromStorage(NPC_WHITEMANE))
        {
            ...
            pWhitemane->movement_gens.push(new movement::PointMovementGenerator(
                1, 1163.113370f, 1398.856812f, 32.527786f, true, true));
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_bHasDied && !m_bHeal && m_pInstance &&
            m_pInstance->GetData(TYPE_MOGRAINE_AND_WHITE_EVENT) == SPECIAL)
        {
            // On ressurection, stop fake death and heal whitemane and resume
            // fight
            if (Creature* pWhitemane =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_WHITEMANE))
            {
...
struct MANGOS_DLL_DECL boss_high_inquisitor_whitemaneAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        if (Creature* pMograine =
                m_pInstance->GetSingleCreatureFromStorage(NPC_MOGRAINE))
        {
    ...
    void JustReachedHome() override
    {
        if (m_pInstance)
        {
            if (!(m_pInstance->GetData(TYPE_MOGRAINE_AND_WHITE_EVENT) ==
                    NOT_STARTED) ||
                !(m_pInstance->GetData(TYPE_MOGRAINE_AND_WHITE_EVENT) == FAIL))
    ...
    {
        // This needs to be empty because Whitemane should NOT aggro while
        // fighting Mograine. Mograine will give us a target.
    }
            ...
            if (m_pInstance)
            {
                        ...
                        m_pInstance->GetSingleCreatureFromStorage(NPC_MOGRAINE))
                {
                    if (pMograine->isAlive() &&
                        pMograine->GetHealthPercent() <= 75.0f)
                        pTarget = pMograine;
                }
            }
