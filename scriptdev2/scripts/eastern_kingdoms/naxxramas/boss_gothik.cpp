...
struct MANGOS_DLL_DECL boss_gothikAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        m_uiPhaseTimer =
            ...
            7 * IN_MILLISECONDS; // last summon at 4:04, next would be 4:09
        m_uiControlZoneTimer =
            urand(120 * IN_MILLISECONDS, 150 * IN_MILLISECONDS);
        m_uiSpeechTimer = 1 * IN_MILLISECONDS;

        // Despawn Adds
        for (GUIDList::const_iterator itr = m_lSummonedAddGuids.begin();
             itr != m_lSummonedAddGuids.end(); itr++)
        {
    ...
    void Aggro(Unit* /*pWho*/) override
    {
        ...
        // Make immune
        m_creature->ApplySpellImmune(
            0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_ALL, true);
        ...
        for (auto& elem : lSummonList)
        {
            if (elem)
            {
                if (uiTraineeCount == 0)
                    break;
                if (index == 1)
                    m_lRiderSummonPosGuids.push_back((elem)->GetObjectGuid());
                else
                {
                    m_lTraineeSummonPosGuids.push_back((elem)->GetObjectGuid());
                    --uiTraineeCount;
                }
                index++;
            }
        ...
        for (auto& elem : lSummonList)
        {
            if (elem)
            {
                if (uiDeathKnightCount == 0)
                    break;
                m_lDeathKnightSummonPosGuids.push_back((elem)->GetObjectGuid());
                --uiDeathKnightCount;
            }
        }
    }
    ...
    void SummonAdds(bool /*bRightSide*/, uint32 uiSummonEntry)
    {
        ...
        {
            if (Creature* pPos =
                    m_creature->GetMap()->GetCreature(plSummonPosGuid))
                m_creature->SummonCreature(uiSummonEntry, pPos->GetX(),
                    pPos->GetY(), pPos->GetZ(), pPos->GetO(),
    ...
    void SummonedCreatureJustDied(Creature* pSummoned) override
    {
        ...
        if (Creature* pAnchor =
                m_pInstance->GetClosestAnchorForGoth(pSummoned, true))
        {
            switch (pSummoned->GetEntry())
            {
            // Wrong caster, it expected to be pSummoned.
            // Mangos deletes the spell event at caster death, so for delayed
            // spell like this
            // it's just a workaround. Does not affect other than the visual
            // though (+ spell takes longer to "travel")
            case NPC_UNREL_TRAINEE:
                m_creature->CastSpell(pAnchor, SPELL_A_TO_ANCHOR_1, true, NULL,
                    NULL, pSummoned->GetObjectGuid());
                break;
            case NPC_UNREL_DEATH_KNIGHT:
                m_creature->CastSpell(pAnchor, SPELL_B_TO_ANCHOR_1, true, NULL,
                    NULL, pSummoned->GetObjectGuid());
                break;
            case NPC_UNREL_RIDER:
                m_creature->CastSpell(pAnchor, SPELL_C_TO_ANCHOR_1, true, NULL,
                    NULL, pSummoned->GetObjectGuid());
                break;
            }
        }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        case PHASE_STOP_SUMMONING:
            if (m_uiPhaseTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT_RIGHT,
                        CAST_TRIGGERED) == CAST_OK)
                {
                    m_uiPhase = m_pInstance ? PHASE_TELEPORTING :
                                              PHASE_STOP_TELEPORTING;
        ...
        case PHASE_TELEPORTING: // Phase is only reached if m_pInstance is valid
            ...
            {
                uint32 uiTeleportSpell =
                    m_pInstance->IsInRightSideGothArea(m_creature) ?
                        SPELL_TELEPORT_LEFT :
                        SPELL_TELEPORT_RIGHT;
            ...
            if (m_creature->GetHealthPercent() <= 30.0f)
            {
                ...
                ProcessCentralDoor();
                // as the doors now open, recheck whether mobs are standing
                // around
        ...
        case PHASE_STOP_TELEPORTING:
            ...
            else if (!m_creature->IsNonMeleeSpellCasted(true))
            {
                ...
                if (Unit* pTarget = m_creature->SelectAttackingTarget(
                        ATTACKING_TARGET_TOPAGGRO, 0, SPELL_SHADOWBOLT,
                        SELECT_FLAG_IN_LOS))
            ...
            if (m_uiControlZoneTimer <= uiDiff)
            {
                ...
                if (m_pInstance && !HasPlayersInLeftSide())
                {
                    ...
                    for (GUIDList::const_iterator itr =
                             m_lSummonedAddGuids.begin();
                         itr != m_lSummonedAddGuids.end(); itr++)
                    {
                        if (Creature* pCreature =
                                m_pInstance->instance->GetCreature(*itr))
                        {
    ...
    SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
    if (uiEffIndex != EFFECT_INDEX_0 ||
        pCreatureTarget->GetEntry() != NPC_SUB_BOSS_TRIGGER)
        return true;
    ...
    {
        if (Creature* pAnchor2 =
                pInstance->GetClosestAnchorForGoth(pCreatureTarget, false))
        {
    ...
    {
        if (Creature* pGoth =
                pInstance->GetSingleCreatureFromStorage(NPC_GOTHIK))
        {
            ...
            pGoth->SummonCreature(uiNpcEntry, pCreatureTarget->GetX(),
                pCreatureTarget->GetY(), pCreatureTarget->GetZ(),
                pCreatureTarget->GetO(), TEMPSUMMON_DEAD_DESPAWN, 0);

            if (uiNpcEntry == NPC_SPECT_RIDER)
                pGoth->SummonCreature(NPC_SPECT_HORSE, pCreatureTarget->GetX(),
                    pCreatureTarget->GetY(), pCreatureTarget->GetZ(),
                    pCreatureTarget->GetO(), TEMPSUMMON_DEAD_DESPAWN, 0);
        }
        return true;
