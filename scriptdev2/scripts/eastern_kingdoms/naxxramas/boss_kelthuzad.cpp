...
SD%Complete: 75
SDComment: Timers will need adjustments, along with tweaking positions and
amounts
...
// - what happens if raid fail, can they start the event as soon after as they
// want?
...
static float M_F_RANGE =
    55.0f; // ~ range from center of chamber to center of alcove
...
struct MANGOS_DLL_DECL boss_kelthuzadAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        m_uiFrostBoltTimer = urand(
            1000, 60000); // It won't be more than a minute without cast it
        m_uiFrostBoltNovaTimer = 15000; // Cast every 15 seconds
        m_uiChainsTimer =
            urand(30000, 60000); // Cast no sooner than once every 30 seconds
        m_uiManaDetonationTimer = 20000; // Seems to cast about every 20 seconds
        m_uiShadowFissureTimer = 25000;  // 25 seconds
        m_uiFrostBlastTimer =
            urand(30000, 60000);   // Random time between 30-60 seconds
        m_uiGuardiansTimer = 5000; // 5 seconds for summoning each Guardian of
                                   // Icecrown in phase 3
        ...
        // it may be some spell should be used instead, to control the intro
        // phase
    ...
    void DespawnIntroCreatures()
    {
        if (m_pInstance)
        {
            for (const auto& elem : m_lIntroMobsSet)
            {
                if (Creature* pCreature =
                        m_pInstance->instance->GetCreature(elem))
    ...
    void DespawnAdds()
    {
        if (m_pInstance)
        {
            for (const auto& elem : m_lAddsSet)
            {
                if (Creature* pCreature =
                        m_pInstance->instance->GetCreature(elem))
                {
    ...
    void SummonIntroCreatures(uint32 packId)
    {
        ...
        for (uint8 uiI = 0; uiI < 14; ++uiI)
        {
            ...
            auto pos = m_creature->GetPointXYZ(G3D::Vector3(fX, fY, fZ),
                2 * M_PI_F * rand_norm_f(), 12.0f * rand_norm_f());
            m_creature->SummonCreature(uiNpcEntry, pos.x, pos.y, pos.z,
                fAngle + M_PI_F, TEMPSUMMON_CORPSE_DESPAWN, 5000);
        }
    }
    ...
    void SummonMob(uint32 uiType)
    {
        ...
        m_creature->SummonCreature(
            uiType, fX, fY, fZ, 0.0f, TEMPSUMMON_CORPSE_DESPAWN, 5000);
    }
    ...
    {
        if (type == movement::gen::point && uiPointId == 0)
            pSummoned->SetInCombatWithZone();
    }
        ...
        else // normal or guardian phase
        {
            if (m_uiFrostBoltTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature->getVictim(),
                        m_bIsRegularMode ? SPELL_FROST_BOLT :
                                           SPELL_FROST_BOLT_H) == CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(m_creature->getVictim(),
                        m_bIsRegularMode ? SPELL_FROST_BOLT_NOVA :
                                           SPELL_FROST_BOLT_NOVA_H) == CAST_OK)
            ...
            if (m_uiManaDetonationTimer < uiDiff)
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0, SPELL_MANA_DETONATION,
                        SELECT_FLAG_PLAYER | SELECT_FLAG_POWER_MANA))
                {
                    if (DoCastSpellIfCan(pTarget, SPELL_MANA_DETONATION) ==
                        CAST_OK)
                    {
                        if (urand(0, 1))
            ...
            {
                if (DoCastSpellIfCan(
                        m_creature->getVictim(), SPELL_FROST_BLAST) == CAST_OK)
                {
                    if (urand(0, 1))
            ...
            if (!m_bIsRegularMode)
            {
                ...
                {
                    if (DoCastSpellIfCan(m_creature->getVictim(),
                            SPELL_CHAINS_OF_KELTHUZAD) == CAST_OK)
                    {
                        DoScriptText(
                            urand(0, 1) ? SAY_CHAIN1 : SAY_CHAIN2, m_creature);
            ...
            else if (m_uiPhase == PHASE_GUARDIANS &&
                     m_uiGuardiansCount < m_uiGuardiansCountMax)
            {
