...
enum
{
    ...
    // SOUND_RANDOM_AGGRO       = 8955,                     // soundId
    // containing the 4 aggro sounds, we not using this
...
struct MANGOS_DLL_DECL boss_faerlinaAI : public ScriptedAI
{
    ...
    // Widow's Embrace prevents frenzy and poison bolt, if it removes frenzy,
    // next frenzy is sceduled in 60s
    // It is likely that this _should_ be handled with some dummy aura(s) - but
    // couldn't find any
    void SpellHit(Unit* /*pCaster*/, const SpellEntry* pSpellEntry) override
    {
        ...
        if (pSpellEntry->Id == SPELL_WIDOWS_EMBRACE ||
            pSpellEntry->Id == SPELL_WIDOWS_EMBRACE_H)
        {
            // If we remove the Frenzy, the Enrage Timer is reseted to 60s
            if (m_creature->has_aura(
                    m_bIsRegularMode ? SPELL_ENRAGE : SPELL_ENRAGE_H))
            {
                m_uiEnrageTimer = 60000;
            ...
            // In any case we prevent Frenzy and Poison Bolt Volley for Widow's
            // Embrace Duration (30s)
            // We do this be setting the timers to at least bigger than 30s
            m_uiEnrageTimer = std::max(m_uiEnrageTimer, (uint32)30000);
            m_uiPoisonBoltVolleyTimer =
                std::max(m_uiPoisonBoltVolleyTimer, urand(33000, 38000));
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiPoisonBoltVolleyTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature->getVictim(),
                    m_bIsRegularMode ? SPELL_POSIONBOLT_VOLLEY :
                                       SPELL_POSIONBOLT_VOLLEY_H) == CAST_OK)
