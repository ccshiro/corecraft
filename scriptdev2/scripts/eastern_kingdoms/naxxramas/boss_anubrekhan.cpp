...
enum
{
    ...
    SPELL_IMPALE =
        28783, // May be wrong spell id. Causes more dmg than I expect
    SPELL_IMPALE_H = 56090,
    SPELL_LOCUSTSWARM =
        28785, // This is a self buff that triggers the dmg debuff
    SPELL_LOCUSTSWARM_H = 54021,

    // spellId invalid
    SPELL_SUMMONGUARD = 29508, // Summons 1 crypt guard at targeted location

    SPELL_SELF_SPAWN_5 = 29105, // This spawns 5 corpse scarabs ontop of us
                                // (most likely the pPlayer casts this on death)
    SPELL_SELF_SPAWN_10 =
        28864, // This is used by the crypt guards when they die
...
struct MANGOS_DLL_DECL boss_anubrekhanAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        m_uiLocustSwarmTimer = urand(80000, 120000); // Random time between 80
                                                     // seconds and 2 minutes
                                                     // for initial cast
        m_uiSummonTimer = m_uiLocustSwarmTimer +
                          45000; // 45 seconds after initial locust swarm
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiImpaleTimer < uiDiff)
        {
            ...
            if (!m_creature->has_aura(SPELL_LOCUSTSWARM) &&
                !m_creature->has_aura(SPELL_LOCUSTSWARM_H))
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0))
                    DoCastSpellIfCan(pTarget,
                        m_bIsRegularMode ? SPELL_IMPALE : SPELL_IMPALE_H);
            }
        ...
        if (m_uiLocustSwarmTimer < uiDiff)
        {
            DoCastSpellIfCan(m_creature,
                m_bIsRegularMode ? SPELL_LOCUSTSWARM : SPELL_LOCUSTSWARM_H);
