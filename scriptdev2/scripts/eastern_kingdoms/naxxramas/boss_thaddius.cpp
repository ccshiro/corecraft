...
enum
{
    ...
    SAY_ELECT = -1533034,
    SAY_DEATH = -1533035,
    // Background screams in Instance if Thaddius still alive, needs general
    // support most likely
    SAY_SCREAM1 = -1533036,
    SAY_SCREAM2 = -1533037,
    SAY_SCREAM3 = -1533038,
    ...
    SPELL_BESERK = 27680,
    SPELL_CLEAR_CHARGES =
        63133, // TODO NYI, cast on death, most likely to remove remaining buffs

    // Stalagg & Feugen Spells
    // SPELL_WARSTOMP                  = 28125,              // Not used in
    // Wotlk Version
...
// Actually this boss behaves like a NoMovement Boss (SPELL_BALL_LIGHTNING) -
// but there are some movement packages used, unknown what this means!
struct MANGOS_DLL_DECL boss_thaddiusAI : public ScriptedAI
{
    ...
    void JustReachedHome() override
    {
        if (m_pInstance)
        {
            ...
            // Respawn Adds:
            Creature* pFeugen =
                m_pInstance->GetSingleCreatureFromStorage(NPC_FEUGEN);
            Creature* pStalagg =
                m_pInstance->GetSingleCreatureFromStorage(NPC_STALAGG);
            if (pFeugen)
            {
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (DoCastSpellIfCan(m_creature, SPELL_BESERK) ==
                CAST_OK) // allow combat movement?
        ...
        if (m_uiChainLightningTimer < uiDiff)
        {
            ...
            if (pTarget &&
                DoCastSpellIfCan(pTarget,
                    m_bIsRegularMode ? SPELL_CHAIN_LIGHTNING :
        ...
        // TODO: Verify, likely that the boss should attack any enemy in melee
        // range before starting to cast
        if (!m_creature->CanReachWithMeleeAttack(m_creature->getVictim()))
        {
            if (m_uiBallLightningTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature->getVictim(),
                        SPELL_BALL_LIGHTNING) == CAST_OK)
                    m_uiBallLightningTimer = 1 * IN_MILLISECONDS;
            }
            else
...
bool EffectDummyNPC_spell_thaddius_encounter(Unit* /*pCaster*/,
    ...
    case SPELL_SHOCK_OVERLOAD:
        if (uiEffIndex == EFFECT_INDEX_0)
        {
            ...
            if (pCreatureTarget->GetEntry() != NPC_THADDIUS ||
                !pCreatureTarget->has_aura(SPELL_THADIUS_SPAWN))
                return true;
            // remove Stun and then Cast
            pCreatureTarget->remove_auras(SPELL_THADIUS_SPAWN);
            pCreatureTarget->CastSpell(
                pCreatureTarget, SPELL_THADIUS_LIGHTNING_VISUAL, false);
        }
        return true;
    case SPELL_THADIUS_LIGHTNING_VISUAL:
        if (uiEffIndex == EFFECT_INDEX_0 &&
            pCreatureTarget->GetEntry() == NPC_THADDIUS)
        {
            if (instance_naxxramas* pInstance =
                    (instance_naxxramas*)pCreatureTarget->GetInstanceData())
            {
...
struct MANGOS_DLL_DECL npc_tesla_coilAI : public ScriptedAI
{
    ...
    // * Keep Chain spells casted when evading after useless EnterCombat caused
    // by 'buffing' the add
    ...
    bool SetupChain()
    {
        ...
        m_bToFeugen =
            m_creature->GetDistanceOrder(pNoxTeslaFeugen, pNoxTeslaStalagg);

        if (DoCastSpellIfCan(m_creature,
                m_bToFeugen ? SPELL_FEUGEN_CHAIN : SPELL_STALAGG_CHAIN) ==
            CAST_OK)
            return true;
    ...
    void ReApplyChain(uint32 uiEntry)
    {
        if (uiEntry) // called from Stalagg/Feugen with their entry
        {
            ...
            if ((uiEntry == NPC_FEUGEN && !m_bToFeugen) ||
                (uiEntry == NPC_STALAGG && m_bToFeugen))
                return;

            m_bReapply = true; // Reapply Chains on next tick
        }
        else // if called from next tick, needed because otherwise the spell
             // doesn't bind
        {
            m_bReapply = false;
            m_creature->InterruptNonMeleeSpells(true);
            GameObject* pGo = m_pInstance->GetSingleGameObjectFromStorage(
                m_bToFeugen ? GO_CONS_NOX_TESLA_FEUGEN :
                              GO_CONS_NOX_TESLA_STALAGG);

            if (pGo && pGo->GetGoType() == GAMEOBJECT_TYPE_BUTTON &&
                pGo->getLootState() == GO_ACTIVATED)
    ...
    void SetOverloading()
    {
        m_uiOverloadTimer = 14 * IN_MILLISECONDS; // it takes some time to
                                                  // overload and activate
                                                  // Thaddius
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiOverloadTimer)
        {
            ...
            {
                m_uiOverloadTimer = 0;
                m_creature->remove_auras(m_bToFeugen ?
                                             SPELL_FEUGEN_TESLA_PASSIVE :
                                             SPELL_STALAGG_TESLA_PASSIVE);
                DoCastSpellIfCan(
                    m_creature, SPELL_SHOCK_OVERLOAD, CAST_INTERRUPT_PREVIOUS);
                DoScriptText(EMOTE_TESLA_OVERLOAD, m_creature);
                m_pInstance->DoUseDoorOrButton(m_bToFeugen ?
                                                   GO_CONS_NOX_TESLA_FEUGEN :
                                                   GO_CONS_NOX_TESLA_STALAGG);
            }
            else
...
struct MANGOS_DLL_DECL boss_thaddiusAddsAI : public ScriptedAI
{
    ...
    Creature*
    GetOtherAdd() // For Stalagg returns pFeugen, for Feugen returns pStalagg
    {
    ...
    void JustRespawned() override
    {
        ...
        for (GUIDList::const_iterator itr = lTeslaGUIDList.begin();
             itr != lTeslaGUIDList.end(); ++itr)
        {
            if (Creature* pTesla = m_pInstance->instance->GetCreature(*itr))
            {
                if (npc_tesla_coilAI* pTeslaAI =
                        dynamic_cast<npc_tesla_coilAI*>(pTesla->AI()))
    ...
    void JustReachedHome() override
    {
        ...
        {
            if (boss_thaddiusAddsAI* pOtherAI =
                    dynamic_cast<boss_thaddiusAddsAI*>(pOther->AI()))
            {
        ...
        if (!m_creature->has_aura(SPELL_FEUGEN_CHAIN) &&
            !m_creature->has_aura(SPELL_STALAGG_CHAIN))
            JustRespawned();
    ...
    bool IsCountingDead() { return m_bFakeDeath || m_creature->isDead(); }
    ...
    virtual void UpdateAddAI(const uint32 /*uiDiff*/) {} // Used for
                                                         // Add-specific spells
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
            ...
            if (m_uiReviveTimer < uiDiff)
            {
                if (Creature* pOther = GetOtherAdd())
                {
                    if (boss_thaddiusAddsAI* pOtherAI =
                            dynamic_cast<boss_thaddiusAddsAI*>(pOther->AI()))
                    {
                        if (!pOtherAI->IsCountingDead()) // Raid was to slow to
                                                         // kill the second add
                            Revive();
                        else
                        {
                            m_bBothDead =
                                true; // Now both adds are counting dead
                            pOtherAI->m_bBothDead = true;
                            // Set both Teslas to overload
                            GUIDList lTeslaGUIDList;
                            m_pInstance->GetThadTeslaCreatures(lTeslaGUIDList);
                            for (GUIDList::const_iterator itr =
                                     lTeslaGUIDList.begin();
                                 itr != lTeslaGUIDList.end(); ++itr)
                            {
                                if (Creature* pTesla =
        ...
        if (m_uiHoldTimer) // A short timer preventing combat movement after
                           // revive
        {
        ...
        if (m_uiWarStompTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature, SPELL_WARSTOMP) == CAST_OK)
                m_uiWarStompTimer = urand(8*IN_MILLISECONDS,
        10*IN_MILLISECONDS);
        }
        else
    ...
    void DamageTaken(Unit* pKiller, uint32& uiDamage) override
    {
        ...
        m_creature->remove_auras_if([this](AuraHolder* h)
            {
                return !(h->IsPassive() && h->GetCaster() == m_creature &&
                           m_creature->player_or_pet()) &&
                       !h->IsDeathPersistent();
            });
...
struct MANGOS_DLL_DECL boss_stalaggAI : public boss_thaddiusAddsAI
{
    ...
    void UpdateAddAI(const uint32 uiDiff) override
    {
        if (m_uiPowerSurgeTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature,
                    m_bIsRegularMode ? SPELL_POWERSURGE : SPELL_POWERSURGE_H) ==
                CAST_OK)
                m_uiPowerSurgeTimer =
...
struct MANGOS_DLL_DECL boss_feugenAI : public boss_thaddiusAddsAI
{
    ...
    void UpdateAddAI(const uint32 uiDiff) override
    {
        ...
        {
            if (DoCastSpellIfCan(m_creature,
                    m_bIsRegularMode ? SPELL_STATIC_FIELD :
                                       SPELL_STATIC_FIELD_H) == CAST_OK)
                m_uiStaticFieldTimer =
                    urand(10 * IN_MILLISECONDS, 15 * IN_MILLISECONDS);
        }
        else
