...
struct MANGOS_DLL_DECL boss_gluthAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            DoCastSpellIfCan(
                m_creature, m_bIsRegularMode ? SPELL_ENRAGE : SPELL_ENRAGE_H);
            m_uiEnrageTimer = 60000;
        }
        else
        ...
        {
            if (Creature* pZombie =
                    m_creature->SummonCreature(NPC_ZOMBIE_CHOW, ADD_1X, ADD_1Y,
                        ADD_1Z, 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 80000))
            {
            ...
            if (!m_bIsRegularMode)
            {
                if (Creature* pZombie = m_creature->SummonCreature(
                        NPC_ZOMBIE_CHOW, ADD_1X, ADD_1Y, ADD_1Z, 0.0f,
                        TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 80000))
                {
                    if (Unit* pTarget = m_creature->SelectAttackingTarget(
                            ATTACKING_TARGET_RANDOM, 0))
                        pZombie->AddThreat(pTarget);
