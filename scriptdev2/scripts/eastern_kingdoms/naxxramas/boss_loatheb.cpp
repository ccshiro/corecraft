...
struct MANGOS_DLL_DECL boss_loathebAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        m_uiSummonTimer =
            urand(10000, 15000); // first seen in vid after approx 12s
        m_uiBerserkTimer =
            MINUTE * 12 * IN_MILLISECONDS; // only in heroic, after 12min
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiInevitableDoomTimer < uiDiff)
        {
            DoCastSpellIfCan(m_creature, m_bIsRegularMode ?
                                             ...
                                             SPELL_INEVITABLE_DOOM_H);
            m_uiInevitableDoomTimer =
                (m_uiNecroticAuraCount <= 40) ? 30000 : 15000;
        }
        else
        ...
        if (m_uiDeathbloomTimer < uiDiff)
        {
            DoCastSpellIfCan(m_creature,
                m_bIsRegularMode ? SPELL_DEATHBLOOM : SPELL_DEATHBLOOM_H);
