...
static const DialogueEntry aNaxxDialogue[] = {
    ...
    {SAY_SAPP_DIALOG5, NPC_KELTHUZAD, 0}, {0, 0, 0}};

instance_naxxramas::instance_naxxramas(Map* pMap)
  : ScriptedInstance(pMap), m_fChamberCenterX(0.0f), m_fChamberCenterY(0.0f),
    m_fChamberCenterZ(0.0f), m_uiTauntTimer(0), m_dialogueHelper(aNaxxDialogue)
{
    Initialize();
}
...
void instance_naxxramas::OnObjectCreate(GameObject* pGo)
{
    ...
    default:
        // Heigan Traps - many different entries which are only required for
        // sorting
        if (pGo->GetGoType() == GAMEOBJECT_TYPE_TRAP)
        {
            uint32 uiGoEntry = pGo->GetEntry();

            if ((uiGoEntry >= 181517 && uiGoEntry <= 181524) ||
                uiGoEntry == 181678)
                m_alHeiganTrapGuids[0].push_back(pGo->GetObjectGuid());
            else if ((uiGoEntry >= 181510 && uiGoEntry <= 181516) ||
                     (uiGoEntry >= 181525 && uiGoEntry <= 181531) ||
                     uiGoEntry == 181533 || uiGoEntry == 181676)
                m_alHeiganTrapGuids[1].push_back(pGo->GetObjectGuid());
            else if ((uiGoEntry >= 181534 && uiGoEntry <= 181544) ||
                     uiGoEntry == 181532 || uiGoEntry == 181677)
...
{
    if (pCreature->GetEntry() == NPC_MR_BIGGLESWORTH &&
        m_auiEncounter[TYPE_KELTHUZAD] != DONE)
        DoOrSimulateScriptTextForThisInstance(
            SAY_KELTHUZAD_CAT_DIED, NPC_KELTHUZAD);
}
...
void instance_naxxramas::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_FOUR_HORSEMEN:
        ...
        if (uiData == DONE)
        {
            ...
            DoRespawnGameObject(instance->IsRegularDifficulty() ?
                                    GO_CHEST_HORSEMEN_NORM :
                                    GO_CHEST_HORSEMEN_HERO,
                30 * MINUTE);
            m_uiTauntTimer = 5000;
        }
        break;
...
void instance_naxxramas::SetGothTriggers()
{
    ...
    for (GUIDList::const_iterator itr = m_lGothTriggerList.begin();
         itr != m_lGothTriggerList.end(); ++itr)
    {
        if (Creature* pTrigger = instance->GetCreature(*itr))
        {
            GothTrigger pGt;
            pGt.bIsAnchorHigh = (pTrigger->GetZ() >= (pGoth->GetZ() - 5.0f));
...
Creature* instance_naxxramas::GetClosestAnchorForGoth(
    Creature* pSource, bool bRightSide)
{
    std::list<Creature*> lList;

    for (auto& elem : m_mGothTriggerMap)
    {
        if (!elem.second.bIsAnchorHigh)
            continue;

        if (elem.second.bIsRightSide != bRightSide)
            continue;

        if (Creature* pCreature = instance->GetCreature(elem.first))
...
void instance_naxxramas::GetGothSummonPointCreatures(
    std::list<Creature*>& lList, bool bRightSide)
{
    for (auto& elem : m_mGothTriggerMap)
    {
        if (elem.second.bIsAnchorHigh)
...
{
    if (GameObject* pCombatGate =
            GetSingleGameObjectFromStorage(GO_MILI_GOTH_COMBAT_GATE))
        return (pCombatGate->GetY() >= pUnit->GetY());

    logging.error("SD2: left/right side check, Gothik combat area failed.");
    return true;
}

void instance_naxxramas::DoTriggerHeiganTraps(
    Creature* pHeigan, uint32 uiAreaIndex)
{
    if (uiAreaIndex >= MAX_HEIGAN_TRAP_AREAS)
        return;

    for (
        GUIDList::const_iterator itr = m_alHeiganTrapGuids[uiAreaIndex].begin();
        itr != m_alHeiganTrapGuids[uiAreaIndex].end(); ++itr)
    {
...
void instance_naxxramas::DoTaunt()
{
    if (m_auiEncounter[TYPE_KELTHUZAD] != DONE)
    {
        ...
        case 1:
            DoOrSimulateScriptTextForThisInstance(
                SAY_KELTHUZAD_TAUNT1, NPC_KELTHUZAD);
            break;
        case 2:
            DoOrSimulateScriptTextForThisInstance(
                SAY_KELTHUZAD_TAUNT2, NPC_KELTHUZAD);
            break;
        case 3:
            DoOrSimulateScriptTextForThisInstance(
                SAY_KELTHUZAD_TAUNT3, NPC_KELTHUZAD);
            break;
        case 4:
            DoOrSimulateScriptTextForThisInstance(
                SAY_KELTHUZAD_TAUNT4, NPC_KELTHUZAD);
            break;
        }
    }
}
...
bool AreaTrigger_at_naxxramas(Player* pPlayer, AreaTriggerEntry const* pAt)
{
    if (pAt->id == AREATRIGGER_KELTHUZAD)
    {
        ...
        instance_naxxramas* pInstance =
            (instance_naxxramas*)pPlayer->GetInstanceData();
        ...
        {
            if (Creature* pKelthuzad =
                    pInstance->GetSingleCreatureFromStorage(NPC_KELTHUZAD))
            {
    ...
    if (pAt->id == AREATRIGGER_THADDIUS_DOOR)
    {
                ...
                (instance_naxxramas*)pPlayer->GetInstanceData())
        {
            ...
            {
                if (Creature* pThaddius =
                        pInstance->GetSingleCreatureFromStorage(NPC_THADDIUS))
                {
