...
SD%Complete: 80
SDComment: Timer need more care; Spells of Adds (Posion Cloud) need Mangos
Fixes, and further handling
...
struct MANGOS_DLL_DECL boss_grobbulusAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        m_uiPoisonCloudTimer =
            urand(20 * IN_MILLISECONDS, 25 * IN_MILLISECONDS);
        ...
        m_uiSlimeStreamTimer =
            5 *
            IN_MILLISECONDS; // The first few secs it is ok to be out of range
    }
    ...
    // This custom selecting function, because we only want to select players
    // without mutagen aura
    bool DoCastMutagenInjection()
    {
        ...
        {
            if (Unit* pTarget =
                    m_creature->GetMap()->GetUnit((*itr)->getUnitGuid()))
            {
                if (pTarget->GetTypeId() == TYPEID_PLAYER &&
                    !pTarget->has_aura(SPELL_MUTATING_INJECTION))
    ...
    {
        if ((pSpell->Id == SPELL_SLIME_SPRAY ||
                pSpell->Id == SPELL_SLIME_SPRAY_H) &&
            pTarget->GetTypeId() == TYPEID_PLAYER)
            m_creature->SummonCreature(NPC_FALLOUT_SLIME, pTarget->GetX(),
                pTarget->GetY(), pTarget->GetZ(), 0.0f,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiSlimeSprayTimer < uiDiff)
        {
            ...
            {
                m_uiSlimeSprayTimer =
                    urand(30 * IN_MILLISECONDS, 60 * IN_MILLISECONDS);
        ...
        if (m_uiInjectionTimer < uiDiff)
        {
            ...
            {
                // Timer dependend on time of encounter  - on enrage time
                // between 5-8s, heroic 2-5s (TODO no reliable source for
                // heroic)
                if (m_bIsRegularMode)
                    m_uiInjectionTimer =
                        urand(10 * IN_MILLISECONDS, 13 * IN_MILLISECONDS) -
                        5 * (m_uiBerserkTimeSecs * IN_MILLISECONDS -
                                m_uiBerserkTimer) /
                            m_uiBerserkTimeSecs;
                else
                    m_uiInjectionTimer =
                        urand(10 * IN_MILLISECONDS, 13 * IN_MILLISECONDS) -
                        8 * (m_uiBerserkTimeSecs * IN_MILLISECONDS -
                                m_uiBerserkTimer) /
                            m_uiBerserkTimeSecs;
            }
        }
