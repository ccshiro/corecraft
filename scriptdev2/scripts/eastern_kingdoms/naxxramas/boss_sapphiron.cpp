...
/* Additional comments:
 * Bugged spells:   28560 (needs maxTarget = 1, Summon of 16474 implementation,
 *TODO, 30s duration)
 *                  28526 (needs ScriptEffect to cast 28522 onto random target)
 *
 * Frost-Breath ability: the dummy spell 30101 is self cast, so it won't take
 *the needed delay of ~7s until it reaches its target
 *                       As Sapphiron is displayed visually in hight (hovering),
 *and the spell is cast with target=self-location
 *                       which is still on the ground, the client shows a nice
 *slow falling of the visual animation
 *                       Insisting on using the Dummy-Effect to cast the
 *breath-dmg spells, would require, to relocate Sapphi internally,
 *                       and to hack the targeting to be "on the ground" - Hence
 *the prefered way as it is now!
 */
...
struct MANGOS_DLL_DECL boss_sapphironAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        m_uiBlizzardTimer = 15000; // "Once the encounter starts,based on your
                                   // version of Naxx, this will be used x2 for
                                   // normal and x6 on HC"
        m_uiFlyTimer = 46000;
    ...
    void Aggro(Unit* /*pWho*/) override
    {
        DoCastSpellIfCan(m_creature,
            m_bIsRegularMode ? SPELL_FROST_AURA : SPELL_FROST_AURA_H);
    ...
    {
        if (uiType == movement::gen::point && m_Phase == PHASE_LIFT_OFF)
        {
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        case PHASE_GROUND:
            ...
            if (m_uiTailSweepTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature,
                        m_bIsRegularMode ? SPELL_TAIL_SWEEP :
                                           SPELL_TAIL_SWEEP_H) == CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(m_creature,
                        m_bIsRegularMode ? SPELL_LIFE_DRAIN :
                                           SPELL_LIFE_DRAIN_H) == CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(m_creature, SPELL_SUMMON_BLIZZARD) ==
                    CAST_OK)
            ...
            if (m_creature->GetHealthPercent() > 10.0f)
            {
                if (m_uiFlyTimer < uiDiff)
                {
                    ...
                    /*     aLiftOffPosition[0], aLiftOffPosition[1], */
                    /*     aLiftOffPosition[2]); */
        ...
        case PHASE_AIR_BREATH:
            if (m_uiFrostBreathTimer)
            {
                ...
                {
                    DoCastSpellIfCan(
                        m_creature, SPELL_FROST_BREATH_A, CAST_TRIGGERED);
                    DoCastSpellIfCan(
                        m_creature, SPELL_FROST_BREATH_B, CAST_TRIGGERED);
