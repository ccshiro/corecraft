...
SD%Complete: 85%
SDComment: TODO: Timers and sounds need confirmation - orb handling for
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiUnbalancingStrikeTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature->getVictim(),
                    SPELL_UNBALANCING_STRIKE) == CAST_OK)
        ...
        if (m_uiDisruptingShoutTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature,
                    m_bIsRegularMode ? SPELL_DISRUPTING_SHOUT :
                                       SPELL_DISRUPTING_SHOUT_H) == CAST_OK)
