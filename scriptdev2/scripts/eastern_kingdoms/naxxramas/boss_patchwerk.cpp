...
struct MANGOS_DLL_DECL boss_patchwerkAI : public ScriptedAI
{
    ...
    {
        // The ability is used on highest HP target choosen of the top 2 (3
        // heroic) targets on threat list being in melee range
        Unit* pTarget = NULL;
        ...
        if (tList.size() > 1) // Check if more than two targets, and start loop
                              // with second-most aggro
        {
            ...
            for (; iter != tList.end(); ++iter)
            {
                ...
                if (Unit* pTempTarget =
                        m_creature->GetMap()->GetUnit((*iter)->getUnitGuid()))
                {
        ...
        DoCastSpellIfCan(pTarget,
            m_bIsRegularMode ? SPELL_HATEFULSTRIKE : SPELL_HATEFULSTRIKE_H);
    }
