...
struct MANGOS_DLL_DECL boss_nothAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell) override
    {
        if (pCaster == m_creature &&
            pSpell->Effect[EFFECT_INDEX_0] == SPELL_EFFECT_LEAP)
            DoCastSpellIfCan(
                m_creature, m_bIsRegularMode ? SPELL_CRIPPLE : SPELL_CRIPPLE_H);
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (m_uiPhaseTimer) // After PHASE_SKELETON_3 we won't have a
                                // balcony phase
            {
            ...
            if (!m_bIsRegularMode) // Blink is used only in 25man
            {
                ...
                {
                    static uint32 const auiSpellBlink[4] = {SPELL_BLINK_1,
                        SPELL_BLINK_2, SPELL_BLINK_3, SPELL_BLINK_4};

                    if (DoCastSpellIfCan(
                            m_creature, auiSpellBlink[urand(0, 3)]) == CAST_OK)
                    {
                        DoResetThreat();
                        m_uiBlinkTimer = 25000;
                    }
            ...
            {
                DoCastSpellIfCan(m_creature, m_bIsRegularMode ?
                                                 SPELL_CURSE_PLAGUEBRINGER :
                                                 SPELL_CURSE_PLAGUEBRINGER_H);
            ...
            if (m_uiSummonTimer < uiDiff)
            {
                ...
                if (m_bIsRegularMode)
                {
                    static uint32 const auiSpellSummonPlaguedWarrior[3] = {
                        SPELL_SUMMON_WARRIOR_1, SPELL_SUMMON_WARRIOR_2,
                        SPELL_SUMMON_WARRIOR_3};

                    for (uint8 i = 0; i < 2; ++i)
                        DoCastSpellIfCan(m_creature,
                            auiSpellSummonPlaguedWarrior[urand(0, 2)],
                            CAST_TRIGGERED);
                }
                else
                {
                    DoCastSpellIfCan(
                        m_creature, SPELL_SUMMON_WARRIOR_THREE, CAST_TRIGGERED);
                }
        ...
        else // PHASE_BALCONY
        {
            if (m_uiPhaseTimer < uiDiff)
            {
                if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT_RETURN) ==
                    CAST_OK)
                {
                    ...
                    case PHASE_SKELETON_3:
                        ...
                        DoCastSpellIfCan(
                            m_creature, SPELL_BERSERK, CAST_TRIGGERED);
                        break;
                    }
                    m_uiPhase = PHASE_GROUND;
            ...
            if (m_uiSummonTimer < uiDiff)
            {
                ...
                static uint32 const auiSpellSummonPlaguedChampion[10] = {
                    SPELL_SUMMON_CHAMP01, SPELL_SUMMON_CHAMP02,
                    SPELL_SUMMON_CHAMP03, SPELL_SUMMON_CHAMP04,
                    SPELL_SUMMON_CHAMP05, SPELL_SUMMON_CHAMP06,
                    SPELL_SUMMON_CHAMP07, SPELL_SUMMON_CHAMP08,
                    SPELL_SUMMON_CHAMP09, SPELL_SUMMON_CHAMP10};

                static uint32 const auiSpellSummonPlaguedGuardian[4] = {
                    SPELL_SUMMON_GUARD01, SPELL_SUMMON_GUARD02,
                    SPELL_SUMMON_GUARD03, SPELL_SUMMON_GUARD04};
                ...
                case PHASE_SKELETON_1:
                {
                    for (uint8 i = 0; i < (m_bIsRegularMode ? 2 : 4); ++i)
                        DoCastSpellIfCan(m_creature,
                            auiSpellSummonPlaguedChampion[urand(0, 9)],
                            CAST_TRIGGERED);
                ...
                case PHASE_SKELETON_2:
                {
                    for (uint8 i = 0; i < (m_bIsRegularMode ? 1 : 2); ++i)
                    {
                        DoCastSpellIfCan(m_creature,
                            ...
                            CAST_TRIGGERED);
                        DoCastSpellIfCan(m_creature,
                            auiSpellSummonPlaguedGuardian[urand(0, 3)],
                            CAST_TRIGGERED);
                    }
                    break;
                }
