...
bool GossipHello_npc_shadowfang_prisoner(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pInstance && pInstance->GetData(TYPE_FREE_NPC) != DONE &&
        ...
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_DOOR,
...
bool GossipSelect_npc_shadowfang_prisoner(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        ...
        if (npc_shadowfang_prisonerAI* pEscortAI =
                dynamic_cast<npc_shadowfang_prisonerAI*>(pCreature->AI()))
            pEscortAI->Start();
    }
    return true;
}
...
static const Waypoint VWWaypoints[] = {
    ...
    {-159.547f, 2178.11f, 128.944f}, // When they come back up, they hit this
                                     // point then walk back down
    {-171.113f, 2182.69f, 129.255f},
    {-177.613f, 2175.59f, 128.161f}, {-185.396f, 2178.35f, 126.413f},
    {-184.004f, 2188.31f, 124.122f}, {-172.781f, 2188.71f, 121.611f},
    {-173.245f, 2176.93f, 119.085f}, {-183.145f, 2176.04f, 116.995f},
    {-185.551f, 2185.77f, 114.784f}, {-177.502f, 2190.75f, 112.681f},
    {-171.218f, 2182.61f, 110.314f}, {-173.857f, 2175.1f, 109.255f}};
...
struct MANGOS_DLL_DECL mob_arugal_voidwalkerAI : public ScriptedAI
{
    ...
    uint8 m_uiCurrentPoint, m_uiPosition; // 0 - leader, 1 - behind-right, 2 -
                                          // behind, 3 - behind-left
        ...
        if (pLeader && pLeader->isAlive())
        {
            m_creature->movement_gens.push(
                new movement::FollowMovementGenerator(
                    pLeader, 1.0f, M_PI / 2 * m_uiPosition));
        }
        else
        {
            Creature* pNewLeader = NULL;
            uint8 uiHighestPosition = 0;
            auto lVoidwalkerList = GetCreatureListWithEntryInGrid(
                m_creature, NPC_VOIDWALKER, 50.0f);
            for (auto& elem : lVoidwalkerList)
            {
                if ((elem)->isAlive())
                {
                    if (mob_arugal_voidwalkerAI* pVoidwalkerAI =
                            dynamic_cast<mob_arugal_voidwalkerAI*>(
                                (elem)->AI()))
                    {
            ...
            if (pNewLeader)
            {
                ...
                if (pNewLeader == m_creature)
                {
                    ...
                    m_creature->movement_gens.push(
                        new movement::FollowMovementGenerator(
                            pNewLeader, 1.0f, M_PI / 2 * m_uiPosition));
            }
            else
            {
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_bIsLeader && m_bWPDone)
        {
            m_creature->movement_gens.push(new movement::PointMovementGenerator(
                m_uiCurrentPoint, VWWaypoints[m_uiCurrentPoint].fX,
                VWWaypoints[m_uiCurrentPoint].fY,
                VWWaypoints[m_uiCurrentPoint].fZ, true, false));
            m_bWPDone = false;
        }
    ...
    {
        if (uiMoveType != movement::gen::point || !m_bIsLeader)
            return;
    ...
    uint8 GetPosition() { return m_uiPosition; }

    void SendWaypoint()
    {
        auto lVoidwalkerList =
            GetCreatureListWithEntryInGrid(m_creature, NPC_VOIDWALKER, 50.0f);
        for (auto& elem : lVoidwalkerList)
        {
            if ((elem)->isAlive())
                if (mob_arugal_voidwalkerAI* pVoidwalkerAI =
                        dynamic_cast<mob_arugal_voidwalkerAI*>((elem)->AI()))
                    pVoidwalkerAI->ReceiveWaypoint(
                        m_uiCurrentPoint, m_bReverse);
        }
    }
...
struct MANGOS_DLL_DECL boss_arugalAI : public ScriptedAI
{
    boss_arugalAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        if (pCreature->GetZ() < HEIGHT_FENRUS_ROOM)
        {
    ...
    uint32 m_uiTeleportTimer, m_uiCurseTimer, m_uiVoidboltTimer,
        m_uiThundershockTimer, m_uiYellTimer, m_uiSpeechTimer;
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_bEventMode)
        {
            ...
            if (m_uiSpeechTimer < uiDiff)
            {
                ...
                case 5:
                {
                    ...
                    for (uint8 i = 0; i < 4; ++i)
                    {
                        pVoidwalker = m_creature->SummonCreature(NPC_VOIDWALKER,
                            VWSpawns[i].fX, VWSpawns[i].fY, VWSpawns[i].fZ,
                            VWSpawns[i].fO, TEMPSUMMON_DEAD_DESPAWN, 1);
        ...
        if (m_uiTeleportTimer < uiDiff)
        {
            ...
            case POSITION_SPAWN_LEDGE:
                DoCastSpellIfCan(
                    m_creature, SPELL_SHADOW_PORT_SPAWN_LEDGE, true);
                break;
            case POSITION_UPPER_LEDGE:
                DoCastSpellIfCan(
                    m_creature, SPELL_SHADOW_PORT_UPPER_LEDGE, true);
                break;
    ...
    {
        return (((float)m_creature->GetPower(POWER_MANA) /
                    (float)m_creature->GetMaxPower(POWER_MANA)) *
                100);
    }
    ...
    inline float GetVictimDistance()
    {
        return (m_creature->getVictim() ?
                    m_creature->GetDistance2d(m_creature->getVictim()) :
                    999.9f);
    }
...
struct MANGOS_DLL_DECL npc_arugalAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiSpeechTimer < uiDiff)
        {
            ...
            case 3:
                // Make him die
                if (Creature* pVincent = GetClosestCreatureWithEntry(
                        m_creature, NPC_VINCENT, 20.0f))
...
struct MANGOS_DLL_DECL npc_deathstalker_vincentAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        if (m_pInstance && m_pInstance->GetData(TYPE_INTRO) == DONE &&
            !m_creature->GetByteValue(UNIT_FIELD_BYTES_1, 0))
    ...
    {
        if (m_creature->isInCombat() &&
            m_creature->getFaction() == FACTION_FRIENDLY)
            EnterEvadeMode();
