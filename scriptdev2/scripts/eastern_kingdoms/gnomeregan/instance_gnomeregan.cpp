...
void instance_gnomeregan::Initialize()
{
    ...
    for (auto& elem : m_asBombFaces)
    {
        elem.m_bActivated = false;
        elem.m_uiBombTimer = 0;
    }
}
...
void instance_gnomeregan::OnCreatureCreate(Creature* pCreature)
{
    if (pCreature->GetEntry() == NPC_BLASTMASTER_SHORTFUSE)
        m_mNpcEntryGuidStore[NPC_BLASTMASTER_SHORTFUSE] =
...
{
    return pFirst && pSecond && pFirst->GetY() < pSecond->GetY();
}
...
void instance_gnomeregan::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_GRUBBIS:
        ...
        if (uiData == IN_PROGRESS)
        {
            ...
            if (!m_luiExplosiveChargeGUIDs.empty())
            {
                ...
                for (GUIDList::const_iterator itr =
                         m_luiExplosiveChargeGUIDs.begin();
                     itr != m_luiExplosiveChargeGUIDs.end(); ++itr)
                {
                ...
                GameObject* pCaveInSouth =
                    GetSingleGameObjectFromStorage(GO_CAVE_IN_SOUTH);
                GameObject* pCaveInNorth =
                    GetSingleGameObjectFromStorage(GO_CAVE_IN_NORTH);
                if (pCaveInSouth && pCaveInNorth)
                {
                    for (auto& lExplosiveCharge : lExplosiveCharges)
                    {
                        if ((lExplosiveCharge)
                                ->GetDistanceOrder(
                                    pCaveInSouth, pCaveInNorth) &&
                            uiCounterSouth < MAX_EXPLOSIVES_PER_SIDE)
                        {
                            m_aExplosiveSortedGuids[0][uiCounterSouth] =
                                (lExplosiveCharge)->GetObjectGuid();
                            ++uiCounterSouth;
                        }
                        else if (uiCounterNorth < MAX_EXPLOSIVES_PER_SIDE)
                        {
                            m_aExplosiveSortedGuids[1][uiCounterNorth] =
                                (lExplosiveCharge)->GetObjectGuid();
                            ++uiCounterNorth;
                        }
                    }
        ...
        if (uiData == DONE)
        {
            for (GUIDList::const_iterator itr = m_lRedRocketGUIDs.begin();
                 itr != m_lRedRocketGUIDs.end(); ++itr)
    ...
    case TYPE_EXPLOSIVE_CHARGE:
        ...
        case DATA_EXPLOSIVE_CHARGE_1:
            ...
            m_luiSpawnedExplosiveChargeGUIDs.push_back(
                m_aExplosiveSortedGuids[0][0]);
            break;
        case DATA_EXPLOSIVE_CHARGE_2:
            DoRespawnGameObject(m_aExplosiveSortedGuids[0][1], HOUR);
            m_luiSpawnedExplosiveChargeGUIDs.push_back(
                m_aExplosiveSortedGuids[0][1]);
            break;
        case DATA_EXPLOSIVE_CHARGE_3:
            DoRespawnGameObject(m_aExplosiveSortedGuids[1][0], HOUR);
            m_luiSpawnedExplosiveChargeGUIDs.push_back(
                m_aExplosiveSortedGuids[1][0]);
            break;
        case DATA_EXPLOSIVE_CHARGE_4:
            DoRespawnGameObject(m_aExplosiveSortedGuids[1][1], HOUR);
            m_luiSpawnedExplosiveChargeGUIDs.push_back(
                m_aExplosiveSortedGuids[1][1]);
            break;
        case DATA_EXPLOSIVE_CHARGE_USE:
            Creature* pBlastmaster =
                GetSingleCreatureFromStorage(NPC_BLASTMASTER_SHORTFUSE);
            if (!pBlastmaster)
                break;
            for (GUIDList::const_iterator itr =
                     m_luiSpawnedExplosiveChargeGUIDs.begin();
                 itr != m_luiSpawnedExplosiveChargeGUIDs.end(); ++itr)
            {
    ...
    case TYPE_THERMAPLUGG:
        ...
        if (uiData == IN_PROGRESS)
        {
            // Make Door locked
            if (GameObject* pDoor =
                    GetSingleGameObjectFromStorage(GO_THE_FINAL_CHAMBER))
            {
