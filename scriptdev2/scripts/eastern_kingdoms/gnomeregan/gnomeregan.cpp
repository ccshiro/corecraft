...
static const sSummonInformation asSummonInfo[MAX_SUMMON_POSITIONS] = {
    ...
    {1, NPC_CAVERNDEEP_AMBUSHER, -568.5875f, -113.7559f, -151.1869f,
     0.06981317f},
    ...
    {1, NPC_CAVERNDEEP_AMBUSHER, -558.9717f, -115.0669f, -151.8799f,
     0.5235988f},
    {1, NPC_CAVERNDEEP_AMBUSHER, -556.6719f, -112.0526f, -152.8255f,
     0.4886922f},
    {1, NPC_CAVERNDEEP_AMBUSHER, -552.6419f, -113.4385f, -153.0727f,
     0.8028514f},
    {1, NPC_CAVERNDEEP_AMBUSHER, -549.1248f, -112.1469f, -153.7987f,
     0.7504916f},
    {1, NPC_CAVERNDEEP_AMBUSHER, -546.7435f, -112.3051f, -154.2225f,
     0.9250245f},
...
struct MANGOS_DLL_DECL npc_blastmaster_emi_shortfuseAI : public npc_escortAI
{
    ...
    void Reset() override
    {
        m_bDidAggroText = false; // Used for 'defend' text, is triggered when
                                 // the npc is attacked
    ...
    {
        for (auto& elem : asSummonInfo)
        {
            // This requires order of the array
            if (elem.uiPosition > uiIndex)
                break;
            if (elem.uiPosition == uiIndex)
                m_creature->SummonCreature(elem.uiEntry, elem.fX, elem.fY,
                    elem.fZ, elem.fO, TEMPSUMMON_DEAD_DESPAWN, 0);
        }
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        {
            if (GameObject* pDoor = m_pInstance->GetSingleGameObjectFromStorage(
                    m_uiPhase > 20 ? GO_CAVE_IN_NORTH : GO_CAVE_IN_SOUTH))
            {
                auto pos = pDoor->GetPoint(frand(0.0f, 2 * M_PI_F), 2.0f);
                pSummoned->movement_gens.push(
                    new movement::PointMovementGenerator(
                        1, pos.x, pos.y, pos.z, true, false));
            }
            break;
        }
        case NPC_GRUBBIS:
    ...
    {
        return m_uiPhase == 11 || m_uiPhase == 13 || m_uiPhase == 26 ||
               m_uiPhase == 28;
    }

    void MoveInLineOfSight(Unit* pWho) override
    {
        // In case we are preparing the explosive charges, we won't start
        // attacking mobs
    ...
    void AttackedBy(Unit* pAttacker) override
    {
        ...
        if (!urand(0, 2))
            DoScriptText(
                urand(0, 1) ? SAY_AGGRO_1 : SAY_AGGRO_2, m_creature, pAttacker);
    }
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
        ...
        for (GUIDList::const_iterator itr = m_luiSummonedMobGUIDs.begin();
             itr != m_luiSummonedMobGUIDs.end(); ++itr)
        {
            ...
            if (m_uiPhaseTimer <= uiDiff)
            {
                ...
                case 1:
                    ...
                    m_creature->SetUInt32Value(
                        UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);
                ...
                case 2:
                    ...
                    m_uiPhaseTimer =
                        3500; // 6s delay, but 2500ms for escortstarting
                    break;
                case 3:
                    Start(false, m_creature->GetMap()->GetPlayer(m_playerGuid),
                        NULL, false, false);
                    m_uiPhaseTimer = 0;
                    break;
                ...
                case 11: // 15s after reached WP 11
                    ...
                    if (m_pInstance)
                        m_pInstance->SetData(
                            TYPE_EXPLOSIVE_CHARGE, DATA_EXPLOSIVE_CHARGE_1);
                    // Remove EMOTE_STATE_USESTANDING state-emote
                    m_creature->HandleEmote(EMOTE_ONESHOT_NONE);

                    m_uiPhaseTimer = 1;
                    break;
                case 12: // Empty Phase, used to store information about set
                         // charge
                    m_uiPhaseTimer = 0;
                    break;
                ...
                case 13: // 10s after reached WP 13
                    ...
                    if (m_pInstance)
                        m_pInstance->SetData(
                            TYPE_EXPLOSIVE_CHARGE, DATA_EXPLOSIVE_CHARGE_2);
                    // Remove EMOTE_STATE_USESTANDING state-emote
                    m_creature->HandleEmote(EMOTE_ONESHOT_NONE);

                    m_uiPhaseTimer = 11000;
                    break;
                case 14: // Empty Phase, used to store information about set
                         // charge
                    m_uiPhaseTimer = 1;
                    break;
                ...
                case 19:
                    ...
                    if (m_pInstance)
                    {
                        ...
                        m_pInstance->SetData(
                            TYPE_EXPLOSIVE_CHARGE, DATA_EXPLOSIVE_CHARGE_USE);
                    }
                    m_uiPhaseTimer = 5000;
                    break;
                case 20:
                ...
                case 26: // 15s after reaching WP 16
                    ...
                    if (m_pInstance)
                        m_pInstance->SetData(
                            TYPE_EXPLOSIVE_CHARGE, DATA_EXPLOSIVE_CHARGE_3);
                    // Remove EMOTE_STATE_USESTANDING state-emote
                    m_creature->HandleEmote(EMOTE_ONESHOT_NONE);

                    m_uiPhaseTimer = 1;
                    break;
                case 27: // Empty Phase, used to store information about set
                         // charge
                    m_uiPhaseTimer = 0;
                    break;
                ...
                case 28: // 10s after reaching WP 17
                    ...
                    if (m_pInstance)
                        m_pInstance->SetData(
                            TYPE_EXPLOSIVE_CHARGE, DATA_EXPLOSIVE_CHARGE_4);
                    // Remove EMOTE_STATE_USESTANDING state-emote
                    m_creature->HandleEmote(EMOTE_ONESHOT_NONE);

                    m_uiPhaseTimer = 10000;
                    break;
                case 29: // Empty Phase, used to store information about set
                         // charge
                    m_uiPhaseTimer = 1;
                    break;
...
bool GossipHello_npc_blastmaster_emi_shortfuse(
    Player* pPlayer, Creature* pCreature)
{
    if (instance_gnomeregan* pInstance =
            (instance_gnomeregan*)pCreature->GetInstanceData())
    {
        if (pInstance->GetData(TYPE_GRUBBIS) == NOT_STARTED ||
            pInstance->GetData(TYPE_GRUBBIS) == FAIL)
        {
...
bool GossipSelect_npc_blastmaster_emi_shortfuse(
    ...
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
                ...
                (instance_gnomeregan*)pCreature->GetInstanceData())
        {
            if (pInstance->GetData(TYPE_GRUBBIS) == NOT_STARTED ||
                pInstance->GetData(TYPE_GRUBBIS) == FAIL)
            {
                if (npc_blastmaster_emi_shortfuseAI* pEmiAI =
                        dynamic_cast<npc_blastmaster_emi_shortfuseAI*>(
                            pCreature->AI()))
