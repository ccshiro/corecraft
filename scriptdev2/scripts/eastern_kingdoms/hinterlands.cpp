        ...
        case 27:
            for (uint8 i = 0; i < 3; ++i)
            {
                auto pos = m_creature->GetPointXYZ(
                    G3D::Vector3(147.927444f, -3851.513428f, 130.893f),
                    2 * M_PI_F * rand_norm_f(), 7.0f * rand_norm_f());
                m_creature->SummonCreature(NPC_MARAUDING_OWL, pos.x, pos.y,
                    pos.z, 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 25000);
            }
            break;
        case 44:
            for (uint8 i = 0; i < 3; ++i)
            {
                auto pos = m_creature->GetPointXYZ(
                    G3D::Vector3(-141.151581f, -4291.213867f, 120.130f),
                    2 * M_PI_F * rand_norm_f(), 7.0f * rand_norm_f());
                m_creature->SummonCreature(NPC_VILE_AMBUSHER, pos.x, pos.y,
                    pos.z, 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 25000);
            }
            break;
        }
...
Location m_afAmbushSpawn[] = {{191.29620f, -2839.329346f, 107.388f},
    {70.972466f, -2848.674805f, 109.459f}};

Location m_afAmbushMoveTo[] = {{166.63038f, -2824.780273f, 108.153f},
...
struct MANGOS_DLL_DECL npc_rinjiAI : public npc_escortAI
{
    ...
    void Aggro(Unit* pWho) override
    {
        if (HasEscortState(STATE_ESCORT_ESCORTING))
        {
            ...
            DoScriptText(
                urand(0, 1) ? SAY_RIN_HELP_1 : SAY_RIN_HELP_2, m_creature);
        }
    }
    ...
    void DoSpawnAmbush(bool bFirst)
    {
        ...
        m_creature->SummonCreature(NPC_RANGER, m_afAmbushSpawn[m_iSpawnId].m_fX,
            m_afAmbushSpawn[m_iSpawnId].m_fY, m_afAmbushSpawn[m_iSpawnId].m_fZ,
            0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 60000);
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        pSummoned->movement_gens.push(new movement::PointMovementGenerator(0,
            m_afAmbushMoveTo[m_iSpawnId].m_fX,
            m_afAmbushMoveTo[m_iSpawnId].m_fY,
            m_afAmbushMoveTo[m_iSpawnId].m_fZ, true, true));
    }
            ...
            if (HasEscortState(STATE_ESCORT_ESCORTING) && m_uiPostEventCount)
            {
                if (m_uiPostEventTimer < uiDiff)
                {
                    ...
                    if (Player* pPlayer = GetPlayerForEscort())
                    {
                        ...
                        case 1:
                            DoScriptText(
                                SAY_RIN_PROGRESS_1, m_creature, pPlayer);
                            ++m_uiPostEventCount;
                            break;
                        case 2:
                            DoScriptText(
                                SAY_RIN_PROGRESS_2, m_creature, pPlayer);
                            m_uiPostEventCount = 0;
                            break;
                        }
                    }
...
bool QuestAccept_npc_rinji(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_RINJI_TRAPPED)
    {
        if (GameObject* pGo = GetClosestGameObjectWithEntry(
                pCreature, GO_RINJI_CAGE, INTERACTION_DISTANCE))
            pGo->UseDoorOrButton();

        if (npc_rinjiAI* pEscortAI =
                dynamic_cast<npc_rinjiAI*>(pCreature->AI()))
            pEscortAI->Start(false, pPlayer, pQuest);
    }
    return true;
}
