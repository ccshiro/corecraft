...
SD%Complete: 75
SDComment: The Stadium event is missing some yells. Seal of Ascension related
event NYI
...
static const DialogueEntry aStadiumDialogue[] = {
    ...
    {NPC_REND_BLACKHAND, 0, 0}, {0, 0, 0},
};

instance_blackrock_spire::instance_blackrock_spire(Map* pMap)
  : ScriptedInstance(pMap), DialogueHelper(aStadiumDialogue),
...
void instance_blackrock_spire::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_EMBERSEER:
        ...
        if (uiData == FAIL)
        {
            for (GUIDList::const_iterator itr = m_lIncarceratorGUIDList.begin();
                 itr != m_lIncarceratorGUIDList.end(); ++itr)
            {
    ...
    case TYPE_STADIUM:
        ...
        {
            // Despawn Nefarius and Rend on fail (the others are despawned
            // OnCreatureEvade())
            if (Creature* pNefarius =
                    GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
                pNefarius->ForcedDespawn();
            if (Creature* pRend =
                    GetSingleCreatureFromStorage(NPC_REND_BLACKHAND))
                pRend->ForcedDespawn();
...
void instance_blackrock_spire::SetData64(uint32 uiType, uint64 uiData)
{
    if (uiType == TYPE_ROOM_EVENT && GetData(TYPE_ROOM_EVENT) == IN_PROGRESS)
    {
        ...
        {
            if (m_aRoomRuneGuid[i]) // This check is used, to ensure which runes
...
void instance_blackrock_spire::DoSortRoomEventMobs()
{
    ...
    for (uint8 i = 0; i < MAX_ROOMS; ++i)
    {
        ...
        {
            for (GUIDList::const_iterator itr = m_lRoomEventMobGUIDList.begin();
                 itr != m_lRoomEventMobGUIDList.end(); ++itr)
            {
                Creature* pCreature = instance->GetCreature(*itr);
                if (pCreature && pCreature->isAlive() &&
                    pCreature->GetDistance(pRune) < 10.0f)
...
void instance_blackrock_spire::OnCreatureDeath(Creature* pCreature)
{
    ...
    case NPC_BLACKHAND_HANDLER:
        // check if it's summoned - some npcs with the same entry are already
        // spawned in the instance
...
void instance_blackrock_spire::OnCreatureEvade(Creature* pCreature)
{
    ...
    case NPC_BLACKHAND_INCARCERATOR:
        if (Creature* pEmberseer =
                GetSingleCreatureFromStorage(NPC_PYROGUARD_EMBERSEER))
...
{
    if (GetData(TYPE_EMBERSEER) == DONE ||
        GetData(TYPE_EMBERSEER) == IN_PROGRESS)
        return;

    if (m_lIncarceratorGUIDList.empty())
    {
        logging.error(
            "SD2: Npc %u couldn't be found. Please check your DB content!",
            ...
            GetSingleCreatureFromStorage(NPC_PYROGUARD_EMBERSEER))
    {
        ...
        if (pEmberseer->has_aura(SPELL_EMBERSEER_GROWING))
            return;
...
void instance_blackrock_spire::DoUseEmberseerRunes(bool bReset)
{
    ...
    for (GUIDList::const_iterator itr = m_lEmberseerRunesGUIDList.begin();
         itr != m_lEmberseerRunesGUIDList.end(); itr++)
    {
        if (bReset)
        {
...
void instance_blackrock_spire::JustDidDialogueStep(int32 iEntry)
{
    ...
    case NPC_BLACKHAND_HANDLER:
        ...
        if (Creature* pNefarius =
                GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
            pNefarius->movement_gens.push(
                new movement::PointMovementGenerator(0, aStadiumLocs[5].m_fX,
                    aStadiumLocs[5].m_fY, aStadiumLocs[5].m_fZ, true, false));
        break;
    case SAY_NEFARIUS_WARCHIEF:
        ...
        if (Creature* pRend = GetSingleCreatureFromStorage(NPC_REND_BLACKHAND))
        {
            ...
            pRend->SetWalk(false);
            pRend->movement_gens.push(
                new movement::PointMovementGenerator(0, aStadiumLocs[6].m_fX,
                    aStadiumLocs[6].m_fY, aStadiumLocs[6].m_fZ, true, true));
        }
    ...
    case NPC_REND_BLACKHAND:
                ...
                GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
        {
            ...
            pNefarius->movement_gens.push(
                new movement::PointMovementGenerator(0, aStadiumLocs[6].m_fX,
                    aStadiumLocs[6].m_fY, aStadiumLocs[6].m_fZ, true, false));
        }
        break;
    }
}
...
void instance_blackrock_spire::DoSendNextStadiumWave()
{
    if (m_uiStadiumWaves < MAX_STADIUM_WAVES)
    {
                ...
                GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
        {
            for (uint8 i = 0; i < MAX_STADIUM_MOBS_PER_WAVE; ++i)
            {
                ...
                auto pos = pNefarius->GetPointXYZ(
                    G3D::Vector3(aStadiumLocs[0].m_fX, aStadiumLocs[0].m_fY,
                        aStadiumLocs[0].m_fZ),
                    2 * M_PI_F * rand_norm_f(), 7.0f * rand_norm_f());
                pos.x = std::min(aStadiumLocs[0].m_fX,
                    pos.x); // Halfcircle - suits better the rectangular form
                if (Creature* pTemp = pNefarius->SummonCreature(
                        aStadiumEventNpcs[m_uiStadiumWaves][i], pos.x, pos.y,
                        pos.z, 0.0f, TEMPSUMMON_DEAD_DESPAWN, 0))
                {
                    // Get some point in the center of the stadium
                    pos = pTemp->GetPointXYZ(
                        G3D::Vector3(aStadiumLocs[2].m_fX, aStadiumLocs[2].m_fY,
                            aStadiumLocs[2].m_fZ),
                        2 * M_PI_F * rand_norm_f(), 5.0f * rand_norm_f());
                    pos.x = std::min(aStadiumLocs[2].m_fX,
                        pos.x); // Halfcircle - suits better the rectangular
                                // form
    ...
    else
    {
        ...
        if (Creature* pNefarius =
                GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
        {
            if (Creature* pTemp = pNefarius->SummonCreature(NPC_GYTH,
                    aStadiumLocs[1].m_fX, aStadiumLocs[1].m_fY,
                    aStadiumLocs[1].m_fZ, 0.0f, TEMPSUMMON_DEAD_DESPAWN, 0))
                pTemp->movement_gens.push(new movement::PointMovementGenerator(
                    0, aStadiumLocs[2].m_fX, aStadiumLocs[2].m_fY,
                    aStadiumLocs[2].m_fZ, true, false));
        }
...
bool AreaTrigger_at_blackrock_spire(
    Player* pPlayer, AreaTriggerEntry const* pAt)
{
    ...
    case AREATRIGGER_ENTER_UBRS:
        if (instance_blackrock_spire* pInstance =
                (instance_blackrock_spire*)pPlayer->GetInstanceData())
    ...
    case AREATRIGGER_STADIUM:
        if (instance_blackrock_spire* pInstance =
                (instance_blackrock_spire*)pPlayer->GetInstanceData())
        {
            if (pInstance->GetData(TYPE_STADIUM) == IN_PROGRESS ||
                pInstance->GetData(TYPE_STADIUM) == DONE)
                return false;

            // Summon Nefarius and Rend for the dialogue event
            // Note: Nefarius and Rend need to be hostile and not attackable
            if (Creature* pNefarius = pPlayer->SummonCreature(
                    NPC_LORD_VICTOR_NEFARIUS, aStadiumLocs[3].m_fX,
                    aStadiumLocs[3].m_fY, aStadiumLocs[3].m_fZ,
                    aStadiumLocs[3].m_fO, TEMPSUMMON_CORPSE_DESPAWN, 0))
                pNefarius->setFaction(FACTION_BLACK_DRAGON);
            if (Creature* pRend = pPlayer->SummonCreature(NPC_REND_BLACKHAND,
                    aStadiumLocs[4].m_fX, aStadiumLocs[4].m_fY,
                    aStadiumLocs[4].m_fZ, aStadiumLocs[4].m_fO,
                    TEMPSUMMON_CORPSE_DESPAWN, 0))
                pRend->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
    ...
    uint32 /*uiEventId*/, Object* pSource, Object* /*pTarget*/, bool bIsStart)
{
    ...
    {
        if (instance_blackrock_spire* pInstance =
                (instance_blackrock_spire*)((Player*)pSource)
                    ->GetInstanceData())
        {
