    ...
    void DoHandleEmberseerGrowing()
    {
        ...
        else if (m_uiGrowingStacks == MAX_GROWING_STACKS)
        {
            ...
            if (m_pInstance)
            {
                ...
                // progress until this point - avoid using the altar again when
                // the boss is fully grown
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiEncageTimer)
        {
            if (m_uiEncageTimer <= uiDiff)
            {
                ...
                for (
                    GUIDList::const_iterator itr = m_lIncarceratorsGuid.begin();
                    itr != m_lIncarceratorsGuid.end(); ++itr)
                {
                    if (Creature* pIncarcerator =
                            m_creature->GetMap()->GetCreature(*itr))
                        pIncarcerator->CastSpell(
                            m_creature, SPELL_ENCAGE_EMBERSEER, false);
                }
...
bool EffectDummyCreature_pyroguard_emberseer(Unit* /*pCaster*/,
    uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
    ...
    {
        if (boss_pyroguard_emberseerAI* pEmberseerAI =
                dynamic_cast<boss_pyroguard_emberseerAI*>(
                    pCreatureTarget->AI()))
