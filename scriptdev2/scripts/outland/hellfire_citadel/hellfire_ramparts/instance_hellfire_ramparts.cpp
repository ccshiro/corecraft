...
instance_ramparts::instance_ramparts(Map* pMap)
  : ScriptedInstance(pMap), m_uiSentryCounter(0)
{
    Initialize();
}
...
{
    LOG_DEBUG(logging,
        "SD2: Instance Ramparts: SetData received for type %u with data %u",
        uiType, uiData);
...
void instance_ramparts::DoFailVazruden()
{
    ...
    m_uiSentryCounter = 0;
    for (auto& elem : m_lSentryGUIDs)
    {
        if (Creature* pSentry = instance->GetCreature(elem))
            pSentry->Respawn();
    }
    ...
    if (Creature* pVazruden = GetSingleCreatureFromStorage(NPC_VAZRUDEN_HERALD))
    {
        ...
        {
            if (ScriptedAI* pVazrudenAI =
                    dynamic_cast<ScriptedAI*>(pVazruden->AI()))
                pVazrudenAI->Reset();
        }
    }
...
bool instance_ramparts::AreSentriesDead(Creature* pBoss)
{
    if (m_lSentryGUIDs.size() == 0)
        return false;

    for (auto& elem : m_lSentryGUIDs)
    {
        if (Creature* pSentry = pBoss->GetMap()->GetCreature(elem))
        {
            if (pSentry->isAlive())
                return false;
        }
