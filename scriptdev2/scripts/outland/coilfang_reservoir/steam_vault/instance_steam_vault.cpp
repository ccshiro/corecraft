...
bool GOUse_go_main_chambers_access_panel(Player* /*pPlayer*/, GameObject* pGo)
{
    ...
    if (pGo->GetEntry() == GO_ACCESS_PANEL_HYDRO ||
        pGo->GetEntry() == GO_ACCESS_PANEL_MEK)
    {
        if (pInstance->GetData(TYPE_HYDROMANCER_THESPIA) != SPECIAL &&
            pInstance->GetData(TYPE_MEKGINEER_STEAMRIGGER) != SPECIAL)
            pGo->MonsterTextEmote("You hear a faint echo...", NULL);
        else
            pGo->MonsterTextEmote(
                "You hear a loud rumble of metal grinding on stone...", NULL);
    }
    ...
    // Move the door guards if both doors are now open
    if (pInstance->GetData(TYPE_HYDROMANCER_THESPIA) == SPECIAL &&
        pInstance->GetData(TYPE_MEKGINEER_STEAMRIGGER) == SPECIAL)
    {
        if (Creature* guard =
                pGo->GetMap()->GetCreature(ObjectGuid(HIGHGUID_UNIT,
                    (uint32)GUARD_ENTRY_ONE, (uint32)GUARD_GUID_ONE)))
            guard->AI()->Notify(10);
        if (Creature* guard =
                pGo->GetMap()->GetCreature(ObjectGuid(HIGHGUID_UNIT,
                    (uint32)GUARD_ENTRY_TWO, (uint32)GUARD_GUID_TWO)))
            guard->AI()->Notify(10);
        if (Creature* guard =
                pGo->GetMap()->GetCreature(ObjectGuid(HIGHGUID_UNIT,
                    (uint32)GUARD_ENTRY_THREE, (uint32)GUARD_GUID_THREE)))
            guard->AI()->Notify(10);
        if (Creature* guard =
                pGo->GetMap()->GetCreature(ObjectGuid(HIGHGUID_UNIT,
                    (uint32)GUARD_ENTRY_FOUR, (uint32)GUARD_GUID_FOUR)))
            guard->AI()->Notify(10);
    }
...
void instance_steam_vault::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_HYDROMANCER_THESPIA:
        if (uiData == DONE)
            DoToggleGameObjectFlags(
                GO_ACCESS_PANEL_HYDRO, GO_FLAG_NO_INTERACT, false);
        if (uiData == SPECIAL)
        {
    ...
    case TYPE_MEKGINEER_STEAMRIGGER:
        if (uiData == DONE)
            DoToggleGameObjectFlags(
                GO_ACCESS_PANEL_MEK, GO_FLAG_NO_INTERACT, false);
        if (uiData == SPECIAL)
        {
    ...
    case TYPE_WARLORD_KALITHRESH:
        ...
        {
            for (
                GUIDList::const_iterator itr = m_lNagaDistillerGuidList.begin();
                itr != m_lNagaDistillerGuidList.end(); ++itr)
            {
                if (Creature* pDistiller = instance->GetCreature(*itr))
                {
                    pDistiller->Respawn();
                    if (!pDistiller->HasFlag(
...
bool GOUse_ArcaneContainerTwoThree(Player* /*plr*/, GameObject* go)
{
    go->Delete();
    return true;
}
...
void AddSC_instance_steam_vault()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "go_arcane_container_two_three";
    pNewScript->pGOUse = GOUse_ArcaneContainerTwoThree;
    pNewScript->RegisterSelf();
}
