...
SD%Complete: 90
SDComment: Persistent Area Auras for each Essence (Aura of Suffering, Aura of
Desire, Aura of Anger) requires core support.
...
static ReliquaryPosition Coords[] = {{450.4f, 212.3f}, {542.1f, 212.3f},
    {542.1f, 168.3f}, {542.1f, 137.4f}, {450.4f, 137.4f}, {450.4f, 168.3f}};
...
struct MANGOS_DLL_DECL npc_enslaved_soulAI : public ScriptedAI
{
    ...
    void Reset() override { m_reliquaryGuid.Clear(); }
    ...
    void DamageTaken(Unit* done_by, uint32& damage) override
    {
        if (damage >= m_creature->GetHealth())
        {
            if (done_by->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                {
                    if ((done_by->GetPower(POWER_MANA) /
                            done_by->GetMaxPower(POWER_MANA)) < 70)
                    {
                        uint32 mana =
                            done_by->GetPower(POWER_MANA) +
                            (uint32)(done_by->GetMaxPower(POWER_MANA) * 0.3);
                        done_by->SetPower(POWER_MANA, mana);
                    }
                    else
                        done_by->SetPower(
                            POWER_MANA, done_by->GetMaxPower(POWER_MANA));
                }
            }
...
struct MANGOS_DLL_DECL boss_reliquary_of_soulsAI : public ScriptedAI
{
    ...
    // 0 = Out of Combat, 1 = Not started, 2 = Suffering, 3 = Souls, 4 = Desire,
    // 5 = Souls, 6 = Anger
    uint32 Phase;
    ...
    void MoveInLineOfSight(Unit* who) override
    {
            ...
            m_creature->IsHostileTo(who))
        {
            ...
            {
                who->remove_auras(SPELL_AURA_MOD_STEALTH);
                ...
                if (m_creature->getThreatManager().getThreatList().empty())
                {
                    ...
                    // m_creature->SetInCombatWith(who);     // Don't know what
                    // is like retail
                    // who->SetInCombatWith(m_creature);
                    m_creature->SetInCombatWithZone(); // Same goes here, but
                                                       // setting to zone will
                                                       // prevent bug if the
                                                       // only player of
                                                       // threatList dies
                }
    ...
    void SummonSoul()
    {
        ...
        Creature* Soul = m_creature->SummonCreature(CREATURE_ENSLAVED_SOUL, x,
            y, m_creature->GetZ(), m_creature->GetO(),
            TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 45000);
        Unit* target =
            m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0);
        if (target && Soul)
        {
            if (npc_enslaved_soulAI* pSoulAI =
                    dynamic_cast<npc_enslaved_soulAI*>(Soul->AI()))
    ...
    void MergeThreatList(Creature* target)
    {
        ...
        for (const auto& elem : tList)
        {
            ...
            {
                m_creature->AddThreat(pUnit); // This is so that we make sure
                                              // the unit is in Reliquary's
                                              // threat list before we reset the
                                              // unit's threat.
                m_creature->getThreatManager().modifyThreatPercent(pUnit, -100);
                float threat = target->getThreatManager().getThreat(pUnit);
                m_creature->AddThreat(pUnit,
                    threat); // This makes it so that the unit has the same
                             // amount of threat in Reliquary's threatlist as in
                             // the target creature's (One of the Essences).
            }
        }
    }
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (Phase == 1)
        {
            ...
            if (SummonEssenceTimer < diff)
            {
                ...
                Creature* EssenceSuffering = m_creature->SummonCreature(23418,
                    m_creature->GetX(), m_creature->GetY(), m_creature->GetZ(),
                    1.57f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000);
        ...
        if (Phase == 2)
        {
            if (m_sufferingGuid)
            {
                Creature* EssenceSuffering =
                    m_creature->GetMap()->GetCreature(m_sufferingGuid);
                ...
                if (!EndingPhase)
                {
                    if (EssenceSuffering)
                    {
                        if (EssenceSuffering->GetHealthPercent() < 10.0f)
                        {
                            ...
                            EssenceSuffering->remove_auras();
                            EssenceSuffering->DeleteThreatList();
                            /* EssenceSuffering->GetMotionMaster()->MoveFollow(
                             */
                            /*     m_creature, 0.0f, 0.0f); */
                            EssenceSuffering->SetFlag(
                ...
                if ((EndingPhase) && (EssenceSuffering) &&
                    (EssenceSuffering->isAlive()))
                {
                    if (AnimationTimer < diff)
                    {
                        // Return
        ...
        if (Phase == 3)
        {
            ...
            if (SoulDeathCount >= SoulCount)
            {
                ...
                if (SummonEssenceTimer < diff)
                {
                    ...
                    Creature* EssenceDesire =
                        m_creature->SummonCreature(23419, m_creature->GetX(),
                            m_creature->GetY(), m_creature->GetZ(), 1.57f,
        ...
        if (Phase == 4)
        {
            ...
            {
                Creature* EssenceDesire =
                    m_creature->GetMap()->GetCreature(m_desireGuid);
                ...
                if (!EndingPhase && EssenceDesire)
                {
                    if (EssenceDesire->GetHealthPercent() < 10.0f)
                    {
                        ...
                        /* EssenceDesire->GetMotionMaster()->MoveFollow( */
                        /*     m_creature, 0.0f, 0.0f); */
                        EssenceDesire->remove_auras();
                        EssenceDesire->DeleteThreatList();

                        DoScriptText(DESI_SAY_RECAP, EssenceDesire);

                        EssenceDesire->SetFlag(
                            UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        DespawnEssenceTimer = 4000;
                        AnimationTimer = 2200;
                        EndingPhase = true;
        ...
        if (Phase == 5)
        {
            ...
            if (SoulDeathCount >= SoulCount)
            {
                ...
                if (SummonEssenceTimer < diff)
                {
                    ...
                    Creature* EssenceAnger =
                        m_creature->SummonCreature(23420, m_creature->GetX(),
                            m_creature->GetY(), m_creature->GetZ(), 1.57f,
        ...
        if (Phase == 6)
        {
            ...
            {
                Creature* EssenceAnger =
                    m_creature->GetMap()->GetCreature(m_angerGuid);
...
{
    boss_essence_of_sufferingAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    void DamageTaken(Unit* done_by, uint32& damage) override
    {
        if ((damage >= m_creature->GetHealth()) && (done_by != m_creature))
        {
            ...
            {
                if (Player* pPlayer =
                        m_creature->GetMap()->GetPlayer(m_statAuraGuid))
                    pPlayer->remove_auras(AURA_OF_SUFFERING_ARMOR);
            }
        }
    }
    ...
    void Aggro(Unit* pWho) override
    {
        ...
        DoCastSpellIfCan(
            m_creature, ESSENCE_OF_SUFFERING_PASSIVE, CAST_TRIGGERED);
    }
    ...
    void KilledUnit(Unit* /*victim*/) override
    {
        ...
        {
        case 0:
            DoScriptText(SUFF_SAY_SLAY1, m_creature);
            break;
        case 1:
            DoScriptText(SUFF_SAY_SLAY2, m_creature);
            break;
        case 2:
            DoScriptText(SUFF_SAY_SLAY3, m_creature);
            break;
        }
    }
    ...
    void CastFixate()
    {
        ...
        targets.sort(
            ObjectDistanceOrder(m_creature)); // Sort players by distance.
        ...
        m_creature->AddThreat(target,
            m_creature->getThreatManager().getThreat(m_creature->getVictim()));
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (m_creature->GetHealthPercent() <= 10.0f)
        {
            if (m_creature->getVictim())
                m_creature->DeleteThreatList(); // Delete our threatlist if
                                                // below 10% as we should no
                                                // longer attack.
            return;
        }
...
{
    boss_essence_of_desireAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    void DamageTaken(Unit* done_by, uint32& damage) override
    {
        ...
        else
        {
            if (done_by && (done_by->GetTypeId() == TYPEID_PLAYER) &&
                done_by->isAlive())
                done_by->DealDamage(done_by, damage / 2, NULL, DIRECT_DAMAGE,
                    SPELL_SCHOOL_MASK_NORMAL, NULL, false, false);
        }
    }
    ...
    void KilledUnit(Unit* /*victim*/) override
    {
        ...
        {
        case 0:
            DoScriptText(DESI_SAY_SLAY1, m_creature);
            break;
        case 1:
            DoScriptText(DESI_SAY_SLAY2, m_creature);
            break;
        case 2:
            DoScriptText(DESI_SAY_SLAY3, m_creature);
            break;
        }
    }
...
struct MANGOS_DLL_DECL boss_essence_of_angerAI : public ScriptedAI
{
    ...
    {
        DoCastSpellIfCan(
            m_creature->getVictim(), AURA_OF_ANGER, CAST_TRIGGERED);
    }
    ...
    {
        DoScriptText(
            urand(0, 1) ? ANGER_SAY_SLAY1 : ANGER_SAY_SLAY2, m_creature);
    }
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (CheckTankTimer < diff)
        {
            if (m_creature->getVictim()->GetObjectGuid() != m_aggroTargetGuid)
            {
                ...
                DoCastSpellIfCan(m_creature->getVictim(), SPELL_ENEMY_SEETHE,
                    CAST_TRIGGERED);
                m_aggroTargetGuid = m_creature->getVictim()->GetObjectGuid();
            }
            CheckTankTimer = 2000;
...
void npc_enslaved_soulAI::JustDied(Unit* /*killer*/)
{
    if (m_reliquaryGuid)
    {
        if (Creature* pReliquary =
                m_creature->GetMap()->GetCreature(m_reliquaryGuid))
        {
            if (boss_reliquary_of_soulsAI* pReliqAI =
                    dynamic_cast<boss_reliquary_of_soulsAI*>(pReliquary->AI()))
