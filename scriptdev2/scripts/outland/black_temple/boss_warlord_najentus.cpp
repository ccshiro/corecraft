...
SD%Complete: 90
SDComment: Core spell support for Needle Spine (spells 39992, 39835) missing, no
change from SD2 needed
...
struct MANGOS_DLL_DECL boss_najentusAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* /*pCaster*/, const SpellEntry* pSpell) override
    {
        ...
        {
            if (m_creature->has_aura(SPELL_TIDAL_SHIELD))
                m_creature->remove_auras(SPELL_TIDAL_SHIELD);
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_bIsShielded && m_uiTidalShieldTimer < 16000 &&
            !m_creature->has_aura(SPELL_TIDAL_SHIELD))
        {
            m_bIsShielded = false;
        ...
        if (m_uiImpalingSpineTimer < uiDiff)
        {
            Unit* pTarget =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1,
                    SPELL_IMPALING_SPINE, SELECT_FLAG_PLAYER);
            ...
            if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                DoScriptText(
                    urand(0, 1) ? SAY_NEEDLE1 : SAY_NEEDLE2, m_creature);
            }
        }
        else
        ...
        {
            DoCastSpellIfCan(m_creature, SPELL_TIDAL_SHIELD,
