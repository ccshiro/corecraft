...
SDName: Black_Temple
SD%Complete: 95
SDComment: Spirit of Olum: Player Teleporter to Seer Kanai Teleport after
defeating Naj'entus and Supremus. TODO: Find proper gossip.
...
bool GossipHello_npc_spirit_of_olum(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pInstance && (pInstance->GetData(TYPE_SUPREMUS) >= DONE) &&
        (pInstance->GetData(TYPE_NAJENTUS) >= DONE))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_OLUM1,
...
bool GossipSelect_npc_spirit_of_olum(Player* pPlayer, Creature* /*pCreature*/,
