...
SD%Complete: 80
SDComment: Saber Lash missing, Fatal Attraction slightly incorrect; need to
damage only if affected players are within range of each other
...
static Locations TeleportPoint[] = {{959.996f, 212.576f, 193.843f},
    {932.537f, 231.813f, 193.838f}, {958.675f, 254.767f, 193.822f},
    {946.955f, 201.316f, 192.535f}, {944.294f, 149.676f, 197.551f},
    {930.548f, 284.888f, 193.367f}, {965.997f, 278.398f, 195.777f}};
...
struct MANGOS_DLL_DECL boss_shahrazAI : public ScriptedAI
{
    ...
    void TeleportPlayers()
    {
        ...
        for (auto& elem : m_targetGuid)
        {
            ...
            {
                elem = pUnit->GetObjectGuid();
                pUnit->CastSpell(pUnit, SPELL_TELEPORT_VISUAL, true);
                DoTeleportPlayer(pUnit, X, Y, Z, pUnit->GetO());
            }
        }
    }
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        // Select 3 random targets (can select same target more than once),
        // teleport to a random location then make them cast explosions until
        // they get away from each other.
        ...
        {
            // Just make them explode three times... they're supposed to keep
            // exploding while they are in range, but it'll take too much code.
            // I'll try to think of an efficient way for it later.
            if (ExplosionCount < 3)
            {
                for (auto& elem : m_targetGuid)
                {
                    ...
                    {
                        if (Player* pPlayer =
                                m_creature->GetMap()->GetPlayer(elem))
