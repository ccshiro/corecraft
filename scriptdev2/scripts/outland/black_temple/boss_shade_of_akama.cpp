...
SD%Complete: 85
SDComment: Seems to be complete. Some little details/cosmetics left (see next
comment section).
SDCategory: Black Temple
EndScriptData */

/* ToDo:
(1) After start event Akama should walk a bit towards Shade of Akama, then stop
(between the two pillars) and begin to channel.
(2) Some minor changes to post event (after killing Shade of Akama):
(2.1) After Shade of Akama is dead Akama should roar in direction to the door
(he must turn around if he reached the stage).
(2.2) Positioning of broken NPCs.
(3) The channelers are casting their spell somestimes even if they are daed
(move out of view distance and then move in - they are dead but they channel -
maybe some clientspecific issue?).
(4) Unbanish Shade of Akama if a ashtongue sorcerer is spawned but not reached
Shade of Akama and channels his spell?
(5) Unsure: If sourcer channels the banish spell, shade should be banished
again?
*/
...
static const uint32 auiRandSpawnEntry[] = {
    NPC_ASH_ELEMENTAL, NPC_ASH_ROGUE, NPC_ASH_SPIRITBIND};
...
static const Location afAkamaWP[] = {
    //{516.885193, 400.836060, LOC_LOW_Z_SPAWN, 0.0},       //not used yet, he
    // moves to here before start channel
    {482.352448f, 401.162720f, LOC_LOW_Z, 0.0f},
    {469.597443f, 402.264404f, LOC_PLATFORM_Z, 0.0f}};

static const Location afBrokenSpawnLoc[] = {
    {541.375916f, 401.439575f, LOC_LOW_Z,
     M_PI_F}, // The place where Akama channels
    {534.130005f, 352.394531f, LOC_LOW_Z,
     2.164150f}, // Behind a 'pillar' which is behind the east alcove
...
struct MANGOS_DLL_DECL boss_shade_of_akamaAI : public ScriptedAI
{
    ...
    uint32 m_uiDefenderTimer; // They are on a flat 15 second timer, independant
                              // of the other summon creature timer.
    ...
    {
        LOG_DEBUG(logging,
            "SD2: Increasing Death Count for Shade of Akama encounter");
        ...
        if (uiGuid)
        {
            if (m_lSorcerersGUIDList.empty())
                logging.error(
                    "SD2: boss_shade_of_akamaAI attempt to remove guid %s from "
                    "Sorcerers list but list is already empty",
                    uiGuid.GetString().c_str());
            else
    ...
    void SummonCreature()
    {
        ...
        if (!urand(0, 2) && (m_uiDeathChannelerCount > 0) &&
            (m_lSorcerersGUIDList.size() < 7))
        {
            if (Creature* pSorcerer = m_creature->SummonCreature(
                    NPC_ASH_SORCERER, afSpawnLoc[uiRand].m_fX,
                    afSpawnLoc[uiRand].m_fY, afSpawnLoc[uiRand].m_fZ,
                    afSpawnLoc[uiRand].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0))
            {
                pSorcerer->SetWalk(false);
                /* pSorcerer->GetMotionMaster()->MovePoint(0,
                 * m_creature->GetX(), */
        ...
        {
            for (auto& elem : auiRandSpawnEntry)
            {
                if (Creature* pSpawn = m_creature->SummonCreature(elem,
                        afSpawnLoc[uiRand].m_fX, afSpawnLoc[uiRand].m_fY,
                        afSpawnLoc[uiRand].m_fZ, afSpawnLoc[uiRand].m_fO,
                        TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 25000))
                {
                    pSpawn->SetWalk(false);
                    /* pSpawn->GetMotionMaster()->MovePoint(0, */
                    /*     LOC_RAND_TO_CENTER_X, LOC_RAND_TO_CENTER_Y, */
                    /*     LOC_RAND_TO_CENTER_Z); */
                }
            }
        }
    }
    ...
    {
        for (GUIDList::const_iterator itr = m_lSorcerersGUIDList.begin();
             itr != m_lSorcerersGUIDList.end(); ++itr)
        {
    ...
    {
        for (GUIDList::const_iterator itr = m_lChannelersGUIDList.begin();
             itr != m_lChannelersGUIDList.end(); ++itr)
        {
    ...
    {
        auto lChannelerList = GetCreatureListWithEntryInGrid(
            m_creature, NPC_ASH_CHANNELER, 50.0f);
        ...
        if (!lChannelerList.empty())
        {
            ...
            for (auto& elem : lChannelerList)
            {
                m_lChannelersGUIDList.push_back((elem)->GetObjectGuid());

                (elem)->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            }
        }
        else
            logging.error(
                "SD2: boss_shade_of_akamaAI unable to find any channelers.");
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_bIsBanished)
        {
            // Akama is set in the threatlist so when we reset, we make sure
            // that he is not included in our check
            ...
            if (m_uiDefenderTimer < uiDiff)
            {
                ...
                if (Creature* pDefender =
                        m_creature->SummonCreature(NPC_ASH_DEFENDER,
                            afSpawnLoc[uiRand].m_fX, afSpawnLoc[uiRand].m_fY,
                            afSpawnLoc[uiRand].m_fZ, afSpawnLoc[uiRand].m_fO,
                            TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 25000))
                {
        ...
        else // No longer banished, let's fight Akama now
        {
            if (m_uiReduceHealthTimer < uiDiff)
            {
                            ...
                            NPC_AKAMA_SHADE))
                {
                    if (pAkama->isAlive())
                    {
                        ...
                        m_creature->DealDamage(pAkama,
                            pAkama->GetMaxHealth() / 10, NULL, DIRECT_DAMAGE,
                            SPELL_SCHOOL_MASK_NORMAL, NULL, false, false);
                        m_uiReduceHealthTimer = 12000;
                    }
                    else
                    {
                        m_bHasKilledAkama = true; // Akama is dead, we stop
                                                  // fighting and disappear
                        EnterEvadeMode();
                        return;
                    }
                }
            ...
            if (m_bHasKilledAkama)
            {
                ...
                {
                    EnterEvadeMode(); // Reset a little while after killing
                                      // Akama
                    return;
                }
                else
...
struct MANGOS_DLL_DECL npc_akamaAI : public ScriptedAI
{
    ...
    void BeginEvent()
    {
        ...
        if (Creature* pShade =
                m_pInstance->GetSingleCreatureFromStorage(NPC_SHADE_OF_AKAMA))
        {
            if (boss_shade_of_akamaAI* pShadeAI =
                    dynamic_cast<boss_shade_of_akamaAI*>(pShade->AI()))
    ...
    void MovementInform(movement::gen uiMoveType, uint32 uiPointId) override
    {
        ...
        case 0:
            ...
            break;
        case 1:
            if (Creature* pShade = m_pInstance->GetSingleCreatureFromStorage(
                    NPC_SHADE_OF_AKAMA))
            {
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_bIsShadeDead && (m_uiWayPointId == 1))
        {
            ...
            /*     afAkamaWP[1].m_fX, afAkamaWP[1].m_fY, afAkamaWP[1].m_fZ); */
            ++m_uiWayPointId;
        }
            ...
            if (m_uiCheckTimer < uiDiff)
            {
                            ...
                            NPC_SHADE_OF_AKAMA))
                {
                    if (!pShade->isAlive())
                    {
                        ...
                        /* m_creature->GetMotionMaster()->MovePoint(m_uiWayPointId,
                         */
                        /*     afAkamaWP[0].m_fX, afAkamaWP[0].m_fY, */
                        /*     afAkamaWP[0].m_fZ); */
                    }
                }
                m_uiCheckTimer = 5000;
        ...
        if (m_uiSummonBrokenTimer &&
            m_uiBrokenSummonIndex < countof(afBrokenSpawnLoc))
        {
            if (m_uiSummonBrokenTimer <= uiDiff)
            {
                ...
                {
                    float x =
                        afBrokenSpawnLoc[m_uiBrokenSummonIndex].m_fX + (i * 5);
                    float y =
                        afBrokenSpawnLoc[m_uiBrokenSummonIndex].m_fY + (1 * 5);
                    float z = afBrokenSpawnLoc[m_uiBrokenSummonIndex].m_fZ;
                    float o = afBrokenSpawnLoc[m_uiBrokenSummonIndex].m_fO;

                    if (Creature* pBroken =
                            m_creature->SummonCreature(NPC_ASH_BROKEN, x, y, z,
                                o, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 360000))
                    {
                        float wx =
                            afBrokenWP[m_uiBrokenSummonIndex].m_fX + (i * 5);
                        float wy =
                            afBrokenWP[m_uiBrokenSummonIndex].m_fY + (i * 5);
            ...
            if (m_uiSoulRetrieveTimer <= uiDiff)
            {
                ...
                case 2:
                    if (!m_lBrokenGUIDList.empty())
                    {
                        ...
                        for (GUIDList::const_iterator itr =
                                 m_lBrokenGUIDList.begin();
                             itr != m_lBrokenGUIDList.end(); ++itr)
                        {
                            if (Creature* pBroken =
                ...
                case 3:
                    for (GUIDList::const_iterator itr =
                         ...
                         itr != m_lBrokenGUIDList.end(); ++itr)
                    {
                        // This is the incorrect spell, but can't seem to find
                        // the right one.
                        if (Creature* pBroken =
                                m_creature->GetMap()->GetCreature(*itr))
            ...
            (ScriptedInstance*)pCreature->GetInstanceData())
    {
        if (pInstance->GetData(TYPE_SHADE) != DONE)
            pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT,
                GOSSIP_ITEM_START_ENCOUNTER, GOSSIP_SENDER_MAIN,
...
struct MANGOS_DLL_DECL mob_ashtongue_channelerAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        if (m_pInstance)
        {
            ...
            if (!m_creature->isAlive() &&
                m_pInstance->GetData(TYPE_SHADE) != DONE)
                m_creature->Respawn();
                ...
                m_pInstance->GetSingleCreatureFromStorage(NPC_SHADE_OF_AKAMA))
        {
            if (pShade->isAlive())
            {
                if (boss_shade_of_akamaAI* pShadeAI =
                    ...
                    logging.error(
                        "SD2: mob_ashtongue_channelerAI dead but unable to "
                        "increment DeathCount for Shade of Akama.");
            }
        }
    }
    ...
    void UpdateAI(const uint32 /*uiDiff*/) override
    {
        ...
        if (!m_creature->IsNonMeleeSpellCasted(false) &&
            !m_creature->getVictim() && m_pInstance)
        {
            if (Creature* pShade = m_pInstance->GetSingleCreatureFromStorage(
                    NPC_SHADE_OF_AKAMA))
                ...
                m_pInstance->GetSingleCreatureFromStorage(NPC_SHADE_OF_AKAMA))
        {
            if (pShade->isAlive())
            {
                if (boss_shade_of_akamaAI* pShadeAI =
                    ...
                    logging.error(
                        "SD2: mob_ashtongue_sorcererAI dead but unable to "
                        "increment DeathCount for Shade of Akama.");
            }
        }
    }
        ...
        if (m_uiCheckTimer < uiDiff)
        {
            ...
            if (pShade && pShade->isAlive() && m_creature->isAlive())
            {
                if (m_creature->IsWithinDist(pShade, 20.0f, false))
                {
                    ...
                    if (DoCastSpellIfCan(pShade, SPELL_SHADE_SOUL_CHANNEL) ==
                        CAST_OK)
