...
static const uint32 aCouncilMember[] = {
    NPC_GATHIOS, NPC_VERAS, NPC_LADY_MALANDE, NPC_ZEREVOR};

struct MANGOS_DLL_DECL mob_blood_elf_council_voice_triggerAI : public ScriptedAI
{
    mob_blood_elf_council_voice_triggerAI(Creature* pCreature)
    ...
    uint8 m_uiYellCounter; // Serves as the counter for both the aggro and
                           // enrage yells
            ...
            if (m_uiAggroYellTimer <= uiDiff)
            {
                if (Creature* pMember =
                        m_pInstance->GetSingleCreatureFromStorage(
                            aCouncilMember[m_uiYellCounter]))
                {
...
struct MANGOS_DLL_DECL mob_illidari_councilAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        if (m_pInstance)
        {
            ...
            for (auto& elem : aCouncilMember)
            {
                if (Creature* pMember =
                        m_pInstance->GetSingleCreatureFromStorage(elem))
                {
                    if (!pMember->isAlive())
                    {
    ...
    void StartEvent(Unit* pTarget)
    {
        ...
        if (pTarget && pTarget->isAlive() && !m_bEventBegun)
        {
            ...
            if (Creature* pVoiceTrigger =
                    m_pInstance->GetSingleCreatureFromStorage(
                        NPC_COUNCIL_VOICE))
            {
                if (mob_blood_elf_council_voice_triggerAI* pVoiceAI =
                        dynamic_cast<mob_blood_elf_council_voice_triggerAI*>(
                            pVoiceTrigger->AI()))
            ...
            if (m_uiEndEventTimer <= diff)
            {
                if (m_uiDeathCount > 3)
                {
                    if (Creature* VoiceTrigger =
                            m_pInstance->GetSingleCreatureFromStorage(
                                NPC_COUNCIL_VOICE))
                        VoiceTrigger->DealDamage(VoiceTrigger,
                            VoiceTrigger->GetHealth(), NULL, DIRECT_DAMAGE,
                ...
                Creature* pMember = m_pInstance->GetSingleCreatureFromStorage(
                    aCouncilMember[m_uiDeathCount]);
                if (pMember && pMember->isAlive())
                    pMember->DealDamage(pMember, pMember->GetHealth(), NULL,
                ...
                for (auto& elem : aCouncilMember)
                {
                            ...
                            m_pInstance->GetSingleCreatureFromStorage(elem))
                    {
                        ...
                        if (Member->isAlive() && !Member->SelectHostileTarget())
                            ++EvadeCheck; // If all members evade, we reset so
                                          // that players can properly reset the
                                          // event
                        else if (!Member->isAlive()) // If even one uiMember
                                                     // dies, kill the rest, set
                                                     // instance data, and kill
                                                     // self.
                        {
...
struct MANGOS_DLL_DECL boss_illidari_councilAI : public ScriptedAI
{
    ...
    void Aggro(Unit* pWho) override
    {
        if (m_pInstance)
        {
            if (Creature* pController =
                    m_pInstance->GetSingleCreatureFromStorage(
                        NPC_ILLIDARI_COUNCIL))
            {
                if (mob_illidari_councilAI* pControlAI =
                        dynamic_cast<mob_illidari_councilAI*>(
                            pController->AI()))
...
struct MANGOS_DLL_DECL boss_gathios_the_shattererAI
    : public boss_illidari_councilAI
{
    boss_gathios_the_shattererAI(Creature* pCreature)
      : boss_illidari_councilAI(pCreature)
    {
        Reset();
    }
    ...
    Unit* SelectCouncilMember()
    {
        ...
        if (uiMember != 0) // No need to create another pointer
            pUnit = m_pInstance->GetSingleCreatureFromStorage(
                aCouncilMember[uiMember]);
    ...
    void CastAuraOnCouncil()
    {
        ...
        {
        case 0:
            uiSpellid = SPELL_DEVOTION_AURA;
            break;
        case 1:
            uiSpellid = SPELL_CHROMATIC_AURA;
            break;
        }
        ...
        for (auto& elem : aCouncilMember)
        {
            if (Creature* pCouncil =
                    m_pInstance->GetSingleCreatureFromStorage(elem))
                pCouncil->CastSpell(pCouncil, uiSpellid, true, NULL, NULL,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiBlessingTimer < uiDiff)
        {
            if (Unit* pUnit = SelectCouncilMember())
                DoCastSpellIfCan(pUnit, urand(0, 1) ? SPELL_BLESS_SPELLWARD :
        ...
        if (m_uiSealTimer < uiDiff)
        {
            DoCastSpellIfCan(m_creature,
                urand(0, 1) ? SPELL_SEAL_OF_COMMAND : SPELL_SEAL_OF_BLOOD);
            m_uiSealTimer = 40000;
        }
        else
...
struct MANGOS_DLL_DECL boss_high_nethermancer_zerevorAI
    : public boss_illidari_councilAI
{
    boss_high_nethermancer_zerevorAI(Creature* pCreature)
      : boss_illidari_councilAI(pCreature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiCooldown)
        {
            ...
            {
                m_uiCooldown -= uiDiff;
                return; // Don't cast any other spells if global cooldown is
                        // still ticking
            }
        }
        ...
        if (m_uiDampenMagicTimer < uiDiff)
        {
            ...
            m_uiArcaneBoltTimer +=
                1000; // Give the Mage some time to spellsteal Dampen.
        }
        else
...
{
    boss_lady_malandeAI(Creature* pCreature)
      : boss_illidari_councilAI(pCreature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            // Currently bugged and puts Malande on the threatlist of the other
            // council members. It also heals players.
...
{
    boss_veras_darkshadowAI(Creature* pCreature)
      : boss_illidari_councilAI(pCreature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (!m_bHasVanished)
        {
            ...
            if (m_uiAppearEnvenomTimer < uiDiff) // Cast Envenom. This is cast 4
                                                 // seconds after Vanish is over
            {
            ...
            if (m_uiVanishTimer < uiDiff) // Disappear and stop attacking, but
                                          // follow a random unit
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0))
                {
                    ...
                    DoResetThreat();
                    // Chase a unit. Check before DoMeleeAttackIfReady prevents
                    // from attacking
        ...
        {
            if (m_uiVanishTimer <
                uiDiff) // Become attackable and poison current target
            {
                ...
                m_creature->RemoveFlag(
                    UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                DoResetThreat();
                m_creature->AddThreat(target, 3000.0f); // Make Veras attack his
                                                        // pTarget for a while,
                                                        // he will cast Envenom
                                                        // 4 seconds after.
            ...
            if (m_uiAppearEnvenomTimer < uiDiff) // Appear 2 seconds before
                                                 // becoming attackable
                                                 // (Shifting out of vanish)
            {
