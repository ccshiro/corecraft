...
SD%Complete: 90
SDComment: Fixating the target is hacky, unknown if other speed-changes happen,
remove AI for trigger mobs in next step
 ...
 * Statet at wowwiki: "If the gaze target is further away than 40 yards, he
 * dashes at about five times the normal run speed until the range is about 20
 * yards."
...
// TODO Remove this 'script' when combat movement can be proper prevented from
// core-side
struct MANGOS_DLL_DECL molten_flameAI : public ScriptedAI
{
    molten_flameAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
...
struct MANGOS_DLL_DECL boss_supremusAI : public ScriptedAI
{
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
        ...
        for (GUIDList::const_iterator itr = m_lSummonedGUIDs.begin();
             itr != m_lSummonedGUIDs.end(); ++itr)
        {
        ...
        if (pSummoned->GetEntry() == NPC_STALKER)
        {
            ...
            if (pTarget)
            {
                pSummoned->movement_gens.push(
                    new movement::FollowMovementGenerator(pTarget));
                pSummoned->CastSpell(pSummoned, SPELL_MOLTEN_FLAME, false, NULL,
                    NULL, m_creature->GetObjectGuid());
            }
        }

        else if (pSummoned->GetEntry() == NPC_VOLCANO)
            pSummoned->CastSpell(pSummoned, SPELL_VOLCANIC_ERUPTION_VOLCANO,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            // This currently is entirely screwed, because the npc is summoned
            // somewhere far away as of big bounding box of supremus
        ...
        if (m_uiPhaseSwitchTimer < uiDiff)
        {
            ...
            {
                m_bTankPhase = true;
                m_creature->remove_auras(SPELL_SLOW_SELF);
            }
            else
            {
                m_bTankPhase = false;
                ...
                DoCastSpellIfCan(
                    m_creature, SPELL_SLOW_SELF, CAST_INTERRUPT_PREVIOUS);
            }
        ...
        if (m_bTankPhase)
        {
            if (m_uiHatefulStrikeTimer < uiDiff)
            {
                ...
                {
                    if (DoCastSpellIfCan(pTarget, SPELL_HATEFUL_STRIKE) ==
                        CAST_OK)
                        m_uiHatefulStrikeTimer = 5000;
                }
            }
        ...
        else // !m_bTankPhase
        {
            if (m_uiSwitchTargetTimer < uiDiff)
            {
                ...
                {
                    DoResetThreat();
                    // This way to simulate some fixating is to be considered a
                    // hack
            ...
            if (m_uiSummonVolcanoTimer < uiDiff)
            {
                ...
                if (DoCastSpellIfCan(
                        ...
                        SPELL_VOLCANIC_ERUPTION_BOSS) == CAST_OK)
                {
            ...
            if (m_uiMoltenPunchTimer < uiDiff)
            {
                ...
                m_uiMoltenPunchTimer = 8000; // might be better with small timer
                                             // and some sort of cast-chance
            }
            else
                m_uiMoltenPunchTimer -= uiDiff;

            /* Not understood how this really must work
             * if (m_creature->GetSpeedRate(MOVE_RUN) > SPEED_CHASE &&
             * m_creature->GetCombatDistance(m_creature->getVictim()) <
             * RANGE_MIN_DASHING)
             *     m_creature->SetSpeedRate(MOVE_RUN, SPEED_CHASE);
             * else if (m_creature->GetCombatDistance(m_creature->getVictim()) >
             * RANGE_MOLTEN_PUNCH)
