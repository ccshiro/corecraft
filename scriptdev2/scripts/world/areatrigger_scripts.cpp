...
bool AreaTrigger_at_childrens_week_spot(
    Player* pPlayer, AreaTriggerEntry const* pAt)
{
    for (auto& elem : TriggerOrphanSpell)
    {
        if (pAt->id == elem[0] && pPlayer->GetMiniPet() &&
            pPlayer->GetMiniPet()->GetEntry() == elem[1])
        {
            pPlayer->CastSpell(pPlayer, elem[2], true);
            return true;
        }
    }
    return false;
...
enum
{
    ...
    QUEST_MARK_OF_VASH_FLAG = 10900
};

bool AreaTrigger_at_coilfang_waterfall(
    Player* pPlayer, AreaTriggerEntry const* /*pAt*/)
{
    if (pPlayer->isGameMaster())
        return false;
    if (GameObject* pGo = GetClosestGameObjectWithEntry(
            pPlayer, GO_COILFANG_WATERFALL, 60.0f))
    {
        if (pGo->getLootState() == GO_READY &&
            pPlayer->GetQuestRewardStatus(QUEST_MARK_OF_VASH_FLAG))
            pGo->UseDoorOrButton();
    }
    return true;
}
...
bool AreaTrigger_at_legion_teleporter(
    Player* pPlayer, AreaTriggerEntry const* /*pAt*/)
{
    if (pPlayer->isAlive() && !pPlayer->isInCombat())
    {
        if (pPlayer->GetTeam() == ALLIANCE &&
            pPlayer->GetQuestRewardStatus(QUEST_GAINING_ACCESS_A))
        {
        ...
        if (pPlayer->GetTeam() == HORDE &&
            pPlayer->GetQuestRewardStatus(QUEST_GAINING_ACCESS_H))
        {
...
{
    if (pPlayer->GetQuestStatus(QUEST_MANOR_RAVENHOLDT) ==
...
bool AreaTrigger_at_scent_larkorwi(Player* pPlayer, AreaTriggerEntry const* pAt)
{
    if (pPlayer->isAlive() && !pPlayer->isGameMaster() &&
        pPlayer->GetQuestStatus(QUEST_SCENT_OF_LARKORWI) ==
            QUEST_STATUS_INCOMPLETE)
    {
        if (!GetClosestCreatureWithEntry(
                pPlayer, NPC_LARKORWI_MATE, 25.0f, false, false))
            pPlayer->SummonCreature(NPC_LARKORWI_MATE, pAt->x, pAt->y, pAt->z,
                3.3f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
