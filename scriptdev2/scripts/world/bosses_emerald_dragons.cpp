...
struct MANGOS_DLL_DECL boss_emerald_dragonAI : public ScriptedAI
{
    ...
    void KilledUnit(Unit* pVictim) override
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_PLAYER)
            pVictim->CastSpell(pVictim, SPELL_MARK_OF_NATURE_PLAYER, true, NULL,
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == NPC_DREAM_FOG)
            pSummoned->CastSpell(pSummoned, SPELL_DREAM_FOG, true, NULL, NULL,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiSeepingFogTimer < uiDiff)
        {
            ...
            m_uiSeepingFogTimer =
                urand(120000, 150000); // Rather Guesswork, but one Fog has 2min
                                       // duration, hence a bit longer
        }
        else
...
struct MANGOS_DLL_DECL boss_emerissAI : public boss_emerald_dragonAI
{
    ...
    void KilledUnit(Unit* pVictim) override
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_PLAYER)
            pVictim->CastSpell(pVictim, SPELL_PUTRID_MUSHROOM, true, NULL, NULL,
                m_creature->GetObjectGuid());
    ...
    bool UpdateDragonAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiVolatileInfectionTimer < uiDiff)
        {
            Unit* pTarget =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0);
            if (pTarget &&
                DoCastSpellIfCan(pTarget, SPELL_VOLATILE_INFECTION) == CAST_OK)
...
enum
{
    ...
    SPELL_SUMMON_SPIRIT_SHADE =
        24810, // Summon spell was removed, was SPELL_EFFECT_SUMMON_DEMON
...
struct MANGOS_DLL_DECL boss_lethonAI : public boss_emerald_dragonAI
{
    ...
    void Aggro(Unit* /*pWho*/) override
    {
        ...
        // Shadow bolt wirl is a periodic aura which triggers a set of
        // shadowbolts every 2 secs; may need some core tunning
        DoCastSpellIfCan(m_creature, SPELL_SHADOW_BOLT_WIRL, CAST_TRIGGERED);
    }

    // Summon a spirit which moves toward the boss and heals him for each player
    // hit by the spell; used at 75%, 50% and 25%
    bool DoSpecialDragonAbility() override
    {
    ...
    void SpellHitTarget(Unit* pTarget, const SpellEntry* pSpell) override
    {
        ...
        if (pTarget->GetTypeId() == TYPEID_PLAYER &&
            pSpell->Id == SPELL_DRAW_SPIRIT)
        {
            // Summon this way, to be able to cast the shade visual spell with
            // player as original caster
            // This might not be supported currently by core, but this spell's
            // visual should be dependend on the player
            // Also possible that this was no problem due to the special way
            // these NPCs had been summoned in classic times
            if (Creature* pSummoned =
                    pTarget->SummonCreature(NPC_SPIRIT_SHADE, 0.0f, 0.0f, 0.0f,
                        pTarget->GetO(), TEMPSUMMON_DEAD_DESPAWN, 0))
                pSummoned->CastSpell(pSummoned, SPELL_SPIRIT_SHAPE_VISUAL, true,
                    NULL, NULL, pTarget->GetObjectGuid());
        }
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == NPC_SPIRIT_SHADE)
            pSummoned->movement_gens.push(
                new movement::FollowMovementGenerator(m_creature, 0.0f, 0.0f));
        else
...
struct MANGOS_DLL_DECL npc_spirit_shadeAI : public ScriptedAI
{
    ...
    void Reset() override { m_bHasHealed = false; }

    void MoveInLineOfSight(Unit* pWho) override
    {
        if (!m_bHasHealed && pWho->GetEntry() == NPC_LETHON &&
            pWho->IsWithinDistInMap(m_creature, 3.0f))
        {
...
struct MANGOS_DLL_DECL boss_taerarAI : public boss_emerald_dragonAI
{
    ...
    void DoUnbanishBoss()
    {
        m_creature->remove_auras(SPELL_SELF_STUN);
        m_creature->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
    ...
    bool UpdateDragonAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiArcaneBlastTimer < uiDiff)
        {
            Unit* pTarget =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0);
            if (pTarget &&
                DoCastSpellIfCan(pTarget, SPELL_ARCANE_BLAST) == CAST_OK)
...
{
    boss_shadeoftaerarAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    bool UpdateDragonAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiLightningWaveTimer < uiDiff)
        {
            Unit* pTarget =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0);
            if (pTarget &&
                DoCastSpellIfCan(pTarget, SPELL_LIGHTNING_WAVE) == CAST_OK)
...
{
    mob_dementeddruidsAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    void Reset() override { m_uiMoonFire_Timer = 3000; }
