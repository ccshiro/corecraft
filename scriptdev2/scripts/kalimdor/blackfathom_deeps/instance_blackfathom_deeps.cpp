...
instance_blackfathom_deeps::instance_blackfathom_deeps(Map* pMap)
  : ScriptedInstance(pMap), m_uiWaveCounter(0)
{
    Initialize();
}
...
void instance_blackfathom_deeps::DoSpawnMobs(uint8 uiWaveIndex)
{
    ...
    for (auto& elem : aWaveSummonInformation)
    {
        if (elem.m_uiWaveIndex != uiWaveIndex)
            continue;

        // Summon mobs at positions
        for (uint8 j = 0; j < MAX_COUNT_POS; ++j)
        {
            for (uint8 k = 0; k < elem.m_aCountAndPos[j].m_uiCount; ++k)
            {
                uint8 uiPos = elem.m_aCountAndPos[j].m_uiSummonPosition;
                ...
                if (elem.m_aCountAndPos[j].m_uiCount > 1)
                    fPosY = fPosY - INTERACTION_DISTANCE / 2 +
                            k * INTERACTION_DISTANCE /
                                elem.m_aCountAndPos[j].m_uiCount;

                if (Creature* pSummoned =
                        pKelris->SummonCreature(elem.m_uiNpcEntry, fPosX, fPosY,
                            fPosZ, fPosO, TEMPSUMMON_DEAD_DESPAWN, 0))
                {
                    pSummoned->movement_gens.push(
                        new movement::PointMovementGenerator(
                            0, fX_resp, fY_resp, fZ_resp, true, false));
                    m_lWaveMobsGuids[uiWaveIndex].push_back(
                        pSummoned->GetGUIDLow());
                }
            }
        }
    }
...
bool instance_blackfathom_deeps::IsWaveEventFinished()
{
    ...
    for (auto& elem : m_lWaveMobsGuids)
    {
        if (!elem.empty())
            return false;
    }
...
{
    instance_blackfathom_deeps* pInstance =
        (instance_blackfathom_deeps*)pGo->GetInstanceData();
