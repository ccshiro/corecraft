...
struct MANGOS_DLL_DECL mobs_risen_husk_spiritAI : public ScriptedAI
{
    ...
    {
        if (m_pCreditPlayer)
            m_pCreditPlayer->KilledMonsterCredit(
                pSummoned->GetEntry(), pSummoned->GetObjectGuid());
    }
    ...
    void DamageTaken(Unit* pDoneBy, uint32& uiDamage) override
    {
        ...
        {
            if (pPlayer->GetQuestStatus(QUEST_WHATS_HAUNTING_WITCH_HILL) ==
                QUEST_STATUS_INCOMPLETE)
            {
                m_pCreditPlayer = pPlayer;
                m_creature->CastSpell(
                    pDoneBy, SPELL_SUMMON_RESTLESS_APPARITION, true);
            }
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiIntangiblePresence_Timer < uiDiff)
        {
            if (m_creature->GetEntry() == NPC_RISEN_SPIRIT)
                DoCastSpellIfCan(
                    m_creature->getVictim(), SPELL_INTANGIBLE_PRESENCE);
...
struct MANGOS_DLL_DECL npc_restless_apparitionAI : public ScriptedAI
{
    ...
    void Reset() override { m_uiTalk_Timer = 1000; }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiTalk_Timer <= uiDiff)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                DoScriptText(SAY_RAND_3, m_creature);
                break;
            case 3:
                DoScriptText(SAY_RAND_4, m_creature);
                break;
            case 4:
                DoScriptText(SAY_RAND_5, m_creature);
                break;
            case 5:
                DoScriptText(SAY_RAND_6, m_creature);
                break;
            case 6:
                DoScriptText(SAY_RAND_7, m_creature);
                break;
            case 7:
                DoScriptText(SAY_RAND_8, m_creature);
                break;
            }
            ...
            if (m_creature->GetHealthPercent() < 30.0f)
            {
                if (Player* pPlayer = GetPlayerForEscort())
                    pPlayer->GroupEventHappens(
                        QUEST_CHALLENGE_MOROKK, m_creature);
...
struct MANGOS_DLL_DECL npc_ogronAI : public npc_escortAI
{
    ...
    {
        if (HasEscortState(STATE_ESCORT_ESCORTING) &&
            pWho->GetEntry() == NPC_REETHE && lCreatureList.empty())
    ...
    Creature* GetCreature(uint32 uiCreatureEntry)
    {
        ...
        {
            for (auto& elem : lCreatureList)
            {
                if ((elem)->GetEntry() == uiCreatureEntry && (elem)->isAlive())
                    return (elem);
            }
        }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == NPC_CALDWELL)
            pSummoned->movement_gens.push(new movement::PointMovementGenerator(
                0, m_afMoveTo[0], m_afMoveTo[1], m_afMoveTo[2], true, true));
        else
        {
            if (Creature* pCaldwell = GetCreature(NPC_CALDWELL))
            {
                ...
                pSummoned->movement_gens.push(
                    new movement::FollowMovementGenerator(
                        pCaldwell, 0.5f, (M_PI / 2) * (int)iSize));
            }
        }
    }
    ...
    void DoStartAttackMe()
    {
        if (!lCreatureList.empty())
        {
            for (auto& elem : lCreatureList)
            {
                if ((elem)->GetEntry() == NPC_REETHE)
                    continue;

                if ((elem)->isAlive())
                {
                    (elem)->setFaction(FACTION_THER_HOSTILE);
                    (elem)->AI()->AttackStart(m_creature);
                }
            }
        }
    }
                ...
                if (m_uiGlobalTimer < uiDiff)
                {
                    ...
                    case PHASE_INTRO:
                    {
                        ...
                        case 5:
                            ...
                            m_creature->SummonCreature(NPC_CALDWELL,
                                m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
                            m_creature->SummonCreature(NPC_HALLAN, m_afSpawn[0],
                                m_afSpawn[1], m_afSpawn[2], 0.0f,
                                TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
                            m_creature->SummonCreature(NPC_SKIRMISHER,
                                m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
                            m_creature->SummonCreature(NPC_SKIRMISHER,
                                m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
...
bool QuestAccept_npc_ogron(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_QUESTIONING)
    {
        if (npc_ogronAI* pEscortAI =
                dynamic_cast<npc_ogronAI*>(pCreature->AI()))
        {
...
struct MANGOS_DLL_DECL npc_private_hendelAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        if (m_creature->getFaction() !=
            m_creature->GetCreatureInfo()->faction_A)
    ...
    {
        if (uiDamage > m_creature->GetHealth() ||
            m_creature->GetHealthPercent() < 20.0f)
        {
            uiDamage = 0;

            if (Player* pPlayer =
                    pDoneBy->GetCharmerOrOwnerPlayerOrPlayerItself())
                pPlayer->GroupEventHappens(
                    QUEST_MISSING_DIPLO_PT16, m_creature);
...
bool QuestAccept_npc_private_hendel(
    Player* /*pPlayer*/, Creature* pCreature, const Quest* pQuest)
{
...
bool AreaTrigger_at_nats_landing(
    Player* pPlayer, const AreaTriggerEntry* /*pAt*/)
{
    if (pPlayer->GetQuestStatus(QUEST_NATS_BARGAIN) ==
            QUEST_STATUS_INCOMPLETE &&
        pPlayer->has_aura(SPELL_FISH_PASTE))
    {
        Creature* pShark =
            GetClosestCreatureWithEntry(pPlayer, NPC_LURKING_SHARK, 20.0f);

        if (!pShark)
            pShark = pPlayer->SummonCreature(NPC_LURKING_SHARK, -4246.243f,
                -3922.356f, -7.488f, 5.0f,
