...
SDName: Winterspring
SD%Complete: 90
SDComment: Quest support: 4901, 5126 (Loraxs' tale missing proper gossip items
text).
...
bool GossipHello_npc_lorax(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pPlayer->GetQuestStatus(5126) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Talk to me",
    ...
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    ...
    case GOSSIP_ACTION_INFO_DEF + 4:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
            "He is not safe, i'll make sure of that.", GOSSIP_SENDER_MAIN,
...
static const DialogueEntry aIntroDialogue[] = {
    ...
    {GO_ELUNE_GEM, 0, 5000}, {SAY_PRIESTESS_ALTAR_9, NPC_PRIESTESS_DATA_1,
                              4000}, // move priestess 1 near m_creature
    ...
    {SAY_PRIESTESS_ALTAR_13, NPC_PRIESTESS_DATA_1,
     8000}, // summon voice and guard of elune
    {NPC_VOICE_ELUNE, 0, 12000},
    {SAY_VOICE_ALTAR_15, NPC_VOICE_ELUNE,
     5000}, // move priestess 2 near m_creature
    ...
    {SAY_PRIESTESS_ALTAR_20, NPC_PRIESTESS_DATA_1,
     4000}, // move the first priestess up
    {SAY_PRIESTESS_ALTAR_21, NPC_PRIESTESS_DATA_2,
...
{
    npc_ranshallaAI(Creature* pCreature)
    ...
    void Reset() override { m_uiDelayTimer = 0; }
    ...
    void DoChannelTorchSpell(bool bIsAltarWaypoint = false)
    {
        ...
        if (bIsAltarWaypoint)
        {
            if (GameObject* pGo = GetClosestGameObjectWithEntry(
                    m_creature, GO_ELUNE_ALTAR, 10.0f))
            {
        ...
        {
            if (GameObject* pGo = GetClosestGameObjectWithEntry(
                    m_creature, GO_ELUNE_FIRE, 10.0f))
                pGo->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_NO_INTERACT);
        }
    ...
    {
        // Summon 2 Elune priestess and make each of them move to a different
        // spot
        if (Creature* pPriestess = m_creature->SummonCreature(
                NPC_PRIESTESS_ELUNE, aWingThicketLocations[0].m_fX,
                aWingThicketLocations[0].m_fY, aWingThicketLocations[0].m_fZ,
                aWingThicketLocations[0].m_fO, TEMPSUMMON_CORPSE_DESPAWN, 0))
        {
            pPriestess->movement_gens.push(new movement::PointMovementGenerator(
                0, aWingThicketLocations[3].m_fX, aWingThicketLocations[3].m_fY,
                aWingThicketLocations[3].m_fZ, true, false));
            m_firstPriestessGuid = pPriestess->GetObjectGuid();
        }
        if (Creature* pPriestess = m_creature->SummonCreature(
                NPC_PRIESTESS_ELUNE, aWingThicketLocations[1].m_fX,
                aWingThicketLocations[1].m_fY, aWingThicketLocations[1].m_fZ,
                aWingThicketLocations[1].m_fO, TEMPSUMMON_CORPSE_DESPAWN, 0))
        {
            // Left priestess should have a distinct move point because she is
            // the one who starts the dialogue at point reach
            pPriestess->movement_gens.push(new movement::PointMovementGenerator(
                1, aWingThicketLocations[4].m_fX, aWingThicketLocations[4].m_fY,
                aWingThicketLocations[4].m_fZ, true, false));
        ...
        Creature* pSummoned, movement::gen type, uint32 uiPointId) override
    {
        if (type != movement::gen::point ||
            pSummoned->GetEntry() != NPC_PRIESTESS_ELUNE || uiPointId != 1)
            return;

        // Start the dialogue when the priestess reach the altar (they should
        // both reach the point in the same time)
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 41:
        {
            ...
            auto EluneLights = GetGameObjectListWithEntryInGrid(
                m_creature, GO_ELUNE_LIGHT, 20.0f);
            for (auto& elem : EluneLights)
            {
                if (!elem->isSpawned())
                {
                    elem->SetRespawnTime(115);
                    elem->Refresh();
                }
            }

            if (GameObject* pAltar =
                    m_creature->GetMap()->GetGameObject(m_altarGuid))
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case SAY_PRIESTESS_ALTAR_8:
            ...
            if (GameObject* pGem = GetClosestGameObjectWithEntry(
                    m_creature, GO_ELUNE_GEM, 10.0f))
            {
                if (pGem->isSpawned())
                    break;
        ...
        case SAY_PRIESTESS_ALTAR_9:
            ...
            if (Creature* pPriestess =
                    m_creature->GetMap()->GetCreature(m_firstPriestessGuid))
                pPriestess->movement_gens.push(
                    new movement::PointMovementGenerator(0,
                        aWingThicketLocations[6].m_fX,
                        aWingThicketLocations[6].m_fY,
                        aWingThicketLocations[6].m_fZ, true, false));
            break;
        case SAY_PRIESTESS_ALTAR_13:
            // summon the Guardian of Elune
            if (Creature* pGuard = m_creature->SummonCreature(
                    NPC_GUARDIAN_ELUNE, aWingThicketLocations[2].m_fX,
                    aWingThicketLocations[2].m_fY,
                    aWingThicketLocations[2].m_fZ,
                    aWingThicketLocations[2].m_fO, TEMPSUMMON_CORPSE_DESPAWN,
                    0))
            {
                pGuard->movement_gens.push(new movement::PointMovementGenerator(
                    0, aWingThicketLocations[5].m_fX,
                    aWingThicketLocations[5].m_fY,
                    aWingThicketLocations[5].m_fZ, true, false));
            ...
            if (GameObject* pAltar =
                    m_creature->GetMap()->GetGameObject(m_altarGuid))
            {
                if (Creature* pVoice = m_creature->SummonCreature(
                        NPC_VOICE_ELUNE, pAltar->GetX(), pAltar->GetY(),
                        pAltar->GetZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 30000))
        ...
        case SAY_VOICE_ALTAR_15:
            ...
            if (Creature* pPriestess =
                    m_creature->GetMap()->GetCreature(m_secondPriestessGuid))
            {
                DoScriptText(SAY_PRIESTESS_ALTAR_14, pPriestess);
                pPriestess->movement_gens.push(
                    new movement::PointMovementGenerator(0,
                        aWingThicketLocations[7].m_fX,
                        aWingThicketLocations[7].m_fY,
                        aWingThicketLocations[7].m_fZ, true, false));
            }
            break;
        case SAY_PRIESTESS_ALTAR_19:
            // make the voice of elune leave
            if (Creature* pGuard =
                    m_creature->GetMap()->GetCreature(m_guardEluneGuid))
            {
                pGuard->movement_gens.push(new movement::PointMovementGenerator(
                    0, aWingThicketLocations[2].m_fX,
                    aWingThicketLocations[2].m_fY,
                    aWingThicketLocations[2].m_fZ, true, false));
        ...
        case SAY_PRIESTESS_ALTAR_20:
                    ...
                    m_creature->GetMap()->GetCreature(m_firstPriestessGuid))
            {
                pPriestess->movement_gens.push(
                    new movement::PointMovementGenerator(0,
                        aWingThicketLocations[0].m_fX,
                        aWingThicketLocations[0].m_fY,
                        aWingThicketLocations[0].m_fZ, true, false));
        ...
        case SAY_PRIESTESS_ALTAR_21:
                    ...
                    m_creature->GetMap()->GetCreature(m_secondPriestessGuid))
            {
                pPriestess->movement_gens.push(
                    new movement::PointMovementGenerator(0,
                        aWingThicketLocations[1].m_fX,
                        aWingThicketLocations[1].m_fY,
                        aWingThicketLocations[1].m_fZ, true, false));
                pPriestess->ForcedDespawn(4000);
            }
            break;
        case DATA_EVENT_END:
...
bool QuestAccept_npc_ranshalla(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_GUARDIANS_ALTAR)
    {
        ...
        if (npc_ranshallaAI* pEscortAI =
                dynamic_cast<npc_ranshallaAI*>(pCreature->AI()))
...
bool GOUse_go_elune_fire(Player* /*pPlayer*/, GameObject* pGo)
{
    ...
    if (Creature* pRanshalla =
            GetClosestCreatureWithEntry(pGo, NPC_RANSHALLA, 10.0f))
    {
        if (npc_ranshallaAI* pEscortAI =
                dynamic_cast<npc_ranshallaAI*>(pRanshalla->AI()))
...
bool GossipHello_npc_rivern_frostwind(Player* player, Creature* creature)
{
    ...
    if (player->GetReputationRank(589) == REP_EXALTED)
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, GOSSIP_TEXT_BROWSE_GOODS,
...
bool GossipSelect_npc_rivern_frostwind(
    Player* player, Creature* creature, uint32, uint32 action)
{
