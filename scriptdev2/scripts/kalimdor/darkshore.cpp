...
// TODO: make concept similar as "ringo" -escort. Find a way to run the scripted
// attacks, _if_ player are choosing road.
struct MANGOS_DLL_DECL npc_kerlonianAI : public FollowerAI
{
    ...
    void Reset() override { m_uiFallAsleepTimer = urand(10000, 45000); }
                ...
                if (Player* pPlayer = GetLeaderForFollower())
                {
                    if (pPlayer->GetQuestStatus(QUEST_SLEEPER_AWAKENED) ==
                        ...
                        pPlayer->GroupEventHappens(
                            QUEST_SLEEPER_AWAKENED, m_creature);
    ...
    void ClearSleeping()
    {
        m_creature->remove_auras(SPELL_SLEEP_VISUAL);
...
bool QuestAccept_npc_kerlonian(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_SLEEPER_AWAKENED)
    {
        if (npc_kerlonianAI* pKerlonianAI =
                dynamic_cast<npc_kerlonianAI*>(pCreature->AI()))
        {
            pCreature->SetStandState(UNIT_STAND_STATE_STAND);
            DoScriptText(SAY_KER_START, pCreature, pPlayer);
            pKerlonianAI->StartFollow(
                pPlayer, FACTION_ESCORT_N_FRIEND_PASSIVE, pQuest);
        }
    }
...
bool QuestAccept_npc_prospector_remtravel(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_ABSENT_MINDED_PT2)
    {
        ...
        if (npc_prospector_remtravelAI* pEscortAI =
                dynamic_cast<npc_prospector_remtravelAI*>(pCreature->AI()))
...
bool GossipHello_npc_threshwackonator(Player* pPlayer, Creature* pCreature)
{
    if (pPlayer->GetQuestStatus(QUEST_GYROMAST_REV) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_INSERT_KEY,
...
bool GossipSelect_npc_threshwackonator(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        ...
        if (npc_threshwackonatorAI* pThreshAI =
                dynamic_cast<npc_threshwackonatorAI*>(pCreature->AI()))
        {
...
static const SummonLocation aVolcorSpawnLocs[] = {
    {4630.2f, 22.6f, 70.1f, 2.4f}, {4603.8f, 53.5f, 70.4f, 5.4f},
    {4627.5f, 100.4f, 62.7f, 5.8f}, {4692.8f, 75.8f, 56.7f, 3.1f},
    {4747.8f, 152.8f, 54.6f, 2.4f}, {4711.7f, 109.1f, 53.5f, 2.4f},
};
...
struct MANGOS_DLL_DECL npc_volcorAI : public npc_escortAI
{
    ...
    void StartEscort(Player* pPlayer, const Quest* pQuest)
    {
        ...
        if (pQuest->GetQuestId() == QUEST_ESCAPE_THROUGH_STEALTH)
        {
            ...
            m_creature->SetFactionTemporary(
                FACTION_FRIENDLY, TEMPFACTION_RESTORE_RESPAWN);
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 2:
            ...
            break;
        case 5:
            m_creature->SummonCreature(NPC_BLACKWOOD_SHAMAN,
                aVolcorSpawnLocs[0].m_fX, aVolcorSpawnLocs[0].m_fY,
                aVolcorSpawnLocs[0].m_fZ, aVolcorSpawnLocs[0].m_fO,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
            m_creature->SummonCreature(NPC_BLACKWOOD_URSA,
                aVolcorSpawnLocs[1].m_fX, aVolcorSpawnLocs[1].m_fY,
                aVolcorSpawnLocs[1].m_fZ, aVolcorSpawnLocs[1].m_fO,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
            break;
        case 6:
        ...
        case 11:
            m_creature->SummonCreature(NPC_BLACKWOOD_SHAMAN,
                aVolcorSpawnLocs[2].m_fX, aVolcorSpawnLocs[2].m_fY,
                aVolcorSpawnLocs[2].m_fZ, aVolcorSpawnLocs[2].m_fO,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
            m_creature->SummonCreature(NPC_BLACKWOOD_URSA,
                aVolcorSpawnLocs[3].m_fX, aVolcorSpawnLocs[3].m_fY,
                aVolcorSpawnLocs[3].m_fZ, aVolcorSpawnLocs[3].m_fO,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
        case 13:
            m_creature->SummonCreature(NPC_BLACKWOOD_URSA,
                aVolcorSpawnLocs[4].m_fX, aVolcorSpawnLocs[4].m_fY,
                aVolcorSpawnLocs[4].m_fZ, aVolcorSpawnLocs[4].m_fO,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
            m_creature->SummonCreature(NPC_BLACKWOOD_URSA,
                aVolcorSpawnLocs[5].m_fX, aVolcorSpawnLocs[5].m_fY,
                aVolcorSpawnLocs[5].m_fZ, aVolcorSpawnLocs[5].m_fO,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
            break;
        case 15:
            ...
            if (Player* pPlayer = GetPlayerForEscort())
                pPlayer->GroupEventHappens(
                    QUEST_ESCAPE_THROUGH_FORCE, m_creature);
            SetEscortPaused(true);
        ...
        case 24:
            if (Player* pPlayer = GetPlayerForEscort())
                pPlayer->GroupEventHappens(
                    QUEST_ESCAPE_THROUGH_STEALTH, m_creature);
            break;
        }
    }
};
...
bool QuestAccept_npc_volcor(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_ESCAPE_THROUGH_FORCE ||
        pQuest->GetQuestId() == QUEST_ESCAPE_THROUGH_STEALTH)
    {
        if (npc_volcorAI* pEscortAI =
                dynamic_cast<npc_volcorAI*>(pCreature->AI()))
