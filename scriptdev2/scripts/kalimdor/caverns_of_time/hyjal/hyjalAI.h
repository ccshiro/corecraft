...
enum YellType
{
    ...
    RALLY = 3,    // Used to rally the raid and warn that the next wave has been
                  // summoned
...
struct MANGOS_DLL_DECL hyjalAI : public ScriptedAI
{
    ...
    // Generically used to reset our variables. Do *not* call in EnterEvadeMode
    // as this may make problems if the raid is still in combat
    void Reset() override;
    ...
    // Used to reset cooldowns for our spells and to inform the raid that we're
    // under attack
    ...
    // Searches for the appropriate yell and sound and uses it to inform the
    // raid of various things
