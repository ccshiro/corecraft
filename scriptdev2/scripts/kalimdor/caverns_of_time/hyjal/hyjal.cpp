...
#define GOSSIP_ITEM_BEGIN_ALLY \
    "My companions and I are with you, Lady Proudmoore."
#define GOSSIP_ITEM_ANETHERON                                        \
    "We are ready for whatever Archimonde might send our way, Lady " \
    "Proudmoore."
...
bool GossipHello_npc_jaina_proudmoore(Player* pPlayer, Creature* pCreature)
{
            ...
            (ScriptedInstance*)pCreature->GetInstanceData())
    {
        if (hyjalAI* pJainaAI = dynamic_cast<hyjalAI*>(pCreature->AI()))
        {
            ...
            {
                // Should not happen that jaina is here now, but for safe we
                // check
                if (pInstance->GetData(TYPE_KAZROGAL) != DONE)
                {
                    if (pInstance->GetData(TYPE_WINTERCHILL) == NOT_STARTED)
                        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
                            GOSSIP_ITEM_BEGIN_ALLY, GOSSIP_SENDER_MAIN,
                            GOSSIP_ACTION_INFO_DEF + 1);
                    else if (pInstance->GetData(TYPE_WINTERCHILL) == DONE &&
                             pInstance->GetData(TYPE_ANETHERON) == NOT_STARTED)
                        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
                            GOSSIP_ITEM_ANETHERON, GOSSIP_SENDER_MAIN,
                            GOSSIP_ACTION_INFO_DEF + 2);
                    else if (pInstance->GetData(TYPE_ANETHERON) == DONE)
                        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
                            GOSSIP_ITEM_RETREAT, GOSSIP_SENDER_MAIN,
                            GOSSIP_ACTION_INFO_DEF + 3);

                    if (pPlayer->isGameMaster())
                        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT,
                            "[GM] Toggle Debug Timers", GOSSIP_SENDER_MAIN,
...
bool GossipSelect_npc_jaina_proudmoore(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    if (hyjalAI* pJainaAI = dynamic_cast<hyjalAI*>(pCreature->AI()))
    {
        ...
        case GOSSIP_ACTION_INFO_DEF:
            ...
            LOG_DEBUG(logging, "SD2: HyjalAI - Debug mode has been toggled");
            break;
        }
    }
            ...
            if (!pThrallAI->m_bIsEventInProgress)
            {
                ...
                if (pInstance->GetData(TYPE_ANETHERON) == DONE)
                {
                    if (pInstance->GetData(TYPE_KAZROGAL) == NOT_STARTED)
                        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
                            GOSSIP_ITEM_BEGIN_HORDE, GOSSIP_SENDER_MAIN,
                            GOSSIP_ACTION_INFO_DEF + 1);
                    else if (pInstance->GetData(TYPE_KAZROGAL) == DONE &&
                             pInstance->GetData(TYPE_AZGALOR) == NOT_STARTED)
                        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
                            GOSSIP_ITEM_AZGALOR, GOSSIP_SENDER_MAIN,
                            GOSSIP_ACTION_INFO_DEF + 2);
...
bool GossipHello_npc_tyrande_whisperwind(Player* pPlayer, Creature* pCreature)
{
            ...
            (ScriptedInstance*)pCreature->GetInstanceData())
    {
        ...
        if (pInstance->GetData(TYPE_AZGALOR) == DONE &&
            !pPlayer->HasItemCount(ITEM_TEAR_OF_GODDESS, 1))
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_TYRANDE,
...
bool GossipSelect_npc_tyrande_whisperwind(Player* pPlayer,
    Creature* /*pCreature*/, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF)
        pPlayer->add_item(ITEM_TEAR_OF_GODDESS, 1);
