...
SD%Complete: 85
SDComment: Doomfires not completely offlike due to core limitations for random
moving. Tyrande and second phase not fully implemented.
...
// text id -1534018 are the text used when previous events complete. Not part of
// this script.
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (CheckTimer < diff)
        {
            if (Creature* Archimonde =
                    m_pInstance->GetSingleCreatureFromStorage(NPC_ARCHIMONDE))
            {
                if (Archimonde->GetHealthPercent() < 2.0f ||
                    !Archimonde->isAlive())
...
/* This script is merely a placeholder for the Doomfire that triggers Doomfire
   spell. It will
   MoveChase the Doomfire Spirit always, until despawn (AttackStart is called
   upon it's spawn) */
...
/* This is the script for the Doomfire Spirit Mob. This mob simply follow
   players or
...
struct MANGOS_DLL_DECL mob_doomfire_targettingAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pWwho) override
    {
        // will update once m_targetGuid is 0. In case noone actually moves(not
        // likely) and this is 0
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiChangeTargetTimer < uiDiff)
        {
            ...
            {
                /* m_creature->GetMotionMaster()->MoveFollow(pPlayer, 0.0f,
                 * 0.0f); */
                m_targetGuid.Clear();
            }
            else
            {
                ...
                /*     2 * M_PI_F * rand_norm_f(), 40.0f * rand_norm_f()); */
...
/* Finally, Archimonde's script. His script isn't extremely complex, most are
   simply spells on timers.
   The only complicated aspect of the battle is Finger of Death and Doomfire,
   with Doomfire being the
   hardest bit to code. Finger of Death is simply a distance check - if no one
   is in melee range, then
   select a random target and cast the spell on them. However, if someone IS in
   melee range, and this
   is NOT the main tank (creature's victim), then we aggro that player and they
   become the new victim.
   For Doomfire, we summon a mob (Doomfire Spirit) for the Doomfire mob to
   follow. It's spirit will
   randomly select it's target to follow and then we create the random movement
...
struct MANGOS_DLL_DECL boss_archimondeAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        CheckDistanceTimer = 30000; // This checks if he's too close to the
                                    // World Tree (75 yards from a point on the
    ...
    bool CanUseFingerOfDeath()
    {
        ...
        if (victim &&
            m_creature->IsWithinDistInMap(
                victim, m_creature->GetAggroDistance(victim)))
            return false;
        ...
        if (target)
        {
            if (!m_creature->IsWithinDistInMap(
                    target, m_creature->GetAggroDistance(target)))
                return true; // Cast Finger of Death
            else             // This target is closest, he is our new tank
                m_creature->AddThreat(
                    target, m_creature->getThreatManager().getThreat(
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == CREATURE_DOOMFIRE)
        {
            ...
            pSummoned->CastSpell(pSummoned, SPELL_DOOMFIRE, true, NULL, NULL,
                m_creature->GetObjectGuid());

            if (Creature* pDoomfireSpirit =
                    m_creature->GetMap()->GetCreature(m_doomfireSpiritGuid))
            {
                /* pSummoned->GetMotionMaster()->MoveFollow( */
                /*     pDoomfireSpirit, 0.0f, 0.0f); */
                m_doomfireSpiritGuid.Clear();
            }
        }
    }

    // this is code doing close to what the summoning spell would do (spell
    // 31903)
    void SummonDoomfire(Unit* target)
    {
        m_creature->SummonCreature(CREATURE_DOOMFIRE_SPIRIT,
            target->GetX() + 15.0, target->GetY() + 15.0, target->GetZ(), 0,
            TEMPSUMMON_TIMED_DESPAWN, 27000);

        m_creature->SummonCreature(CREATURE_DOOMFIRE, target->GetX() - 15.0,
            target->GetY() - 15.0, target->GetZ(), 0, TEMPSUMMON_TIMED_DESPAWN,
            27000);
    }
    ...
    void UpdateAI(const uint32 diff) override
    {
        if (!m_creature->isInCombat())
        {
            if (m_pInstance)
            {
                // Do not let the raid skip straight to Archimonde. Visible and
                // hostile ONLY if Azagalor is finished.
                if ((m_pInstance->GetData(TYPE_AZGALOR) < DONE) &&
                    ((m_creature->GetVisibility() != VISIBILITY_OFF) ||
                        (m_creature->getFaction() != 35)))
                {
                    m_creature->SetVisibility(VISIBILITY_OFF);
                    m_creature->setFaction(35);
                }
                else if ((m_pInstance->GetData(TYPE_AZGALOR) >= DONE) &&
            ...
            if (DrainNordrassilTimer < diff)
            {
                if (!IsChanneling)
                {
                    Creature* temp = m_creature->SummonCreature(
                        CREATURE_CHANNEL_TARGET, NORDRASSIL_X, NORDRASSIL_Y,
                        NORDRASSIL_Z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 1200000);

                    if (temp)
                        m_worldTreeGuid = temp->GetObjectGuid();

                    if (Creature* Nordrassil =
                            m_creature->GetMap()->GetCreature(m_worldTreeGuid))
                    {
                        Nordrassil->SetFlag(
                ...
                if (Creature* Nordrassil =
                        m_creature->GetMap()->GetCreature(m_worldTreeGuid))
                {
                    Nordrassil->CastSpell(
                        m_creature, SPELL_DRAIN_WORLD_TREE_2, true);
        ...
        if (m_creature->GetHealthPercent() < 10.0f && !BelowTenPercent &&
            !Enraged)
        ...
        if (!Enraged)
        {
            ...
            {
                // To simplify the check, we simply summon a creature in the
                // location and then check how far we are from the creature
                Creature* Check = m_creature->SummonCreature(
                    CREATURE_CHANNEL_TARGET, NORDRASSIL_X, NORDRASSIL_Y,
        ...
        if (BelowTenPercent)
        {
            if (!HasProtected)
            {
                ...
                DoCastSpellIfCan(
                    m_creature->getVictim(), SPELL_PROTECTION_OF_ELUNE);
                HasProtected = true;
                Enraged = true;
            }
            ...
            {
                DoSpawnCreature(CREATURE_ANCIENT_WISP, urand(0, 39),
                    urand(0, 39), 0, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                    15000);
