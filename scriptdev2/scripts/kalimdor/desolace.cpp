...
enum
{
    ...
    SPELL_KODO_KOMBO_PLAYER_BUFF = 18172, // spells here have unclear function,
                                          // but using them at least for visual
                                          // parts and checks
...
struct MANGOS_DLL_DECL npc_aged_dying_ancient_kodoAI : public ScriptedAI
{
    ...
    void Reset() override { m_uiDespawnTimer = 0; }
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        if (pWho->GetEntry() == NPC_SMEED)
        {
            ...
            if (m_creature->IsWithinDistInMap(pWho, 10.0f))
            {
                ...
                // spell have no implemented effect (dummy), so useful to notify
                // spellHit
                m_creature->CastSpell(
                    m_creature, SPELL_KODO_KOMBO_GOSSIP, true);
            }
        }
    }
    ...
    {
        // timer should always be == 0 unless we already updated entry of
        // creature. Then not expect this updated to ever be in combat.
...
bool EffectDummyCreature_npc_aged_dying_ancient_kodo(Unit* pCaster,
    uint32 spellId, SpellEffectIndex effIndex, Creature* pCreatureTarget)
{
    // always check spellid and effectindex
    if (spellId == SPELL_KODO_KOMBO_ITEM && effIndex == EFFECT_INDEX_0)
    {
        // no effect if player/creature already have aura from spells
        if (pCaster->has_aura(SPELL_KODO_KOMBO_PLAYER_BUFF) ||
            pCreatureTarget->has_aura(SPELL_KODO_KOMBO_DESPAWN_BUFF))
            return true;
            ...
            pCreatureTarget->GetEntry() == NPC_ANCIENT_KODO)
        {
            ...
            pCreatureTarget->CastSpell(
                pCreatureTarget, SPELL_KODO_KOMBO_DESPAWN_BUFF, false);

            pCreatureTarget->movement_gens.remove_all(movement::gen::waypoint);
            pCreatureTarget->movement_gens.push(
                new movement::FollowMovementGenerator(pCaster));
        }
...
bool GossipHello_npc_aged_dying_ancient_kodo(
    Player* pPlayer, Creature* pCreature)
{
    if (pPlayer->has_aura(SPELL_KODO_KOMBO_PLAYER_BUFF) &&
        pCreature->has_aura(SPELL_KODO_KOMBO_DESPAWN_BUFF))
    {
        // the expected quest objective
        pPlayer->TalkedToCreature(
            pCreature->GetEntry(), pCreature->GetObjectGuid());

        pPlayer->remove_auras(SPELL_KODO_KOMBO_PLAYER_BUFF);
        pCreature->movement_gens.remove_all(movement::gen::follow);
    }
        ...
        if (uiPointId == 18)
        {
            if (Player* pPlayer = GetPlayerForEscort())
                pPlayer->GroupEventHappens(
                    QUEST_RETURN_TO_VAHLARRIEL, m_creature);
        }
    }
};
...
bool QuestAccept_npc_dalinda_malem(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_RETURN_TO_VAHLARRIEL)
    {
        if (npc_dalinda_malemAI* pEscortAI =
                dynamic_cast<npc_dalinda_malemAI*>(pCreature->AI()))
        {
            // TODO This faction change needs confirmation, also possible that
            // we need to drop her PASSIVE flag
...
static const DialogueEntry aIntroDialogue[] = {
    ...
    {POINT_ID_EVENT_COMPLETE, 0, 2000}, {SAY_MELIZZA_1, NPC_MELIZZA, 4000},
    {SAY_MELIZZA_2, NPC_MELIZZA, 5000}, {SAY_MELIZZA_3, NPC_MELIZZA, 4000},
    {NPC_MELIZZA, 0, 0}, {0, 0, 0},
};
...
static const SummonLocation aMarauderSpawn[] = {
    {-1291.492f, 2644.650f, 111.556f}, {-1306.730f, 2675.163f, 111.561f},
};
...
{
    npc_melizza_brimbuzzleAI(Creature* m_creature)
      : npc_escortAI(m_creature), DialogueHelper(aIntroDialogue)
    {
    ...
    void JustStartedEscort() override
    {
        if (GameObject* pCage = GetClosestGameObjectWithEntry(
                m_creature, GO_MELIZZAS_CAGE, INTERACTION_DISTANCE))
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 1:
            ...
            break;
        case 4:
            for (auto& elem : aMarauderSpawn)
            {
                for (uint8 j = 0; j < MAX_MARAUDERS; ++j)
                {
                    // Summon 2 Marauders on each point
                    auto pos = m_creature->GetPointXYZ(
                        G3D::Vector3(elem.m_fX, elem.m_fY, elem.m_fZ),
                        2 * M_PI_F * rand_norm_f(), 7.0f * rand_norm_f());
                    m_creature->SummonCreature(NPC_MARAUDINE_MARAUDER, pos.x,
                        pos.y, pos.z, 0.0f, TEMPSUMMON_DEAD_DESPAWN, 0);
                }
            }
            break;
        case 9:
            ...
            {
                auto pos = m_creature->GetPointXYZ(
                    G3D::Vector3(aWranglerSpawn.m_fX, aWranglerSpawn.m_fY,
                        aWranglerSpawn.m_fZ),
                    2 * M_PI_F * rand_norm_f(), 10.0f * rand_norm_f());
                m_creature->SummonCreature(NPC_MARAUDINE_BONEPAW, pos.x, pos.y,
                ...
                pos = m_creature->GetPointXYZ(
                    G3D::Vector3(aWranglerSpawn.m_fX, aWranglerSpawn.m_fY,
                        aWranglerSpawn.m_fZ),
                    2 * M_PI_F * rand_norm_f(), 10.0f * rand_norm_f());
                m_creature->SummonCreature(NPC_MARAUDINE_WRANGLER, pos.x, pos.y,
                    pos.z, 0.0f, TEMPSUMMON_DEAD_DESPAWN, 0);
            }
            break;
        case 12:
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case QUEST_GET_ME_OUT_OF_HERE:
            if (Player* pPlayer = GetPlayerForEscort())
            {
                ...
                pPlayer->GroupEventHappens(
                    QUEST_GET_ME_OUT_OF_HERE, m_creature);
            }
...
bool QuestAccept_npc_melizza_brimbuzzle(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_GET_ME_OUT_OF_HERE)
    {
        if (npc_melizza_brimbuzzleAI* pEscortAI =
                dynamic_cast<npc_melizza_brimbuzzleAI*>(pCreature->AI()))
            pEscortAI->Start(false, pPlayer, pQuest);
    }
...
void AddSC_desolace()
{
    ...
    pNewScript->pEffectDummyNPC =
        &EffectDummyCreature_npc_aged_dying_ancient_kodo;
