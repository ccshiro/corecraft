...
{
    npc_cairne_bloodhoofAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
...
bool GossipHello_npc_cairne_bloodhoof(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pPlayer->GetQuestStatus(925) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,
            "I know this is rather silly but a young ward who is a bit shy "
            "would like your hoofprint.",
            GOSSIP_SENDER_MAIN, GOSSIP_SENDER_INFO);
...
bool GossipSelect_npc_cairne_bloodhoof(
