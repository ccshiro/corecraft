...
struct MANGOS_DLL_DECL npc_lazy_peonAI : public ScriptedAI
{
    ...
    void Reset() override { m_uiResetSleepTimer = 0; }

    void MovementInform(movement::gen uiMotionType, uint32 uiPointId) override
    {
        if (uiMotionType != movement::gen::point || !uiPointId)
            return;
...
bool EffectDummyCreature_lazy_peon_awake(Unit* pCaster, uint32 uiSpellId,
    SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
    // always check spellid and effectindex
    if (uiSpellId == SPELL_AWAKEN_PEON && uiEffIndex == EFFECT_INDEX_0)
    {
        if (!pCreatureTarget->has_aura(SPELL_PEON_SLEEP) ||
            pCaster->GetTypeId() != TYPEID_PLAYER ||
            pCreatureTarget->GetEntry() != NPC_SLEEPING_PEON)
            return true;

        if (GameObject* pLumber = GetClosestGameObjectWithEntry(
                pCreatureTarget, GO_LUMBERPILE, 15.0f))
        {
            pCreatureTarget->remove_auras(SPELL_PEON_SLEEP);
            DoScriptText(urand(0, 1) ? SAY_PEON_AWAKE_1 : SAY_PEON_AWAKE_2,
                pCreatureTarget);
            ((Player*)pCaster)
                ->KilledMonsterCredit(pCreatureTarget->GetEntry());

            auto pos = pLumber->GetPoint(pCreatureTarget);
            ...
            logging.error(
                "SD2: No GameObject of entry %u was found in range or "
