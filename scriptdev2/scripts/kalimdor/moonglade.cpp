...
SDName: Moonglade
SD%Complete: 100
SDComment: Quest support: 30, 272, 5929, 5930, 8736, 10965. Special Flight Paths
for Druid class.
...
bool GossipHello_npc_bunthen_plainswind(Player* pPlayer, Creature* pCreature)
{
    ...
    {
        if (pPlayer->GetQuestStatus(QUEST_SEA_LION_ALLY) ==
            QUEST_STATUS_INCOMPLETE)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_AQ_END,
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_THUNDER,
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

        if (pPlayer->GetQuestStatus(QUEST_SEA_LION_HORDE) ==
            QUEST_STATUS_INCOMPLETE)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_AQ_END,
...
bool GossipSelect_npc_bunthen_plainswind(
...
struct MANGOS_DLL_DECL npc_clintar_dw_spiritAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i) override
    {
        ...
        case 27:
            ...
            break;
        case 31:
            m_creature->SummonCreature(NPC_ASPECT_OF_RAVEN, 7465.321f,
                -3088.515f, 429.006f, 5.550f,
    ...
    void Reset() override
    {
        ...
        // m_creature are expected to always be spawned, but not visible for
        // player
        // spell casted from quest_template.SrcSpell require this to be this way
        // we handle the triggered spell to get a "hook" to our guy so he can be
        // escorted on quest accept

        if (CreatureInfo const* pTemp =
                GetCreatureTemplateStore(m_creature->GetEntry()))
    ...
    {
        // not the best way, maybe check in DummyEffect if this creature are
        // "free" and not in escort.
        if (HasEscortState(STATE_ESCORT_ESCORTING))
            return;

        m_creature->SetVisibility(VISIBILITY_ON);
        m_creature->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
        Start(false, pStarter && pStarter->GetTypeId() == TYPEID_PLAYER ?
                         (Player*)pStarter :
                         NULL);
    }
...
bool EffectDummyCreature_npc_clintar_dw_spirit(Unit* pCaster, uint32 spellId,
    SpellEffectIndex effIndex, Creature* pCreatureTarget)
{
    // always check spellid and effectindex
    if (spellId == SPELL_EMERALD_DREAM && effIndex == EFFECT_INDEX_0)
    {
        if (pCaster->GetTypeId() != TYPEID_PLAYER ||
            pCaster->has_aura(SPELL_EMERALD_DREAM))
            return true;
        ...
        if (CreatureInfo const* pTemp =
                GetCreatureTemplateStore(NPC_CLINTAR_SPIRIT))
            pCreatureTarget->SetDisplayId(Creature::ChooseDisplayId(pTemp));
        else
            return true;

        // done here, escort can start
        if (npc_clintar_dw_spiritAI* pSpiritAI =
                dynamic_cast<npc_clintar_dw_spiritAI*>(pCreatureTarget->AI()))
...
#define GOSSIP_BEAR4                                                       \
    "I have heard your words, Great Bear Spirit, and I understand. I now " \
    "seek your blessings to fully learn the way of the Claw."
...
bool GossipHello_npc_great_bear_spirit(Player* pPlayer, Creature* pCreature)
{
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BEAR1,
...
bool GossipSelect_npc_great_bear_spirit(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    switch (uiAction)
    {
    case GOSSIP_ACTION_INFO_DEF:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BEAR2,
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        pPlayer->SEND_GOSSIP_MENU(4721, pCreature->GetObjectGuid());
        break;
    case GOSSIP_ACTION_INFO_DEF + 1:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BEAR3,
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
        pPlayer->SEND_GOSSIP_MENU(4733, pCreature->GetObjectGuid());
        break;
    case GOSSIP_ACTION_INFO_DEF + 2:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BEAR4,
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
...
bool GossipHello_npc_silva_filnaveth(Player* pPlayer, Creature* pCreature)
{
    ...
    else if (pPlayer->GetTeam() != ALLIANCE)
    {
        if (pPlayer->GetQuestStatus(QUEST_SEA_LION_HORDE) ==
            QUEST_STATUS_INCOMPLETE)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_AQ_AGI,
                GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_RUTHERAN,
            GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
bool GossipSelect_npc_silva_filnaveth(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    switch (uiAction)
    {
...
enum
{
    ...
    // SPELL_MOONGLADE_TRANQUILITY = unk,            // spell which acts as a
    // spotlight over Eranikus after he is redeemed

    NPC_ERANIKUS_TYRANT = 15491,
    NPC_NIGHTMARE_PHANTASM = 15629, // shadows summoned during the event -
                                    // should cast 17228 and 21307
    NPC_REMULOS = 11832,
    NPC_TYRANDE_WHISPERWIND = 15633, // appears with the priestess during the
                                     // event to help the players - should cast
                                     // healing spells
...
static const DialogueEntry aIntroDialogue[] = {
    ...
    {NPC_ERANIKUS_TYRANT, 0, 0}, // target player - restart the escort and move
                                 // Eranikus above the village
    {SAY_REMULOS_DEFEND_2, NPC_REMULOS, 6000}, // face Eranikus
    {SAY_ERANIKUS_SHADOWS, NPC_ERANIKUS_TYRANT, 4000},
    {SAY_REMULOS_DEFEND_3, NPC_REMULOS, 0}, {0, 0, 0},
};
...
static EventLocations aTyrandeLocations[] = {
    // Tyrande should appear along the pathway, but because of the missing
    // pathfinding we'll summon here closer to Eranikus
...
static EventLocations aShadowsLocations[] = {
    ...
    {7832.78f, -2604.57f, 489.29f, 0}, {7826.68f, -2538.46f, 489.30f, 0},
    {7811.48f, -2573.20f, 488.49f, 0},
    // Outside shade points - basically only the first set of coords is used for
    // the summoning; there is no solid proof of using the other coords
    {7888.32f, -2566.25f, 487.02f, 0}, {7946.12f, -2577.10f, 489.97f, 0},
    {7963.00f, -2492.03f, 487.84f, 0}};

struct MANGOS_DLL_DECL npc_keeper_remulosAI : public npc_escortAI,
                                              private DialogueHelper
{
    npc_keeper_remulosAI(Creature* pCreature)
      : npc_escortAI(pCreature), DialogueHelper(aIntroDialogue)
    {
        Reset();
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        case NPC_ERANIKUS_TYRANT:
            ...
            // ToDo: uncomment the fly effect when it will be possible to cancel
            // it properly
            // pSummoned->SetByteValue(UNIT_FIELD_BYTES_1, 3,
        ...
        Creature* pSummoned, movement::gen type, uint32 uiPointId) override
    {
        if (type != movement::gen::point ||
            pSummoned->GetEntry() != NPC_ERANIKUS_TYRANT)
            return;
    ...
    void JustDied(Unit* pKiller) override
    {
        ...
        if (Creature* pEranikus =
                m_creature->GetMap()->GetCreature(m_eranikusGuid))
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case NPC_ERANIKUS_TYRANT:
            ...
            if (Creature* pEranikus =
                    m_creature->GetMap()->GetCreature(m_eranikusGuid))
                pEranikus->movement_gens.push(
                    new movement::PointMovementGenerator(
                        POINT_ID_ERANIKUS_FLIGHT, aEranikusLocations[1].m_fX,
                        aEranikusLocations[1].m_fY, aEranikusLocations[1].m_fZ,
                        true, false));
    ...
    void UpdateEscortAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiOutroTimer)
        {
            if (m_uiOutroTimer <= uiDiff)
            {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    // Despawn Remulos after the outro is finished - he will
                    // respawn automatically at his home position after a few
                    // min
            ...
            if (m_uiShadesummonTimer <= uiDiff)
            {
                ...
                if (m_bIsFirstWave)
                {
                    ...
                    for (uint8 i = 0; i < MAX_SHADOWS; ++i)
                        m_creature->SummonCreature(NPC_NIGHTMARE_PHANTASM,
                            aShadowsLocations[i].m_fX,
                            aShadowsLocations[i].m_fY,
                            aShadowsLocations[i].m_fZ, 0,
                ...
                // Summon 3 shades per turn until the maximum summon turns are
                // reached
                // Randomize the summon point
                uint8 uiSummonPoint =
                    roll_chance_i(70) ?
                        (uint8)MAX_SHADOWS :
                        (uint8)urand(MAX_SHADOWS + 1, MAX_SHADOWS + 2);
                ...
                if (m_uiSummonCount < MAX_SUMMON_TURNS)
                {
                    for (uint8 i = 0; i < MAX_SHADOWS; ++i)
                    {
                        auto pos = m_creature->GetPointXYZ(
                            G3D::Vector3(aShadowsLocations[uiSummonPoint].m_fX,
                                aShadowsLocations[uiSummonPoint].m_fY,
                                aShadowsLocations[uiSummonPoint].m_fZ),
                            2 * M_PI_F * rand_norm_f(), 10.0f * rand_norm_f());
                        m_creature->SummonCreature(NPC_NIGHTMARE_PHANTASM,
                            pos.x, pos.y, pos.z, 0.0f, TEMPSUMMON_DEAD_DESPAWN,
                            0);
                    }
                ...
                // We don't count the dead shades, because the boss is usually
                // set in combat before all shades are dead
                if (m_uiSummonCount == MAX_SUMMON_TURNS)
                {
                    m_uiShadesummonTimer = 0;
                            ...
                            m_creature->GetMap()->GetCreature(m_eranikusGuid))
                    {
                        ...
                        pEranikus->movement_gens.push(
                            new movement::PointMovementGenerator(
                                POINT_ID_ERANIKUS_COMBAT,
                                aEranikusLocations[2].m_fX,
                                aEranikusLocations[2].m_fY,
                                aEranikusLocations[2].m_fZ, false, true));
                    }
                }
                else
        ...
        {
            if (Unit* pTarget =
                    DoSelectLowestHpFriendly(DEFAULT_VISIBILITY_DISTANCE))
            {
                switch (urand(0, 2))
                {
                case 0:
...
bool QuestAccept_npc_keeper_remulos(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_NIGHTMARE_MANIFESTS)
    {
        if (npc_keeper_remulosAI* pEscortAI =
                dynamic_cast<npc_keeper_remulosAI*>(pCreature->AI()))
            pEscortAI->Start(true, pPlayer, pQuest);

        return true;
    }

    // Return false for other quests in order to handle DB scripts. Example:
    // quest 8447
    return false;
}

bool EffectDummyCreature_conjure_rift(Unit* pCaster, uint32 uiSpellId,
    SpellEffectIndex uiEffIndex, Creature* /*pCreatureTarget*/)
{
    // always check spellid and effectindex
    if (uiSpellId == SPELL_CONJURE_RIFT && uiEffIndex == EFFECT_INDEX_0)
    {
        pCaster->SummonCreature(NPC_ERANIKUS_TYRANT, aEranikusLocations[0].m_fX,
            aEranikusLocations[0].m_fY, aEranikusLocations[0].m_fZ,
            aEranikusLocations[0].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0);
...
enum
{
    ...
    SPELL_ARCANE_CHANNELING =
        23017, // used by Tyrande - not sure if it's the right id
...
struct MANGOS_DLL_DECL boss_eranikusAI : public ScriptedAI
{
    ...
    void EnterEvadeMode(bool by_group = false) override
    {
        if (m_creature->GetHealthPercent() < 20.0f)
        {
            ...
            if (Creature* pRemulos =
                    GetClosestCreatureWithEntry(m_creature, NPC_REMULOS, 50.0f))
            {
        ...
        {
            // There may be a core issue related to the reached home function
            // for summoned creatures so we are cleaning things up here
            // if the creature evades while the event is in progress then we
            // despawn all the summoned, including himself
            m_creature->ForcedDespawn();
            DoDespawnSummoned();

            if (Creature* pTyrande =
                    m_creature->GetMap()->GetCreature(m_tyrandeGuid))
    ...
    void DoSummonHealers()
    {
        ...
        {
            auto pos = m_creature->GetPointXYZ(
                G3D::Vector3(aTyrandeLocations[0].m_fX,
                    aTyrandeLocations[0].m_fY, aTyrandeLocations[0].m_fZ),
                2 * M_PI_F * rand_norm_f(), 10.0f * rand_norm_f());
            m_creature->SummonCreature(NPC_ELUNE_PRIESTESS, pos.x, pos.y, pos.z,
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        case NPC_TYRANDE_WHISPERWIND:
            ...
            pSummoned->movement_gens.push(
                new movement::PointMovementGenerator(POINT_ID_TYRANDE_HEAL,
                    aTyrandeLocations[1].m_fX, aTyrandeLocations[1].m_fY,
                    aTyrandeLocations[1].m_fZ, true, true));
            break;
        case NPC_ELUNE_PRIESTESS:
            m_lPriestessList.push_back(pSummoned->GetObjectGuid());
            auto pos = m_creature->GetPointXYZ(
                G3D::Vector3(aTyrandeLocations[1].m_fX,
                    aTyrandeLocations[1].m_fY, aTyrandeLocations[1].m_fZ),
                2 * M_PI_F * rand_norm_f(), 10.0f * rand_norm_f());
            pSummoned->movement_gens.push(new movement::PointMovementGenerator(
                POINT_ID_TYRANDE_HEAL, pos.x, pos.y, pos.z, true, true));
            break;
        }
    }
    ...
    {
        for (GUIDList::const_iterator itr = m_lPriestessList.begin();
             itr != m_lPriestessList.end(); ++itr)
        {
        ...
        Creature* pSummoned, movement::gen type, uint32 uiPointId) override
    {
        ...
        case POINT_ID_TYRANDE_HEAL:
            ...
            // Unmount the priestess - unk what is their exact purpose (maybe
            // healer)
    ...
    void MovementInform(movement::gen uiType, uint32 uiPointId) override
    {
        if (uiType != movement::gen::point ||
            uiPointId != POINT_ID_ERANIKUS_REDEEMED)
            return;
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiEventTimer)
        {
            if (m_uiEventTimer <= uiDiff)
            {
                ...
                {
                case 0:
                    // Eranikus is redeemed - make Tyrande kneel and stop
                    // casting
                    if (Creature* pTyrande =
                            m_creature->GetMap()->GetCreature(m_tyrandeGuid))
                    {
                    ...
                    if (Creature* pRemulos =
                            m_creature->GetMap()->GetCreature(m_remulosGuid))
                        pRemulos->SetFacingToObject(m_creature);
                    // Note: this emote was a world wide yellow emote before
                    // WotLK
                    DoScriptText(EMOTE_ERANIKUS_REDEEM, m_creature);
                    // DoCastSpellIfCan(m_creature,
                    // SPELL_MOONGLADE_TRANQUILITY);        // spell id unk for
                    // the moment
                ...
                case 3:
                    ...
                    m_creature->movement_gens.push(
                        new movement::PointMovementGenerator(
                            POINT_ID_ERANIKUS_REDEEMED,
                            aEranikusLocations[3].m_fX,
                            aEranikusLocations[3].m_fY,
                            aEranikusLocations[3].m_fZ, true, false));
                    m_uiEventTimer = 0;
                    break;
                case 4:
                ...
                case 7:
                    ...
                    if (Creature* pRemulos =
                            m_creature->GetMap()->GetCreature(m_remulosGuid))
                        ((npc_keeper_remulosAI*)pRemulos->AI())
                            ->DoHandleOutro(m_creature);
        ...
        if (m_uiTyrandeMoveTimer)
        {
            if (m_uiTyrandeMoveTimer <= uiDiff)
            {
                if (Creature* pTyrande =
                        m_creature->GetMap()->GetCreature(m_tyrandeGuid))
                    pTyrande->movement_gens.push(
                        new movement::PointMovementGenerator(
                            POINT_ID_TYRANDE_ABSOLUTION,
                            aTyrandeLocations[2].m_fX,
                            aTyrandeLocations[2].m_fY,
                            aTyrandeLocations[2].m_fZ, true, false));
                m_uiTyrandeMoveTimer = 0;
            }
            else
        ...
        // Not sure if this should be handled by health percent, but this is the
        // only reasonable way
        if (m_creature->GetHealthPercent() < m_uiHealthCheck)
        {
            ...
            case 85:
                ...
                // Here Tyrande only yells but she doesn't appear anywhere - we
                // summon here for 1 second just to handle the yell
                if (Creature* pTyrande = m_creature->SummonCreature(
                        NPC_TYRANDE_WHISPERWIND, aTyrandeLocations[0].m_fX,
                        aTyrandeLocations[0].m_fY, aTyrandeLocations[0].m_fZ, 0,
            ...
            case 50:
                ...
                m_creature->SummonCreature(NPC_TYRANDE_WHISPERWIND,
                    aTyrandeLocations[0].m_fX, aTyrandeLocations[0].m_fY,
                    aTyrandeLocations[0].m_fZ, 0, TEMPSUMMON_CORPSE_DESPAWN, 0);
                m_uiHealthCheck = 35;
                break;
            case 35:
        ...
        {
            if (DoCastSpellIfCan(m_creature, SPELL_SHADOWBOLT_VOLLEY) ==
                CAST_OK)
