...
instance_dire_maul::instance_dire_maul(Map* pMap)
  : ScriptedInstance(pMap), m_bWallDestroyed(false)
{
    Initialize();
}
...
void instance_dire_maul::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_ZEVRIM:
        if (uiData == DONE)
        {
            ...
            if (Creature* pIronbark =
                    GetSingleCreatureFromStorage(NPC_OLD_IRONBARK))
            {
    ...
    case TYPE_IRONBARK:
        m_auiEncounter[uiType] = uiData;
        break;
    case TYPE_ALZZIN: // This Encounter is expected to be handled within Acid
                      // (reason handling at 50% hp)
        if (uiData == DONE)
        {
            if (!m_bWallDestroyed)
            {
            ...
            {
                for (
                    GUIDList::const_iterator itr = m_lFelvineShardGUIDs.begin();
                    itr != m_lFelvineShardGUIDs.end(); ++itr)
    ...
    case TYPE_IMMOLTHAR:
        ...
        {
            if (Creature* pPrince =
                    GetSingleCreatureFromStorage(NPC_PRINCE_TORTHELDRIN))
            {
                DoScriptText(SAY_FREE_IMMOLTHAR, pPrince);
                pPrince->setFaction(FACTION_HOSTILE);
                // Despawn Chest-Aura
                if (GameObject* pChestAura =
                        GetSingleGameObjectFromStorage(GO_PRINCES_CHEST_AURA))
...
{
    if (m_auiEncounter[TYPE_PYLON_1] != DONE ||
        m_auiEncounter[TYPE_PYLON_2] != DONE ||
        m_auiEncounter[TYPE_PYLON_3] != DONE ||
        m_auiEncounter[TYPE_PYLON_4] != DONE ||
        m_auiEncounter[TYPE_PYLON_5] != DONE)
        return false;
...
void instance_dire_maul::ProcessForceFieldOpening()
{
    ...
    for (GUIDList::const_iterator itr = m_luiHighborneSummonerGUIDs.begin();
         itr != m_luiHighborneSummonerGUIDs.end(); ++itr)
    {
...
void instance_dire_maul::SortPylonGuards()
{
    if (!m_lGeneratorGuardGUIDs.empty())
    {
        ...
        {
            GameObject* pGenerator =
                instance->GetGameObject(m_aCrystalGeneratorGuid[i]);
            ...
            for (GUIDList::iterator itr = m_lGeneratorGuardGUIDs.begin();
                 itr != m_lGeneratorGuardGUIDs.end();)
            {
                Creature* pGuard = instance->GetCreature(*itr);
                if (!pGuard ||
                    pGuard->isDead()) // Remove invalid guids and dead guards
                {
...
void instance_dire_maul::PylonGuardJustDied(Creature* pCreature)
{
    for (uint8 i = 0; i < MAX_GENERATORS; ++i)
    {
        ...
        if (m_sSortedGeneratorGuards[i].find(pCreature->GetGUIDLow()) !=
            m_sSortedGeneratorGuards[i].end())
        {
