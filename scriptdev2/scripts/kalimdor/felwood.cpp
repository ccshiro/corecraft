...
4506, 7603, 7603 (Summon Pollo Grande)
SDCategory: Felwood
EndScriptData */
...
struct MANGOS_DLL_DECL npc_kittenAI : public FollowerAI
{
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        ...
        if (!m_creature->getVictim() &&
            !m_creature->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP) &&
            HasFollowState(STATE_FOLLOW_INPROGRESS) &&
            pWho->GetEntry() == NPC_WINNA)
        {
            ...
            if (HasFollowState(STATE_FOLLOW_PAUSED))
            {
                if (m_uiMoonwellCooldown < uiDiff)
                {
                    m_creature->CastSpell(
                        m_creature, SPELL_CORRUPT_SABER_VISUAL, false);
    ...
    SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
    ...
    if (uiSpellId == SPELL_CORRUPT_SABER_VISUAL && uiEffIndex == EFFECT_INDEX_0)
    {
        ...
        if (const CreatureInfo* pTemp =
                GetCreatureTemplateStore(NPC_CORRUPT_SABER))
        {
...
bool GossipHello_npc_corrupt_saber(Player* pPlayer, Creature* pCreature)
{
    if (pPlayer->GetQuestStatus(QUEST_CORRUPT_SABER) == QUEST_STATUS_INCOMPLETE)
    {
        if (GetClosestCreatureWithEntry(
                pCreature, NPC_WINNA, INTERACTION_DISTANCE))
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_RELEASE,
...
bool GossipSelect_npc_corrupt_saber(
    Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        ...
        if (npc_kittenAI* pKittenAI =
                dynamic_cast<npc_kittenAI*>(pCreature->AI()))
...
bool GossipHello_npcs_riverbreeze_and_silversky(
    Player* pPlayer, Creature* pCreature)
{
    ...
    case NPC_ARATHANDIS_SILVERSKY:
        ...
        {
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_BEACON,
                GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        if (m_bEventStarted)
        {
            if (m_uiSummonTimer <= uiDiff)
            {
                ...
                case 1:
                    m_creature->movement_gens.push(
                        new movement::PointMovementGenerator(
                            0, 5407.19f, -753.00f, 350.82f, true, false));
                ...
                case 3:
                    ...
                    break;
                case 4:
                    if (Creature* pImpsy = GetClosestCreatureWithEntry(
                            m_creature, NPC_IMPSY, 20.0))
                    {
...
bool QuestRewarded_npc_niby_the_almighty(
    Player* /*pPlayer*/, Creature* pCreature, Quest const* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_KROSHIUS)
    {
        if (npc_niby_the_almightyAI* pNibyAI =
                dynamic_cast<npc_niby_the_almightyAI*>(pCreature->AI()))
        {
            pNibyAI->StartEvent();
        }
    }
...
struct MANGOS_DLL_DECL npc_kroshiusAI : public ScriptedAI
{
    ...
    void JustDied(Unit* /*pKiller*/) override { m_uiPhase = 0; }
    ...
    uint32 uiEventId, Object* pSource, Object* /*pTarget*/, bool /*bIsStart*/)
{
    if (uiEventId == EVENT_KROSHIUS_REVIVE)
    {
        ...
        {
            if (Creature* pKroshius = GetClosestCreatureWithEntry(
                    (Player*)pSource, NPC_KROSHIUS, 20.0f))
            {
                if (npc_kroshiusAI* pKroshiusAI =
                        dynamic_cast<npc_kroshiusAI*>(pKroshius->AI()))
