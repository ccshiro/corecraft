...
SDName: Mulgore
SD%Complete: 100
SDComment: Quest support: 11129. Skorn Whitecloud: Just a story if not rewarded
for quest
SDCategory: Mulgore
EndScriptData */
...
struct MANGOS_DLL_DECL npc_kyle_the_frenziedAI : public ScriptedAI
{
    ...
    {
        if (uiType != movement::gen::point || !m_bEvent)
            return;
        ...
        if (m_bEvent)
        {
            ...
            if (m_uiEventTimer < diff)
            {
                            ...
                            m_creature->GetMap()->GetPlayer(m_playerGuid))
                    {
                        ...
                        if (!pGo)
                        {
                            const SpellEntry* pSpell =
                                GetSpellStore()->LookupEntry(SPELL_LUNCH);

                            uint32 uiGameobjectEntry =
                                pSpell->EffectMiscValue[EFFECT_INDEX_1];

                            pGo = GetClosestGameObjectWithEntry(pPlayer,
                        ...
                        if (pGo)
                        {
                            ...
                            auto pos = pGo->GetPoint(m_creature);
                            m_creature->movement_gens.push(
                                new movement::PointMovementGenerator(POINT_ID,
                ...
                case 3:
                    if (Player* pPlayer =
                            m_creature->GetMap()->GetPlayer(m_playerGuid))
                        pPlayer->TalkedToCreature(m_creature->GetEntry(),
                ...
                case 5:
                    ...
                    Reset();
                    m_creature->movement_gens.reset();
                    break;
                }
            }
            else
...
bool GossipHello_npc_skorn_whitecloud(Player* pPlayer, Creature* pCreature)
{
    ...
    if (!pPlayer->GetQuestRewardStatus(770))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Tell me a story, Skorn.",
...
bool GossipSelect_npc_skorn_whitecloud(
