...
#define GOSSIP_BLY                                                             \
    "That's it! I'm tired of helping you out. It's time we settled things on " \
    "the battlefield!"
...
struct MANGOS_DLL_DECL npc_sergeant_blyAI : public ScriptedAI
{
    ...
    uint32 m_uiRevengeTimer; // this is wrong, spell should never be used unless
                             // m_creature->getVictim() dodge, parry or block
                             // attack. Mangos support required.
...
bool GossipHello_npc_sergeant_bly(Player* pPlayer, Creature* pCreature)
{
    ...
    {*/
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BLY, GOSSIP_SENDER_MAIN,
...
bool GossipSelect_npc_sergeant_bly(
...
{
    // event not implemented yet, this is only placeholder for future
    // developement
    /*if (m_pInstance->GetData(0) == DONE)
    {
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_WEEGLI,
    GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        pPlayer->SEND_GOSSIP_MENU(1514, pCreature->GetObjectGuid());//if event
    can proceed to end
    }
    else if (m_pInstance->GetData(0) == IN_PROGRESS)
        pPlayer->SEND_GOSSIP_MENU(1513, pCreature->GetObjectGuid());//if event
    are in progress
    else*/
    pPlayer->SEND_GOSSIP_MENU(
        1511, pCreature->GetObjectGuid()); // if event not started
    return true;
}

bool GossipSelect_npc_weegli_blastfuse(Player* pPlayer, Creature* /*pCreature*/,
    uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
        // here we make him run to door, set the charge and run away off to
        // nowhere
    }
    return true;
}

bool ProcessEventId_event_go_zulfarrak_gong(
    uint32 /*uiEventId*/, Object* pSource, Object* /*pTarget*/, bool bIsStart)
{
    if (bIsStart && pSource->GetTypeId() == TYPEID_PLAYER)
    {
        if (instance_zulfarrak* pInstance =
                (instance_zulfarrak*)((Player*)pSource)->GetInstanceData())
        {
            if (pInstance->GetData(TYPE_GAHZRILLA) == NOT_STARTED ||
                pInstance->GetData(TYPE_GAHZRILLA) == FAIL)
            {
...
bool AreaTrigger_at_zulfarrak(Player* pPlayer, AreaTriggerEntry const* pAt)
{
    if (pAt->id == AREATRIGGER_ANTUSUL)
    {
        ...
        instance_zulfarrak* pInstance =
            (instance_zulfarrak*)pPlayer->GetInstanceData();

        if (!pInstance)
            return false;

        if (pInstance->GetData(TYPE_ANTUSUL) == NOT_STARTED ||
            pInstance->GetData(TYPE_ANTUSUL) == FAIL)
        {
            if (Creature* pAntuSul =
                    pInstance->GetSingleCreatureFromStorage(NPC_ANTUSUL))
            {
                if (pAntuSul->isAlive())
