...
struct MANGOS_DLL_DECL npc_draenei_survivorAI : public ScriptedAI
{
    ...
    {
        if (m_bCanSayHelp && pWho->GetTypeId() == TYPEID_PLAYER &&
            m_creature->IsFriendlyTo(pWho) &&
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiSayThanksTimer)
        {
            ...
            {
                m_creature->remove_auras(SPELL_IRRIDATION);

                if (Player* pPlayer =
                        m_creature->GetMap()->GetPlayer(m_casterGuid))
                {
                ...
                m_creature->movement_gens.push(
                    new movement::PointMovementGenerator(0, -4115.053711f,
                        -13754.831055f, 73.508949f, true, false));
...
struct MANGOS_DLL_DECL npc_engineer_spark_overgrindAI : public ScriptedAI
{
    npc_engineer_spark_overgrindAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        if (pCreature->GetAreaId() == AREA_COVE ||
            pCreature->GetAreaId() == AREA_ISLE)
            m_bIsTreeEvent = true;
    }
...
bool GossipHello_npc_engineer_spark_overgrind(
    Player* pPlayer, Creature* pCreature)
{
    if (pPlayer->GetQuestStatus(QUEST_GNOMERCY) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_FIGHT,
...
bool GossipSelect_npc_engineer_spark_overgrind(
...
struct MANGOS_DLL_DECL npc_injured_draeneiAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 /*diff*/) override { return; }
};
...
struct MANGOS_DLL_DECL npc_magwinAI : public npc_escortAI
{
    ...
    void Aggro(Unit* who) override { DoScriptText(SAY_AGGRO, m_creature, who); }

    void Reset() override {}
};

bool QuestAccept_npc_magwin(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_A_CRY_FOR_HELP)
    {
        ...
        if (npc_magwinAI* pEscortAI =
                dynamic_cast<npc_magwinAI*>(pCreature->AI()))
