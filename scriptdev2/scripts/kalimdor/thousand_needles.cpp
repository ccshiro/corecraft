...
struct MANGOS_DLL_DECL npc_kanatiAI : public npc_escortAI
{
    ...
    {
        for (int i = 0; i < 3; ++i)
            m_creature->SummonCreature(NPC_GALAK_ASS, m_afGalakLoc[0],
...
bool QuestAccept_npc_lakota_windsong(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_FREE_AT_LAST)
    {
        ...
        if (npc_lakota_windsongAI* pEscortAI =
                dynamic_cast<npc_lakota_windsongAI*>(pCreature->AI()))
...
float m_afWyvernLoc[3][3] = {{-4990.606f, -906.057f, -5.343f},
    {-4970.241f, -927.378f, -4.951f}, {-4985.364f, -952.528f, -5.199f}};
...
struct MANGOS_DLL_DECL npc_paoka_swiftmountainAI : public npc_escortAI
{
    ...
    void DoSpawnWyvern()
    {
        for (auto& elem : m_afWyvernLoc)
            m_creature->SummonCreature(NPC_WYVERN, elem[0], elem[1], elem[2],
...
bool QuestAccept_npc_paoka_swiftmountain(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_HOMEWARD)
    {
        ...
        if (npc_paoka_swiftmountainAI* pEscortAI =
                dynamic_cast<npc_paoka_swiftmountainAI*>(pCreature->AI()))
            pEscortAI->Start(false, pPlayer, pQuest);
    }
    return true;
}
            ...
            if (m_uiResetTimer < uiDiff)
            {
                if (!m_creature->getVictim())
                    ...
                    m_creature->RemoveFlag(
                        UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
...
{
    if (pPlayer->GetQuestStatus(QUEST_SCOOP) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_QUEST,
...
bool GossipSelect_npc_plucky_johnson(
