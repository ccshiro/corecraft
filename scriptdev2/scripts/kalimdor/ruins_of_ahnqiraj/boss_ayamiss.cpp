...
enum
{
    ...
    // SPELL_SUMMON_SWARMER  = 25844,                //might be 25708    -
    // spells were removed since 2.0.1
    ...
    SPELL_FEED =
        25721, // cast by the Larva when reaches the player on the altar
...
static const SummonLocation aAyamissSpawnLocs[] = {
    ...
    {-9717.18f, 1517.72f, 27.4677f}, // teleport location - need to be hardcoded
                                     // because the player is teleported after
                                     // the larva is summoned
};
...
struct MANGOS_DLL_DECL boss_ayamissAI : public ScriptedAI
{
    ...
    void Aggro(Unit* /*pWho*/) override
    {
        ...
        /* m_creature->GetMotionMaster()->MovePoint(0, m_creature->GetX(), */
        /*     m_creature->GetY(), m_creature->GetZ() + 15.0f); */
    }
    ...
    void JustSummoned(Creature* pSummoned) override
    {
        ...
        else if (pSummoned->GetEntry() == NPC_LARVA)
        {
            ...
            /* pSummoned->GetMotionMaster()->MovePoint(1, */
            /*     aAyamissSpawnLocs[3].m_fX, aAyamissSpawnLocs[3].m_fY, */
            /*     aAyamissSpawnLocs[3].m_fZ); */
        }
    ...
    void SummonedMovementInform(Creature* pSummoned,
        movement::gen /*uiMotionType*/, uint32 uiPointId) override
    {
        ...
        if (Unit* pTarget = m_creature->GetMap()->GetUnit(m_paralyzeTarget))
            pSummoned->CastSpell(pTarget, SPELL_FEED, true, NULL, NULL,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiParalyzeTimer < uiDiff)
        {
            Unit* pTarget = m_creature->SelectAttackingTarget(
                ATTACKING_TARGET_RANDOM, 1, SPELL_PARALYZE, SELECT_FLAG_PLAYER);
            if (!pTarget)
            ...
            if (DoCastSpellIfCan(pTarget, SPELL_PARALYZE) == CAST_OK)
            {
                ...
                m_creature->SummonCreature(NPC_LARVA,
                    aAyamissSpawnLocs[uiLoc].m_fX,
                    aAyamissSpawnLocs[uiLoc].m_fY,
                    aAyamissSpawnLocs[uiLoc].m_fZ, 0,
        ...
        {
            // The spell which summons these guys was removed in 2.0.1 ->
            // therefore we need to summon them manually at a random location
            // around the area
            // The summon locations is guesswork - the real location is supposed
            // to be handled by world triggers
            // There should be about 24 swarmers per min
            for (uint8 i = 0; i < 2; ++i)
            {
                auto pos = m_creature->GetPointXYZ(
                    G3D::Vector3(aAyamissSpawnLocs[2].m_fX,
                        aAyamissSpawnLocs[2].m_fY, aAyamissSpawnLocs[2].m_fZ),
                    2 * M_PI_F * rand_norm_f(), 80.0f * rand_norm_f());
                m_creature->SummonCreature(NPC_SWARMER, pos.x, pos.y, pos.z,
        ...
        {
            for (GUIDList::const_iterator itr = m_lSwarmersGuidList.begin();
                 itr != m_lSwarmersGuidList.end(); ++itr)
            {
        ...
        else
        {
            ...
            {
                if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_LASH) ==
                    CAST_OK)
            ...
            {
                if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_TRASH) ==
                    CAST_OK)
...
struct MANGOS_DLL_DECL npc_hive_zara_larvaAI : public ScriptedAI
{
    ...
    {
        m_pInstance =
            (instance_ruins_of_ahnqiraj*)m_creature->GetInstanceData();
        Reset();
    }
