...
struct MANGOS_DLL_DECL boss_kurinnaxxAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiSandTrapTimer < uiDiff)
        {
            ...
            pTarget->CastSpell(pTarget, SPELL_SANDTRAP, true, NULL, NULL,
        ...
        if (m_uiTrapTriggerTimer)
        {
            ...
            {
                if (GameObject* pTrap =
                        m_creature->GetMap()->GetGameObject(m_sandtrapGuid))
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_WIDE_SLASH) ==
                CAST_OK)
