...
struct MANGOS_DLL_DECL boss_moamAI : public ScriptedAI
{
    ...
    void Aggro(Unit* /*pWho*/) override
    {
        ...
        m_creature->SetMaxPower(
            POWER_MANA, m_creature->GetCreatureInfo()->maxmana);
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        case PHASE_ATTACKING:
            ...
            {
                m_uiCheckoutManaTimer = 1500;
                if (m_creature->GetPower(POWER_MANA) * 100 /
            ...
            if (m_uiSummonManaFiendsTimer <= uiDiff)
            {
                DoCastSpellIfCan(m_creature->getVictim(),
                    SPELL_SUMMON_MANAFIEND_1, CAST_TRIGGERED);
                DoCastSpellIfCan(m_creature->getVictim(),
                    SPELL_SUMMON_MANAFIEND_2, CAST_TRIGGERED);
                DoCastSpellIfCan(m_creature->getVictim(),
                    SPELL_SUMMON_MANAFIEND_3, CAST_TRIGGERED);
                m_uiSummonManaFiendsTimer = 90000;
            }
            else
            ...
            if (m_uiManaDrainTimer <= uiDiff)
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0, SPELL_DRAIN_MANA,
        ...
        case PHASE_ENERGIZING:
            if (m_uiCheckoutManaTimer <= uiDiff)
            {
                ...
                if (m_creature->GetPower(POWER_MANA) ==
                    m_creature->GetMaxPower(POWER_MANA))
                {
                    m_creature->remove_auras(SPELL_ENERGIZE);
