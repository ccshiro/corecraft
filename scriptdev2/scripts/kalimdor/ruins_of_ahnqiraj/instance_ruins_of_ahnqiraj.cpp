...
instance_ruins_of_ahnqiraj::instance_ruins_of_ahnqiraj(Map* pMap)
    ...
    case TYPE_KURINNAXX:
        if (uiData == DONE)
        {
            ...
            DoOrSimulateScriptTextForThisInstance(
                SAY_OSSIRIAN_INTRO, NPC_OSSIRIAN);
        }
...
void instance_ruins_of_ahnqiraj::DoSapwnAndorovIfCan()
{
    ...
    for (auto& aAndorovSpawnLoc : aAndorovSpawnLocs)
        pPlayer->SummonCreature(aAndorovSpawnLoc.m_uiEntry,
            aAndorovSpawnLoc.m_fX, aAndorovSpawnLoc.m_fY, aAndorovSpawnLoc.m_fZ,
            aAndorovSpawnLoc.m_fO, TEMPSUMMON_DEAD_DESPAWN, 0);
}
