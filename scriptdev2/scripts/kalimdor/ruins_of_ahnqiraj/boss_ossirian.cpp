...
static const uint32 aWeaknessSpell[] = {SPELL_WEAKNESS_FIRE,
    SPELL_WEAKNESS_FROST, SPELL_WEAKNESS_NATURE, SPELL_WEAKNESS_ARCANE,
...
struct MANGOS_DLL_DECL boss_ossirianAI : public ScriptedAI
{
    ...
    void DoSpawnNextCrystal()
    {
        ...
        if (m_uiCrystalPosition == 0)
        {
            ...
            pOssirianTrigger =
                m_pInstance->GetSingleCreatureFromStorage(NPC_OSSIRIAN_TRIGGER);
            if (pOssirianTrigger && !pOssirianTrigger->isAlive())
                pOssirianTrigger->Respawn();
        }
        else
        {
            // Summon a new crystal trigger at some position depending on
            // m_uiCrystalPosition
        }
        if (!pOssirianTrigger)
            return;
        ...
        if (GameObject* pCrystal = GetClosestGameObjectWithEntry(
                pOssirianTrigger, GO_OSSIRIAN_CRYSTAL, 10.0f))
            m_pInstance->DoRespawnGameObject(
                pCrystal->GetObjectGuid(), 5 * MINUTE);
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell) override
    {
        if (pCaster->GetTypeId() == TYPEID_UNIT &&
            pCaster->GetEntry() == NPC_OSSIRIAN_TRIGGER)
        {
            ...
            m_creature->remove_auras(SPELL_SUPREME);
            m_uiSupremeTimer = 45000;
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        ...
        if (!m_bSaidIntro && pWho->GetTypeId() == TYPEID_PLAYER &&
            m_creature->IsWithinDistInMap(pWho, 75.0f, false))
        {
            switch (urand(0, 2))
            {
            case 0:
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            if (DoCastSpellIfCan(m_creature, SPELL_SUPREME,
                    CAST_AURA_NOT_PRESENT) == CAST_OK)
            {
                switch (urand(0, 2))
                {
                case 0:
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_CYCLONE) ==
                CAST_OK)
                m_uiCycloneTimer = 20000;
        }
        else
...
{
    if (Creature* pOssirianTrigger =
            GetClosestCreatureWithEntry(pGo, NPC_OSSIRIAN_TRIGGER, 10.0f))
        pOssirianTrigger->CastSpell(
            pOssirianTrigger, aWeaknessSpell[urand(0, 4)], false);
