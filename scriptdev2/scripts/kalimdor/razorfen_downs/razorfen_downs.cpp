...
enum
{
    ...
    // SPELL_SUMMON_2                  = 14802,              //
    // NPC_DEATHS_HEAD_GEOMANCER
    // SPELL_SUMMON_3                  = 14801,              //
...
struct MANGOS_DLL_DECL npc_belnistraszAI : public npc_escortAI
{
    ...
    void AttackedBy(Unit* pAttacker) override
    {
        if (HasEscortState(STATE_ESCORT_PAUSED))
        {
            if (!m_bAggro)
            {
                DoScriptText(urand(0, 1) ? SAY_BELNISTRASZ_AGGRO_1 :
    ...
    void SpawnerSummon(Creature* pSummoner)
    {
        ...
        {
            pSummoner->SummonCreature(NPC_PLAGUEMAW_THE_ROTTING,
                pSummoner->GetX(), pSummoner->GetY(), pSummoner->GetZ(),
                pSummoner->GetO(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                60000);
            return;
        }
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            auto pos = pSummoner->GetPoint(2.0f, angle);
            ...
            pSummoner->SummonCreature(uiEntry, pos.x, pos.y, pos.z, 0.0f,
    ...
    {
        m_creature->SummonCreature(NPC_IDOL_ROOM_SPAWNER,
            m_fSpawnerCoord[iType][0], m_fSpawnerCoord[iType][1],
            m_fSpawnerCoord[iType][2], m_fSpawnerCoord[iType][3],
    ...
    void UpdateEscortAI(const uint32 uiDiff) override
    {
        if (HasEscortState(STATE_ESCORT_PAUSED))
        {
            if (m_uiRitualTimer < uiDiff)
            {
                ...
                case 9:
                    DoScriptText(
                        SAY_BELNISTRASZ_FINISH, m_creature, m_creature);
                ...
                case 10:
                {
                    if (Player* pPlayer = GetPlayerForEscort())
                    {
                        pPlayer->GroupEventHappens(
                            QUEST_EXTINGUISHING_THE_IDOL, m_creature);

                        if (GameObject* pGo = GetClosestGameObjectWithEntry(
                                m_creature, GO_BELNISTRASZ_BRAZIER, 10.0f))
                        {
                            if (!pGo->isSpawned())
                            {
                    ...
                    m_creature->remove_auras(SPELL_IDOL_SHUTDOWN);
...
bool QuestAccept_npc_belnistrasz(
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_EXTINGUISHING_THE_IDOL)
    {
        if (npc_belnistraszAI* pEscortAI =
                dynamic_cast<npc_belnistraszAI*>(pCreature->AI()))
        {
