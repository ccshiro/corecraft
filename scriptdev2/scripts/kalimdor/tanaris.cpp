...
struct MANGOS_DLL_DECL mob_aquementasAI : public ScriptedAI
{
    ...
    void SendItem(Unit* receiver)
    {
        ...
        {
            ((Player*)receiver)->add_item(11522, 1);
        }
    }
...
struct MANGOS_DLL_DECL npc_custodian_of_timeAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i) override
    {
        ...
        case 24:
            ...
            // below here is temporary workaround, to be removed when spell
            // works properly
    ...
    void SummonedBy(WorldObject* obj) override
    {
        ...
        {
            Player* sum = static_cast<Player*>(obj);
            if (sum->has_aura(34877) &&
                sum->GetQuestStatus(10277) == QUEST_STATUS_INCOMPLETE)
            {
                Start(false, sum);
            }
        }
...
bool GossipSelect_npc_marin_noggenfogger(
...
struct MANGOS_DLL_DECL npc_oox17tnAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i) override
    {
        ...
        case 22:
            ...
            m_creature->SummonCreature(NPC_SCORPION, -8340.70f, -4448.17f,
                9.17f, 3.10f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
            m_creature->SummonCreature(NPC_SCORPION, -8343.18f, -4444.35f,
                9.44f, 2.35f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
            m_creature->SummonCreature(NPC_SCORPION, -8348.70f, -4457.80f,
                9.58f, 2.02f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
            break;
        // 2. Ambush: 2 Rogues & 1 Shadow Mage
        case 28:
            DoScriptText(SAY_OOX_AMBUSH, m_creature);

            m_creature->SummonCreature(NPC_SCOFFLAW, -7488.02f, -4786.56f,
                10.67f, 3.74f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
            m_creature->SummonCreature(NPC_SHADOW_MAGE, -7486.41f, -4791.55f,
                10.54f, 3.26f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);

            if (Creature* pCreature = m_creature->SummonCreature(NPC_SCOFFLAW,
                    -7488.47f, -4800.77f, 9.77f, 2.50f,
    ...
    Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_RESCUE_OOX_17TN)
    {
        ...
        if (npc_oox17tnAI* pEscortAI =
                dynamic_cast<npc_oox17tnAI*>(pCreature->AI()))
...
#define GOSSIP_ITEM_NORGANNON_4                                               \
    "Excuse me? We've been \"reschedueled for visitations\"? What does that " \
    "mean?!"
#define GOSSIP_ITEM_NORGANNON_5 "So, what's inside Uldum?"
#define GOSSIP_ITEM_NORGANNON_6 "I will return when i have the Plates of Uldum."

bool GossipHello_npc_stone_watcher_of_norgannon(
    Player* pPlayer, Creature* pCreature)
{
    ...
    if (pPlayer->GetQuestStatus(2954) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_NORGANNON_1,
...
struct MANGOS_DLL_DECL npc_toogaAI : public FollowerAI
{
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        ...
        if (!m_creature->getVictim() &&
            !HasFollowState(STATE_FOLLOW_COMPLETE | STATE_FOLLOW_POSTEVENT) &&
            pWho->GetEntry() == NPC_TORTA)
        {
            if (m_creature->IsWithinDistInMap(pWho, INTERACTION_DISTANCE))
            {
                ...
                {
                    if (pPlayer->GetQuestStatus(QUEST_TOOGA) ==
                    ...
                    case 6:
                        ...
                        m_creature->movement_gens.push(
                            new movement::PointMovementGenerator(
                                POINT_ID_TO_WATER, m_afToWaterLoc[0],
                                m_afToWaterLoc[1], m_afToWaterLoc[2], true,
                                false));
                        break;
                    }
