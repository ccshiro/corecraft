...
enum
{
    ...
    SPELL_SPIRIT_HEAL_MANA =
        44535, // in battlegrounds player get this no-mana-cost-buff

    SPELL_WAITING_TO_RESURRECT =
        2584 // players who cancel this aura don't want a resurrection
};
...
struct MANGOS_DLL_DECL npc_spirit_guideAI : public ScriptedAI
{
    ...
    void CorpseRemoved(uint32&) override
    {
        ...
        for (const auto& elem : PlayerList)
        {
            ...
            // NOTE: m_creature is no longer in world
            if (!pPlayer || !pPlayer->IsWithinDist(m_creature, 20.0f) ||
                !pPlayer->has_aura(SPELL_WAITING_TO_RESURRECT))
                continue;

            // repop player again - now this node won't be counted and another
            // node is searched
    ...
    {
        if (pSpellEntry->Id == SPELL_SPIRIT_HEAL &&
            pUnit->GetTypeId() == TYPEID_PLAYER &&
            pUnit->has_aura(SPELL_WAITING_TO_RESURRECT))
